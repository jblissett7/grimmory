...
class MetadataControllerTest {
  ...
  @Mock private BookMetadataService bookMetadataService;
  ...
  @Mock private MetadataManagementService metadataManagementService;
  ...
  private MetadataUpdateContext captureContextFromUpdate(MetadataReplaceMode replaceMode) {
    ...
    when(bookRepository.findAllWithMetadataByIds(java.util.Collections.singleton(bookId)))
        .thenReturn(java.util.List.of(bookEntity));
    ...
    ArgumentCaptor<MetadataUpdateContext> captor =
        ArgumentCaptor.forClass(MetadataUpdateContext.class);
  ...
  void updateMetadata_shouldPassReplaceModeFromParam() {
    MetadataUpdateContext context =
        captureContextFromUpdate(MetadataReplaceMode.REPLACE_WHEN_PROVIDED);
