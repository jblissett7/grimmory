...
class LibraryRescanTaskTest {
  ...
  @Mock private LibraryRescanHelper libraryRescanHelper;
  @Mock private TaskCancellationManager cancellationManager;

  @InjectMocks private LibraryRescanTask libraryRescanTask;
  ...
  void execute_shouldRescanAllLibraries() {
    ...
    verify(libraryRescanHelper)
        .handleRescanOptions(argThat(ctx -> ctx.getLibraryId().equals(1L)), eq("task-123"));
    verify(libraryRescanHelper)
        .handleRescanOptions(argThat(ctx -> ctx.getLibraryId().equals(2L)), eq("task-123"));
  }
  ...
  void execute_shouldContinue_whenDataAccessExceptionOccurs() {
    ...
    doThrow(new InvalidDataAccessApiUsageException("DB Error"))
        .when(libraryRescanHelper)
        .handleRescanOptions(any(), any());
