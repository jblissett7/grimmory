...
class BookRecommendationUpdaterTaskTest {
  ...
  @Mock private BookVectorService vectorService;
  @Mock private NotificationService notificationService;

  @InjectMocks private BookRecommendationUpdaterTask task;
  ...
  void execute_shouldProcessBooks() {
    BookEntity book1 =
        BookEntity.builder()
            .id(1L)
            .metadata(BookMetadataEntity.builder().title("B1").build())
            .build();
    BookEntity book2 =
        BookEntity.builder()
            .id(2L)
            .metadata(BookMetadataEntity.builder().title("B2").build())
            .build();
    ...
    lenient()
        .when(vectorService.findTopKSimilar(any(), anyList(), anyInt()))
    ...
    org.mockito.ArgumentCaptor<List<BookEntity>> captor =
  ...
  void execute_shouldFail_whenEmbeddingGenerationThrows() {
    ...
    when(vectorService.generateEmbedding(any()))
  ...
  void execute_shouldContinue_whenSimilarityCalculationThrows() {
    ...
    when(vectorService.cosineSimilarity(any(), any()))
        .thenThrow(new RuntimeException("Math error"));
