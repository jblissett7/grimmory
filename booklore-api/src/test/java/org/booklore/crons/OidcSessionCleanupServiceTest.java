...
class OidcSessionCleanupServiceTest {
  ...
  @InjectMocks private OidcSessionCleanupService oidcSessionCleanupService;
  ...
  void cleanupOidcSessions_revokedCutoffIsApproximately7DaysAgo() {
    ...
    assertThat(revokedCaptor.getValue())
        .isBetween(expectedRevoked.minusSeconds(2), expectedRevoked.plusSeconds(2));
  }
  ...
  void cleanupOidcSessions_maxAgeCutoffIsApproximately30DaysAgo() {
    ...
    assertThat(maxAgeCaptor.getValue())
        .isBetween(expectedMaxAge.minusSeconds(2), expectedMaxAge.plusSeconds(2));
  }
  ...
  void cleanupOidcSessions_revokedCleanupCalledBeforeMaxAgeCleanup() {
    ...
    inOrder
        .verify(oidcSessionRepository)
        .deleteByRevokedTrueAndCreatedAtBefore(org.mockito.ArgumentMatchers.any(Instant.class));
    inOrder
        .verify(oidcSessionRepository)
        .deleteByCreatedAtBefore(org.mockito.ArgumentMatchers.any(Instant.class));
  }
}
