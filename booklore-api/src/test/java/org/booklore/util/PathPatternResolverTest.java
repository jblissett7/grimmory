...
class PathPatternResolverTest {
  ...
  private static final int MAX_FILESYSTEM_COMPONENT_BYTES =
      245; // From PathPatternResolver.MAX_FILESYSTEM_COMPONENT_BYTES
  private static final int MAX_FILENAME_BYTES =
      245; // From PathPatternResolver.MAX_FILESYSTEM_COMPONENT_BYTES
  private static final Charset FILENAME_CHARSET = StandardCharsets.UTF_8;

  public static final List<String> LONG_AUTHOR_LIST =
      new ArrayList<>(
          List.of(
              "梁思成",
              "叶嘉莹",
              ...
              "顾毓琇",
              "陈从周",
              "（加拿大）伊莎白（Isabel Crook）（美）柯临清（Christina Gilmartin）",
              "傅莹"));
  ...
  void testResolvePattern_titleWithExtension() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{title}.{extension}", "original.pdf");
  ...
  void testResolvePattern_multiplePlaceholders() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{authors} - {title} ({year})", "original.pdf");

    // Authors from a Set may be in any order
    assertTrue(
        result.equals("John Doe, Jane Smith - Test Book (2023).pdf")
  ...
  void testResolvePattern_authorsList() {
    BookMetadata metadata =
        BookMetadata.builder().authors(List.of("Author One", "Author Two")).build();

    String result = PathPatternResolver.resolvePattern(metadata, "{authors}", "original.pdf");

    // Authors from a Set may be in any order
    assertTrue(
        result.equals("Author One, Author Two.pdf") || result.equals("Author Two, Author One.pdf"));
  }
  ...
  void testResolvePattern_seriesInfo() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{series} #{seriesIndex} - {title}", "original.pdf");
  ...
  void testResolvePattern_optionalBlock_allPresent() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").authors(List.of("Author Name")).build();

    String result =
        PathPatternResolver.resolvePattern(metadata, "{title}< - {authors}>", "original.pdf");
  ...
  void testResolvePattern_isbnPriority() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{title} - {isbn}", "original.pdf");
  ...
  void testResolvePattern_isbn10Fallback() {
    BookMetadata metadata = BookMetadata.builder().title("Book Title").isbn10("1234567890").build();
  ...
  void testResolvePattern_nullMetadata() {
    String result =
        PathPatternResolver.resolvePattern((BookMetadata) null, "{title}", "original.pdf");
  ...
  void testResolvePattern_nullTitle() {
    BookMetadata metadata = BookMetadata.builder().title(null).build();
  ...
  void testResolvePattern_currentFilename() {
    BookMetadata metadata = BookMetadata.builder().title("Book Title").build();

    String result =
        PathPatternResolver.resolvePattern(metadata, "{currentFilename}", "original.pdf");
  ...
  void testResolvePattern_specialCharacters() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book: Title? *With* Special/Chars").build();
  ...
  void testResolvePattern_emptyAuthors() {
    BookMetadata metadata = BookMetadata.builder().title("Book Title").authors(List.of()).build();
  ...
  void testResolvePattern_sanitizesIllegalCharacters() {
    BookMetadata metadata = BookMetadata.builder().title("Title/With:Slashes?And*Stars").build();
  ...
  void testResolvePattern_emptyOptionalBlocks() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{title}< [{series}]>< ({year})>", "original.pdf");
  ...
  void testResolvePattern_complexPattern() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{authors} - {title}< [{series} #{seriesIndex}]>< ({year})>", "original.pdf");

    // Authors from a Set may be in any order
    assertTrue(
        result.equals("John Doe, Jane Smith - The Great Book [Awesome Series #03] (2023).pdf")
            || result.equals(
                "Jane Smith, John Doe - The Great Book [Awesome Series #03] (2023).pdf"));
  }
  ...
  void testResolvePattern_truncatesLongAuthorList() {
    BookMetadata metadata =
        BookMetadata.builder().title("中国文化合集").authors(LONG_AUTHOR_LIST).build();

    String result =
        PathPatternResolver.resolvePattern(metadata, "{authors}/{title}", "original.epub");
    ...
    for (String component : pathComponents) {
      ...
      assertTrue(
          byteLength <= MAX_FILESYSTEM_COMPONENT_BYTES,
          String.format(
              "Component '%s' byte length %d should not exceed filesystem limit %d",
    ...
    assertTrue(
        authorsBytes <= MAX_AUTHORS_BYTES,
        String.format(
            "Authors part should be truncated to <= %d bytes, got: %d",
  ...
  void testResolvePattern_authorsWithinLimit() {
    ...
    BookMetadata metadata = BookMetadata.builder().title("Test Book").authors(authors).build();
  ...
  void testResolvePattern_appliesAuthorTruncation() {
    ...
    BookMetadata metadata = BookMetadata.builder().title("Test").authors(shortAuthorList).build();
    ...
    BookMetadata longMetadata =
        BookMetadata.builder().title("Test").authors(LONG_AUTHOR_LIST).build();
    ...
    assertTrue(
        longAuthorsBytes <= MAX_AUTHORS_BYTES,
        String.format(
            "Long authors should be truncated to <= %d bytes, got: %d",
            MAX_AUTHORS_BYTES, longAuthorsBytes));

    assertTrue(
        longAuthorsBytes < LONG_AUTHOR_LIST.toString().getBytes(FILENAME_CHARSET).length,
  ...
  void testResolvePattern_truncatesSingleVeryLongAuthor() {
    ...
    BookMetadata metadata =
        BookMetadata.builder().title("Test").authors(List.of(veryLongAuthor)).build();
    ...
    assertTrue(
        authorsBytes <= MAX_AUTHORS_BYTES,
        String.format(
            "Single long author should be truncated to <= %d bytes, got: %d",
            MAX_AUTHORS_BYTES, authorsBytes));
    assertFalse(authorsPart.isEmpty(), "Should not be empty after truncation");
    assertTrue(
        authorsBytes < veryLongAuthor.getBytes(FILENAME_CHARSET).length,
  ...
  void testResolvePattern_addsEtAlWhenTruncated() {
    BookMetadata metadata = BookMetadata.builder().title("Test").authors(LONG_AUTHOR_LIST).build();

    String result = PathPatternResolver.resolvePattern(metadata, "{authors}", "test.epub");

    assertTrue(
        result.contains("et al."),
        "Should contain truncation indicator when authors are truncated");
  }
  ...
  void testResolvePattern_validatesFinalPathWithCombinedLongComponents() {
    ...
    BookMetadata metadata =
        BookMetadata.builder().title(longTitle).authors(LONG_AUTHOR_LIST).build();

    String result =
        PathPatternResolver.resolvePattern(metadata, "{title} - {authors}", "test.epub");
    ...
    for (String component : components) {
      if (!component.contains(".")) { // Skip filename with extension
        ...
        assertTrue(
            byteLength <= MAX_FILESYSTEM_COMPONENT_BYTES,
            String.format(
                "Path component should be <= %d bytes, got: %d for component: %s",
  ...
  void testResolvePattern_removesTrailingDots() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").authors(List.of("Author Name Jr.")).build();

    // Pattern: {authors}/{title}
    String result =
        PathPatternResolver.resolvePattern(metadata, "{authors}/{title}", "original.pdf");
    ...
    assertTrue(
        authorDir.equals("Author Name Jr"),
        "Expected 'Author Name Jr' but got '" + authorDir + "'");
  }
  ...
  void testResolvePattern_removesTrailingDotsFromMultipleComponents() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{authors}/{series}/{title}", "original.pdf");

    String[] components = result.split("/");
    for (int i = 0; i < components.length - 1; i++) { // Check directories
      assertFalse(
          components[i].endsWith("."),
          "Component " + i + " should not end with dot: " + components[i]);
    }
  ...
  @Test
  @DisplayName(
      "Should preserve extension for files with numeric patterns in name (e.g., Chapter 8.1.cbz)")
  void testResolvePattern_filenameWithNumericPattern() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{series} - Chapter {seriesIndex}", "original.cbz");

    assertEquals(
        "Series Name - Chapter 08.1.cbz",
        result,
        "Extension should be preserved for files with numeric patterns");
  }
  ...
  void testResolvePattern_filenameWithMultipleDots() {
    BookMetadata metadata = BookMetadata.builder().title("My.Awesome.Book").build();
    ...
    assertEquals(
        ...
        result,
        "Extension should be preserved for files with dots in title");
  }
  ...
  void testResolvePattern_currentFilenameWithPath() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "books/{currentFilename}", "My.File.With.Dots.epub");

    assertEquals(
        "books/My.File.With.Dots.epub",
        result,
        "Extension should not be added when {currentFilename} is used, even with dots in name");
  }
  ...
  void testResolvePattern_titleWithDotsAndNumericSuffix() {
    BookMetadata metadata = BookMetadata.builder().title("Chapter.8.1").build();
  ...
  void testResolvePattern_comicWithChapterNumber() {
    BookMetadata metadata =
        BookMetadata.builder().seriesName("One Punch Man").seriesNumber(8.1f).build();

    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{series} - Chapter {seriesIndex}", "One Punch Man - Chapter 8.1.cbz");

    assertEquals(
        "One Punch Man - Chapter 08.1.cbz", result, "CBZ extension should be preserved for comics");
  }
  ...
  void testResolvePattern_numericExtensionLikePattern() {
    BookMetadata metadata = BookMetadata.builder().title("Book Version 2").build();

    String result = PathPatternResolver.resolvePattern(metadata, "{title}.1", "original.epub");

    assertEquals(
        "Book Version 2.1.epub",
        result,
        "Should add real extension even when pattern ends with .1");
  }
  ...
  void testResolvePattern_veryLongExtension_truncatesOrWarns() {
    ...
    BookMetadata metadata = BookMetadata.builder().title("Test").build();

    String result = PathPatternResolver.resolvePattern(metadata, "{title}", "test" + longExtension);

    assertNotNull(result, "Result should not be null");
    assertTrue(
        result.contains("Test"), "Should preserve title even with long extension. Got: " + result);

    int resultBytes = result.getBytes(FILENAME_CHARSET).length;
    assertTrue(
        resultBytes <= MAX_FILENAME_BYTES,
        String.format(
            "Result should not exceed filesystem limits: %d bytes (max: %d). Result: %s",
            resultBytes, MAX_FILENAME_BYTES, result));

    int lastDotIndex = result.lastIndexOf('.');
    assertTrue(lastDotIndex > 0, "Should have an extension separator. Result: " + result);
  }

  @Test
  @DisplayName("Should handle hidden file (starts with dot)")
  void testResolvePattern_hiddenFile() {
    BookMetadata metadata = BookMetadata.builder().title("Hidden File").build();
  ...
  void testResolvePattern_multipleTrailingDots() {
    ...
    for (int i = 0; i < components.length - 1; i++) { // Check directories (not filename)
      assertFalse(
          components[i].endsWith("."), "Component should not end with dot: " + components[i]);
    }
  }
  ...
  void testResolvePattern_unknownPlaceholder() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{title} - {unknown}", "test.epub");
  ...
  void testResolvePattern_optionalBlockAllPresent() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{title}< - {authors}>< [{series}]>", "test.epub");
  ...
  void testResolvePattern_currentFilenameNoExtension() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{currentFilename}", "fileWithoutExtension");
  ...
  void testResolvePattern_emptyFilename_noTitle_usesDefault() {
    ...
    if (result.isBlank()) {
      ...
      assertTrue(
          result.toLowerCase().contains("untitled"),
  ...
  void testResolvePattern_firstAuthorExceedsLimit() {
    ...
    assertTrue(
        authorsBytes <= MAX_AUTHORS_BYTES,
        String.format(
            "Authors should be truncated to <= %d bytes, got: %d",
            MAX_AUTHORS_BYTES, authorsBytes));
  ...
  void testTruncateFilenameWithExtension_truncatesLongFilename() {
    ...
    assertTrue(
        result.getBytes(StandardCharsets.UTF_8).length <= MAX_FILENAME_BYTES,
  ...
  void testTruncateFilenameWithExtension_hiddenFile() {
    ...
    assertTrue(
        result.startsWith(".config"),
        "Should still start with .config (or be treated as filename)");
    assertTrue(
        result.getBytes(StandardCharsets.UTF_8).length <= MAX_FILENAME_BYTES,
        "Result bytes should be <= " + MAX_FILENAME_BYTES);
  }
  ...
  void testElseClause_leftSideUsedWhenPresent() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").seriesName("My Series").build();

    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series}|Standalone>/{title}", "original.pdf");
  ...
  void testElseClause_fallbackWithPlaceholders() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").authors(List.of("John Doe")).build();

    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series}/{seriesIndex} - {title}|{title}>", "original.pdf");
  ...
  void testElseClause_backwardCompatibleNoPipe() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "<{series}/>{title}", "original.pdf");
  ...
  void testElseClause_literalFallback() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series}|No Series>/{title}", "original.pdf");
  ...
  void testElseClause_mixedBlocks() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").authors(List.of("Author")).build();

    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series}|Standalone>/<{year} - >{title}", "original.pdf");
  ...
  void testModifier_firstMultipleAuthors() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{authors:first}/{title}", "original.pdf");
  ...
  void testModifier_firstSingleAuthor() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").authors(List.of("Patrick Rothfuss")).build();
  ...
  void testModifier_sort() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{authors:sort}/{title}", "original.pdf");
  ...
  void testModifier_sortSingleWord() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").authors(List.of("Plato")).build();
  ...
  void testModifier_initialTitle() {
    BookMetadata metadata = BookMetadata.builder().title("The Name of the Wind").build();

    String result =
        PathPatternResolver.resolvePattern(metadata, "{title:initial}/{title}", "original.pdf");
  ...
  void testModifier_initialAuthors() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{authors:initial}/{authors:sort}/{title}", "original.pdf");
  ...
  void testModifier_insideElseClause() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series}|{authors:sort}>/{title}", "original.pdf");
  ...
  void testModifier_sortThreeWordName() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").authors(List.of("Mary Jane Watson")).build();
  ...
  void testModifier_initialSingleWordAuthor() {
    ...
    String result =
        PathPatternResolver.resolvePattern(metadata, "{authors:initial}/{title}", "original.pdf");
  ...
  void testModifier_firstOnNonAuthorsField() {
    BookMetadata metadata = BookMetadata.builder().title("Intro, Outro, Epilogue").build();
  ...
  void testModifier_sortOnTitle() {
    BookMetadata metadata = BookMetadata.builder().title("The Wind").build();
  ...
  void testElseClause_multipleBlocks() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series}|Standalone>/<{year}|Unknown Year> - {title}", "original.pdf");
  ...
  @Test
  @DisplayName(
      "Else clause: primary side partially missing with multiple placeholders triggers fallback")
  ...
  void testModifier_inPrimarySideOfElseClause() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series} by {authors:sort}|{title}>", "original.pdf");
  ...
  void testModifier_chainedDifferentFields() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{title:initial}/{authors:first}/{title:lower}", "original.pdf");
  ...
  @Test
  @DisplayName(
      "Modifier on field used in optional block where field is missing - block should be removed")
  ...
  void testModifier_withElseClauseAndExtension() {
    BookMetadata metadata =
        BookMetadata.builder().title("My Book").authors(List.of("Jane Doe")).build();

    String result =
        PathPatternResolver.resolvePattern(
            metadata, "<{series:upper}|{authors:sort}>/{title}.{extension}", "original.epub");
  ...
  void testModifier_initialLowercaseTitle() {
    BookMetadata metadata = BookMetadata.builder().title("lowercase title").build();
  ...
  void testElseClause_primaryCompleteIgnoresFallback() {
    BookMetadata metadata =
        BookMetadata.builder().title("Book Title").seriesName("My Series").seriesNumber(5f).build();
  ...
  void testElseClause_existingPatternsUnchanged() {
    ...
    String result =
        PathPatternResolver.resolvePattern(
            metadata, "{authors}/<{series}/><{seriesIndex}. >{title}< ({year})>", "original.epub");
