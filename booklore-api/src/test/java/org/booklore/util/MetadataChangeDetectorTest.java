...
public class MetadataChangeDetectorTest {
  ...
  void testIsDifferent_whenFieldChanges_returnsTrue(
      String fieldName, Consumer<BookMetadata> modifier) {
  ...
  static Stream<Arguments> fieldChangeProvider() {
    return Stream.of(
        ...
        Arguments.of(
            "publishedDate",
            (Consumer<BookMetadata>) m -> m.setPublishedDate(LocalDate.of(2021, 6, 15))),
        Arguments.of(
            "description", (Consumer<BookMetadata>) m -> m.setDescription("New Description")),
        ...
        Arguments.of(
            "amazonReviewCount", (Consumer<BookMetadata>) m -> m.setAmazonReviewCount(2000)),
        Arguments.of("goodreadsRating", (Consumer<BookMetadata>) m -> m.setGoodreadsRating(4.3)),
        Arguments.of(
            "goodreadsReviewCount", (Consumer<BookMetadata>) m -> m.setGoodreadsReviewCount(30000)),
        Arguments.of("hardcoverRating", (Consumer<BookMetadata>) m -> m.setHardcoverRating(4.2)),
        Arguments.of(
            "hardcoverReviewCount", (Consumer<BookMetadata>) m -> m.setHardcoverReviewCount(750)),
        ...
        Arguments.of(
            "audibleReviewCount", (Consumer<BookMetadata>) m -> m.setAudibleReviewCount(3000)));
  }
  ...
  void testHasValueChangesForFileWrite_whenNoChanges_returnsFalse() {
    boolean result =
        MetadataChangeDetector.hasValueChangesForFileWrite(newMeta, existingMeta, clearFlags);
  ...
  void testEdgeCase_nullToEmptyCollection_returnsFalse() {
    BookMetadataEntity testExisting =
        BookMetadataEntity.builder()
            .bookId(1L)
            .title("Original Title")
            .subtitle("Original Subtitle")
    ...
    assertFalse(
        result,
        "Should return false because toNameSet converts null to empty set, so null → empty collection is no change");
  }
  ...
  void testIsDifferent_whenFieldLockedAndOnlyValueDifferent_returnsFalse() {
    ...
    boolean result = MetadataChangeDetector.isDifferent(newMeta, existingMeta, clearFlags);
    assertFalse(
        result, "Should not detect value change when field is locked and lock is unchanged");
  }
  ...
  void testClearFlag_whenFieldUnlocked_triggersChange() {
    ...
    assertTrue(
        result,
        "Clear flag should trigger change when field is unlocked, even with identical values");
  }
  ...
  void testIsDifferent_whenClearFlagTrue_andValuesIdentical_returnsTrue() {
    ...
    assertTrue(
        result,
        "isDifferent should return true when clear flag is set, even if values are identical");
  }
  ...
  void testEdgeCase_whitespaceNormalization() {
    ...
    boolean result = MetadataChangeDetector.isDifferent(newMeta, existingMeta, clearFlags);
    assertFalse(
        result, "Should return false when strings are equal after whitespace normalization");
  }
