...
class BookUtilsTest {
  ...
  void testCleanAndTruncateSearchTerm_longText() {
    String longText =
        "This is a very long search term that should be truncated because it exceeds sixty characters in length and needs to be shortened";
    String result = BookUtils.cleanAndTruncateSearchTerm(longText);
    assertTrue(
        result.length() <= MAX_SEARCH_TERM_LENGTH,
        String.format(
            "Result length %d should not exceed max %d", result.length(), MAX_SEARCH_TERM_LENGTH));
  ...
  void testCleanAndTruncateSearchTerm_longTextWithSpecialChars() {
    String longText =
        "This-is,a@very#long$search%term^with&special*chars(that)should[be]truncated{because}it<exceeds>sixty?characters";
    String result = BookUtils.cleanAndTruncateSearchTerm(longText);
    assertTrue(
        result.length() <= MAX_SEARCH_TERM_LENGTH,
        String.format(
  ...
  void testCleanAndTruncateSearchTerm_exactly60Chars() {
    ...
    assertEquals(
        MAX_SEARCH_TERM_LENGTH, result.length(), "Text at max length should not be truncated");
  }
  ...
  void testNormalizeForSearch_programmingLanguages() {
    ...
    assertEquals(
        "effective c++ programming", BookUtils.normalizeForSearch("Effective C++ Programming"));
  ...
  void testSearchMatchingWithAndWithoutDiacritics() {
    ...
    assertTrue(
        storedSearchText.contains("francois muller"),
        "Stored text should contain normalized author: " + storedSearchText);

    assertTrue(
        storedSearchText.contains(searchWithoutDiacritics),
        "Search without diacritics should match");
    assertTrue(
        storedSearchText.contains(searchWithDiacritics), "Search with diacritics should match");
  }
  ...
  void testCleanFileName_parameterized(String input, String expected) {
    ...
    assertEquals(
        expected, result, String.format("Input: '%s' should result in: '%s'", input, expected));
  }
