...
/**
 * Test builder for creating Library-related test objects. Provides fluent API for creating
 * LibraryEntity and Library DTO objects with sensible defaults.
 */
public class LibraryTestBuilder {
  ...
  public LibraryTestBuilder(
      MockedStatic<FileUtils> fileUtilsMock,
    ...
    fileFingerprintMock
        .when(() -> FileFingerprint.generateHash(any(Path.class)))
        .then(
            invocation -> {
    ...
    when(bookRepositoryMock.findAllByLibraryPathIdAndFileSubPathStartingWith(
        ...
        .thenAnswer(
            invocation -> {
              ...
              return bookRepository.values().stream()
                  .filter(
                      book ->
                          book.getLibraryPath().getId().equals(libraryPathId)
    ...
    lenient()
        .when(bookAdditionalFileRepositoryMock.save(any(BookFileEntity.class)))
        .thenAnswer(
            invocation -> {
  ...
  public LibraryTestBuilder addDefaultLibrary() {
    return addLibrary(DEFAULT_LIBRARY_NAME).addPath(DEFAULT_LIBRARY_PATH);
  }
  ...
  public List<LibraryFile> getLibraryFiles() {
    if (libraryFiles.isEmpty()) {
      throw new IllegalStateException(
          "No library files available. Please add a library file first.");
    }
  ...
  public LibraryTestBuilder addPath(String path) {
    if (libraries.isEmpty()) {
      throw new IllegalStateException(
          "No library available to add a path. Please add a library first.");
    }
    ...
    var id =
        libraries.stream()
                ...
                .orElse(0L)
            + 1;
    LibraryPathEntity libraryPath = LibraryPathEntity.builder().id(id).path(path).build();
  ...
  public LibraryTestBuilder addLibraryFile(String fileSubPath, String fileName, String hash) {
    ...
    Path filePath =
        Path.of(lastLibraryFiles.getLibraryPathEntity().getPath(), fileSubPath, fileName);
    if (libraryFileHashes.containsKey(filePath)) {
      throw new IllegalArgumentException(
          "File with the same path and name already exists: " + fileSubPath + "/" + fileName);
    }
  ...
  public LibraryTestBuilder addLibraryFile(String fileSubPath, String fileName) {
    if (libraries.isEmpty()) {
      throw new IllegalStateException(
          "No library available to add a book. Please add a library first.");
    }

    var library = libraries.getLast();
    if (library.getLibraryPaths().isEmpty()) {
      throw new IllegalStateException(
          "No library path available to add a book. Please add a path first.");
    }
  ...
  private FileProcessResult processFileResult(LibraryFile libraryFile) {
    ...
    return FileProcessResult.builder().book(book).status(FileProcessStatus.NEW).build();
  }
  ...
  private @NotNull BookFileEntity saveBookAdditionalFile(BookFileEntity additionalFile) {
    ...
    if (additionalFile.isBookFormat()
        && bookAdditionalFileRepository.values().stream()
            .anyMatch(
                existingFile ->
                    existingFile.getCurrentHash().equals(additionalFile.getCurrentHash()))) {
      throw new IllegalArgumentException(
          "File with the same hash already exists: " + additionalFile.getCurrentHash());
    }
