...
class SecurityContextVirtualThreadTest {
  ...
  @Test
  void testRunWithSecurityContext_executesRunnable()
  ...
  @Test
  void testRunWithSecurityContext_preservesSecurityContext()
      throws ExecutionException, InterruptedException {
  ...
  @Test
  void testRunWithSecurityContext_maintainsMainThreadContext()
      throws ExecutionException, InterruptedException {
    CompletableFuture<Void> future = new CompletableFuture<>();
    SecurityContextVirtualThread.runWithSecurityContext(
        () -> {
  ...
  @Test
  void testRunWithSecurityContext_withProvidedContext()
      throws ExecutionException, InterruptedException {
    Authentication differentAuth =
        new UsernamePasswordAuthenticationToken("differentUser", "password", null);
  ...
  @Test
  void testRunWithSecurityContext_withProvidedContext_clearsAfterExecution()
      throws ExecutionException, InterruptedException {
    Authentication differentAuth =
        new UsernamePasswordAuthenticationToken("differentUser", "password", null);
    SecurityContext differentContext = new SecurityContextImpl(differentAuth);
    ...
    SecurityContextVirtualThread.runWithSecurityContext(
        ...
        () -> {
          try {
            ...
            assertEquals(
                "differentUser", SecurityContextHolder.getContext().getAuthentication().getName());
            future.complete(null);
          } catch (Exception e) {
            future.completeExceptionally(e);
          }
  ...
  @Test
  void testRunWithSecurityContext_handlesException()
      throws ExecutionException, InterruptedException {
