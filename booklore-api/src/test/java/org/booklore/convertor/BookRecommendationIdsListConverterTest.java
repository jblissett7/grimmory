...
class BookRecommendationIdsListConverterTest {
  ...
  void convertToEntityAttribute_shouldDeserializeJsonStringToSet() {
    ...
    BookRecommendationLite book1 =
        result.stream().filter(b -> b.getB() == 1L).findFirst().orElse(null);
    assertNotNull(book1);
    assertEquals(0.95, book1.getS(), 0.001);

    BookRecommendationLite book2 =
        result.stream().filter(b -> b.getB() == 2L).findFirst().orElse(null);
    assertNotNull(book2);
