...
class HardcoverSyncServiceTest {
  ...
  void setUp() throws Exception {
    ...
    when(hardcoverSyncSettingsService.getSettingsForUserId(TEST_USER_ID))
        .thenReturn(hardcoverSyncSettings);
  ...
  void syncProgressToHardcover_whenApiError_shouldNotThrow() {
    ...
    when(responseSpec.body(Map.class))
        .thenReturn(Map.of("errors", List.of(Map.of("message", "Unauthorized"))));
  ...
  void findHardcoverBookById_emptyResponse_shouldReturnNull() throws Exception {
    ...
    Method method =
        HardcoverSyncService.class.getDeclaredMethod("findHardcoverBookById", Integer.class);
    method.setAccessible(true);
