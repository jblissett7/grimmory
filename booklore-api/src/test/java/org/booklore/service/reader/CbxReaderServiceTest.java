...
class CbxReaderServiceTest {
  ...
  @InjectMocks CbxReaderService cbxReaderService;
  ...
  void testGetAvailablePages_CBZ_Fallback_Success() throws Exception {
    ...
    try (MockedStatic<FileUtils> fileUtilsStatic = mockStatic(FileUtils.class)) {
      ...
      when(zipFileFail.getEntries())
          .thenThrow(new IllegalArgumentException("Corrupt extra fields"));
      ...
      when(builder.setUseUnicodeExtraFields(anyBoolean()))
          .thenAnswer(
              invocation -> {
                return builder;
              });
  ...
  void testStreamPageImage_CBZ_Success() throws Exception {
    ...
    try (MockedStatic<FileUtils> fileUtilsStatic = mockStatic(FileUtils.class)) {
      ...
      when(zipFile.getInputStream(entry1))
  ...
  void testGetAvailablePages_CBZ_ThrowsOnMissingBook() {
    ...
    assertThrows(
        ...
        () -> cbxReaderService.getAvailablePages(2L));
  }
          ...
          mockConstruction(
              ...
              (mock, context) -> {
                ...
                doAnswer(
                    ...
                    .when(mock)
                    .extractFile(eq(header), any(OutputStream.class));
              })) {
        Files.deleteIfExists(cbrPath);
        Files.createFile(cbrPath);
        Files.setLastModifiedTime(cbrPath, FileTime.fromMillis(System.currentTimeMillis()));
  ...
  void testGetAvailablePages_UnsupportedArchive_Throws() throws Exception {
    ...
    try (MockedStatic<FileUtils> fileUtilsStatic = mockStatic(FileUtils.class)) {
      fileUtilsStatic
          .when(() -> FileUtils.getBookFullPath(bookEntity))
          .thenReturn(unknownPath.toString());
