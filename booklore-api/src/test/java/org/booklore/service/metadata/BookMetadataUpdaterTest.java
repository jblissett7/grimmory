...
class BookMetadataUpdaterTest {
  ...
  @InjectMocks private BookMetadataUpdater updater;
  ...
  void setUp() {
    ...
    bookEntity =
        BookEntity.builder().id(1L).metadata(metadataEntity).bookFiles(new ArrayList<>()).build();
  ...
  private void mockSettingsAndChangeDetector(
      MockedStatic<MetadataChangeDetector> mcd, boolean hasDiff, boolean hasValueChanges) {
    ...
    when(appSettingService.getAppSettings())
        .thenReturn(AppSettings.builder().metadataPersistenceSettings(settings).build());
    ...
    mcd.when(() -> MetadataChangeDetector.hasValueChanges(any(), any(), any()))
        .thenReturn(hasValueChanges);
    mcd.when(() -> MetadataChangeDetector.hasLockChanges(any(), any())).thenReturn(false);
    mcd.when(() -> MetadataChangeDetector.hasValueChangesForFileWrite(any(), any(), any()))
        .thenReturn(false);
  }
  ...
  void setBookMetadata_replaceWhenProvided_setsWhenNewValueNotBlank() {
    ...
    MetadataUpdateContext context =
        buildContext(newMeta, MetadataReplaceMode.REPLACE_WHEN_PROVIDED);
    ...
    MetadataUpdateContext context =
        MetadataUpdateContext.builder()
            ...
            .metadataUpdateWrapper(
                MetadataUpdateWrapper.builder().metadata(newMeta).clearFlags(clearFlags).build())
  ...
  void setBookMetadata_updatesLocks() {
    BookMetadata newMeta =
        BookMetadata.builder().title("T").titleLocked(true).publisherLocked(false).build();
    MetadataUpdateContext context = buildContext(newMeta, MetadataReplaceMode.REPLACE_ALL);
  ...
  class BasicFieldUpdates {
    ...
    void updatesAgeRatingAndContentRating() {
      BookMetadata newMeta =
          BookMetadata.builder().title("T").ageRating(18).contentRating("Mature").build();
      MetadataUpdateContext context = buildContext(newMeta, MetadataReplaceMode.REPLACE_ALL);
  ...
  class ComicMetadataUpdates {
    ...
    void updatesComicLocks() {
      ...
      ComicMetadata comicDto =
          ComicMetadata.builder()
              .issueNumberLocked(true)
              .volumeNameLocked(true)
              .charactersLocked(true)
              .pencillersLocked(true)
              .build();
      BookMetadata newMeta = BookMetadata.builder().title("T").comicMetadata(comicDto).build();
      MetadataUpdateContext context = buildContext(newMeta, MetadataReplaceMode.REPLACE_ALL);
    ...
    void updatesCreators_pencillers() {
      ...
      try (MockedStatic<MetadataChangeDetector> mcd = mockStatic(MetadataChangeDetector.class)) {
        ...
        assertThat(existing.getCreatorMappings().iterator().next().getRole())
            .isEqualTo(ComicCreatorRole.PENCILLER);
      }
    }

    @Test
    void creatorRole_locked_notUpdated() {
      ComicMetadataEntity existing =
          ComicMetadataEntity.builder().bookId(1L).pencillersLocked(true).build();
      metadataEntity.setComicMetadata(existing);
    ...
    void charactersLocked_notUpdated() {
      ComicMetadataEntity existing =
          ComicMetadataEntity.builder().bookId(1L).charactersLocked(true).build();
      existing.getCharacters().add(ComicCharacterEntity.builder().id(1L).name("Batman").build());
      metadataEntity.setComicMetadata(existing);
  ...
  class MatchScoreAndDelegation {
    ...
    void matchScoreExceptionDoesNotFailUpdate() {
      ...
      when(metadataMatchService.calculateMatchScore(bookEntity))
    ...
    void sidecarWriter_exceptionDoesNotFailUpdate() {
      ...
      try (MockedStatic<MetadataChangeDetector> mcd = mockStatic(MetadataChangeDetector.class)) {
        ...
        when(appSettingService.getAppSettings())
            .thenReturn(AppSettings.builder().metadataPersistenceSettings(settings).build());
        when(sidecarMetadataWriter.isWriteOnUpdateEnabled()).thenReturn(true);
        doThrow(new RuntimeException("sidecar error"))
            .when(sidecarMetadataWriter)
            .writeSidecarMetadata(any());
  ...
  class FileMoveAfterUpdate {
    ...
    void movesFile_whenSettingEnabled() {
      BookFileEntity primaryFile =
          BookFileEntity.builder().fileName("book.epub").bookType(BookFileType.EPUB).build();
      ...
      FileMoveResult moveResult =
          FileMoveResult.builder()
              .moved(true)
              .newFileName("new-book.epub")
              .newFileSubPath("Author/new-book.epub")
              .build();
  ...
  class NetworkStorageGating {
    ...
    void updateBookMetadata_networkStorage_skipsFileWrite() {
      ...
      BookFileEntity bookFile =
          BookFileEntity.builder().bookType(BookFileType.EPUB).isBookFormat(true).build();
