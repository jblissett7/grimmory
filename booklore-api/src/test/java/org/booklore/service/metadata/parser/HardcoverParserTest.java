 ...
 *
 * <p>These tests verify: - Combined title+author search strategy for better reliability - ISBN
 * search behavior - Author filtering logic - Mood/genre/tag mapping with quality filtering - Edge
 * cases and error handling
 */
class HardcoverParserTest {

  @Mock private HardcoverBookSearchService hardcoverBookSearchService;
  ...
  class SearchStrategyTests {
    ...
    void fetchMetadata_titleAndAuthor_searchesCombined() {
      ...
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Lamb").author("Christopher Moore").build();

      GraphQLResponse.Hit hit =
          createHitWithAuthor("Lamb: The Gospel According to Biff", "Christopher Moore");
    ...
    void fetchMetadata_combinedSearchEmpty_fallsBackToTitleOnly() {
      ...
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Some Book").author("Unknown Author").build();
      ...
      when(hardcoverBookSearchService.searchBooks("Some Book")).thenReturn(List.of(hit));
    ...
    @Test
    @DisplayName(
        "Should fall back to title-only search when combined search returns results but they are filtered out")
    void fetchMetadata_combinedSearchFilteredOut_fallsBackToTitleOnly() {
      Book book = Book.builder().title("Portrait of a Thief").build();
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Portrait of a Thief").author("Grace D. Li").build();

      // Simulate combined search returning a result that DOES NOT match the author (e.g. some other
      // book matched the string)
      ...
      when(hardcoverBookSearchService.searchBooks("Portrait of a Thief Grace D. Li"))
          .thenReturn(
              List.of(
                  badHit)); // Returns a hit, but fuzzy score will fail or simple check will fail
    ...
    void fetchMetadata_isbnProvided_searchesByIsbn() {
      ...
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Any Title").isbn("978-0316769488").build();
      ...
      when(hardcoverBookSearchService.searchBookByIsbn("9780316769488")).thenReturn(List.of(hit));
    ...
    void fetchMetadata_isbnSearchNoEditions_returnsEmptyList() {
      Book book = Book.builder().title("Any Title").build();
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Any Title").isbn("9780316769488").build();
    ...
    void fetchMetadata_isbnSearchNullEditions_returnsEmptyList() {
      ...
      GraphQLResponse.BookWithEditions bookWithNullEditions =
          new GraphQLResponse.BookWithEditions();
    ...
    void fetchMetadata_isbnSearchReturnsNull_returnsEmptyList() {
      ...
      when(hardcoverBookSearchService.searchBookByIsbn("9780316769488")).thenReturn(null);
    ...
    void fetchMetadata_noAuthor_searchesTitleOnly() {
      ...
      FetchMetadataRequest request = FetchMetadataRequest.builder().title("The Prince").build();

      GraphQLResponse.Hit hit = createHitWithAuthor("The Prince", "Niccolò Machiavelli");
      when(hardcoverBookSearchService.searchBooks("The Prince")).thenReturn(List.of(hit));
  ...
  class AuthorFilteringTests {
    ...
    void fetchMetadata_authorProvided_filtersResults() {
      Book book = Book.builder().title("The Prince").build();
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("The Prince").author("Machiavelli").build();
      ...
      when(hardcoverBookSearchService.searchBooks("The Prince Machiavelli")).thenReturn(hits);
    ...
    void fetchMetadata_fuzzyAuthorMatch_includesPartialMatches() {
      ...
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Test Book").author("Moore").build();

      GraphQLResponse.Hit hit = createHitWithAuthor("Test Book", "Christopher Moore");
      when(hardcoverBookSearchService.searchBooks("Test Book Moore")).thenReturn(List.of(hit));
  ...
  class MetadataMappingTests {
    ...
    void fetchMetadata_fullDocument_mapsAllFields_fromISBN() {
      Book book = Book.builder().title("Test").build();
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Test").isbn("9781234567897").build();

      GraphQLResponse.BookWithEditions hit = createBookWithEditions();
      when(hardcoverBookSearchService.searchBookByIsbn("9781234567897")).thenReturn(List.of(hit));
    ...
    void fetchMetadata_fullDocument_isbn10() {
      Book book = Book.builder().title("Test").build();
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Test").isbn("123456789X").build();

      GraphQLResponse.BookWithEditions hit = createBookWithEditions();
      when(hardcoverBookSearchService.searchBookByIsbn("123456789X")).thenReturn(List.of(hit));
    ...
    void fetchMetadata_fullDocument_noIsbn10() {
      Book book = Book.builder().title("Test").build();
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Test").isbn("9791111111112").build();

      GraphQLResponse.BookWithEditions hit = createBookWithEditions();
      hit.getEditions().get(0).setIsbn13("9791111111112");
      hit.getEditions().get(0).setIsbn10(null);

      when(hardcoverBookSearchService.searchBookByIsbn("9791111111112")).thenReturn(List.of(hit));
  ...
  class MoodFilteringTests {
    ...
    void fetchMetadata_withBookId_fetchesDetailedMoods() {
      ...
      when(hardcoverBookSearchService.fetchBookDetails(12345)).thenReturn(details);
    ...
    void fetchMetadata_detailFetchFails_fallsBackToBasicFilter() {
      ...
      when(hardcoverBookSearchService.searchBooks("Test")).thenReturn(List.of(hit));
      when(hardcoverBookSearchService.fetchBookDetails(12345)).thenReturn(null); // Simulate failure
  ...
  class EdgeCaseTests {
    ...
    void fetchMetadata_nullResponse_returnsEmptyList() {
      ...
      when(hardcoverBookSearchService.searchBooks(anyString())).thenReturn(null);
    ...
    void fetchMetadata_emptyResponse_returnsEmptyList() {
      ...
      FetchMetadataRequest request =
          FetchMetadataRequest.builder().title("Nonexistent Book").build();

      when(hardcoverBookSearchService.searchBooks(anyString())).thenReturn(Collections.emptyList());
    ...
    void fetchMetadata_nullTitle_returnsEmptyList() {
      ...
      FetchMetadataRequest request = FetchMetadataRequest.builder().author("Some Author").build();
  ...
  class FetchTopMetadataTests {
    ...
    void fetchTopMetadata_hasResults_returnsFirst() {
      ...
      when(hardcoverBookSearchService.searchBooks("Test")).thenReturn(hits);
    ...
    void fetchTopMetadata_noResults_returnsNull() {
      ...
      FetchMetadataRequest request = FetchMetadataRequest.builder().title("Nonexistent").build();
  ...
  private GraphQLResponse.Hit createFullyPopulatedHit() {
    ...
    doc.setIsbns(
        List.of("9781111111113", "1111111111", "9781234567897", "123456789X", "9791111111112"));
  ...
  private GraphQLResponse.BookWithEditions createBookWithEditions() {
    ...
    cachedTags.setMood(
        List.of(
            createCachedTag("adventurous", 15),
            createCachedTag("exciting", 12),
            createCachedTag("novotes", 0)));
    cachedTags.setGenre(
        List.of(
            createCachedTag("fiction", 20),
            createCachedTag("fantasy", 18),
            createCachedTag("novotes", 0)));
    cachedTags.setTag(
        List.of(
            createCachedTag("epic", 10),
            createCachedTag("quest", 8),
            createCachedTag("novotes", 0)));
