...
class BookReviewUpdateServiceTest {
  ...
  private BookMetadataEntity entityWithReviews(Set<BookReviewEntity> reviews) {
    return BookMetadataEntity.builder().reviewsLocked(false).reviews(reviews).build();
  }

  private BookReview review(MetadataProvider provider, String name, Instant date) {
    return BookReview.builder().metadataProvider(provider).reviewerName(name).date(date).build();
  }
  ...
  class ReviewLockRespect {
    ...
    void skipsUpdateWhenReviewsLocked() {
      BookMetadataEntity entity =
          BookMetadataEntity.builder().reviewsLocked(true).reviews(new HashSet<>()).build();
    ...
    void skipsUpdateWhenReviewsLockedIsNull() {
      BookMetadataEntity entity =
          BookMetadataEntity.builder().reviewsLocked(null).reviews(new HashSet<>()).build();
      BookMetadata metadata =
          BookMetadata.builder()
              .bookReviews(List.of(review(MetadataProvider.Amazon, "r1", Instant.now())))
              .build();
  ...
  class NullAndEmptyReviewHandling {
    ...
    void filtersReviewsWithNullProvider() {
      ...
      BookMetadata metadata = BookMetadata.builder().bookReviews(List.of(noProvider)).build();
      service.updateBookReviews(metadata, entity, new MetadataClearFlags(), true);
  ...
  class MergeVsReplace {
    ...
    void replaceRemovesExistingReviews() {
      ...
      BookMetadata metadata =
          BookMetadata.builder()
              .bookReviews(
                  List.of(review(MetadataProvider.GoodReads, "replacement", Instant.now())))
              .build();
  ...
  class PerProviderLimit {
    ...
    void limitsToFiveReviewsPerProvider() {
      ...
      List<BookReview> sevenReviews =
          IntStream.range(0, 7)
              .mapToObj(
                  i -> review(MetadataProvider.Amazon, "r" + i, base.minus(i, ChronoUnit.DAYS)))
              .toList();
  ...
  class AddReviewsToBook {
    ...
    void addReviewsToBookRespectsLock() {
      ...
      service.addReviewsToBook(
          List.of(review(MetadataProvider.Amazon, "r1", Instant.now())), entity);
