...
class MetadataTaskServiceTest {
  ...
  @Mock private MetadataFetchJobRepository metadataFetchTaskRepository;

  @Mock private MetadataFetchProposalRepository proposalRepository;

  @Mock private FetchedProposalMapper fetchedProposalMapper;

  @Mock private AuthenticationService authenticationService;

  @InjectMocks private MetadataTaskService service;

  private MetadataFetchJobEntity buildTask(
      String taskId, MetadataFetchTaskStatus status, List<MetadataFetchProposalEntity> proposals) {
  ...
  private MetadataFetchProposalEntity buildProposal(
      Long id, MetadataFetchJobEntity job, FetchedMetadataProposalStatus status) {
  ...
  class GetTaskWithProposals {
    ...
    void returnsResponseWithOnlyFetchedProposals() {
      MetadataFetchJobEntity task =
          buildTask("t1", MetadataFetchTaskStatus.COMPLETED, new ArrayList<>());
      MetadataFetchProposalEntity fetched =
          buildProposal(1L, task, FetchedMetadataProposalStatus.FETCHED);
      MetadataFetchProposalEntity accepted =
          buildProposal(2L, task, FetchedMetadataProposalStatus.ACCEPTED);
      MetadataFetchProposalEntity rejected =
          buildProposal(3L, task, FetchedMetadataProposalStatus.REJECTED);
      ...
      when(fetchedProposalMapper.toDto(fetched))
          .thenReturn(FetchedProposal.builder().proposalId(1L).build());
    ...
    void mapsTaskFieldsCorrectly() {
      MetadataFetchJobEntity task =
          buildTask("t2", MetadataFetchTaskStatus.IN_PROGRESS, new ArrayList<>());
  ...
  class UpdateProposalStatus {
    ...
    void updatesProposalStatusSuccessfully() {
      ...
      MetadataFetchJobEntity job =
          buildTask("t1", MetadataFetchTaskStatus.COMPLETED, new ArrayList<>());
      MetadataFetchProposalEntity proposal =
          buildProposal(10L, job, FetchedMetadataProposalStatus.FETCHED);
    ...
    void returnsFalseWhenProposalTaskIdMismatch() {
      ...
      MetadataFetchJobEntity differentJob =
          buildTask("other-task", MetadataFetchTaskStatus.COMPLETED, new ArrayList<>());
      MetadataFetchProposalEntity proposal =
          buildProposal(10L, differentJob, FetchedMetadataProposalStatus.FETCHED);
      when(proposalRepository.findById(10L)).thenReturn(Optional.of(proposal));
  ...
  class GetActiveTasks {
    ...
    void filtersOutInProgressAndCancelledTasks() {
      MetadataFetchJobEntity inProgress =
          buildTask("ip", MetadataFetchTaskStatus.IN_PROGRESS, new ArrayList<>());
      MetadataFetchJobEntity cancelled =
          buildTask("ca", MetadataFetchTaskStatus.CANCELLED, new ArrayList<>());
      MetadataFetchJobEntity completed =
          buildTask(
              "co",
    ...
    void completedTaskCountsAcceptedAsCompleted() {
      MetadataFetchProposalEntity accepted1 =
          MetadataFetchProposalEntity.builder()
              .proposalId(1L)
              .status(FetchedMetadataProposalStatus.ACCEPTED)
              .build();
      MetadataFetchProposalEntity accepted2 =
          MetadataFetchProposalEntity.builder()
              .proposalId(2L)
              .status(FetchedMetadataProposalStatus.ACCEPTED)
              .build();
      ...
      MetadataFetchProposalEntity rejected =
          MetadataFetchProposalEntity.builder()
              .proposalId(4L)
              .status(FetchedMetadataProposalStatus.REJECTED)
              .build();
    ...
    void errorTaskFallsBackToRemainingSizeWhenTotalBooksCountNull() {
      MetadataFetchProposalEntity fetched1 =
          MetadataFetchProposalEntity.builder()
              .proposalId(1L)
              .status(FetchedMetadataProposalStatus.FETCHED)
              .build();
      MetadataFetchProposalEntity fetched2 =
          MetadataFetchProposalEntity.builder()
              .proposalId(2L)
              .status(FetchedMetadataProposalStatus.FETCHED)
              .build();
    ...
    void allNotificationsHaveIsReviewTrue() {
      ...
      MetadataFetchJobEntity task =
          buildTask("t1", MetadataFetchTaskStatus.COMPLETED, List.of(fetched));
