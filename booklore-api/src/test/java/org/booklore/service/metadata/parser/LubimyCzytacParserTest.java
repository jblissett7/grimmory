...
class LubimyCzytacParserTest {
  ...
  @InjectMocks private LubimyCzytacParser parser;
  ...
  void testFetchMetadata_ProviderDisabled() {
    ...
    MetadataProviderSettings.Lubimyczytac lubimyCzytac =
        new MetadataProviderSettings.Lubimyczytac();
  ...
  void testFetchMetadata_Integration_RealBook() {
    ...
    FetchMetadataRequest request =
        FetchMetadataRequest.builder().title("Wiedźmin").author("Andrzej Sapkowski").build();
    ...
    assertTrue(
        firstResult.getAuthors() != null && !firstResult.getAuthors().isEmpty(),
