...
class KoreaderUserServiceTest {
  ...
  void upsertUser_createsNew_whenAbsent() {
    ...
    when(koreaderUserRepository.save(any(KoreaderUserEntity.class)))
        .thenAnswer(
            invocation -> {
    ...
    when(koreaderUserMapper.toDto(any(KoreaderUserEntity.class)))
        .thenAnswer(
            invocation -> {
              KoreaderUserEntity u = invocation.getArgument(0);
              return new KoreaderUser(
                  u.getId(),
                  u.getUsername(),
                  u.getPassword(),
                  u.getPasswordMD5(),
                  u.isSyncEnabled(),
                  u.isSyncWithBookloreReader());
            });
  ...
  void upsertUser_throws_whenOwnerMissing() {
    ...
    assertThrows(APIException.class, () -> service.upsertUser("x", "y"));
  }
