...
class BookNoteServiceTest {
  ...
  void setUp() {
    ...
    service =
        new BookNoteService(
            bookNoteRepository, bookRepository, userRepository, mapper, authenticationService);
  ...
  void createOrUpdateNote_createsNewNote_whenIdIsNull() {
    CreateBookNoteRequest req =
        CreateBookNoteRequest.builder().bookId(bookId).title("t").content("c").build();
  ...
  void createOrUpdateNote_updatesExistingNote_whenIdIsPresent() {
    ...
    BookNoteEntity existing =
        BookNoteEntity.builder().id(noteId).title("old").content("old").build();
    BookNoteEntity saved =
        BookNoteEntity.builder().id(noteId).title("new title").content("new content").build();
    BookNote dto = BookNote.builder().id(noteId).title("new title").content("new content").build();

    when(bookRepository.findById(bookId))
        .thenReturn(Optional.of(BookEntity.builder().id(bookId).build()));
    when(userRepository.findById(userId))
        .thenReturn(
            Optional.of(BookLoreUserEntity.builder().id(userId).isDefaultPassword(false).build()));
  ...
  void createOrUpdateNote_throwsIfBookNotFound() {
    ...
    RuntimeException ex =
        assertThrows(RuntimeException.class, () -> service.createOrUpdateNote(req));

    assertTrue(
        ex.getMessage().contains("BOOK_NOT_FOUND")
            || ex.getMessage().contains(String.valueOf(bookId)),
  ...
  void createOrUpdateNote_throwsIfNoteNotFoundForUpdate() {
    CreateBookNoteRequest req =
        CreateBookNoteRequest.builder().id(noteId).bookId(bookId).title("t").content("c").build();
