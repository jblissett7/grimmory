...
class ReadingProgressServiceTest {
  ...
  @InjectMocks private ReadingProgressService readingProgressService;
  ...
  void fetchUserProgress_shouldReturnProgressMap() {
    ...
    Map<Long, UserBookProgressEntity> result =
  ...
  void fetchUserFileProgress_emptyBookIds_shouldReturnEmptyMap() {
    ...
    verify(userBookFileProgressRepository, never())
        .findByUserIdAndBookFileBookIdIn(anyLong(), anySet());
  }
  ...
  void updateReadProgress_epub_shouldSaveProgress() {
    ...
    UserBookProgressEntity progress = new UserBookProgressEntity();
    when(userBookProgressRepository.findByUserIdAndBookId(2L, bookId))
        .thenReturn(Optional.of(progress));

    when(userBookFileProgressRepository.findByUserIdAndBookFileId(2L, 1L))
  ...
  void resetProgress_booklore_shouldCallBulkReset() {
    ...
    List<BookStatusUpdateResponse> result =
        readingProgressService.resetProgress(bookIds, ResetProgressType.BOOKLORE);

    verify(userBookProgressRepository)
        .bulkResetBookloreProgress(eq(1L), eq(new ArrayList<>(bookIds)), any());
    assertEquals(2, result.size());
  }
