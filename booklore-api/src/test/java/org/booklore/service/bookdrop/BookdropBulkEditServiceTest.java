...
class BookdropBulkEditServiceTest {
  ...
  @Mock private BookdropMetadataHelper metadataHelper;

  @InjectMocks private BookdropBulkEditService bulkEditService;

  @Captor private ArgumentCaptor<List<BookdropFileEntity>> filesCaptor;
  ...
  void bulkEdit_WithSingleValueFields_ShouldUpdateTextAndNumericFields() {
    ...
    when(metadataHelper.resolveFileIds(false, null, List.of(1L, 2L))).thenReturn(List.of(1L, 2L));
    when(bookdropFileRepository.findAllById(anyList())).thenReturn(List.of(file1, file2));
  ...
  void bulkEdit_WithArrayFieldsMergeMode_ShouldMergeArrays() {
    ...
    when(metadataHelper.resolveFileIds(false, null, List.of(1L))).thenReturn(List.of(1L));
  ...
  void bulkEdit_WithSelectAll_ShouldProcessAllFiles() {
    ...
    when(metadataHelper.resolveFileIds(true, List.of(2L), null)).thenReturn(List.of(1L, 3L));
    when(bookdropFileRepository.findAllById(anyList())).thenReturn(List.of(file1, file3));
  ...
  void bulkEdit_WithOneFileError_ShouldContinueWithOthers() {
    ...
    doThrow(new RuntimeException("JSON serialization error"))
        .when(metadataHelper)
        .updateFetchedMetadata(eq(file2), any());
  ...
  void bulkEdit_WithLargeSelection_ShouldProcessInBatches() {
    ...
    when(metadataHelper.resolveFileIds(false, null, manyIds)).thenReturn(manyIds);

    when(bookdropFileRepository.findAllById(anyList())).thenReturn(batch1, batch2, batch3);
