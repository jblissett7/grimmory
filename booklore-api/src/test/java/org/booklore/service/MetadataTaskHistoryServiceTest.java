...
class MetadataTaskHistoryServiceTest {
  ...
  @Mock private MetadataFetchJobRepository jobRepository;
  ...
  void getActiveTasks_shouldReturnOnlyTasksWithRemainingProposals() {
    ...
    MetadataBatchProgressNotification n1 =
        notifications.stream().filter(n -> n.getTaskId().equals("task1")).findFirst().orElseThrow();
    ...
    MetadataBatchProgressNotification n2 =
        notifications.stream().filter(n -> n.getTaskId().equals("task2")).findFirst().orElseThrow();
