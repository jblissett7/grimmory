...
class VersionServiceTest {
  ...
  class VersionComparison {
    ...
    void init() throws Exception {
      cmp = VersionService.class.getDeclaredMethod("isVersionGreater", String.class, String.class);
      cmp.setAccessible(true);
    }

    @Test
    void returnsTrueWhenMajorIncreases() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "2.0.0", "1.9.9")).isTrue();
    }

    @Test
    void returnsFalseWhenMajorDecreases() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "1.0.0", "2.0.0")).isFalse();
    }

    @Test
    void returnsTrueForPatchIncrease() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "1.0.1", "1.0.0")).isTrue();
    }

    @Test
    void returnsFalseForPatchDecrease() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "1.0.0", "1.0.1")).isFalse();
    }

    @Test
    void returnsFalseWhenEqual() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "1.2.3", "1.2.3")).isFalse();
    }

    @Test
    void handlesDifferentLengthVersions() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "1.2.0", "1.1")).isTrue();
      assertThat((Boolean) cmp.invoke(service, "1.0", "1.0.1")).isFalse();
    }

    @Test
    void ignoresPrefixAndSafelyHandlesInvalid() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "v1.10", "v1.9.9")).isTrue();
      assertThat((Boolean) cmp.invoke(service, "x.y", "1.0")).isFalse();
    }

    @Test
    void returnsFalseWhenVersion2IsNull() throws Exception {
      assertThat((Boolean) cmp.invoke(service, "1.0.0", null)).isFalse();
    }

    @Test
    void returnsFalseWhenVersion1IsNull() throws Exception {
      assertThat((Boolean) cmp.invoke(service, null, "1.0.0")).isFalse();
    }

    @Test
    void returnsFalseWhenBothVersionsNull() throws Exception {
      assertThat((Boolean) cmp.invoke(service, null, null)).isFalse();
    }
  }
  ...
  class GetVersionInfoTests {
    ...
    void includesAppAndLatestOnSuccess() {
      Mockito.doReturn("v9.9.9").when(spyService).fetchLatestGitHubReleaseVersion();

      VersionInfo info = spyService.getVersionInfo();

      assertThat(info.getCurrent()).isEqualTo(service.appVersion);
      assertThat(info.getLatest()).isEqualTo("v9.9.9");
    }
    ...
    void usesUnknownIfFetchFails() {
      ...
      assertThat(info.getLatest()).isEqualTo("unknown");
    }
  }
  ...
  class GetChangelogSinceCurrentVersionTests {
    ...
    void returnsNotesWhenAvailable() {
      ...
      Mockito.doReturn(List.of(note)).when(spyService).fetchReleaseNotesSince(service.appVersion);
    ...
    void returnsEmptyListWhenNoNewReleases() {
      Mockito.doReturn(List.of()).when(spyService).fetchReleaseNotesSince(service.appVersion);
