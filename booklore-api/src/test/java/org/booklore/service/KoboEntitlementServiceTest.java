...
class KoboEntitlementServiceTest {
  ...
  @Mock private KoboUrlBuilder koboUrlBuilder;

  @Mock private BookQueryService bookQueryService;

  @Mock private AppSettingService appSettingService;

  @Mock private KoboCompatibilityService koboCompatibilityService;
  ...
  @Mock private KoboReadingStateRepository readingStateRepository;

  @Mock private KoboReadingStateMapper readingStateMapper;

  @Mock private KoboReadingStateBuilder readingStateBuilder;

  @Mock private KoboSettingsService koboSettingsService;

  @InjectMocks private KoboEntitlementService koboEntitlementService;
  ...
  void getMetadataForBook_shouldUseCompatibilityServiceFilter() {
    ...
    when(bookQueryService.findAllWithMetadataByIds(Set.of(bookId))).thenReturn(List.of(cbxBook));
    when(koboCompatibilityService.isBookSupportedForKobo(cbxBook)).thenReturn(true);
  ...
  void generateTags_shouldReturnTagsForShelvesAndMagicShelves() {
    ...
    ShelfEntity koboShelf =
        ShelfEntity.builder()
            .id(100L)
            .name(ShelfType.KOBO.getName())
            .bookEntities(Set.of(book1, book2))
            .build();
    ShelfEntity userShelf =
        ShelfEntity.builder().id(101L).name("My Favorites").bookEntities(Set.of(book1)).build();

    MagicShelfEntity magicShelf =
        MagicShelfEntity.builder()
            .id(201L)
            .userId(user.getId())
            .name("Sci-Fi Books")
            .icon("pi-book")
    ...
    // Verify shelf tag
    KoboTagWrapper shelfTag =
        tags.stream()
            .filter(
                t ->
                    t.getChangedTag() != null
                        && t.getChangedTag().getTag().getId().equals("BL-S-101"))
            .findFirst()
            .orElseThrow();
    ...
    KoboTagWrapper magicShelfTag =
        tags.stream()
            .filter(
                t ->
                    t.getChangedTag() != null
                        && t.getChangedTag().getTag().getId().equals("BL-MS-201"))
            .findFirst()
            .orElseThrow();
  ...
  void generateTags_shouldReturnDeletedTagWhenNoMatchingBooks() {
    ...
    ShelfEntity userShelf =
        ShelfEntity.builder().id(101L).name("My Favorites").bookEntities(Set.of(book2)).build();
  ...
  void generateTags_shouldFilterBooksNotSupportedForKobo() {
    ...
    ShelfEntity koboShelf =
        ShelfEntity.builder()
            .id(100L)
            .name(ShelfType.KOBO.getName())
            .bookEntities(Set.of(supportedBook, unsupportedBook))
            .build();
    ShelfEntity userShelf =
        ShelfEntity.builder()
            .id(101L)
  ...
  void generateTags_shouldSetCorrectTagItemType() {
    ...
    ShelfEntity koboShelf =
        ShelfEntity.builder()
            ...
            .build();
    ShelfEntity userShelf =
        ShelfEntity.builder().id(101L).name("Reading").bookEntities(Set.of(book1)).build();
  ...
  void generateTags_shouldIncludeMultipleBooksInSingleTag() {
    ...
    ShelfEntity koboShelf =
        ShelfEntity.builder()
            .id(100L)
            .name(ShelfType.KOBO.getName())
            .bookEntities(Set.of(book1, book2, book3))
            .build();
    ShelfEntity userShelf =
        ShelfEntity.builder()
            .id(101L)
  ...
  class GenerateNewEntitlements {
    ...
    void generateNewEntitlements_supportedBooks() {
      ...
      when(readingStateRepository.findByEntitlementIdAndUserId(anyString(), any()))
          .thenReturn(Optional.empty());
      when(readingStateRepository
      ...
      List<NewEntitlement> result =
          koboEntitlementService.generateNewEntitlements(Set.of(1L), "token1");

      assertEquals(1, result.size());
      assertNotNull(result.getFirst().getNewEntitlement());
      assertNotNull(result.getFirst().getNewEntitlement().getBookMetadata());
      assertEquals(
          "Test EPUB Book 1", result.getFirst().getNewEntitlement().getBookMetadata().getTitle());
    }
    ...
    void generateNewEntitlements_emptyIds() {
      ...
      List<NewEntitlement> result =
          koboEntitlementService.generateNewEntitlements(Set.of(), "token1");
  ...
  class GenerateChangedEntitlements {
    ...
    void generateChangedEntitlements_removed() {
      ...
      List<? extends Entitlement> result =
          koboEntitlementService.generateChangedEntitlements(Set.of(1L), "token1", true);
    ...
    void generateChangedEntitlements_changed() {
      ...
      List<? extends Entitlement> result =
          koboEntitlementService.generateChangedEntitlements(Set.of(1L), "token1", false);
  ...
  class GenerateChangedReadingStates {
    ...
    void generateChangedReadingStates_fromProgress() {
      ...
      KoboReadingState.CurrentBookmark bookmark =
          KoboReadingState.CurrentBookmark.builder().progressPercent(50).build();
      ...
      List<ChangedReadingState> result =
          koboEntitlementService.generateChangedReadingStates(List.of(progress));
    ...
    void generateChangedReadingStates_emptyBookmark() {
      ...
      KoboReadingState.CurrentBookmark emptyBookmark =
          KoboReadingState.CurrentBookmark.builder().build();
      when(readingStateBuilder.buildEmptyBookmark(any(OffsetDateTime.class)))
          .thenReturn(emptyBookmark);
      when(readingStateBuilder.buildStatusInfoFromProgress(eq(progress), anyString()))
          .thenReturn(
              KoboReadingState.StatusInfo.builder()
    ...
    void generateChangedReadingStates_twoWaySync() {
      ...
      KoboReadingState.CurrentBookmark bookmark =
          KoboReadingState.CurrentBookmark.builder().progressPercent(70).build();
      when(readingStateBuilder.buildBookmarkFromProgress(eq(progress), any(OffsetDateTime.class)))
          .thenReturn(bookmark);
      when(readingStateBuilder.buildStatusInfoFromProgress(eq(progress), anyString()))
          .thenReturn(
