...
class CbxConversionServiceTest {
  ...
  @Test
  void convertCbxToEpub_WithValidCbzFile_ShouldGenerateValidEpub()
      throws IOException, TemplateException, RarException {
    File epubFile =
        cbxConversionService.convertCbxToEpub(testCbzFile, tempDir.toFile(), testBookEntity, 85);
  ...
  void convertCbxToEpub_WithNullCbxFile_ShouldThrowException() {
    assertThatThrownBy(
            () -> cbxConversionService.convertCbxToEpub(null, tempDir.toFile(), testBookEntity, 85))
  ...
  void convertCbxToEpub_WithNonExistentFile_ShouldThrowException() {
    ...
    assertThatThrownBy(
            () ->
                cbxConversionService.convertCbxToEpub(
                    nonExistentFile, tempDir.toFile(), testBookEntity, 85))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessageContaining("Invalid CBX file");
  }
  ...
  void convertCbxToEpub_WithUnsupportedFileFormat_ShouldThrowException() throws IOException {
    ...
    assertThatThrownBy(
            () ->
                cbxConversionService.convertCbxToEpub(
                    unsupportedFile, tempDir.toFile(), testBookEntity, 85))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void convertCbxToEpub_WithNullTempDir_ShouldThrowException() {
    assertThatThrownBy(
            () -> cbxConversionService.convertCbxToEpub(testCbzFile, null, testBookEntity, 85))
        .isInstanceOf(IllegalArgumentException.class)
  ...
  void convertCbxToEpub_WithEmptyCbzFile_ShouldThrowException() throws IOException {
    ...
    assertThatThrownBy(
            () ->
                cbxConversionService.convertCbxToEpub(
                    emptyCbzFile, tempDir.toFile(), testBookEntity, 85))
  ...
  @Test
  void convertCbxToEpub_WithNullBookEntity_ShouldUseDefaultMetadata()
      throws IOException, TemplateException, RarException {
  ...
  @Test
  void convertCbxToEpub_WithMultipleImages_ShouldPreservePageOrder()
      throws IOException, TemplateException, RarException {
    File multiPageCbzFile = createMultiPageCbzFile();

    File epubFile =
        cbxConversionService.convertCbxToEpub(
            multiPageCbzFile, tempDir.toFile(), testBookEntity, 85);
  ...
  @Test
  void convertCbxToEpub_WithZipNamedAsCbr_ShouldGenerateValidEpub()
      throws IOException, TemplateException, RarException {
    File zipAsCbr = new File(tempDir.toFile(), "fake.cbr");
    try (ZipArchiveOutputStream zipOut =
        new ZipArchiveOutputStream(new FileOutputStream(zipAsCbr))) {
    ...
    File epubFile =
        cbxConversionService.convertCbxToEpub(zipAsCbr, tempDir.toFile(), testBookEntity, 85);
  ...
  private File createTestCbzFile() throws IOException {
    ...
    try (ZipArchiveOutputStream zipOut =
        new ZipArchiveOutputStream(new FileOutputStream(cbzFile))) {
      BufferedImage testImage = createTestImage("Page 1", Color.RED);
  ...
  private void verifyPageOrderInEpub(File epubFile, int expectedPageCount) throws IOException {
    try (ZipFile zipFile = ZipFile.builder().setFile(epubFile).get()) {
      ...
      List<ZipArchiveEntry> htmlEntries =
          Collections.list(zipFile.getEntries()).stream()
