...
class KoboAutoShelfServiceTest {
  ...
  @InjectMocks private KoboAutoShelfService koboAutoShelfService;
  ...
  @BeforeEach
  void setUp() {
    testBook = BookEntity.builder().id(1L).shelves(new HashSet<>()).build();

    testUser1 = BookLoreUserEntity.builder().id(100L).isDefaultPassword(false).build();

    testUser2 = BookLoreUserEntity.builder().id(200L).isDefaultPassword(false).build();

    koboShelf1 =
        ShelfEntity.builder().id(10L).name(ShelfType.KOBO.getName()).user(testUser1).build();

    koboShelf2 =
        ShelfEntity.builder().id(20L).name(ShelfType.KOBO.getName()).user(testUser2).build();

    settings1 =
        KoboUserSettingsEntity.builder()
            .userId(100L)
            .autoAddToShelf(true)
            .syncEnabled(true)
            .build();
  ...
  void autoAddBookToKoboShelves_withNullShelves_shouldInitializeAndAdd() {
    testBook = BookEntity.builder().id(1L).shelves(null).build();
