...
class KoboLibrarySnapshotServiceTest {
  ...
  @Mock private KoboLibrarySnapshotRepository koboLibrarySnapshotRepository;

  @Mock private KoboSnapshotBookRepository koboSnapshotBookRepository;

  @Mock private ShelfRepository shelfRepository;

  @Mock private BookEntityToKoboSnapshotBookMapper mapper;

  @Mock private KoboDeletedBookProgressRepository koboDeletedBookProgressRepository;

  @Mock private KoboCompatibilityService koboCompatibilityService;

  @Mock private AuthenticationService authenticationService;

  @InjectMocks private KoboLibrarySnapshotService service;
  ...
  void setUp() {
    ...
    LibraryEntity ownersLibrary = LibraryEntity.builder().users(List.of(owner)).build();

    LibraryEntity othersLibrary = LibraryEntity.builder().users(List.of(otherUser)).build();

    ownersBook = BookEntity.builder().id(101L).library(ownersLibrary).build();
    ...
    otherUsersBook = BookEntity.builder().id(202L).library(othersLibrary).build();
    ...
    BookLoreUser mockUser =
        BookLoreUser.builder().id(owner.getId()).permissions(userPermissions).build();
  ...
  void create_shouldIncludeOnlyBooksOwnedBySnapshotUser() {
    ...
    doAnswer(
        ...
        .when(mapper)
        .toKoboSnapshotBook(any(BookEntity.class));

    ArgumentCaptor<KoboLibrarySnapshotEntity> snapshotCaptor =
        ArgumentCaptor.forClass(KoboLibrarySnapshotEntity.class);
    ...
    assertThat(created.getBooks())
        .extracting(KoboSnapshotBookEntity::getBookId)
