...
class CbxConversionIntegrationTest {
  ...
  void convertCbzToEpub_MainConversionTest() throws IOException, TemplateException, RarException {
    ...
    File epubFile =
        conversionService.convertCbxToEpub(testCbzFile, tempDir.toFile(), bookMetadata, 85);

    assertThat(epubFile).exists().hasExtension("epub");

    assertThat(epubFile.length()).as("EPUB file should not be empty").isGreaterThan(0);
  ...
  private void verifyEpubContents(File epubFile, BookEntity expectedMetadata) throws IOException {
    try (ZipFile zipFile = ZipFile.builder().setFile(epubFile).get()) {
      List<String> entryNames =
          Collections.list(zipFile.getEntries()).stream().map(ZipArchiveEntry::getName).toList();
      ...
      assertThat(imageCount).as("Should have converted all comic pages to images").isEqualTo(4);
  ...
  private void verifyMimetypeEntry(ZipFile zipFile) throws IOException {
    ...
    assertThat(mimetypeEntry).as("Mimetype entry should exist").isNotNull();
  ...
  private void verifyContentOpf(ZipFile zipFile, BookEntity expectedMetadata) throws IOException {
    ...
    assertThat(contentOpfEntry).as("Content.opf should exist").isNotNull();
