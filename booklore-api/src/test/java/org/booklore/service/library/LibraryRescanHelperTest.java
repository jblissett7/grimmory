...
class LibraryRescanHelperTest {
  ...
  void setUp() {
    ...
    LibraryRescanOptions options =
        LibraryRescanOptions.builder()
            .metadataReplaceMode(MetadataReplaceMode.REPLACE_ALL)
            .updateMetadataFromFiles(true)
            .build();

    rescanContext = RescanLibraryContext.builder().libraryId(1L).options(options).build();
  ...
  void handleRescanOptions_shouldThrowException_whenLibraryNotFound() {
    ...
    assertTrue(
        exception.getMessage().contains("LIBRARY_NOT_FOUND")
  ...
  void handleRescanOptions_shouldProcessAllBooks_whenLibraryHasBooks() {
    ...
    when(metadataExtractorFactory.extractMetadata(eq(BookFileType.EPUB), any(File.class)))
        .thenReturn(metadata1);
    when(metadataExtractorFactory.extractMetadata(eq(BookFileType.PDF), any(File.class)))
    ...
    verify(libraryRepository).findById(1L);
    verify(metadataExtractorFactory, times(2))
        .extractMetadata(any(BookFileType.class), any(File.class));
    verify(bookMetadataUpdater, times(2)).setBookMetadata(any(MetadataUpdateContext.class));
    verify(notificationService, times(4))
        .sendMessage(eq(Topic.TASK_PROGRESS), any(TaskProgressPayload.class));
  }
  ...
  void handleRescanOptions_shouldSkipDeletedBooks() {
    ...
    verify(metadataExtractorFactory, times(1))
        .extractMetadata(any(BookFileType.class), any(File.class));
  ...
  void handleRescanOptions_shouldContinue_whenMetadataUpdateThrowsException() {
    ...
    doThrow(new RuntimeException("Update failed"))
        .doNothing()
        .when(bookMetadataUpdater)
        .setBookMetadata(any(MetadataUpdateContext.class));
  ...
  void handleRescanOptions_shouldCancel_whenTaskCancellationRequested() {
    ...
    verify(notificationService, atLeastOnce())
        .sendMessage(eq(Topic.TASK_PROGRESS), payloadCaptor.capture());
  ...
  void handleRescanOptions_shouldContinue_whenNotificationFails() {
    ...
    doThrow(new RuntimeException("Notification failed"))
        ...
        .sendMessage(any(Topic.class), any(TaskProgressPayload.class));
  ...
  void handleRescanOptions_shouldUpdateAudiobookTechnicalMetadata() {
    ...
    AudiobookMetadata audiobookMeta =
    ...
    when(metadataExtractorFactory.extractMetadata(eq(BookFileType.AUDIOBOOK), any(File.class)))
        .thenReturn(metadata);
    when(cancellationManager.isTaskCancelled(taskId)).thenReturn(false);
