...
class BookReviewServiceTest {
  ...
  @Mock private BookReviewMapper mapper;
  ...
  private MetadataPublicReviewsSettings createReviewSettings(
      boolean enabled, MetadataProvider... providers) {
  ...
  void getByBookId_returnsEmptyList_whenUserLacksPermissions() {
    ...
    BookEntity bookEntity = new BookEntity();
    AppSettings appSettings = new AppSettings();
    appSettings.setMetadataPublicReviewsSettings(
        createReviewSettings(true, MetadataProvider.Amazon));
  ...
  void getByBookId_fetchesAndSavesReviews_whenAdminUserAndNoExistingReviews() {
    ...
    metadataMap.put(
        ...
        BookMetadata.builder().bookReviews(Collections.singletonList(freshReview)).build());
    ...
    verify(bookReviewUpdateService)
        .addReviewsToBook(Collections.singletonList(freshReview), bookEntity.getMetadata());
  ...
  void getByBookId_fetchesAndSavesReviews_whenLibraryManipulatorAndNoExistingReviews() {
    ...
    AppSettings appSettings = new AppSettings();
    appSettings.setMetadataPublicReviewsSettings(
        createReviewSettings(true, MetadataProvider.GoodReads));
  ...
  void getByBookId_throwsException_whenBookNotFound() {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> service.getByBookId(bookId));
  ...
  void fetchBookReviews_filtersEnabledProviders() {
    BookEntity bookEntity = new BookEntity();
    MetadataPublicReviewsSettings settings =
        createReviewSettings(true, MetadataProvider.Amazon, MetadataProvider.GoodReads);
  ...
  void fetchBookReviews_aggregatesReviewsFromMultipleProviders() {
    ...
    metadataMap.put(
        MetadataProvider.Amazon,
        BookMetadata.builder().bookReviews(Collections.singletonList(amazonReview)).build());
    metadataMap.put(
        MetadataProvider.GoodReads,
        BookMetadata.builder().bookReviews(Collections.singletonList(goodreadsReview)).build());
  ...
  void fetchBookReviews_skipsMetadataWithNullReviews() {
    ...
    Map<MetadataProvider, BookMetadata> metadataMap = new EnumMap<>(MetadataProvider.class);
    metadataMap.put(MetadataProvider.Amazon, BookMetadata.builder().bookReviews(null).build());
  ...
  void delete_throwsException_whenReviewNotFound() {
    ...
    EntityNotFoundException exception =
        assertThrows(EntityNotFoundException.class, () -> service.delete(reviewId));
