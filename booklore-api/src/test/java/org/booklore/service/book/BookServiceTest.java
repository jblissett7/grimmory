...
class BookServiceTest {
  ...
  void getBookDTOs_adminUser_returnsBooksWithProgress() {
    Book book =
        Book.builder()
            .id(1L)
            .primaryFile(BookFile.builder().bookType(BookFileType.PDF).build())
            .shelves(Set.of())
            .build();
    when(bookQueryService.getAllBooks(anyBoolean())).thenReturn(List.of(book));
    when(readingProgressService.fetchUserProgress(anyLong(), anySet()))
        .thenReturn(Map.of(1L, new UserBookProgressEntity()));
  ...
  void getBooksByIds_returnsMappedBooksWithProgress() {
    ...
    when(readingProgressService.fetchUserProgress(anyLong(), anySet()))
        .thenReturn(Map.of(2L, new UserBookProgressEntity()));
    Book mappedBook =
        Book.builder()
            .id(2L)
    ...
    try (MockedStatic<FileUtils> fileUtilsMock = mockStatic(FileUtils.class)) {
      fileUtilsMock
          .when(() -> FileUtils.getBookFullPath(entity))
          .thenReturn("/tmp/library/book.epub");
  ...
  void getBook_existingBook_returnsBookWithProgress() {
    ...
    when(userBookProgressRepository.findByUserIdAndBookId(anyLong(), eq(3L)))
    ...
    try (MockedStatic<FileUtils> fileUtilsMock = mockStatic(FileUtils.class)) {
      fileUtilsMock
          .when(() -> FileUtils.getBookFullPath(entity))
          .thenReturn("/tmp/library/book.pdf");
  ...
  void getBookViewerSetting_epub_returnsEpubSettings() {
    ...
    when(ebookViewerPreferenceRepository.findByBookIdAndUserId(4L, testUser.getId()))
  ...
  void getBookContent_fileNotFound_throwsException() {
    ...
    try (MockedStatic<FileUtils> fileUtilsMock = mockStatic(FileUtils.class)) {
      fileUtilsMock
          .when(() -> FileUtils.getBookFullPath(entity))
          .thenReturn("/tmp/nonexistentfile.txt");
