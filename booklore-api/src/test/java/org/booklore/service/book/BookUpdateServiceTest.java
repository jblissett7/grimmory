...
class BookUpdateServiceTest {
  ...
  @Mock private EbookViewerPreferenceRepository ebookViewerPreferenceRepository;
  ...
  void updateBookViewerSetting_pdf_shouldUpdatePdfPrefs() {
    ...
    when(pdfViewerPreferencesRepository.findByBookIdAndUserId(bookId, 2L))
        .thenReturn(Optional.of(pdfPrefs));
    BookViewerSettings settings =
        BookViewerSettings.builder()
            .pdfSettings(PdfViewerPreferences.builder().zoom("1.5").spread("spread").build())
            .build();
  ...
  void updateBookViewerSetting_epub_shouldUpdateEpubPrefsV2() {
    ...
    when(ebookViewerPreferenceRepository.findByBookIdAndUserId(bookId, 2L))
        .thenReturn(Optional.of(epubPrefs));
    BookViewerSettings settings =
        BookViewerSettings.builder()
            .ebookSettings(
  ...
  void updateBookViewerSetting_unsupportedType_shouldThrow() {
    ...
    assertThrows(
        APIException.class, () -> bookUpdateService.updateBookViewerSetting(bookId, settings));
  }
  ...
  void updateReadStatus_shouldUpdateExistingAndCreateNew() {
    ...
    when(userBookProgressRepository.findExistingProgressBookIds(1L, new HashSet<>(bookIds)))
        .thenReturn(existing);
    ...
    verify(userBookProgressRepository)
        .bulkUpdateReadStatus(
            eq(1L), eq(new ArrayList<>(existing)), eq(ReadStatus.READ), any(), any());
  ...
  void updatePersonalRating_shouldUpdateAndCreate() {
    ...
    verify(userBookProgressRepository)
        .bulkUpdatePersonalRating(eq(1L), eq(new ArrayList<>(existing)), eq(5));
    verify(userBookProgressRepository).saveAll(anyList());
    assertEquals(3, result.size());
  ...
  void resetPersonalRating_shouldUpdateExisting() {
    ...
    verify(userBookProgressRepository)
        .bulkUpdatePersonalRating(eq(1L), eq(new ArrayList<>(bookIds)), isNull());
  ...
  void assignShelvesToBooks_shouldAssignAndReturnBooks() {
    ...
    when(bookQueryService.findAllWithMetadataByIds(bookIds))
        .thenReturn(Arrays.asList(bookEntity1, bookEntity2));
    ...
    when(readingProgressService.fetchUserProgress(eq(1L), anySet()))
