...
class BookMarkServiceTest {
  ...
  @Mock private BookmarkProperties bookmarkProperties;
  ...
  void setUp() {
    ...
    bookmarkEntity =
        BookMarkEntity.builder()
            ...
            .build();
    bookmarkDto =
        BookMark.builder().id(bookmarkId).bookId(bookId).cfi("cfi").title("title").build();
  }

  @Test
  void getBookmarksForBook_Success() {
    when(authenticationService.getAuthenticatedUser()).thenReturn(userDto);
    when(bookMarkRepository.findByBookIdAndUserIdOrderByPriorityAscCreatedAtDesc(bookId, userId))
        .thenReturn(List.of(bookmarkEntity));
  ...
  void createBookmark_Success() {
    CreateBookMarkRequest request =
        CreateBookMarkRequest.builder().bookId(bookId).cfi("new-cfi").title("New Bookmark").build();

    when(authenticationService.getAuthenticatedUser()).thenReturn(userDto);
    when(bookmarkProperties.getDefaultPriority()).thenReturn(3);
    when(bookMarkRepository.existsByCfiAndBookIdAndUserId("new-cfi", bookId, userId))
        .thenReturn(false);
  ...
  void deleteBookmark_Success() {
    when(authenticationService.getAuthenticatedUser()).thenReturn(userDto);
    when(bookMarkRepository.findByIdAndUserId(bookmarkId, userId))
        .thenReturn(Optional.of(bookmarkEntity));
  ...
  void updateBookmark_NotFound() {
    var updateRequest = UpdateBookMarkRequest.builder().title("Updated Title").build();
