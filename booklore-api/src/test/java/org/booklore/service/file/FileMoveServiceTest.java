...
class FileMoveServiceTest {
  ...
  static class TestableFileMoveService extends FileMoveService {
    TestableFileMoveService(
        AppProperties appProperties,
        BookRepository bookRepository,
        BookAdditionalFileRepository bookFileRepository,
  ...
  private BookFileEntity createBookFile(
      Long id, String name, String subPath, boolean isBook, boolean folderBased) {
  ...
  private void mockStandardBehavior(BookEntity book, Path targetPath) throws IOException {
    ...
    when(fileMoveHelper.generateNewFilePath(
            eq(book), any(BookFileEntity.class), any(LibraryPathEntity.class), anyString()))
        .thenAnswer(
            inv -> {
              BookFileEntity bf = inv.getArgument(1);
              String ext =
                  bf.getFileName().contains(".")
                      ? bf.getFileName().substring(bf.getFileName().lastIndexOf('.'))
                      : "";
    ...
    when(fileMoveHelper.moveFileWithBackup(any()))
        .thenAnswer(inv -> ((Path) inv.getArgument(0)).resolveSibling("temp"));
  ...
  private void mockMonitoredLibrary() {
    ...
    when(monitoringRegistrationService.getPathsForLibraries(Set.of(1L)))
        .thenReturn(Set.of(Paths.get("/library")));
  ...
  class MoveSingleFileSuccess {
    ...
    void movesMultiFormatBook() throws IOException {
      ...
      verify(bookFileRepository, times(2))
          .updateFileNameAndSubPath(anyLong(), anyString(), anyString());
    }
    ...
    void movesBookWithMultipleAdditionalFiles() throws IOException {
      ...
      verify(bookFileRepository, times(4))
          .updateFileNameAndSubPath(anyLong(), anyString(), anyString());
    }
  ...
  class BulkMoveFilesSuccess {
    ...
    private void mockBulkMoveSetup(BookEntity book, Path targetPath) throws IOException {
      ...
      when(fileMoveHelper.generateNewFilePath(eq(book), eq(targetLibraryPath), anyString()))
          .thenReturn(targetPath);
      when(fileMoveHelper.generateNewFilePath(
              eq(book), any(BookFileEntity.class), eq(targetLibraryPath), anyString()))
          .thenAnswer(
              inv -> {
                BookFileEntity bf = inv.getArgument(1);
                String ext =
    ...
    void movesMultiFormatBook() throws IOException {
      ...
      verify(bookFileRepository, times(3))
          .updateFileNameAndSubPath(anyLong(), anyString(), anyString());
    }
  ...
  class BulkMoveFilesMonitoring {
    ...
    void unregistersAllLibraries() throws IOException {
      ...
      when(fileMoveHelper.getFileNamingPattern(targetLibrary)).thenReturn("{title}");
      when(fileMoveHelper.generateNewFilePath(eq(book), eq(targetPath), anyString()))
          .thenReturn(Paths.get("/target/new/NewName.epub"));
      when(fileMoveHelper.generateNewFilePath(
              eq(book), any(BookFileEntity.class), eq(targetPath), anyString()))
          .thenReturn(Paths.get("/target/new/NewName.epub"));
