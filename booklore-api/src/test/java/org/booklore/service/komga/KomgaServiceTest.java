...
class KomgaServiceTest {
  ...
  @Mock private KomgaMapper komgaMapper;

  @Mock private MagicShelfService magicShelfService;

  @Mock private CbxReaderService cbxReaderService;

  @Mock private AppSettingService appSettingService;

  @InjectMocks private KomgaService komgaService;
  ...
  void shouldReturnAllBooksWhenUnpagedIsTrue() {
    ...
    KomgaPageableDto<KomgaBookDto> result =
        komgaService.getBooksBySeries("1-test-series", 0, 20, true);
  ...
  void shouldReturnPagedBooksWhenUnpagedIsFalse() {
    ...
    KomgaPageableDto<KomgaBookDto> result =
        komgaService.getBooksBySeries("1-test-series", 0, 20, false);
  ...
  void shouldHandleNullPageCountInGetBookPages() {
    ...
    BookMetadataEntity metadata =
        BookMetadataEntity.builder().title("Test Book").pageCount(null).build();
  ...
  void shouldReturnCorrectPagesWhenPageCountIsValid() {
    ...
    BookMetadataEntity metadata =
        BookMetadataEntity.builder().title("Test Book").pageCount(5).build();
  ...
  void shouldGetAllSeriesOptimized() {
    ...
    when(komgaMapper.toKomgaSeriesDto(eq("Series A"), anyLong(), any()))
        .thenReturn(
            KomgaSeriesDto.builder().id("1-series-a").name("Series A").booksCount(2).build());
    when(komgaMapper.toKomgaSeriesDto(eq("Series B"), anyLong(), any()))
        .thenReturn(
            KomgaSeriesDto.builder().id("1-series-b").name("Series B").booksCount(2).build());
