...
class KoboReadingStateServiceTest {
  ...
  @Mock private KoboReadingStateRepository repository;

  @Mock private KoboReadingStateMapper mapper;
  ...
  @Mock private KoboSettingsService koboSettingsService;

  @Mock private KoboReadingStateBuilder readingStateBuilder;

  @Mock private HardcoverSyncService hardcoverSyncService;

  @Mock private UserBookFileProgressRepository fileProgressRepository;

  @InjectMocks private KoboReadingStateService service;
  ...
  @BeforeEach
  void setUp() {
    testUser = BookLoreUser.builder().id(1L).username("testuser").isDefaultPassword(true).build();
  ...
  void testSyncKoboProgressToUserBookProgress_PreserveExistingFinishedDate() {
    ...
    KoboReadingState.CurrentBookmark bookmark =
        KoboReadingState.CurrentBookmark.builder().progressPercent(100).build();

    KoboReadingState readingState =
        KoboReadingState.builder().entitlementId(entitlementId).currentBookmark(bookmark).build();
    ...
    when(progressRepository.findByUserIdAndBookId(1L, 100L))
    ...
    assertEquals(
        originalFinishedDate,
        savedProgress.getDateFinished(),
  ...
  void testSyncKoboProgressToUserBookProgress_IgnoreHardcoverUpdateWhenNoChange() {
    ...
    KoboReadingState.CurrentBookmark bookmark =
        KoboReadingState.CurrentBookmark.builder().progressPercent(12).build();
  ...
  void testSyncKoboProgressToUserBookProgress_BookNotFound() {
    ...
    KoboReadingState readingState =
        KoboReadingState.builder()
            .entitlementId(entitlementId)
            .currentBookmark(KoboReadingState.CurrentBookmark.builder().progressPercent(50).build())
            .build();
  ...
  void testGetReadingState_ConstructFromProgress() {
    ...
    KoboReadingState expectedState =
        KoboReadingState.builder()
            .entitlementId(entitlementId)
            .currentBookmark(
    ...
    when(readingStateBuilder.buildReadingStateFromProgress(entitlementId, progress))
    ...
    assertEquals(
        "epubcfi(/6/4[chap01ref]!/4/2/1:3)", state.getCurrentBookmark().getLocation().getValue());
  ...
  @Test
  @DisplayName(
      "Should return null when no Kobo reading state exists and UserBookProgress has no Kobo data")
  ...
  void testGetReadingState_ExistingState() {
    String entitlementId = "100";
    KoboReadingState existingState =
        KoboReadingState.builder().entitlementId(entitlementId).build();

    KoboReadingStateEntity entity = new KoboReadingStateEntity();
    when(repository.findByEntitlementIdAndUserId(entitlementId, 1L))
  ...
  void testSyncKoboProgressToUserBookProgress_NullBookmark() {
    String entitlementId = "100";
    KoboReadingState readingState =
        KoboReadingState.builder().entitlementId(entitlementId).currentBookmark(null).build();
    ...
    ArgumentCaptor<UserBookProgressEntity> progressCaptor =
        ArgumentCaptor.forClass(UserBookProgressEntity.class);
    when(progressRepository.save(progressCaptor.capture()))
        .thenReturn(new UserBookProgressEntity());
  ...
  void testSyncKoboProgressToUserBookProgress_NullProgressPercent() {
    String entitlementId = "100";
    KoboReadingState.CurrentBookmark bookmark =
        KoboReadingState.CurrentBookmark.builder().progressPercent(null).build();
  ...
  void testSaveReadingState_PerFieldMerge() throws Exception {
    ...
    KoboReadingState.StatusInfo incomingStatus =
        KoboReadingState.StatusInfo.builder()
            .lastModified(newerTimestamp)
            .status(KoboReadStatus.FINISHED)
            .timesStartedReading(2)
            .build();
    KoboReadingState.CurrentBookmark incomingBookmark =
        KoboReadingState.CurrentBookmark.builder()
            .lastModified(olderTimestamp)
            .progressPercent(10)
            .build();

    KoboReadingState incomingState =
        KoboReadingState.builder()
            .entitlementId(entitlementId)
            .lastModified(newerTimestamp)
    ...
    ArgumentCaptor<KoboReadingStateEntity> entityCaptor =
        ArgumentCaptor.forClass(KoboReadingStateEntity.class);
    service.saveReadingState(List.of(incomingState));
    verify(repository).save(entityCaptor.capture());

    KoboReadingStateEntity saved = entityCaptor.getValue();
    ObjectMapper objectMapper = new ObjectMapper();
    KoboReadingState.StatusInfo savedStatus =
        objectMapper.readValue(saved.getStatusInfoJson(), KoboReadingState.StatusInfo.class);
    KoboReadingState.CurrentBookmark savedBookmark =
        objectMapper.readValue(
            saved.getCurrentBookmarkJson(), KoboReadingState.CurrentBookmark.class);
    KoboReadingState.Statistics savedStatistics =
        objectMapper.readValue(saved.getStatisticsJson(), KoboReadingState.Statistics.class);
  ...
  @Test
  @DisplayName(
      "Should overlay web reader bookmark when two-way sync is ON and web reader data exists")
  void testGetReadingState_overlayWebReaderBookmark() {
    ...
    KoboReadingState.CurrentBookmark webBookmark =
        KoboReadingState.CurrentBookmark.builder()
            .progressPercent(65)
  ...
  void testSyncKoboProgress_noCrossPopulateWhenToggleOff() {
    ...
    KoboReadingState readingState =
        KoboReadingState.builder()
            .entitlementId(entitlementId)
            .currentBookmark(KoboReadingState.CurrentBookmark.builder().progressPercent(70).build())
            .build();
  ...
  void testSyncKoboProgress_skipCrossPopulateWhenWebReaderNewer() {
    ...
    when(progressRepository.findByUserIdAndBookId(1L, 100L))
        .thenReturn(Optional.of(existingProgress));
    when(fileProgressRepository.findByUserIdAndBookFileId(1L, 10L))
  ...
  @Test
  @DisplayName(
      "Should construct reading state from web reader progress when toggle ON and no Kobo data")
  void testGetReadingState_constructFromWebReaderProgress_whenToggleOn() {
    ...
    KoboReadingState expectedState =
        KoboReadingState.builder()
            .entitlementId(entitlementId)
            .currentBookmark(KoboReadingState.CurrentBookmark.builder().progressPercent(60).build())
            .build();
  ...
  void testSyncKoboProgress_configurableThresholds() {
    ...
    KoboReadingState readingState =
        KoboReadingState.builder()
            .entitlementId(entitlementId)
            .currentBookmark(KoboReadingState.CurrentBookmark.builder().progressPercent(5).build())
            .build();
