...
class BookNoteV2ServiceTest {
  ...
  void setUp() {
    ...
    service =
        new BookNoteV2Service(
            bookNoteV2Repository, bookRepository, userRepository, authenticationService, mapper);
  ...
  void getNoteById_throwsEntityNotFoundException_whenNotFound() {
    ...
    EntityNotFoundException ex =
        assertThrows(EntityNotFoundException.class, () -> service.getNoteById(noteId));
  ...
  void createNote_createsNewNote_withDefaultColor() {
    CreateBookNoteV2Request request =
        CreateBookNoteV2Request.builder()
            .bookId(bookId)
            .cfi(cfi)
    ...
    BookLoreUserEntity userEntity =
        BookLoreUserEntity.builder().id(userId).isDefaultPassword(false).build();
    BookNoteV2Entity savedEntity =
        BookNoteV2Entity.builder()
            .id(noteId)
            .cfi(cfi)
            .selectedText(selectedText)
            .noteContent(noteContent)
  ...
  void createNote_createsNewNote_withCustomColor() {
    ...
    BookLoreUserEntity userEntity =
        BookLoreUserEntity.builder().id(userId).isDefaultPassword(false).build();
    BookNoteV2Entity savedEntity = BookNoteV2Entity.builder().id(noteId).color(customColor).build();
  ...
  void createNote_throwsAPIException_whenDuplicateCfiExists() {
    CreateBookNoteV2Request request =
        CreateBookNoteV2Request.builder().bookId(bookId).cfi(cfi).noteContent(noteContent).build();
  ...
  void createNote_throwsEntityNotFoundException_whenBookNotFound() {
    ...
    EntityNotFoundException ex =
        assertThrows(EntityNotFoundException.class, () -> service.createNote(request));
  ...
  void updateNote_updatesAllFields() {
    UpdateBookNoteV2Request request =
        UpdateBookNoteV2Request.builder()
            .noteContent("Updated content")
            .color("#00FF00")
            .chapterTitle("Updated chapter")
            .build();

    BookNoteV2Entity existingEntity =
        BookNoteV2Entity.builder()
            .id(noteId)
            .cfi(cfi)
            .noteContent(noteContent)
    ...
    when(bookNoteV2Repository.findByIdAndUserId(noteId, userId))
  ...
  void updateNote_updatesOnlyNonNullFields() {
    UpdateBookNoteV2Request request =
        UpdateBookNoteV2Request.builder().noteContent("Updated content").build();
  ...
  void updateNote_throwsEntityNotFoundException_whenNoteNotFound() {
    ...
    EntityNotFoundException ex =
        assertThrows(EntityNotFoundException.class, () -> service.updateNote(noteId, request));
