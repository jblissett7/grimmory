...
class TaskServiceTest {
  ...
  void testRunAsUserSyncTask() {
    ...
    when(mockTask.execute(any()))
        .thenReturn(TaskCreateResponse.builder().taskType(TaskType.CLEANUP_TEMP_METADATA).build());
  ...
  void testExecuteTaskThrowsForUnknownTaskType() {
    TaskCreateRequest req =
        TaskCreateRequest.builder()
            .taskType(TaskType.CLEANUP_DELETED_BOOKS)
            .triggeredByCron(false)
            .build();
    BookLoreUser user = new BookLoreUser();
    user.setId(1L);
  ...
  void testParallelTaskAllowsMultipleRuns() {
    ...
    when(mockTask.execute(any()))
        .thenReturn(TaskCreateResponse.builder().taskType(parallelType).build());
    BookLoreUser user = new BookLoreUser();
    user.setId(2L);
    user.setUsername("parallelUser");
    when(authenticationService.getAuthenticatedUser()).thenReturn(user);

    TaskCreateRequest req1 =
        TaskCreateRequest.builder().taskType(parallelType).triggeredByCron(false).build();
    TaskCreateRequest req2 =
        TaskCreateRequest.builder().taskType(parallelType).triggeredByCron(false).build();
  ...
  void testNonParallelTaskBlocksSecondRunAsUser() {
    ...
    when(nonParallelTask.execute(any()))
        .thenReturn(TaskCreateResponse.builder().taskType(nonParallelType).build());
    ...
    TaskCreateRequest req =
        TaskCreateRequest.builder().taskType(nonParallelType).triggeredByCron(false).build();
  ...
  void testAsyncTaskReturnsAcceptedStatus() {
    ...
    when(asyncTask.execute(any()))
        .thenReturn(TaskCreateResponse.builder().taskType(asyncType).build());
    ...
    TaskCreateRequest req =
        TaskCreateRequest.builder().taskType(asyncType).triggeredByCron(false).build();
    TaskCreateResponse resp = taskService.runAsUser(req);
  ...
  void testNullOptionsHandledGracefully() {
    ...
    TaskCreateRequest req =
        TaskCreateRequest.builder().taskType(type).options(null).triggeredByCron(false).build();
    TaskCreateResponse resp = taskService.runAsUser(req);
  ...
  void testExceptionInTaskExecutionPropagates() {
    ...
    TaskCreateRequest req =
        TaskCreateRequest.builder().taskType(type).triggeredByCron(false).build();
  ...
  void testRescheduleTaskDoesNotThrowWhenEnabled() {
    CronConfig cronConfig =
        CronConfig.builder()
            ...
            .build();
    when(taskCronService.getCronConfigOrDefault(TaskType.CLEANUP_TEMP_METADATA))
        .thenReturn(cronConfig);
