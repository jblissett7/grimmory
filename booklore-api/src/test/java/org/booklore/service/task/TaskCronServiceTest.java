...
class TaskCronServiceTest {
  ...
  @Mock private TaskCronConfigurationRepository repository;

  @Mock private AuthenticationService authService;

  @InjectMocks private TaskCronService service;
  ...
  void testGetAllEnabledCronConfigs_returnsList() {
    List<TaskCronConfigurationEntity> configs =
        List.of(buildEntity(TaskType.CLEANUP_DELETED_BOOKS, "0 0 1 * * *", true));
  ...
  void testValidateCronExpression_valid() {
    ...
    assertDoesNotThrow(
        () -> {
          var method =
              TaskCronService.class.getDeclaredMethod("validateCronExpression", String.class);
  ...
  void testValidateTaskTypeForCron_supported() {
    ...
    assertDoesNotThrow(
        () -> {
          var method =
              TaskCronService.class.getDeclaredMethod("validateTaskTypeForCron", TaskType.class);
          method.setAccessible(true);
  ...
  void testValidateTaskTypeForCron_notSupported() {
    ...
    Exception ex =
        assertThrows(
            ...
            () -> {
              var method =
                  TaskCronService.class.getDeclaredMethod(
                      "validateTaskTypeForCron", TaskType.class);
              method.setAccessible(true);
              method.invoke(service, type);
            });
    Throwable cause = ex.getCause();
