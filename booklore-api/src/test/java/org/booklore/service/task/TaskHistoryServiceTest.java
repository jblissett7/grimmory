...
class TaskHistoryServiceTest {
  ...
  void testGetLatestTasksForEachType_success() {
    ...
    assertTrue(
        histories.stream()
            .anyMatch(
                h ->
                    TaskType.REFRESH_LIBRARY_METADATA.equals(h.getType())
                        && "t1".equals(h.getId())));
    assertTrue(
        histories.stream()
            .anyMatch(
                h -> TaskType.SYNC_LIBRARY_FILES.equals(h.getType()) && "t2".equals(h.getId())));
    assertFalse(
        histories.stream().anyMatch(h -> h.getType() != null && h.getType().isHiddenFromUI()));
  ...
  void testGetLatestTasksForEachType_skipsInvalidType() {
    ...
    when(taskHistoryRepository.findLatestTaskForEachType())
        .thenReturn(Collections.singletonList(invalidTask));

    TasksHistoryResponse response = taskHistoryService.getLatestTasksForEachType();
    assertNotNull(response);
    assertTrue(
        response.getTaskHistories().stream()
            .allMatch(h -> h.getId() == null || h.getType() != null));
  }
  ...
  void testGetLatestTasksForEachType_allTypesHidden() {
    ...
    TaskHistoryEntity dummyTask =
        TaskHistoryEntity.builder()
            ...
            .build();
    when(taskHistoryRepository.findLatestTaskForEachType())
        .thenReturn(Collections.singletonList(dummyTask));
  ...
  void testBuildTaskDescription_largeBookIdsListTruncated() {
    Set<Long> ids =
        LongStream.rangeClosed(1, 500).boxed().collect(Collectors.toCollection(LinkedHashSet::new));
    Map<String, Object> options = new HashMap<>();
