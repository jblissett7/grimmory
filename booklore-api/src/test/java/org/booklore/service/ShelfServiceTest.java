...
class ShelfServiceTest {
  ...
  void createShelf_withNullIcon_shouldPersistNullIconValues() {
    ShelfCreateRequest request =
        ShelfCreateRequest.builder().name("My Shelf").icon(null).iconType(null).build();
    ...
    when(shelfMapper.toShelf(any(ShelfEntity.class)))
        .thenReturn(Shelf.builder().name("My Shelf").build());
  ...
  void updateShelf_withNullIcon_shouldClearIconValues() {
    ...
    ShelfCreateRequest request =
        ShelfCreateRequest.builder().name("Updated Shelf").icon(null).iconType(null).build();

    when(shelfRepository.findById(1L)).thenReturn(Optional.of(existingShelf));
    when(shelfRepository.save(any(ShelfEntity.class)))
        .thenAnswer(invocation -> invocation.getArgument(0));
    when(shelfMapper.toShelf(any(ShelfEntity.class)))
        .thenReturn(Shelf.builder().name("Updated Shelf").build());
