...
class KoreaderServiceTest {
  ...
  @InjectMocks KoreaderService service;
  ...
  void authorizeUser_success() {
    ...
    when(koreaderUserRepo.findByUsername("u")).thenReturn(Optional.of(userEntity));
    when(details.getPassword()).thenReturn("MD5PWD");
  ...
  void getProgress_success() {
    ...
    when(progressRepo.findByUserIdAndBookId(42L, 99L)).thenReturn(Optional.of(prog));
  ...
  void getProgress_noProgress() {
    when(details.isSyncEnabled()).thenReturn(true);
    when(bookRepo.findByCurrentHash("h")).thenReturn(Optional.of(new BookEntity()));
    when(progressRepo.findByUserIdAndBookId(anyLong(), isNull())).thenReturn(Optional.empty());
    assertThrows(APIException.class, () -> service.getProgress("h"));
  }
  ...
  void getProgress_includesTimestamp() {
    ...
    when(progressRepo.findByUserIdAndBookId(42L, 100L)).thenReturn(Optional.of(prog));
  ...
  void getProgress_nullTimestamp() {
    ...
    when(progressRepo.findByUserIdAndBookId(42L, 101L)).thenReturn(Optional.of(prog));
  ...
  void saveProgress_createsNew() {
    ...
    when(progressRepo.findByUserIdAndBookId(42L, 7L)).thenReturn(Optional.empty());
    ...
    ArgumentCaptor<UserBookProgressEntity> cap =
  ...
  void normalizeProgressPercent_handlesNullAndRanges() throws Exception {
    Method method =
        KoreaderService.class.getDeclaredMethod("normalizeProgressPercent", Float.class);
