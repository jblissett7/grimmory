...
class KoboStatusSyncProtectionTest {
  ...
  class PendingSyncProtection {
    ...
    @Test
    @DisplayName(
        "User marks book READ in BookLore -> Kobo sends 50% progress before sync completes -> READ status preserved until synced")
    ...
    @Test
    @DisplayName(
        "User marks book UNREAD in BookLore -> Kobo sends 100% progress before sync -> UNREAD preserved until synced")
    ...
    @Test
    @DisplayName(
        "User changes status AFTER last sync -> new status preserved until next sync completes")
    void preserveStatus_WhenModifiedAfterSent() {
      ...
      KoboReadingState readingState =
          KoboReadingState.builder()
              .entitlementId("100")
              .currentBookmark(
                  KoboReadingState.CurrentBookmark.builder().progressPercent(75).build())
              .build();
  ...
  class BufferWindowProtection {
    ...
    @Test
    @DisplayName(
        "Status synced 5s ago -> Kobo sends progress -> preserve status (handles rapid Kobo requests)")
    ...
    @Test
    @DisplayName(
        "Status synced 15s ago -> buffer expired -> Kobo progress can now update status (e.g., re-reading)")
    ...
    void setUnread_ForLowProgress() {
      ...
      KoboReadingState readingState =
          KoboReadingState.builder()
              .entitlementId("100")
              .currentBookmark(
                  KoboReadingState.CurrentBookmark.builder().progressPercent(1).build())
              .build();
  ...
  class MultipleRapidRequests {
    ...
    void handleMultipleRapidRequests() {
      ...
      // First request
      KoboReadingState firstRequest =
          KoboReadingState.builder()
              .entitlementId("100")
              .currentBookmark(
                  KoboReadingState.CurrentBookmark.builder().progressPercent(52).build())
              .build();
      ...
      KoboReadingState secondRequest =
          KoboReadingState.builder()
              .entitlementId("100")
              .currentBookmark(
                  KoboReadingState.CurrentBookmark.builder().progressPercent(55).build())
              .build();
