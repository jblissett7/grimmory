...
class FileStreamingServiceTest {
  ...
  void streamWithRangeSupport_noRangeHeader_streamsFullFile() throws IOException {
    ...
    fileStreamingService.streamWithRangeSupport(
        testFile, "application/octet-stream", request, response);
  ...
  @ParameterizedTest
  @CsvSource({"0, 0", "0, 9999", "5000, 5000", "9999, 9999"})
  void streamWithRangeSupport_variousValidRanges_streamsCorrectContent(int start, int end)
      throws IOException {
    HttpServletRequest request = mock(HttpServletRequest.class);
    HttpServletResponse response = mock(HttpServletResponse.class);
    ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
