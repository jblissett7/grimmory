...
class AppSeriesControllerTest {
  ...
  @Mock private AppSeriesService mobileSeriesService;

  @InjectMocks private AppSeriesController controller;
  ...
  void getSeriesBooks_encodedSeriesName_passedAsIs() {
    AppPageResponse<AppBookSummary> expected = buildBookPage();
    when(mobileSeriesService.getSeriesBooks(
            "A Song of Ice and Fire", 0, 20, "seriesNumber", "asc", null))
        .thenReturn(expected);
    ...
    assertEquals(200, response.getStatusCode().value());
    verify(mobileSeriesService)
        .getSeriesBooks("A Song of Ice and Fire", 0, 20, "seriesNumber", "asc", null);
  }

  // ---- Helpers ----

  private AppPageResponse<AppSeriesSummary> buildSeriesPage() {
    AppSeriesSummary summary =
