...
public class OidcTokenValidator {
  ...
  private final ConcurrentMap<String, ConfigurableJWTProcessor<SecurityContext>> processorCache =
      new ConcurrentHashMap<>();

  public JWTClaimsSet validateIdToken(
      String idTokenStr,
      String issuerUri,
      String clientId,
  ...
  public JWTClaimsSet validateIdToken(
      String idTokenStr, String issuerUri, String clientId, String expectedNonce) {
  ...
  private ConfigurableJWTProcessor<SecurityContext> buildProcessor(String issuerUri) {
    ...
    var jwkSetCache =
        new DefaultJWKSetCache(JWKS_CACHE_TTL_MS, JWKS_REFRESH_MS, TimeUnit.MILLISECONDS);
    JWKSource<SecurityContext> jwkSource =
        new RemoteJWKSet<>(uriToUrl(jwksUri), resourceRetriever, jwkSetCache);
  ...
  private void validateAudience(JWTClaimsSet claims, String clientId) {
    if (claims.getAudience() == null || !claims.getAudience().contains(clientId)) {
      throw ApiError.OIDC_INVALID_TOKEN.createException(
  ...
  public JWTClaimsSet validateLogoutToken(
      String logoutTokenStr, String issuerUri, String clientId) {
    try {
      var processor = getOrCreateProcessor(issuerUri);
      ...
      if (events == null
        ...
        throw ApiError.GENERIC_BAD_REQUEST.createException(
            "Logout token missing backchannel-logout event claim");
      }
