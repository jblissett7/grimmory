...
public class AuthRateLimitService {
  ...
  public AuthRateLimitService(AuditService auditService) {
    ...
    this.attemptCache =
        Caffeine.newBuilder().maximumSize(10000).expireAfterWrite(Duration.ofMinutes(15)).build();
  }
  ...
  public void checkLoginRateLimit(String ip) {
    checkRateLimit(
        "login:ip:" + ip, AuditAction.LOGIN_RATE_LIMITED, "Login rate limited for IP: " + ip);
  }
  ...
  public void checkLoginRateLimitByUsername(String username) {
    ...
    checkRateLimit(
        ...
        "Login rate limited for username: " + normalizedUsername);
  }
  ...
  public void checkRefreshRateLimit(String ip) {
    checkRateLimit(
        "refresh:" + ip, AuditAction.REFRESH_RATE_LIMITED, "Refresh rate limited for IP: " + ip);
  }
