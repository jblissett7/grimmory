...
public class SecurityConfig {
  ...
  private static final String[] COMMON_PUBLIC_ENDPOINTS = {
    ...
    "/api/v1/healthcheck/**" // Healthcheck endpoints (must remain accessible for Docker
    // healthchecks)
  };

  private static final String[] COMMON_UNAUTHENTICATED_ENDPOINTS = {
    "/api/v1/opds/search.opds", "/api/v2/opds/search.opds"
  };
  ...
  public SecurityFilterChain opdsBasicAuthSecurityChain(HttpSecurity http) throws Exception {
    List<String> unauthenticatedEndpoints =
        new ArrayList<>(Arrays.asList(COMMON_UNAUTHENTICATED_ENDPOINTS));
    http.securityMatcher("/api/v1/opds/**", "/api/v2/opds/**")
        .csrf(AbstractHttpConfigurer::disable)
        .sessionManagement(
            session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
        .authorizeHttpRequests(
            auth ->
                auth.requestMatchers(unauthenticatedEndpoints.toArray(new String[0]))
                    .permitAll()
                    .anyRequest()
                    .authenticated())
        .httpBasic(
                ...
                basic
                    ...
                    .authenticationEntryPoint(
                        (request, response, authException) -> {
                          ...
                          response
                              .getWriter()
                              .write("HTTP Status 401 - " + authException.getMessage());
                        }));
  ...
  public SecurityFilterChain komgaBasicAuthSecurityChain(HttpSecurity http) throws Exception {
    http.securityMatcher("/komga/api/v1/**", "/komga/api/v2/**")
        .csrf(AbstractHttpConfigurer::disable)
        .sessionManagement(
            session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
                ...
                basic
                    ...
                    .authenticationEntryPoint(
                        (request, response, authException) -> {
                          response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                          response.setHeader(
                              "WWW-Authenticate", "Basic realm=\"Booklore Komga API\"");
                          response
                              .getWriter()
                              .write("HTTP Status 401 - " + authException.getMessage());
                        }));
  ...
  @Bean
  @Order(3)
  public SecurityFilterChain koreaderSecurityChain(
      HttpSecurity http, KoreaderAuthFilter koreaderAuthFilter) throws Exception {
  ...
  @Bean
  @Order(4)
  public SecurityFilterChain coverJwtApiSecurityChain(
      HttpSecurity http, CoverJwtFilter coverJwtFilter) throws Exception {
    http.securityMatcher("/api/v1/media/**")
        .csrf(AbstractHttpConfigurer::disable)
        .sessionManagement(
            session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
  ...
  @Bean
  @Order(5)
  public SecurityFilterChain customFontSecurityChain(
      HttpSecurity http, CustomFontJwtFilter customFontJwtFilter) throws Exception {
    http.securityMatcher("/api/v1/custom-fonts/*/file")
  ...
  @Bean
  @Order(6)
  public SecurityFilterChain epubStreamingSecurityChain(
      HttpSecurity http, EpubStreamingJwtFilter epubStreamingJwtFilter) throws Exception {
    http.securityMatcher("/api/v1/epub/*/file/**")
        .cors(cors -> cors.configurationSource(corsConfigurationSource()))
        .csrf(AbstractHttpConfigurer::disable)
        .sessionManagement(
            session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
  ...
  @Bean
  @Order(7)
  public SecurityFilterChain audiobookStreamingSecurityChain(
      HttpSecurity http, AudiobookStreamingJwtFilter audiobookStreamingJwtFilter) throws Exception {
  ...
  public SecurityFilterChain jwtApiSecurityChain(HttpSecurity http) throws Exception {
    ...
    http.securityMatcher("/api/**", "/komga/**", "/ws/**")
        .cors(cors -> cors.configurationSource(corsConfigurationSource()))
        .csrf(AbstractHttpConfigurer::disable)
        .sessionManagement(
            session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))
  ...
  public CorsConfigurationSource corsConfigurationSource() {
    ...
    if ("*".equals(allowedOriginsStr) || allowedOriginsStr.isEmpty()) {
      ...
      List<String> origins =
          Arrays.stream(ALLOWED.split(allowedOriginsStr))
              .filter(s -> !s.isEmpty())
              .map(String::trim)
              .toList();
    ...
    configuration.setAllowedHeaders(
        List.of("Authorization", "Cache-Control", "Content-Type", "Range", "If-None-Match"));
