...
public class AuthenticationService {
  ...
  public OpdsUserDetails getOpdsUser() {
    Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
    if (authentication != null
        && authentication.getPrincipal() instanceof OpdsUserDetails opdsUser) {
      return opdsUser;
    }
  ...
  public ResponseEntity<Map<String, String>> loginUser(UserLoginRequest loginRequest) {
    if (appSettingService.getAppSettings().isOidcForceOnlyMode()) {
      BookLoreUserEntity oidcCheckUser =
  ...
  public ResponseEntity<Map<String, String>> loginRemote(
    ...
    if (user.isEmpty() && appProperties.getRemoteAuth().isCreateNewUsers()) {
      user =
          Optional.of(
              userProvisioningService.provisionRemoteUserFromHeaders(
                  name, username, email, groups));
    }

    if (user.isEmpty()) {
      throw ApiError.INTERNAL_SERVER_ERROR.createException(
          "User not found and remote user creation is disabled");
    }
  ...
  public ResponseEntity<Map<String, String>> loginUser(
      BookLoreUserEntity user, Long customRefreshTokenExpirationMs) {
    String accessToken = jwtUtils.generateAccessToken(user);
    String refreshToken = jwtUtils.generateRefreshToken(user);

    long expirationMs =
        customRefreshTokenExpirationMs != null
            ? customRefreshTokenExpirationMs
  ...
  public ResponseEntity<Map<String, String>> refreshToken(String token) {
    ...
    return ResponseEntity.ok(
        Map.of("accessToken", jwtUtils.generateAccessToken(user), "refreshToken", newRefreshToken));
  }
}
