...
public class LibraryAccessAspect {
  ...
  public void checkLibraryAccess(JoinPoint joinPoint) {
    ...
    Long libraryId =
        extractLibraryId(
            methodSignature.getParameterNames(), joinPoint.getArgs(), annotation.libraryIdParam());

    if (libraryId == null) {
      throw ApiError.GENERIC_BAD_REQUEST.createException(
          "Library ID not found in method parameters.");
    }
    ...
    boolean hasAccess =
        user.getAssignedLibraries().stream().anyMatch(lib -> lib.getId().equals(libraryId));
