...
public class BackchannelLogoutService {
  ...
  private static final Cache<String, Instant> PROCESSED_JTIS =
  ...
  public void handleLogoutToken(String logoutToken) {
    ...
    JWTClaimsSet claims =
        oidcTokenValidator.validateLogoutToken(
    ...
    if (sid != null) {
      ...
      sessions =
          oidcSessionRepository.findByOidcSubjectAndOidcIssuerAndRevokedFalse(
              sub, providerDetails.getIssuerUri());
    }
    ...
    for (OidcSessionEntity session : sessions) {
      ...
      notificationService.sendMessageToUser(
          user.getUsername(), Topic.SESSION_REVOKED, Map.of("reason", "backchannel_logout"));

      auditService.log(
          AuditAction.BACKCHANNEL_LOGOUT,
          "User",
          user.getId(),
