...
public class KoboAuthFilter extends OncePerRequestFilter {
  ...
  private List<GrantedAuthority> getAuthorities(UserPermissionsEntity permissions) {
    ...
    if (permissions != null) {
      ...
      addAuthorityIfPermissionGranted(
          authorities, "ROLE_DOWNLOAD", permissions.isPermissionDownload());
      addAuthorityIfPermissionGranted(
          authorities, "ROLE_EDIT_METADATA", permissions.isPermissionEditMetadata());
      addAuthorityIfPermissionGranted(
          authorities, "ROLE_MANAGE_LIBRARY", permissions.isPermissionManageLibrary());
      addAuthorityIfPermissionGranted(authorities, "ROLE_ADMIN", permissions.isPermissionAdmin());
      addAuthorityIfPermissionGranted(
          authorities, "ROLE_SYNC_KOBO", permissions.isPermissionSyncKobo());
    }
    return authorities;
  }

  private void addAuthorityIfPermissionGranted(
      List<GrantedAuthority> authorities, String role, boolean permissionGranted) {
