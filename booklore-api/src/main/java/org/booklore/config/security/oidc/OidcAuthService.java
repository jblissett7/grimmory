...
public class OidcAuthService {
  ...
  public ResponseEntity<Map<String, String>> exchangeCodeForTokens(
    ...
    OidcProviderDetails providerDetails =
        appSettingService.getAppSettings().getOidcProviderDetails();
    ...
    var tokenResponse =
        oidcTokenClient.exchangeAuthorizationCode(code, codeVerifier, redirectUri, providerDetails);
    ...
    if (tokenResponse.accessToken() != null) {
      userInfo =
          oidcTokenClient.fetchUserInfo(
              tokenResponse.accessToken(), providerDetails.getIssuerUri());
    }

    var userClaims =
        oidcClaimExtractor.extractClaims(claims, providerDetails.getClaimMapping(), userInfo);
    ...
    persistOidcSession(
        user, userClaims.subject(), providerDetails.getIssuerUri(), tokenResponse, idToken, claims);
    ...
    auditService.log(
        AuditAction.OIDC_LOGIN_SUCCESS,
        "User",
        user.getId(),
        "OIDC login successful for user: " + user.getUsername());
    return response;
  }
  ...
  private void validateRedirectUri(String redirectUri, HttpServletRequest httpRequest) {
    ...
    try {
      ...
      if (requestOrigin != null && !requestOrigin.equals(redirectOrigin)) {
        log.warn(
            "OIDC redirect URI origin mismatch: redirect={}, request={}",
            redirectOrigin,
            requestOrigin);
        throw ApiError.OIDC_INVALID_REDIRECT_URI.createException();
      }
  ...
  private void persistOidcSession(
      BookLoreUserEntity user,
      String subject,
      String issuer,
      OidcTokenClient.TokenResponse tokenResponse,
      String rawIdToken,
      JWTClaimsSet claims) {
  ...
  private BookLoreUserEntity findOrProvisionUser(
      OidcClaimExtractor.OidcUserClaims userClaims, String issuerUri) {
    ...
    boolean allowLocalLinking =
        provisionDetails != null && provisionDetails.isAllowLocalAccountLinking();

    return userRepository
        .findByOidcIssuerAndOidcSubject(issuerUri, userClaims.subject())
        .map(user -> syncExistingUser(user, userClaims, issuerUri))
        .or(
            () ->
                userRepository
                    .findByUsername(userClaims.username())
                    .filter(
                        user ->
                            user.getProvisioningMethod() == ProvisioningMethod.OIDC
                                || allowLocalLinking)
        ...
        .orElseGet(
            () -> {
              if (!autoProvision) {
                log.warn(
                    "OIDC user '{}' not found and auto-provisioning is disabled",
                    userClaims.username());
                throw ApiError.OIDC_USER_NOT_PROVISIONED.createException(userClaims.username());
              }

              ReentrantLock lock =
                  userLocks.computeIfAbsent(userClaims.username(), _ -> new ReentrantLock());
              lock.lock();
              try {
                return userRepository
                    .findByOidcIssuerAndOidcSubject(issuerUri, userClaims.subject())
                    ...
                    .orElseGet(
                        () -> {
                          ...
                          auditService.log(
                              AuditAction.OIDC_USER_PROVISIONED,
                              "User",
                              newUser.getId(),
  ...
  private BookLoreUserEntity linkExistingUser(
      BookLoreUserEntity user, OidcClaimExtractor.OidcUserClaims userClaims, String issuerUri) {
    if (user.getProvisioningMethod() != ProvisioningMethod.OIDC) {
      log.info(
          "OIDC login: linking local user '{}' (was {}) to OIDC identity",
          user.getUsername(),
          user.getProvisioningMethod());
      user.setProvisioningMethod(ProvisioningMethod.OIDC);
      auditService.log(
          AuditAction.OIDC_ACCOUNT_LINKED,
          "User",
          user.getId(),
  ...
  private BookLoreUserEntity syncExistingUser(
      BookLoreUserEntity user, OidcClaimExtractor.OidcUserClaims userClaims, String issuerUri) {
    ...
    if ((user.getOidcSubject() == null || user.getOidcSubject().isBlank())
        && userClaims.subject() != null) {
