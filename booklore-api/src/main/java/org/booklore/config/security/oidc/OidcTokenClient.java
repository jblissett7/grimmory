...
public class OidcTokenClient {
  ...
  public TokenResponse exchangeAuthorizationCode(
      String code, String codeVerifier, String redirectUri, OidcProviderDetails providerDetails) {
    ...
    if (tokenEndpoint == null || tokenEndpoint.isBlank()) {
      throw ApiError.OIDC_PROVIDER_UNREACHABLE.createException(
    ...
    try {
      ...
      Map<String, Object> response =
          oidcRestTemplate.postForObject(tokenEndpoint, new HttpEntity<>(body, headers), Map.class);

      if (response == null) {
        throw ApiError.OIDC_TOKEN_EXCHANGE_FAILED.createException(
            "Empty response from token endpoint");
      }
