...
public class MetadataRefreshOptions {
  ...
  /**
   * Controls how fetched metadata replaces existing metadata. REPLACE_ALL: Replace all fields with
   * fetched values (even if existing values are present) REPLACE_MISSING: Only fill in fields that
   * are currently empty/null Default is REPLACE_MISSING to preserve existing metadata unless user
   * explicitly wants to overwrite.
   */
  @Builder.Default private MetadataReplaceMode replaceMode = MetadataReplaceMode.REPLACE_MISSING;
  ...
  public static class EnabledFields {
    ...
    /**
     * Default constructor that initializes all fields to true. This ensures that by default, all
     * metadata fields are enabled for fetching/updating.
     */
