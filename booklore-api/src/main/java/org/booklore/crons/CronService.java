...
public class CronService {
  ...
  protected boolean postData(String url, Object body) {
    try {
      restClient.post().uri(url).body(body).retrieve().body(String.class);
      return true;
    } catch (Exception ex) {
  ...
  /**
   * Determines if a task should run immediately on startup. Returns false for new installations (no
   * last run recorded) to follow normal schedule. Returns true if more than INTERVAL_HOURS have
   * passed since the last run, preventing data gaps when the server restarts close to scheduled
   * execution time.
   *
   * <p>Example: Telemetry normally runs at 2:00 AM daily. If the server restarts at 1:55 AM, the
   * scheduled task would reset and not run until 2:00 AM the next day (48 hours later). This method
   * checks if 24+ hours have passed since the last run and executes immediately on startup if
   * needed, ensuring data is sent at 1:55 AM instead of waiting another 24 hours.
   */
  ...
  /**
   * Checks if the app version has changed since the last ping. Returns true if this is an
   * established installation with a version change.
   */
