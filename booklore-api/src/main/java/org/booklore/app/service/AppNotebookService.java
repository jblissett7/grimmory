...
public class AppNotebookService {
  ...
  public AppPageResponse<AppNotebookBookSummary> getBooksWithAnnotations(
      int page, int size, String search) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    Pageable pageable = PageRequest.of(page, size);

    Page<NotebookEntryRepository.BookWithCountProjection> booksPage =
        notebookEntryRepository.findBooksWithAnnotationsPaginated(
            userId, wrapSearch(search), pageable);
    ...
    Map<Long, List<String>> authorsByBook =
        authorRepository.findAuthorNamesByBookIds(bookIds).stream()
            .collect(
                Collectors.groupingBy(
                    AuthorRepository.AuthorBookProjection::getBookId,
                    Collectors.mapping(
                        AuthorRepository.AuthorBookProjection::getAuthorName,
                        Collectors.toList())));

    List<AppNotebookBookSummary> summaries =
        content.stream()
            .map(
                p ->
                    AppNotebookBookSummary.builder()
                        .bookId(p.getBookId())
  ...
  @Transactional(readOnly = true)
  public AppPageResponse<AppNotebookEntry> getEntriesForBook(
      Long bookId, int page, int size, Set<String> types, String search, String sort) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    Set<String> entryTypes =
        (types == null || types.isEmpty()) ? Set.of("HIGHLIGHT", "NOTE", "BOOKMARK") : types;
    Pageable pageable = PageRequest.of(page, size, toSort(sort));

    Page<NotebookEntryRepository.EntryProjection> entriesPage =
        notebookEntryRepository.findEntries(
            userId, entryTypes, bookId, wrapSearch(search), pageable);

    List<AppNotebookEntry> entries =
        entriesPage.getContent().stream().map(AppNotebookService::toMobileEntry).toList();
  ...
  private static String escapeLike(String input) {
    return input.trim().replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_");
  }
