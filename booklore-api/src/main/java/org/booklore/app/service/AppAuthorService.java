...
public class AppAuthorService {
  ...
  public AppPageResponse<AppAuthorSummary> getAuthors(
    ...
    String fromClause =
        " FROM AuthorEntity a LEFT JOIN a.bookMetadataEntityList bm LEFT JOIN bm.book b";
    ...
    String dataJpql =
        "SELECT a, COUNT(DISTINCT bm.id)" + fromClause + whereClause + " GROUP BY a" + orderClause;
    ...
    List<AppAuthorSummary> summaries =
        results.stream()
            .map(
                row -> {
                  AuthorEntity author = (AuthorEntity) row[0];
                  long bookCount = (Long) row[1];
                  boolean authorHasPhoto =
                      Files.exists(Paths.get(fileService.getAuthorThumbnailFile(author.getId())));
    ...
    if (hasPhoto != null) {
      summaries =
          summaries.stream().filter(s -> s.isHasPhoto() == hasPhoto).collect(Collectors.toList());
      // Adjust total count for hasPhoto filter — requires a separate count
      long filteredTotal =
          countAuthorsWithPhotoFilter(accessibleLibraryIds, libraryId, search, hasPhoto);
  ...
  public AppAuthorDetail getAuthorDetail(Long authorId) {
    ...
    if (accessibleLibraryIds != null) {
      if (accessibleLibraryIds.isEmpty()
          || !authorRepository.existsByIdAndLibraryIds(authorId, accessibleLibraryIds)) {
  ...
  private long countAuthorsWithPhotoFilter(
      Set<Long> accessibleLibraryIds, Long libraryId, String search, boolean hasPhoto) {
    ...
    String jpql =
        "SELECT DISTINCT a FROM AuthorEntity a LEFT JOIN a.bookMetadataEntityList bm LEFT JOIN bm.book b"
            + whereClause;
    ...
    return query.getResultList().stream()
        .filter(
            a -> Files.exists(Paths.get(fileService.getAuthorThumbnailFile(a.getId()))) == hasPhoto)
        .count();
  }

  private void buildLibraryFilter(
      StringBuilder whereClause, Set<Long> accessibleLibraryIds, Long libraryId) {
    if (libraryId != null) {
  ...
  private void setQueryParams(
      TypedQuery<?> query, Set<Long> accessibleLibraryIds, Long libraryId, String search) {
    if (libraryId != null) {
