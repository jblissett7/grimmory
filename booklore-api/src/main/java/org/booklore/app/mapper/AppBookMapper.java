...
public interface AppBookMapper {
  ...
  @Mapping(
      target = "categories",
      source = "book.metadata.categories",
  ...
  @Mapping(
      target = "audiobookProgress",
      source = "fileProgress",
      qualifiedByName = "mapAudiobookProgress")
  @Mapping(
      target = "koreaderProgress",
      source = "progress",
      qualifiedByName = "mapKoreaderProgress")
  AppBookDetail toDetail(
      BookEntity book, UserBookProgressEntity progress, UserBookFileProgressEntity fileProgress);
  ...
  default List<AppShelfSummary> mapShelves(Set<ShelfEntity> shelves) {
    ...
    return shelves.stream().map(this::toShelfSummary).collect(Collectors.toList());
  }
  ...
  default AppBookDetail.AudiobookProgress mapAudiobookProgress(
    ...
    if (library.getLibraryPaths() != null && !library.getLibraryPaths().isEmpty()) {
      paths =
          library.getLibraryPaths().stream()
              .map(
                  lp ->
                      AppLibrarySummary.PathSummary.builder()
                          .id(lp.getId())
                          .path(lp.getPath())
                          .build())
              .collect(Collectors.toList());
