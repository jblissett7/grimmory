...
public class AppBookSpecification {
  ...
  public static Specification<BookEntity> withReadStatus(ReadStatus status, Long userId) {
    return (root, query, cb) -> {
      ...
      subquery
          .select(progressRoot.get("book").get("id"))
          .where(
  ...
  public static Specification<BookEntity> searchText(String searchQuery) {
    return (root, query, cb) -> {
      ...
      Join<BookMetadataEntity, AuthorEntity> authorsJoin =
          metadataJoin.join("authors", JoinType.LEFT);
  ...
  public static Specification<BookEntity> notDeleted() {
    return (root, query, cb) ->
        cb.or(cb.isNull(root.get("deleted")), cb.equal(root.get("deleted"), false));
  }
  ...
  public static Specification<BookEntity> hasAudiobookFile() {
    return (root, query, cb) -> {
      ...
      subquery
          .select(bookFileRoot.get("book").get("id"))
  ...
  /**
   * Filter books where the user's personal rating is <= maxRating. Use maxRating=0 to find unrated
   * books.
   */
  public static Specification<BookEntity> withMaxRating(int maxRating, Long userId) {
    return (root, query, cb) -> {
      ...
      if (maxRating == 0) {
        ...
        ratedSubquery
            .select(ratedRoot.get("book").get("id"))
            .where(
