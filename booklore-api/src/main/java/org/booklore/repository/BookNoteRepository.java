...
public interface BookNoteRepository extends JpaRepository<BookNoteEntity, Long> {
  ...
  @Query(
      "SELECT n FROM BookNoteEntity n WHERE n.bookId = :bookId AND n.userId = :userId ORDER BY n.updatedAt DESC")
  List<BookNoteEntity> findByBookIdAndUserIdOrderByUpdatedAtDesc(
