...
public interface BookMarkRepository extends JpaRepository<BookMarkEntity, Long> {
  ...
  @Query(
      "SELECT b FROM BookMarkEntity b WHERE b.bookId = :bookId AND b.userId = :userId ORDER BY b.priority ASC, b.createdAt DESC")
  List<BookMarkEntity> findByBookIdAndUserIdOrderByPriorityAscCreatedAtDesc(
  ...
  @Query(
      "SELECT COUNT(b) > 0 FROM BookMarkEntity b WHERE b.cfi = :cfi AND b.bookId = :bookId AND b.userId = :userId AND b.id != :excludeId")
  ...
  @Query(
      "SELECT COUNT(b) > 0 FROM BookMarkEntity b WHERE b.bookId = :bookId AND b.userId = :userId "
  ...
  boolean existsByPositionMsNearAndBookIdAndUserId(
      @Param("positionMs") Long positionMs,
      @Param("trackIndex") Integer trackIndex,
      @Param("bookId") Long bookId,
      @Param("userId") Long userId);
  ...
  @Query(
      "SELECT b FROM BookMarkEntity b JOIN FETCH b.book bk JOIN FETCH bk.metadata WHERE b.userId = :userId ORDER BY b.createdAt DESC")
