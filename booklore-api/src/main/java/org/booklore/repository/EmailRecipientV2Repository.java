...
public interface EmailRecipientV2Repository extends JpaRepository<EmailRecipientV2Entity, Long> {
  ...
  @Transactional
  @Query(
      "UPDATE EmailRecipientV2Entity e SET e.defaultRecipient = false WHERE e.defaultRecipient = true AND e.userId = :userId")
  void updateAllRecipientsToNonDefault(Long userId);

  @Query(
      "SELECT e FROM EmailRecipientV2Entity e WHERE e.defaultRecipient = true AND e.userId = :userId")
