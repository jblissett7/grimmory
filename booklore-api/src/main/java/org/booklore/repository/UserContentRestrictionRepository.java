...
public interface UserContentRestrictionRepository
    extends JpaRepository<UserContentRestrictionEntity, Long> {

  List<UserContentRestrictionEntity> findByUserId(Long userId);

  List<UserContentRestrictionEntity> findByUserIdAndRestrictionType(
      Long userId, ContentRestrictionType type);
  ...
  @Query(
      "SELECT r FROM UserContentRestrictionEntity r WHERE r.user.id = :userId AND r.restrictionType = :type AND r.mode = :mode")
  ...
  boolean existsByUserIdAndRestrictionTypeAndValue(
      Long userId, ContentRestrictionType type, String value);
}
