...
public interface NotebookEntryRepository extends Repository<AnnotationEntity, Long> {
  ...
  @Query(
      value =
          "SELECT t.id, t.type, t.book_id AS bookId, t.book_title AS bookTitle, "
      ...
      nativeQuery = true)
  Page<EntryProjection> findEntries(
  ...
  @Query(
      value =
          "SELECT DISTINCT t.book_id AS bookId, t.book_title AS bookTitle FROM ("
      ...
      nativeQuery = true)
  List<BookProjection> findBooksWithAnnotations(
      @Param("userId") Long userId, @Param("search") String search, Pageable pageable);

  @Query(
      value =
          "SELECT t.book_id AS bookId, t.book_title AS bookTitle, "
      ...
      countQuery =
          "SELECT COUNT(*) FROM ("
              ...
              + ") cnt",
      nativeQuery = true)
  Page<BookWithCountProjection> findBooksWithAnnotationsPaginated(
      @Param("userId") Long userId, @Param("search") String search, Pageable pageable);
}
