  ...
  public TaskCreateResponse execute(TaskCreateRequest request) {
    ...
    lastNotificationTime =
        sendTaskProgressNotification(
            taskId,
            0,
            "Starting book recommendation update",
    ...
    lastNotificationTime =
        sendTaskProgressNotification(
            taskId,
            5,
            String.format("Loaded %d books, generating embeddings...", totalBooks),
            TaskStatus.IN_PROGRESS,
            lastNotificationTime,
            false);
    ...
    for (BookEntity targetBook : allBooks) {
      try {
        ...
        String targetSeries =
            Optional.ofNullable(targetBook.getMetadata())
                .map(BookMetadataEntity::getSeriesName)
                .map(String::toLowerCase)
                .orElse(null);

        List<BookVectorService.ScoredBook> candidates =
            allBooks.stream()
                .filter(candidate -> !candidate.getId().equals(targetBook.getId()))
                .filter(
                    candidate -> {
                      String candidateSeries =
                          Optional.ofNullable(candidate.getMetadata())
                              .map(BookMetadataEntity::getSeriesName)
                              .map(String::toLowerCase)
                              .orElse(null);
        ...
        List<BookVectorService.ScoredBook> topSimilar =
            vectorService.findTopKSimilar(targetVector, candidates, RECOMMENDATION_LIMIT);
        ...
        log.error(
            "{}: Error updating similar books for book ID {}",
            getTaskType(),
            targetBook.getId(),
            e);
      }
    ...
    lastNotificationTime =
        sendTaskProgressNotification(
            taskId,
            85,
            String.format("Saving recommendations for %d books...", booksToUpdate.size()),
            TaskStatus.IN_PROGRESS,
            lastNotificationTime,
            false);
    ...
    sendTaskProgressNotification(
        ...
        100,
        String.format(
            "Updated recommendations for %d books in %d ms", totalBooks, endTime - startTime),
  ...
  private long sendTaskProgressNotification(
    ...
    if (force || (currentTime - lastNotificationTime) >= MIN_NOTIFICATION_INTERVAL_MS) {
      try {
        ...
        log.error(
            "Failed to send task progress notification for taskId={}: {}",
            taskId,
            e.getMessage(),
            e);
      }
