...
public class TempFetchedMetadataCleanupTask implements Task {
  ...
  public TaskCreateResponse execute(TaskCreateRequest request) {
    TaskCreateResponse.TaskCreateResponseBuilder builder =
        TaskCreateResponse.builder().taskId(UUID.randomUUID().toString()).taskType(getTaskType());
    ...
    try {
      ...
      if (request.isTriggeredByCron()) {
        ...
        log.info(
            "{}: Removed {} metadata fetch jobs older than {}", getTaskType(), deleted, cutoff);
      } else {
        deleted = metadataFetchJobRepository.deleteAllRecords();
        log.info(
            "{}: Removed all {} metadata fetch jobs (on-demand execution)", getTaskType(), deleted);
      }
