...
public class AppSettingController {
  ...
  public void updateSettings(
      @Parameter(description = "List of settings to update") @RequestBody
  ...
  public OidcDiagnosticService.OidcTestResult testOidcConnection(
      @RequestBody OidcProviderDetails providerDetails) {
    var result = oidcDiagnosticService.testConnection(providerDetails);
    auditService.log(
        AuditAction.OIDC_CONNECTION_TEST,
        "OIDC connection test: " + (result.success() ? "passed" : "failed"));
    return result;
  }
}
