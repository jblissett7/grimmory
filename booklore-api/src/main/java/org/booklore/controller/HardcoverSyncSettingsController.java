...
public class HardcoverSyncSettingsController {
  ...
  @GetMapping
  @PreAuthorize(
      "@securityUtil.canSyncKobo() or @securityUtil.canSyncKoReader() or @securityUtil.isAdmin()")
  ...
  @PreAuthorize(
      "@securityUtil.canSyncKobo() or @securityUtil.canSyncKoReader() or @securityUtil.isAdmin()")
  public ResponseEntity<HardcoverSyncSettings> updateSettings(
      @RequestBody @Valid HardcoverSyncSettings settings) {
    HardcoverSyncSettings updated =
        hardcoverSyncSettingsService.updateCurrentUserSettings(settings);
