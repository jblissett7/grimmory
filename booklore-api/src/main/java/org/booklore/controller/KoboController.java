...
public class KoboController {
  ...
  private static final Pattern KOBO_V1_PRODUCTS_NEXTREAD_PATTERN =
      Pattern.compile(".*/v1/products/\\d+/nextread.*");
  ...
  public ResponseEntity<Resource> getThumbnail(
    ...
    if (imageId.startsWith("BL-")) {
      ...
      String cdnUrl =
          String.format(
              "https://cdn.kobo.com/book-images/%s/%d/%d/false/image.jpg", imageId, width, height);
    ...
    if (imageId.startsWith("BL-")) {
      ...
      String cdnUrl =
          String.format(
              ...
              imageId, width, height, quality, isGreyscale);
      return koboServerProxy.proxyExternalUrl(cdnUrl);
    }
  }
  ...
  public ResponseEntity<?> getBookMetadata(
    ...
    if (StringUtils.isNumeric(bookId)) {
      return ResponseEntity.ok(
          List.of(koboEntitlementService.getMetadataForBook(Long.parseLong(bookId), token)));
    } else {
  ...
  public ResponseEntity<?> getState(
    ...
    if (StringUtils.isNumeric(bookId)) {
      return ResponseEntity.ok(
          new KoboReadingStateList(koboReadingStateService.getReadingState(bookId)));
    } else {
      return koboServerProxy.proxyCurrentRequest(null, false);
    }
  }
  ...
  public ResponseEntity<?> updateState(
      @Parameter(description = "Book ID") @PathVariable String bookId,
      @Parameter(description = "Reading state update body") @RequestBody
          KoboReadingStateRequest body) {
    if (StringUtils.isNumeric(bookId)) {
    ...
    if (StringUtils.isNumeric(bookId)) {
      ...
      if (userKoboShelf != null) {
        bookService.assignShelvesToBooks(
            Set.of(Long.valueOf(bookId)), Set.of(), Set.of(userKoboShelf.getId()));
      }
  ...
  public ResponseEntity<JsonNode> catchAll(
      HttpServletRequest request, @RequestBody(required = false) Object body) {
