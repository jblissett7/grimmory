...
@Tag(
    ...
    description = "Endpoints for managing user content restrictions")
@RestController
...
public class ContentRestrictionController {
  ...
  @Operation(
      summary = "Get user content restrictions",
      description = "Retrieve all content restrictions for a specific user")
  @ApiResponses({
  ...
  @Operation(
      summary = "Add content restriction",
      description = "Add a new content restriction for a user")
  @ApiResponses({
    @ApiResponse(responseCode = "201", description = "Content restriction created successfully"),
    @ApiResponse(
        responseCode = "400",
        description = "Invalid request or restriction already exists"),
  ...
  public ResponseEntity<ContentRestriction> addRestriction(
      @Parameter(description = "ID of the user") @PathVariable Long userId,
      @Parameter(description = "Content restriction to add") @RequestBody @Valid
  ...
  @Operation(
      summary = "Update all content restrictions",
      description = "Replace all content restrictions for a user")
  @ApiResponses({
  ...
  public ResponseEntity<List<ContentRestriction>> updateRestrictions(
      @Parameter(description = "ID of the user") @PathVariable Long userId,
      @Parameter(description = "List of content restrictions") @RequestBody @Valid
          List<ContentRestriction> restrictions) {
    return ResponseEntity.ok(contentRestrictionService.updateRestrictions(userId, restrictions));
  }

  @Operation(
      summary = "Delete content restriction",
      description = "Delete a specific content restriction")
  @ApiResponses({
  ...
  public ResponseEntity<Void> deleteRestriction(
      @Parameter(description = "ID of the user") @PathVariable Long userId,
      @Parameter(description = "ID of the restriction to delete") @PathVariable
          Long restrictionId) {
  ...
  @Operation(
      summary = "Delete all user content restrictions",
      description = "Delete all content restrictions for a user")
  @ApiResponses({
    @ApiResponse(
        responseCode = "204",
        description = "All content restrictions deleted successfully"),
    @ApiResponse(responseCode = "403", description = "Forbidden - requires admin")
  })
  @DeleteMapping
  @PreAuthorize("@securityUtil.isAdmin()")
