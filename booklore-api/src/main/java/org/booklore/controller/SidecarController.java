...
public class SidecarController {
  ...
  @Operation(
      summary = "Get sidecar content",
      description = "Get the content of the sidecar JSON file for a book")
  @ApiResponses({
  ...
  public ResponseEntity<SidecarMetadata> getSidecarContent(
    ...
    return sidecar.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
  }

  @Operation(
      summary = "Get sidecar sync status",
      description = "Get the synchronization status between database and sidecar file")
  @ApiResponses({
  ...
  public ResponseEntity<Map<String, SidecarSyncStatus>> getSyncStatus(
      @Parameter(description = "Book ID") @PathVariable Long bookId) {
  ...
  @Operation(
      summary = "Export metadata to sidecar",
      description = "Export book metadata to a sidecar JSON file")
  @ApiResponses({
  ...
  public ResponseEntity<Map<String, String>> exportToSidecar(
      @Parameter(description = "Book ID") @PathVariable Long bookId) {
  ...
  @Operation(
      summary = "Import metadata from sidecar",
      description = "Import book metadata from a sidecar JSON file")
  @ApiResponses({
  ...
  public ResponseEntity<Map<String, String>> importFromSidecar(
      @Parameter(description = "Book ID") @PathVariable Long bookId) {
  ...
  @Operation(
      summary = "Bulk export sidecar for library",
      description = "Generate sidecar JSON files for all books in a library")
  @ApiResponses({
  ...
  public ResponseEntity<Map<String, Object>> bulkExport(
      @Parameter(description = "Library ID") @PathVariable Long libraryId) {
    int exported = sidecarService.bulkExport(libraryId);
    return ResponseEntity.ok(Map.of("message", "Bulk export completed", "exported", exported));
  }

  @Operation(
      summary = "Bulk import sidecar for library",
      description = "Import metadata from all sidecar JSON files in a library")
  @ApiResponses({
  ...
  public ResponseEntity<Map<String, Object>> bulkImport(
    ...
    return ResponseEntity.ok(Map.of("message", "Bulk import completed", "imported", imported));
  }
}
