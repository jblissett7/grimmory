...
public class BookController {
  ...
  public ResponseEntity<Book> getBook(
      ...
      @Parameter(description = "Include book description in the response")
          @RequestParam(required = false, defaultValue = "false")
          boolean withDescription) {
  ...
  @Operation(
      ...
      description =
          "Create a physical book without digital files. Requires library management permission or admin.")
  @ApiResponses({
  ...
  public ResponseEntity<Book> createPhysicalBook(
      @Parameter(description = "Physical book creation request") @RequestBody @Valid
          CreatePhysicalBookRequest request) {
  ...
  @Operation(
      ...
      description = "Extract embedded metadata from the book file.")
  ...
  public void getBookContent(
      @Parameter(description = "ID of the book") @PathVariable long bookId,
      @Parameter(description = "Optional book type for alternative format (e.g., EPUB, PDF, MOBI)")
  ...
  public ResponseEntity<Void> updateBookViewerSettings(
      @Parameter(description = "Viewer settings to update") @RequestBody @Valid
          BookViewerSettings bookViewerSettings,
      @Parameter(description = "ID of the book") @PathVariable long bookId) {
  ...
  public ResponseEntity<List<Book>> addBookToShelf(
      @Parameter(description = "Shelves assignment request") @RequestBody @Valid
          ShelvesAssignmentRequest request) {
    return ResponseEntity.ok(
        bookService.assignShelvesToBooks(
            request.getBookIds(), request.getShelvesToAssign(), request.getShelvesToUnassign()));
  }
  ...
  public ResponseEntity<Void> addBookToProgress(
      @Parameter(description = "Read progress request") @RequestBody @Valid
  ...
  public ResponseEntity<List<BookRecommendation>> getRecommendations(
      ...
      @Parameter(description = "Maximum number of recommendations to return (max 25)")
  ...
  @PostMapping("/status")
  public List<BookStatusUpdateResponse> updateReadStatus(
  ...
  public ResponseEntity<List<BookStatusUpdateResponse>> resetProgress(
      @Parameter(description = "List of book IDs to reset progress for")
          @RequestBody
          @Size(max = 500)
          List<Long> bookIds,
  ...
  public ResponseEntity<List<PersonalRatingUpdateResponse>> updatePersonalRating(
      @Parameter(description = "Personal rating update request") @RequestBody @Valid
          PersonalRatingUpdateRequest request) {
    return ResponseEntity.ok(
        bookUpdateService.updatePersonalRating(request.ids(), request.rating()));
  }
  ...
  public ResponseEntity<List<PersonalRatingUpdateResponse>> resetPersonalRating(
      @Parameter(description = "List of book IDs to reset personal rating for")
          @RequestBody
          @Size(max = 500)
          List<Long> bookIds) {
    if (bookIds == null || bookIds.isEmpty()) {
    ...
    List<PersonalRatingUpdateResponse> updatedBooks =
        bookUpdateService.resetPersonalRating(bookIds);
    return ResponseEntity.ok(updatedBooks);
  }

  @Operation(
      summary = "Find duplicate books",
      description =
          "Detect potential duplicate books in a library using configurable matching strategies.")
  ...
  public ResponseEntity<List<DuplicateGroup>> findDuplicates(
      @Parameter(description = "Duplicate detection configuration") @RequestBody @Valid
          DuplicateDetectionRequest request) {
    return ResponseEntity.ok(duplicateDetectionService.findDuplicates(request));
  }

  @Operation(
      summary = "Toggle physical book flag",
      description = "Mark or unmark a book as a physical book.")
  @ApiResponses({
  ...
  @ApiResponses({
    ...
    @ApiResponse(
        responseCode = "400",
        description =
            "Invalid request - books must be in same library, sources must have exactly one file each"),
    @ApiResponse(
        responseCode = "403",
  ...
  public ResponseEntity<AttachBookFileResponse> attachBookFiles(
      @Parameter(description = "ID of the target book to attach the files to") @PathVariable
          Long targetBookId,
      @Parameter(description = "Request containing source book IDs and delete option")
          @RequestBody
          @Valid
          AttachBookFileRequest request) {
    return ResponseEntity.ok(
        bookFileAttachmentService.attachBookFiles(
            targetBookId, request.getSourceBookIds(), request.isMoveFiles()));
  }
}
