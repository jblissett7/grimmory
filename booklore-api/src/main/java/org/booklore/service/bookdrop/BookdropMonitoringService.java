  ...
  public void start() {
    ...
    if (Files.notExists(bookdrop)) {
      try {
        ...
        log.warn(
            "Bookdrop folder is not available at '{}'. Bookdrop monitoring is disabled. "
                + "Mount a volume at this path to enable it.",
            bookdrop);
        this.disabled = true;
        return;
      }
          ...
          if (Files.isDirectory(fullPath)) {
            ...
            try (Stream<Path> pathStream = Files.walk(fullPath)) {
              pathStream
                  ...
                  .filter(
                      path ->
                          BookFileExtension.fromFileName(path.getFileName().toString()).isPresent())
                  .forEach(
                      path -> eventHandler.enqueueFile(path, StandardWatchEventKinds.ENTRY_CREATE));
            } catch (IOException e) {
  ...
  private void scanExistingBookdropFiles() {
    ...
    try (Stream<Path> files = Files.walk(bookdrop)) {
      supportedFiles =
          files
              ...
              .filter(path -> !FileUtils.shouldIgnore(path))
              .filter(
                  path -> BookFileExtension.fromFileName(path.getFileName().toString()).isPresent())
              .toList();
    } catch (IOException e) {
    ...
    if (!supportedFiles.isEmpty()) {
      List<String> supportedFilePaths =
          supportedFiles.stream().map(Path::toAbsolutePath).map(Path::toString).toList();
