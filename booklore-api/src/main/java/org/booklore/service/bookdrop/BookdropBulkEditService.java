...
public class BookdropBulkEditService {
  ...
  public BookdropBulkEditResult bulkEdit(BookdropBulkEditRequest request) {
    List<Long> fileIds =
        metadataHelper.resolveFileIds(
            request.isSelectAll(), request.getExcludedIds(), request.getSelectedIds());

    return processBulkEditInBatches(fileIds, request);
  }

  private BookdropBulkEditResult processBulkEditInBatches(
      List<Long> fileIds, BookdropBulkEditRequest request) {
    ...
    for (int batchStart = 0; batchStart < fileIds.size(); batchStart += BATCH_SIZE) {
      ...
      log.debug(
          "Processed batch {}-{} of {}: {} successful, {} failed",
          batchStart,
          batchEnd,
          totalFiles,
  ...
  private BatchEditResult processSingleBatch(
      List<Long> allFileIds, int batchStart, int batchEnd, BookdropBulkEditRequest request) {
    ...
    List<BookdropFileEntity> filesToSave =
        batchFiles.stream().filter(file -> !failedFileIds.contains(file.getId())).toList();
  ...
  private void updateFileMetadata(BookdropFileEntity file, BookdropBulkEditRequest request) {
    ...
    updateArrayField(
        "authors",
        enabledFields,
        currentMetadata.getAuthors(),
        updates.getAuthors(),
        currentMetadata::setAuthors,
        mergeArrays);
    updateArrayField(
        "categories",
        enabledFields,
        currentMetadata.getCategories(),
        updates.getCategories(),
        currentMetadata::setCategories,
        mergeArrays);
    updateArrayField(
        "moods",
        enabledFields,
        currentMetadata.getMoods(),
        updates.getMoods(),
        currentMetadata::setMoods,
        mergeArrays);
    updateArrayField(
        "tags",
        enabledFields,
        currentMetadata.getTags(),
        updates.getTags(),
        currentMetadata::setTags,
        mergeArrays);
