...
public class FilenamePatternExtractor {
  ...
  private static final Map<String, PlaceholderConfig> PLACEHOLDER_CONFIGS =
      Map.ofEntries(
  ...
  private BookdropPatternExtractResult processPreviewExtraction(
      List<Long> fileIds, ParsedPattern pattern) {
    List<Long> limitedFileIds =
        fileIds.size() > PREVIEW_FILE_LIMIT ? fileIds.subList(0, PREVIEW_FILE_LIMIT) : fileIds;
  ...
  private BookdropPatternExtractResult processFullExtractionInBatches(
      List<Long> fileIds, ParsedPattern pattern) {
    ...
    for (int batchStart = 0; batchStart < fileIds.size(); batchStart += BATCH_SIZE) {
      ...
      BatchExtractionResult batchResult =
          processSingleExtractionBatch(fileIds, batchStart, batchEnd, pattern);
      ...
      log.debug(
          "Processed pattern extraction batch {}-{} of {}: {} successful, {} failed",
          batchStart,
          batchEnd,
          totalFiles,
  ...
  private BatchExtractionResult processSingleExtractionBatch(
      List<Long> allFileIds, int batchStart, int batchEnd, ParsedPattern pattern) {
  ...
  private BookMetadata extractFromFilenameWithParsedPattern(
      String filename, ParsedPattern parsedPattern) {
    String nameOnly = FilenameUtils.getBaseName(filename);

    Optional<Matcher> matcherResult =
        executeRegexMatchingWithTimeout(parsedPattern.compiledPattern(), nameOnly);
  ...
  private Optional<Matcher> executeRegexMatchingWithTimeout(Pattern pattern, String input) {
    Future<Optional<Matcher>> future =
        regexExecutor.submit(
            () -> {
    ...
    try {
      ...
      future.cancel(true);
      log.warn(
          "Pattern matching exceeded {} second timeout for: {}",
  ...
  private BookdropPatternExtractResult.FileExtractionResult extractFromFile(
      BookdropFileEntity file, ParsedPattern parsedPattern) {
    try {
      BookMetadata extracted =
          extractFromFilenameWithParsedPattern(file.getFileName(), parsedPattern);

      if (extracted == null) {
        String errorMsg =
            "Pattern did not match filename structure. Check if the pattern aligns with the filename format.";
  ...
  private ParsedPattern parsePattern(String pattern) {
    ...
    for (int i = 0; i < placeholderMatches.size(); i++) {
      ...
      String placeholderWithFormat =
          formatParameter != null ? placeholderName + ":" + formatParameter : placeholderName;
  ...
  private List<PlaceholderMatch> findAllPlaceholders(String pattern) {
    ...
    while (matcher.find()) {
      ...
      placeholderMatches.add(
          new PlaceholderMatch(matcher.start(), matcher.end(), placeholderName, formatParameter));
    }
  ...
  private String determineRegexForPlaceholder(
      PlaceholderConfig config, boolean shouldUseGreedyMatching) {
    if (config != null) {
  ...
  private void applyValueToMetadata(
      BookMetadata metadata, String placeholderName, String value, String formatParameter) {
  ...
  private void setPublishedDate(BookMetadata metadata, String value, String dateFormat) {
    String detectedFormat =
        (dateFormat == null || dateFormat.isBlank()) ? detectDateFormat(value) : dateFormat;
    ...
    try {
      ...
      log.warn(
          "Failed to parse date '{}' with format '{}': {}", value, detectedFormat, e.getMessage());
    } catch (IllegalArgumentException e) {
      log.warn(
          "Invalid date format '{}' for value '{}': {}", detectedFormat, value, e.getMessage());
    }
  }
  ...
  private void persistExtractedMetadata(
      List<BookdropPatternExtractResult.FileExtractionResult> results,
      List<BookdropFileEntity> files) {
    ...
    for (BookdropPatternExtractResult.FileExtractionResult result : results) {
      ...
      try {
        ...
        log.error(
            "Error persisting extracted metadata for file {} ({}): {}",
            file.getId(),
            file.getFileName(),
            e.getMessage(),
            e);
    ...
    List<BookdropFileEntity> filesToSave =
        files.stream().filter(file -> !failedFileIds.contains(file.getId())).toList();
  ...
  private record BatchExtractionResult(
      List<BookdropPatternExtractResult.FileExtractionResult> results,
      int successCount,
      int failureCount) {}
}
