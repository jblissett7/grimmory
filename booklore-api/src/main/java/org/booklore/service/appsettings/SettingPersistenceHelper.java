...
public class SettingPersistenceHelper {
  ...
  public <T> T getJsonSetting(
      Map<String, String> settingsMap,
      AppSettingKey key,
      Class<T> clazz,
      T defaultValue,
      boolean persistDefault) {
    return getJsonSettingInternal(
        settingsMap,
        key,
        defaultValue,
        persistDefault,
  ...
  private <T> T getJsonSettingInternal(
      Map<String, String> settingsMap,
      AppSettingKey key,
      T defaultValue,
      boolean persistDefault,
    ...
    if (json != null && !json.isBlank()) {
      try {
        ...
        log.error(
            "Failed to parse JSON for setting key '{}'. Using default value. Error: {}",
            key,
            e.getMessage());
        return defaultValue;
      }
    ...
    if (defaultValue != null && persistDefault) {
      try {
        ...
        log.error(
            "Failed to persist default value for setting key '{}'. Error: {}", key, e.getMessage());
      }
    }
    return defaultValue;
  }
  ...
  MetadataRefreshOptions getDefaultMetadataRefreshOptions() {
    MetadataRefreshOptions.FieldProvider amazonProvider =
        MetadataRefreshOptions.FieldProvider.builder().p1(MetadataProvider.Amazon).build();

    MetadataRefreshOptions.FieldProvider nullProvider =
        MetadataRefreshOptions.FieldProvider.builder().build();
  ...
  public MetadataPersistenceSettings getDefaultMetadataPersistenceSettings() {
    MetadataPersistenceSettings.FormatSettings epubSettings =
        MetadataPersistenceSettings.FormatSettings.builder()
            .enabled(false)
            .maxFileSizeInMb(250)
            .build();

    MetadataPersistenceSettings.FormatSettings pdfSettings =
        MetadataPersistenceSettings.FormatSettings.builder()
            .enabled(false)
            .maxFileSizeInMb(250)
            .build();

    MetadataPersistenceSettings.FormatSettings cbxSettings =
        MetadataPersistenceSettings.FormatSettings.builder()
            .enabled(false)
            .maxFileSizeInMb(250)
            .build();
