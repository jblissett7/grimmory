...
public class KomgaService {
  ...
  public KomgaPageableDto<KomgaSeriesDto> getAllSeries(
      Long libraryId, int page, int size, boolean unpaged) {
    ...
    if (groupUnknown) {
      ...
      if (libraryId != null) {
        sortedSeriesNames =
            bookRepository.findDistinctSeriesNamesGroupedByLibraryId(
                libraryId, komgaMapper.getUnknownSeriesName());
      } else {
        sortedSeriesNames =
            bookRepository.findDistinctSeriesNamesGrouped(komgaMapper.getUnknownSeriesName());
      }
    } else {
    ...
    for (String seriesName : pageSeriesNames) {
      try {
        ...
        if (libraryId != null) {
          if (groupUnknown) {
            seriesBooks =
                bookRepository.findBooksBySeriesNameGroupedByLibraryId(
                    seriesName, libraryId, komgaMapper.getUnknownSeriesName());
          } else {
            seriesBooks =
                bookRepository.findBooksBySeriesNameUngroupedByLibraryId(seriesName, libraryId);
          }
        } else {
  ...
  public KomgaPageableDto<KomgaBookDto> getBooksBySeries(
      String seriesId, int page, int size, boolean unpaged) {
    // Parse seriesId to extract library and series name
    String[] parts = seriesId.split("-", 2);
    if (parts.length < 2) {
      throw new RuntimeException("Invalid series ID");
    ...
    if (unpaged) {
      ...
      content =
          seriesBooks.subList(fromIndex, toIndex).stream()
              .map(book -> komgaMapper.toKomgaBookDto(book))
              .collect(Collectors.toList());
      actualPage = page;
      actualSize = size;
  ...
  public KomgaPageableDto<KomgaBookDto> getAllBooks(Long libraryId, int page, int size) {
    ...
    List<KomgaBookDto> content =
        books.subList(fromIndex, toIndex).stream()
            .map(book -> komgaMapper.toKomgaBookDto(book))
            .collect(Collectors.toList());
  ...
  public List<KomgaPageDto> getBookPages(Long bookId) {
    ...
    if (pageCount > 0) {
      for (int i = 1; i <= pageCount; i++) {
        pages.add(
            KomgaPageDto.builder().number(i).fileName("page-" + i).mediaType("image/jpeg").build());
      }
    }
  ...
  public KomgaPageableDto<KomgaCollectionDto> getCollections(int page, int size, boolean unpaged) {
    ...
    List<KomgaCollectionDto> allCollections =
        magicShelves.stream()
  ...
  public Resource getBookPageImage(Long bookId, Integer pageNumber, boolean convertToPng)
      throws IOException {
    log.debug(
        "Getting page {} from book {} (convert to PNG: {})", pageNumber, bookId, convertToPng);
