...
public class MigrateProgressToFileProgressMigration implements Migration {
    ...
    for (UserBookProgressEntity progress : allProgress) {
      ...
      try {
        ...
        if (bookFileOpt.isEmpty()) {
          log.debug(
              "No matching book file found for progress id={}, bookId={}",
              progress.getId(),
        ...
        if (existingFileProgress.isPresent()) {
          log.debug(
              "File progress already exists for userId={}, bookFileId={}",
        ...
        log.warn(
            "Failed to migrate progress for progressId={}: {}", progress.getId(), e.getMessage());
        skippedCount++;
      }
    }

    log.info(
        "Migration '{}' completed. Migrated: {}, Skipped: {}",
        getKey(),
        migratedCount,
        skippedCount);
  }
  ...
  private Optional<BookFileEntity> findBookFileForProgress(UserBookProgressEntity progress) {
    ...
    if (progress.getPdfProgress() != null) {
      ...
      return findBookFileByTypes(
          bookFiles, BookFileType.EPUB, BookFileType.FB2, BookFileType.MOBI, BookFileType.AZW3);
  ...
  private Optional<BookFileEntity> findBookFileByType(
      List<BookFileEntity> bookFiles, BookFileType type) {
    return bookFiles.stream()
        .filter(bf -> bf.isBookFormat() && bf.getBookType() == type)
        .findFirst();
  }

  private Optional<BookFileEntity> findBookFileByTypes(
      List<BookFileEntity> bookFiles, BookFileType... types) {
  ...
  private UserBookFileProgressEntity createFileProgress(
      UserBookProgressEntity progress, BookFileEntity bookFile) {
