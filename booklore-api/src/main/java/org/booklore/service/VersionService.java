...
public class VersionService {
  ...
  public String fetchLatestGitHubReleaseVersion() {
    try {
      String response =
          REST_CLIENT.get().uri(BASE_URI + "/releases/latest").retrieve().body(String.class);
  ...
  public List<ReleaseNote> fetchReleaseNotesSince(String currentVersion) {
    if ("development".equals(currentVersion)) {
      log.warn(
          "Skipping fetch of release notes because current version is '{}', which is a local development build.",
          currentVersion);
    ...
    try {
      ...
      for (JsonNode release : releases) {
        ...
        LocalDateTime published =
            LocalDateTime.parse(
                release.path("published_at").asText(), DateTimeFormatter.ISO_DATE_TIME);
        updates.add(
            new ReleaseNote(
                tag,
                release.path("name").asText(tag),
