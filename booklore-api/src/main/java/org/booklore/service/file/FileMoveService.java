...
public class FileMoveService {
  ...
  public void bulkMoveFiles(FileMoveRequest request) {
    ...
    Set<Path> libraryPaths =
        monitoringRegistrationService.getPathsForLibraries(allAffectedLibraryIds);
    ...
    try {
      ...
      // Ensure any file system events from the moves are drained/ignored while we are still
      // unregistered
      ...
      for (Long libraryId : allAffectedLibraryIds) {
        libraryRepository
            ...
            .ifPresent(
                library ->
                    monitoringRegistrationService.registerLibrary(
                        libraryMapper.toLibrary(library)));
      }
    }
  }
  ...
  private void processSingleMove(FileMoveRequest.Move move) {
    ...
    try {
      ...
      Optional<LibraryPathEntity> optionalLibraryPathEntity =
          targetLibrary.getLibraryPaths().stream()
              .filter(libraryPath -> Objects.equals(libraryPath.getId(), targetLibraryPathId))
              .findFirst();
      if (optionalLibraryPathEntity.isEmpty()) {
        log.warn(
            "Target library path not found for move operation: libraryId={}, pathId={}",
            targetLibraryId,
            targetLibraryPathId);
        return;
      }
      ...
      if (targetParentDir == null) {
        log.warn(
            "Target parent directory could not be determined for move operation: bookId={}",
            bookId);
        return;
      }
      ...
      for (var bookFile : bookEntity.getBookFiles()) {
        ...
        if (!fileMoveHelper.validateSourceExists(sourcePath, bookFile.isFolderBased())) {
          log.warn(
              "Source {} not found: bookId={}, path={}",
              bookFile.isFolderBased() ? "folder" : "file",
              bookId,
              sourcePath);
          return;
        }
      ...
      for (var bookFile : bookEntity.getBookFiles()) {
        ...
        if (bookFile.isBook()) {
          targetPath =
              fileMoveHelper.generateNewFilePath(bookEntity, bookFile, libraryPathEntity, pattern);
        } else {
        ...
        plannedMovesByBookFileId.put(
            bookFile.getId(), new PlannedMove(sourcePath, tempPath, targetPath));
      ...
      try {
        transactionTemplate.executeWithoutResult(
            status -> {
              for (var bookFile : bookEntity.getBookFiles()) {
                ...
                if (bookFile.isBook()) {
                  Path targetPath =
                      fileMoveHelper.generateNewFilePath(
                          bookEntity, bookFile, libraryPathEntity, pattern);
                  newFileName = targetPath.getFileName().toString();
                } else {
                  newFileName = bookFile.getFileName();
                }
                bookFileRepository.updateFileNameAndSubPath(
                    bookFile.getId(), newFileName, newFileSubPath);
              }

              bookRepository.updateLibrary(
                  bookEntity.getId(), targetLibrary.getId(), libraryPathEntity);
            });
      } catch (Exception e) {
        log.error(
            "Database update failed after files were moved. Attempting to rollback file moves for book ID {}",
            bookId,
            e);
        for (PlannedMove committed : committedMoves) {
          try {
            ...
            log.error(
                "Failed to rollback file move (Target -> Source) for book ID {}: {} -> {}",
                bookId,
                committed.target(),
                committed.source(),
                rollbackEx);
      ...
      notificationService.sendMessage(
          Topic.BOOK_UPDATE, bookMapper.toBookWithDescription(fresh, false));
  ...
  public FileMoveResult moveSingleFile(BookEntity bookEntity) {
    ...
    Path libraryRoot =
        Paths.get(bookEntity.getLibraryPath().getPath()).toAbsolutePath().normalize();
    ...
    try {
      Set<Path> existingPaths =
          monitoringRegistrationService.getPathsForLibraries(Set.of(libraryId));
      isLibraryMonitoredWhenCalled =
          monitoringRegistrationService.isLibraryMonitored(libraryId) || !existingPaths.isEmpty();

      // Ensure bookFiles are loaded
      BookEntity bookWithFiles = bookEntity;
      if (bookEntity.getBookFiles() == null || bookEntity.getBookFiles().isEmpty()) {
        Optional<BookEntity> optionalBook =
            bookRepository.findByIdWithBookFiles(bookEntity.getId());
        if (optionalBook.isEmpty()) {
      ...
      String pattern =
          fileMoveHelper.getFileNamingPattern(bookWithFiles.getLibraryPath().getLibrary());
      Path currentPrimaryFilePath = bookWithFiles.getFullFilePath();
      Path expectedPrimaryFilePath =
          fileMoveHelper.generateNewFilePath(
              bookWithFiles, bookWithFiles.getLibraryPath(), pattern);
      ...
      log.info(
          "Files for book ID {} need to be moved to match library pattern", bookWithFiles.getId());

      String newFileSubPath =
          fileMoveHelper.extractSubPath(expectedPrimaryFilePath, bookWithFiles.getLibraryPath());
      ...
      if (isLibraryMonitoredWhenCalled) {
        ...
        monitoringRegistrationService.waitForEventsDrainedByPaths(
            existingPaths, EVENT_DRAIN_TIMEOUT_MS);
      }
      ...
      for (var bookFile : bookWithFiles.getBookFiles()) {
        ...
        if (bookFile.isBook()) {
          targetPath =
              fileMoveHelper.generateNewFilePath(
                  bookWithFiles, bookFile, bookWithFiles.getLibraryPath(), pattern);
        } else {
          targetPath = targetParentDir.resolve(bookFile.getFileName());
        }
                ...
                if (bookFile.isBook()) {
                  Path targetPath =
                      fileMoveHelper.generateNewFilePath(
                          finalBookWithFiles,
                          bookFile,
                          finalBookWithFiles.getLibraryPath(),
                          pattern);
                  newFileName = targetPath.getFileName().toString();
                } else {
                  newFileName = bookFile.getFileName();
      ...
      Set<Path> libraryRoots =
          bookWithFiles.getLibraryPath().getLibrary().getLibraryPaths().stream()
              .map(LibraryPathEntity::getPath)
              .map(Paths::get)
              .map(Path::toAbsolutePath)
              .map(Path::normalize)
      ...
      try {
        ...
        log.warn(
            "Failed to move sidecar files for book ID {}: {}", bookEntity.getId(), e.getMessage());
      }

      if (isLibraryMonitoredWhenCalled) {
        // Ensure any file system events from the move and cleanup are drained/ignored while we are
        // still unregistered
        sleep(EVENT_DRAIN_TIMEOUT_MS);
      }
    ...
    if (!appProperties.isLocalStorage()) {
      throw new IllegalStateException(
          "File move operations are only supported on local storage. "
              + "Current disk type is configured as: "
              + appProperties.getDiskType()
              + ". "
