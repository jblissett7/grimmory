...
public class FileMoveHelper {
  ...
  public boolean waitForFileAccessible(Path path) {
    for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (attempt < MAX_ATTEMPTS) {
        log.debug(
            "File not accessible on attempt {}/{}, waiting {}ms: {}",
            attempt,
            MAX_ATTEMPTS,
            RETRY_DELAY_MS,
            path);
  ...
  public void moveFile(Path source, Path target) throws IOException {
    if (!waitForFileAccessible(source)) {
      throw new NoSuchFileException(
          source.toString(), null, "Source file not accessible after retries");
    }
  ...
  public void commitMove(Path tempPath, Path target) throws IOException {
    if (!waitForFileAccessible(tempPath)) {
      throw new NoSuchFileException(
          tempPath.toString(), null, "Temporary file not accessible before commit");
    }
  ...
  public void rollbackMove(Path tempPath, Path originalSource) {
    ...
    for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        if (attempt == MAX_ATTEMPTS) {
          log.error(
              "Failed to rollback file move from {} to {} after {} attempts. "
        ...
        log.warn(
            "Rollback attempt {}/{} failed, retrying: {}", attempt, MAX_ATTEMPTS, e.getMessage());
        waitBeforeRetry();
      }
    }
  }
  ...
  public String getFileNamingPattern(LibraryEntity library) {
    ...
    if (pattern == null || pattern.trim().isEmpty()) {
      try {
        ...
        log.debug(
            "Using default pattern for library {} as no custom pattern is set", library.getName());
      } catch (Exception e) {
        log.warn(
            "Failed to get default upload pattern for library {}: {}",
            library.getName(),
            e.getMessage());
      }
    }
    if (pattern == null || pattern.trim().isEmpty()) {
      ...
      log.info(
          "No file naming pattern available for library {}. Using fallback pattern: {currentFilename}",
          library.getName());
    }
  ...
  public Path generateNewFilePath(
      BookEntity book, LibraryPathEntity libraryPathEntity, String pattern) {
  ...
  public Path generateNewFilePath(
      ...
      String pattern) {
    String newRelativePathStr =
        PathPatternResolver.resolvePattern(book, bookFile, pattern, bookFile.isFolderBased());
    if (newRelativePathStr.startsWith("/") || newRelativePathStr.startsWith("\\")) {
      newRelativePathStr = newRelativePathStr.substring(1);
    }
    String path = libraryPathEntity.getPath();
  ...
  private void executeWithRetry(FileOperation operation) throws IOException {
    for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        log.warn(
            "File operation failed (attempt {}/{}), retrying in {}ms: {}",
            attempt,
            MAX_ATTEMPTS,
            RETRY_DELAY_MS,
            e.getMessage());
