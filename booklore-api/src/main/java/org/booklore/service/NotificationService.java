...
public class NotificationService {
  ...
  public void sendMessageToUser(String username, Topic topic, Object message) {
    try {
      ...
      log.error(
          "Error sending message to user {} on topic {}: {}", username, topic, e.getMessage(), e);
    }
  }

  @Transactional(readOnly = true)
  public void sendMessageToPermissions(
      Topic topic, Object message, Set<PermissionType> permissionTypes) {
    ...
    try {
      ...
      log.error(
          "Error sending message to users with permissions {}: {}",
          permissionTypes,
          e.getMessage(),
          e);
    }
  ...
  private List<String> findUsernamesWithPermissions(Set<PermissionType> permissionTypes) {
    ...
    String jpql =
        "SELECT u.username FROM BookLoreUserEntity u JOIN u.permissions p WHERE " + conditions;
