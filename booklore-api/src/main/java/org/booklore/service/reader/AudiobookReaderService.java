...
/**
 * Service for audiobook reader operations. Orchestrates metadata extraction, file access, and
 * streaming.
 */
@Slf4j
@Service
...
public class AudiobookReaderService {
  ...
  /**
   * Get the path to an audio file for streaming. For folder-based audiobooks, returns the path to a
   * specific track.
   */
  ...
  public void streamWithRangeSupport(
      Path filePath, HttpServletRequest request, HttpServletResponse response) throws IOException {
  ...
  public byte[] getEmbeddedCoverArt(Long bookId, String bookType) {
    BookFileEntity bookFile = getAudiobookFile(bookId, bookType);
    Path audioPath =
        bookFile.isFolderBased() ? bookFile.getFirstAudioFile() : bookFile.getFullFilePath();
  ...
  private BookFileEntity getAudiobookFile(Long bookId, String bookType) {
    ...
    return bookEntity.getBookFiles().stream()
        ...
        .orElseThrow(
            () ->
                ApiError.FILE_NOT_FOUND.createException(
                    "No audiobook file found for book " + bookId));
  }
}
