...
public class LibraryFileEventProcessor {
  ...
  private final ConcurrentMap<Path, ScheduledFuture<?>> pendingFolderCreates =
  ...
  public void processEvent(
      WatchEvent.Kind<?> eventKind, long libraryId, Path fullPath, boolean isDirectory) {
    ...
    if (eventKind == StandardWatchEventKinds.ENTRY_DELETE) {
      ...
      if (pendingDelete != null) {
        ...
        log.debug(
            "[DEBOUNCE] CREATE after pending DELETE for '{}', treating as file modification", path);
      }

      if (isDirectory) {
        log.debug(
            "[DEBOUNCE] Scheduling folder create for '{}' with {}ms delay",
            path,
            FOLDER_CREATE_DEBOUNCE_MS);
        ScheduledFuture<?> existingFolder =
            pendingFolderCreates.put(
                path,
                scheduler.schedule(
                    () -> {
          ...
          if (path.startsWith(entry.getKey())) {
            ...
            pendingFolderCreates.put(
                ...
                scheduler.schedule(
                    ...
                    TimeUnit.MILLISECONDS));
            log.debug(
                "[DEBOUNCE] File '{}' tracked, reset folder debounce for '{}'",
                path.getFileName(),
                folderPath);
            break;
          }
        ...
        if (!insidePendingFolder) {
          scheduleWithStabilityCheck(
              path, () -> eventQueue.offer(new FileEvent(eventKind, libraryId, path, false)));
        }
      }
    } else {
  ...
  private void handleEvent(FileEvent event) {
    ...
    if (isDirectory) {
      switch (event.eventKind().name()) {
        ...
        default ->
            log.warn(
                "[SKIP] Folder event '{}' ignored for '{}'", event.eventKind().name(), fileName);
      }
      return;
    }
    ...
    switch (event.eventKind().name()) {
      ...
      default ->
          log.debug("[SKIP] File event '{}' ignored for '{}'", event.eventKind().name(), fileName);
    }
  }
  ...
  private void handleFileDelete(LibraryEntity library, Path path) {
    ...
    try {
      ...
      LibraryPathEntity libPathEntity =
          bookFilePersistenceService.getLibraryPathEntityForFile(library, libPath);
      ...
      bookFilePersistenceService
          .findBookFileByLibraryPathSubPathAndFileName(libPathEntity.getId(), fileSubPath, fileName)
          .ifPresentOrElse(
              bookFile -> {
                ...
                log.info(
                    "[PENDING_DELETE] Book '{}' deferred deletion (grace period {}ms)",
                    fileName,
                    PENDING_DELETION_GRACE_MS);
              },
              () ->
                  log.debug(
                      "[NOT_FOUND] BookFile for deleted path '{}' not found (likely renamed/moved)",
                      path));
  ...
  private void handleFolderCreate(LibraryEntity library, Path folderPath) {
    ...
    var mode =
        library.getOrganizationMode() != null
            ? library.getOrganizationMode()
    ...
    if (analysis.isFolderBasedAudiobook()) {
      log.info(
          "[FOLDER_AUDIOBOOK] Detected folder-based audiobook: {} ({} audio files)",
          folderPath.getFileName(),
          analysis.audioFileCount());
      try {
        ...
        if (cleared > 0) {
          log.debug(
              "[FOLDER_AUDIOBOOK] Cleared {} tracked files that are now part of folder audiobook",
              cleared);
        }
      } catch (Exception e) {
  ...
  private void processFilesInFolderAsOneBook(LibraryEntity library, Path folderPath) {
    ...
    Optional<PendingDeletionPool.FolderMatchResult> folderMatch =
        pendingDeletionPool.matchFolderByHashes(fileHashes);
    if (folderMatch.isPresent()) {
      var match = folderMatch.get();
      String matchedLibPath =
          bookFilePersistenceService.findMatchingLibraryPath(library, folderPath);
      LibraryPathEntity matchedLibPathEntity =
          bookFilePersistenceService.getLibraryPathEntityForFile(library, matchedLibPath);

      for (var bookEntry : match.pending().affectedBooks().entrySet()) {
        var bookSnap = bookEntry.getValue();
        bookFilePersistenceService.recoverFolderBook(
            bookSnap, matchedLibPathEntity, folderPath, fileHashes);
      }
      log.info(
          "[FOLDER_CREATE] Folder '{}' matched pending deletion, recovered {} books",
          folderPath,
          match.pending().affectedBooks().size());
      return;
    }
    ...
    for (Path p : bookFiles) {
      try {
        ...
        if (poolMatch.isPresent()) {
          ...
          LibraryPathEntity lpEntity =
              bookFilePersistenceService.getLibraryPathEntityForFile(library, libPath);
          String subPath = FileUtils.getRelativeSubPath(lpEntity.getPath(), p);
          pendingDeletionPool.recoverBook(
              pmatch, lpEntity, subPath, p.getFileName().toString(), hash);
          log.info(
              "[FOLDER_CREATE] File '{}' matched pending deletion, recovered book id={}",
              p,
              pmatch.book().bookId());
          continue;
        }
        ...
        if (existing.isPresent()) {
          ...
          log.info(
              "[FOLDER_CREATE] File '{}' recognized as moved file, updated existing book's path",
              p);
        } else {
          unmatched.add(p);
        }
    ...
    LibraryPathEntity libPathEntity =
        bookFilePersistenceService.getLibraryPathEntityForFile(library, libraryPath);
  ...
  private FolderAnalysis analyzeFolderForAudiobook(Path folderPath) {
    ...
    boolean isSeriesFolder =
        audioFiles.size() >= MIN_AUDIO_FILES_FOR_FOLDER_AUDIOBOOK
            && FileUtils.isSeriesFolder(audioFiles);

    return new FolderAnalysis(audioFiles.size(), hasNonAudioBook, isSeriesFolder);
  }

  private record FolderAnalysis(
      int audioFileCount, boolean hasNonAudioBook, boolean isSeriesFolder) {
    boolean isFolderBasedAudiobook() {
      return audioFileCount >= MIN_AUDIO_FILES_FOR_FOLDER_AUDIOBOOK
          && !hasNonAudioBook
          && !isSeriesFolder;
    }
  }
  ...
  private void handleFolderDelete(LibraryEntity library, Path folderPath) {
    ...
    try {
      ...
      String relativePrefix =
          Path.of(libPathEntity.getPath()).relativize(folderPath).toString().replace("\\", "/");
      List<BookEntity> booksWithFiles =
          bookRepository.findBooksWithFilesUnderPath(libPathEntity.getId(), relativePrefix);
      ...
      log.info(
          "[PENDING_DELETE] {} books under '{}' deferred deletion (grace period {}ms)",
          booksWithFiles.size(),
          folderPath,
          PENDING_DELETION_GRACE_MS);
    } catch (Exception e) {
  ...
  private void scheduleWithStabilityCheck(Path path, Runnable onStable) {
    scheduler.schedule(
        () -> checkStability(path, onStable, System.currentTimeMillis()), 0, TimeUnit.MILLISECONDS);
  }
  ...
  private void checkStability(Path path, Runnable onStable, long startTime) {
    try {
      ...
      if (msSinceMod < STABILITY_CHECK_INTERVAL_MS) {
        log.debug(
            "[STABILITY] File '{}' modified {}ms ago, rechecking in {}ms",
            path,
            msSinceMod,
            STABILITY_CHECK_INTERVAL_MS);
        scheduler.schedule(
            () -> checkStability(path, onStable, startTime),
            STABILITY_CHECK_INTERVAL_MS,
            TimeUnit.MILLISECONDS);
      } else {
        onStable.run();
  ...
  public boolean hasPendingEventsForPaths(Set<Path> paths) {
    boolean inQueue =
        eventQueue.stream().anyMatch(e -> paths.stream().anyMatch(p -> e.fullPath().startsWith(p)));
    boolean inDeletes =
        pendingDeletes.keySet().stream().anyMatch(p -> paths.stream().anyMatch(p::startsWith));
    boolean inFolderCreates =
        pendingFolderCreates.keySet().stream()
