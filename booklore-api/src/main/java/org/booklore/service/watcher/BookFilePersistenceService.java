...
public class BookFilePersistenceService {
  ...
  @Transactional
  public void updatePathIfChanged(
      BookEntity book, LibraryEntity libraryEntity, Path path, String currentHash) {
    ...
    boolean changed =
        !Objects.equals(newSubPath, matchedFile.getFileSubPath())
    ...
    if (changed) {
      ...
      log.info(
          "[FILE_CREATE] Updated path / undeleted existing book with hash '{}': '{}'",
          currentHash,
          path);
    } else {
      log.info(
          "[FILE_CREATE] Book with hash '{}' already exists at same path. Skipping update.",
          currentHash);
    }
  ...
  String findMatchingLibraryPath(LibraryEntity libraryEntity, Path filePath) {
    return libraryEntity.getLibraryPaths().stream()
        ...
        .orElseThrow(
            () ->
                ApiError.LIBRARY_NOT_FOUND.createException(
                    "No matching libraryPath for: " + filePath));
  }
  ...
  public int markAllBooksUnderPathAsDeleted(long libraryPathId, String relativeFolderPath) {
    ...
    List<BookEntity> books =
        bookRepository.findAllByLibraryPathIdAndFileSubPathStartingWith(
            libraryPathId, relativeFolderPath);
    books.forEach(
        book -> {
  ...
  public Optional<BookEntity> findByLibraryPathSubPathAndFileName(
      long libraryPathId, String fileSubPath, String fileName) {
    return bookRepository.findByLibraryPath_IdAndFileSubPathAndFileName(
        libraryPathId, fileSubPath, fileName);
  }

  @Transactional(readOnly = true)
  public Optional<BookFileEntity> findBookFileByLibraryPathSubPathAndFileName(
      long libraryPathId, String fileSubPath, String fileName) {
    return bookFileRepository.findByLibraryPathIdAndFileSubPathAndFileName(
        libraryPathId, fileSubPath, fileName);
  }
  ...
  @Transactional
  public void recoverFolderBook(
      PendingDeletionPool.BookSnapshot bookSnap,
              ...
              bf -> {
                for (var entry : fileHashes.entrySet()) {
                  if (entry.getValue().equals(fs.currentHash())) {
                    ...
                    String newSubPath =
                        FileUtils.getRelativeSubPath(newLibraryPath.getPath(), filePath);
