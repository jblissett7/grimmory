...
public class BookFileTransactionalHandler {
  ...
  public void handleNewBookFile(long libraryId, Path path) {
    ...
    notificationService.sendMessageToPermissions(
        Topic.LOG,
        LogNotification.info("Started processing file: " + filePath),
        Set.of(ADMIN, MANAGE_LIBRARY));

    LibraryPathEntity libraryPathEntity =
        bookFilePersistenceService.getLibraryPathEntityForFile(libraryEntity, libraryPath);
    String fileSubPath = FileUtils.getRelativeSubPath(libraryPathEntity.getPath(), path);

    Optional<BookFileEntity> existingAtPath =
        bookFilePersistenceService.findBookFileByLibraryPathSubPathAndFileName(
            libraryPathEntity.getId(), fileSubPath, fileName);
    if (existingAtPath.isPresent()) {
      ...
      if (wasDeleted) {
        ...
        notificationService.sendMessageToPermissions(
            Topic.LOG,
            LogNotification.info("Finished processing file: " + filePath),
            Set.of(ADMIN, MANAGE_LIBRARY));
        return;
      }
    ...
    Optional<PendingDeletionPool.MatchResult> poolMatch =
        pendingDeletionPool.matchByHash(currentHash);
    if (poolMatch.isPresent()) {
      ...
      log.info(
          "[CREATE] File '{}' matched pending deletion, recovered book id={}",
          filePath,
          match.book().bookId());
      notificationService.sendMessageToPermissions(
          Topic.LOG,
    ...
    Optional<BookEntity> existingByHash =
        bookRepository.findByCurrentHashIncludingRecentlyDeleted(
            currentHash, Instant.now().minus(60, ChronoUnit.SECONDS));
    if (existingByHash.isPresent()) {
      bookFilePersistenceService.updatePathIfChanged(
          existingByHash.get(), libraryEntity, path, currentHash);
      log.info(
          "[CREATE] File '{}' recognized as moved file, updated existing book's path", filePath);
      notificationService.sendMessageToPermissions(
          Topic.LOG,
          LogNotification.info("Finished processing file: " + filePath),
          Set.of(ADMIN, MANAGE_LIBRARY));
    ...
    var mode =
        libraryEntity.getOrganizationMode() != null
            ? libraryEntity.getOrganizationMode()
            : LibraryOrganizationMode.AUTO_DETECT;

    BookEntity filelessMatch =
        (mode == LibraryOrganizationMode.AUTO_DETECT)
    ...
    if (filelessMatch != null) {
      ...
      log.info(
          "[CREATE] Attached file '{}' to fileless book id={}", filePath, filelessMatch.getId());
      notificationService.sendMessageToPermissions(
          Topic.LOG,
          LogNotification.info("Finished processing file: " + filePath),
          Set.of(ADMIN, MANAGE_LIBRARY));
    ...
    if (mode == LibraryOrganizationMode.BOOK_PER_FILE) {
      ...
      if (matchingBook == null) {
        BookFileType fileType =
            BookFileExtension.fromFileName(fileName).map(BookFileExtension::getType).orElse(null);
    ...
    if (matchingBook != null) {
      ...
      LibraryFile libraryFile =
          LibraryFile.builder()
              ...
              .fileName(fileName)
              .bookFileType(
                  BookFileExtension.fromFileName(fileName)
                      .map(BookFileExtension::getType)
                      .orElseThrow(
                          () ->
                              new IllegalArgumentException(
                                  "Unsupported book file type: " + fileName)))
              .build();
  ...
  public void handleNewFolderAudiobook(long libraryId, Path folderPath) {
    ...
    String libraryPath =
        bookFilePersistenceService.findMatchingLibraryPath(libraryEntity, folderPath);

    notificationService.sendMessageToPermissions(
        Topic.LOG,
        LogNotification.info("Started processing folder audiobook: " + folderPath),
        Set.of(ADMIN, MANAGE_LIBRARY));
    ...
    Optional<PendingDeletionPool.MatchResult> poolMatch =
        pendingDeletionPool.matchByHash(folderHash);
    if (poolMatch.isPresent()) {
      var match = poolMatch.get();
      pendingDeletionPool.recoverBook(
          match, libraryPathEntity, fileSubPath, folderName, folderHash);
      log.info(
          "[CREATE] Folder audiobook '{}' matched pending deletion, recovered book id={}",
          folderName,
          match.book().bookId());
      notificationService.sendMessageToPermissions(
          Topic.LOG,
          LogNotification.info("Finished processing folder audiobook: " + folderPath),
          Set.of(ADMIN, MANAGE_LIBRARY));
      return;
    }

    BookEntity matchingBook =
        findMatchingBookForFolderAudiobook(libraryPathEntity.getId(), fileSubPath, folderName);

    if (matchingBook != null) {
      autoAttachFolderAudiobook(matchingBook, folderName, fileSubPath, folderPath);
      String primaryFileName =
          matchingBook.hasFiles()
              ? matchingBook.getPrimaryBookFile().getFileName()
              : "book#" + matchingBook.getId();
      log.info(
          "[CREATE] Auto-attached folder audiobook '{}' to existing book '{}'",
          folderName,
          primaryFileName);
    } else {
      LibraryFile libraryFile =
  ...
  private BookEntity findMatchingFilelessBook(
      LibraryEntity library, String fileName, LibraryPathEntity fileLibraryPath) {
    ...
    for (BookEntity book : filelessBooks) {
      if (book.getLibraryPath() != null
          && !book.getLibraryPath().getId().equals(fileLibraryPath.getId())) {
        continue;
      }
  ...
  private BookEntity findExactFilelessBook(
      LibraryEntity library, String fileName, LibraryPathEntity fileLibraryPath) {
    List<BookEntity> filelessBooks = bookRepository.findFilelessBooksByLibraryId(library.getId());
    String fileBaseName = BookFileGroupingUtils.extractGroupingKey(fileName);
  ...
  private BookEntity findBookInSameFolder(Long libraryPathId, String fileSubPath) {
    ...
    List<BookEntity> booksInDirectory =
        bookRepository.findAllByLibraryPathIdAndFileSubPath(libraryPathId, fileSubPath);
    List<BookEntity> activeBooks =
        booksInDirectory.stream()
    ...
    return activeBooks.stream()
        .max(
            Comparator.comparingLong(
                book -> book.getBookFiles() != null ? book.getBookFiles().size() : 0))
        .orElse(null);
  }
  ...
  private BookEntity findNearestAncestorBookWithEbook(Long libraryPathId, String subPath) {
    ...
    while (true) {
      ...
      List<BookEntity> booksAtLevel =
          bookRepository.findAllByLibraryPathIdAndFileSubPath(libraryPathId, current);
      for (BookEntity book : booksAtLevel) {
        if (book.getDeleted() != null && book.getDeleted()) {
          continue;
        }
        if (book.getBookFiles() != null
            && book.getBookFiles().stream()
                .anyMatch(bf -> bf.getBookType() != BookFileType.AUDIOBOOK)) {
          log.debug(
              "BOOK_PER_FOLDER: Audio absorption matched to ancestor book id={} at '{}'",
              book.getId(),
              current);
          return book;
        }
  ...
  private BookEntity findMatchingBook(Long libraryPathId, String fileSubPath, String fileName) {
    ...
    for (BookEntity book : booksInDirectory) {
      ...
      String existingGroupingKey =
          BookFileGroupingUtils.extractGroupingKey(primaryFile.getFileName());
      ...
      double similarity =
          BookFileGroupingUtils.calculateSimilarity(fileGroupingKey, existingGroupingKey);
    ...
    if (fuzzyMatch != null) {
      String primaryFileName =
          fuzzyMatch.hasFiles()
              ? fuzzyMatch.getPrimaryBookFile().getFileName()
              : "book#" + fuzzyMatch.getId();
      log.debug(
          "Fuzzy matched '{}' to '{}' with similarity {}",
          fileName,
          primaryFileName,
          bestSimilarity);
    }
  ...
  private BookEntity findMatchingBookForFolderAudiobook(
      Long libraryPathId, String fileSubPath, String folderName) {
    String parentPath =
        Optional.ofNullable(fileSubPath)
            .filter(p -> !p.isEmpty())
            .map(
                p -> {
    ...
    List<BookEntity> booksInParent =
        bookRepository.findAllByLibraryPathIdAndFileSubPath(libraryPathId, parentPath);
    ...
    for (BookEntity book : booksInParent) {
      ...
      double similarity =
          BookFileGroupingUtils.calculateSimilarity(folderGroupingKey, existingGroupingKey);
      if (similarity >= FUZZY_MATCH_THRESHOLD && similarity > bestSimilarity) {
        bestSimilarity = similarity;
        fuzzyMatch = book;
      }
    ...
    if (fuzzyMatch != null) {
      String primaryFileName =
          fuzzyMatch.hasFiles()
              ...
              : "book#" + fuzzyMatch.getId();
      log.debug(
          "Fuzzy matched folder '{}' to '{}' with similarity {}",
          folderName,
          primaryFileName,
          bestSimilarity);
    }
  ...
  private void autoAttachFolderAudiobook(
      BookEntity book, String folderName, String fileSubPath, Path folderPath) {
    String hash = FileFingerprint.generateFolderHash(folderPath);
    BookFileEntity additionalFile =
    ...
    String primaryFileName =
        book.hasFiles() ? book.getPrimaryBookFile().getFileName() : "book#" + book.getId();
