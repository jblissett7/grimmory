...
public class PendingDeletionPool {
  ...
  public record BookSnapshot(
      Long bookId, Long libraryPathId, String fileSubPath, List<FileSnapshot> files) {}
  ...
  public void addFileDeletion(
    ...
    FileSnapshot fileSnapshot =
        new FileSnapshot(
            ...
            bookFile.getFileSubPath(),
            bookFile.getCurrentHash(),
            bookFile.isFolderBased(),
            bookFile.getBookType());
    ...
    PendingDeletion pending =
        new PendingDeletion(path, false, libraryId, Instant.now(), timer, hashMap, affectedBooks);
    ...
    log.debug(
        "[POOL] Added file deletion: path='{}', hash='{}', bookId={}",
        path,
        bookFile.getCurrentHash(),
        book.getId());
  }

  public void addFolderDeletion(
      Path folderPath, long libraryId, List<BookEntity> booksWithFiles, ScheduledFuture<?> timer) {
    Map<String, Long> hashMap = new ConcurrentHashMap<>();
    Map<Long, BookSnapshot> affectedBooks = new ConcurrentHashMap<>();
      ...
      if (!fileSnapshots.isEmpty()) {
        BookSnapshot bs =
            new BookSnapshot(
                book.getId(),
                book.getLibraryPath() != null ? book.getLibraryPath().getId() : null,
                book.getBookFiles().getFirst().getFileSubPath(),
                fileSnapshots);
    ...
    PendingDeletion pending =
        new PendingDeletion(
            folderPath, true, libraryId, Instant.now(), timer, hashMap, affectedBooks);
    pendingByPath.put(folderPath, pending);

    log.debug(
        "[POOL] Added folder deletion: path='{}', books={}, hashes={}",
        folderPath,
        affectedBooks.size(),
        hashMap.size());
  }
  ...
  public Optional<FolderMatchResult> matchFolderByHashes(Map<Path, String> fileHashes) {
    ...
    if (bestMatch != null) {
      ...
      log.debug(
          "[POOL] Folder matched by hashes: path='{}', overlap={}",
          bestMatch.originalPath(),
          bestOverlap);
    ...
    for (BookSnapshot bookSnap : pending.affectedBooks().values()) {
      ...
      if (remainingFiles <= 1) {
        ...
        for (FileSnapshot fs : bookSnap.files()) {
          bookFileRepository
              .findById(fs.bookFileId())
              .ifPresent(
                  bf -> {
                    bookFileRepository.delete(bf);
                    log.info(
                        "[EXPIRED] BookFile id={} removed from book id={}",
                        fs.bookFileId(),
                        book.getId());
                  });
        }
  ...
  public void expireFolderDeletion(Path path) {
    ...
    if (!bookIds.isEmpty()) {
      notificationService.sendMessageToPermissions(
          Topic.BOOKS_REMOVE, bookIds, Set.of(PermissionType.ADMIN, PermissionType.MANAGE_LIBRARY));
    }
  }
  ...
  public void recoverBook(
      MatchResult match,
      org.booklore.model.entity.LibraryPathEntity newLibraryPath,
      String newFileSubPath,
      String newFileName,
      String hash) {
    BookEntity book =
        bookRepository
            .findById(match.book().bookId())
            .orElseThrow(
                () -> new IllegalStateException("Book not found: " + match.book().bookId()));

    BookFileEntity bookFile =
        bookFileRepository
            .findById(match.file().bookFileId())
            .orElseThrow(
                () ->
                    new IllegalStateException("BookFile not found: " + match.file().bookFileId()));
      ...
      if (pd.isDirectory() && filePath.startsWith(entry.getKey())) {
        for (BookSnapshot bookSnap : pd.affectedBooks().values()) {
          for (FileSnapshot fileSnap : bookSnap.files()) {
            ...
            String relPath =
                entry.getKey().getParent() != null
                    ? entry.getKey().getParent().relativize(filePath).toString()
                    : filePath.toString();
            if (relPath.endsWith(fileSnap.fileName())) {
              ...
              if (pd.hashToBookFileId().isEmpty()) {
                ...
                log.debug(
                    "[POOL] All files matched for path='{}', cancelled timer", entry.getKey());
              }
              return true;
            }
          }
  ...
  public boolean hasPendingForPaths(Set<Path> paths) {
    return pendingByPath.keySet().stream().anyMatch(p -> paths.stream().anyMatch(p::startsWith));
  }
}
