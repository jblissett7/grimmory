...
/**
 * Service to sync reading progress to Hardcover. Uses per-user Hardcover API tokens for reading
 * progress sync. Each user can configure their own Hardcover API key in their sync settings.
 */
@Slf4j
@Service
public class HardcoverSyncService {
  ...
  public HardcoverSyncService(
      HardcoverSyncSettingsService hardcoverSyncSettingsService, BookRepository bookRepository) {
    this.hardcoverSyncSettingsService = hardcoverSyncSettingsService;
    this.bookRepository = bookRepository;
    this.restClient = RestClient.builder().baseUrl(HARDCOVER_API_URL).build();
  }

  /**
   * Asynchronously sync Kobo reading progress to Hardcover. This method is non-blocking and will
   * not fail the calling process if sync fails. Uses the user's personal Hardcover API key if
   * configured.
   *
  ...
  public void syncProgressToHardcover(Long bookId, Float progressPercent, Long userId) {
    try {
      ...
      HardcoverSyncSettings userSettings =
          hardcoverSyncSettingsService.getSettingsForUserId(userId);

      if (!isHardcoverSyncEnabledForUser(userSettings)) {
        log.trace(
            "Hardcover sync skipped for user {}: not enabled or no API token configured", userId);
        return;
      }
      ...
      try {
        ...
        if (metadata.getHardcoverBookId() != null) {
          ...
          if (fetched != null) {
            ...
            log.debug(
                "Fetched from Hardcover: editionId={}, pages={}",
                hardcoverBook.editionId,
                hardcoverBook.pages);
          } else {
            log.warn(
                "Could not fetch edition info from Hardcover for book ID: {}",
                hardcoverBook.bookId);
          }
        } else {
          // Search by ISBN
        ...
        log.info(
            "Progress calculation: userId={}, progressPercent={}%, totalPages={}, progressPages={}",
        ...
        boolean success =
            upsertReadingProgress(userBookId, hardcoverBook.editionId, progressPages, isFinished);
      ...
      log.error(
          "Failed to sync progress to Hardcover for book {} (user {}): {}",
          bookId,
          userId,
          e.getMessage());
    }
  ...
  /**
   * Find a book on Hardcover by ISBN or hardcoverId. Returns the numeric book_id, edition_id, and
   * page count.
   */
  private HardcoverBookInfo findHardcoverBook(BookMetadataEntity metadata) {
    ...
    try {
      ...
      // Fetch page count from the edition (prioritizing edition page count over book-level page
      // count)
      ...
      log.info(
          "Found Hardcover book: bookId={}, editionId={}, pages={}",
          info.bookId,
          info.editionId,
          info.pages);
  ...
  /**
   * Find an edition by ISBN for a given book. This queries Hardcover's editions table to match by
   * ISBN.
   */
    ...
    try {
      ...
      if (info.editionId != null) {
        ...
        if (edition != null && edition.pages != null && edition.pages > 0) {
          info.pages = edition.pages;
          log.debug(
              "Using page count from default physical edition {}: {} pages",
              info.editionId,
              info.pages);
        }
      }
  ...
  private Integer insertOrGetUserBook(Integer bookId, Integer editionId, int statusId) {
    ...
    try {
      ...
      if (error != null && !error.isBlank()) {
        log.debug(
            "insert_user_book returned error: {} - book may already exist, trying to find it",
            error);
  ...
  private boolean upsertReadingProgress(
      Integer userBookId, Integer editionId, int progressPages, boolean isFinished) {
    log.info(
        "upsertReadingProgress: userBookId={}, editionId={}, progressPages={}, isFinished={}",
        userBookId,
        editionId,
        progressPages,
        isFinished);
  ...
  private boolean insertUserBookRead(
      Integer userBookId, Integer editionId, int progressPages, boolean isFinished) {
    String mutation =
        """
  ...
  private boolean updateUserBookRead(
      Integer readId, Integer editionId, int progressPages, boolean isFinished) {
    String mutation =
        """
