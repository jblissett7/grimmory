...
public class TaskHistoryService {
  ...
  private String buildTaskDescription(TaskType type, Map<String, Object> options) {
    ...
    if (bookIds instanceof Collection<?> ids && !ids.isEmpty()) {
      ...
      while (it.hasNext()) {
        ...
        if (isLast
            && sb.length() + separator.length() + id.length() + 1 <= MAX_DESCRIPTION_LENGTH) {
          sb.append(separator).append(id).append(")");
        } else if (!isLast
            && sb.length() + separator.length() + id.length() + truncationSuffix.length()
  ...
  public TasksHistoryResponse getLatestTasksForEachType() {
    ...
    try {
      ...
      log.warn(
          "Error fetching latest tasks, possibly due to removed enum values: {}", e.getMessage());
      latestTasks = Collections.emptyList();
    }

    Map<TaskType, TaskHistoryEntity> taskHistoryMap =
        latestTasks.stream()
            .filter(
                task -> {
                  try {
            ...
            .collect(
                Collectors.toMap(
                    TaskHistoryEntity::getType, task -> task, (existing, replacement) -> existing));
    ...
    return TasksHistoryResponse.builder().taskHistories(allTasks).build();
  }
