...
public class TaskCronService {
  ...
  public CronConfig getCronConfigOrDefault(TaskType taskType) {
    ...
    return repository
        ...
        .orElse(CronConfig.builder().taskType(taskType).enabled(false).build());
  }
  ...
  private void validateTaskTypeForCron(TaskType taskType) {
    ...
    if (!taskType.isCronSupported()) {
      throw new APIException(
          "Task type " + taskType + " does not support cron scheduling", HttpStatus.BAD_REQUEST);
    }
  }
  ...
  private void validateCronExpression(String cronExpression) {
    ...
    if (fields.length != 6) {
      throw new APIException(
          "Invalid cron expression format. Expected 6 fields (second minute hour day month day-of-week)",
