...
public class KoboServerProxy {
  ...
  private static final Set<String> HEADERS_OUT_INCLUDE =
      Set.of(
  ...
  private static final Set<String> HEADERS_OUT_EXCLUDE = Set.of(KoboHeaders.X_KOBO_SYNCTOKEN);
  ...
  public ResponseEntity<JsonNode> proxyCurrentRequest(Object body, boolean includeSyncToken) {
    ...
    if (includeSyncToken) {
      ...
      if (syncToken == null
          || syncToken.getRawKoboSyncToken() == null
          || syncToken.getRawKoboSyncToken().isBlank()) {
  ...
  public ResponseEntity<Resource> proxyExternalUrl(String url) {
    try {
      ...
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to fetch image", e);
    }
  }

  private ResponseEntity<JsonNode> executeProxyRequest(
    ...
    try {
      ...
      if (responseHeaders.getFirst(KoboHeaders.X_KOBO_SYNCTOKEN) != null
          && includeSyncToken
          && syncToken != null) {
        String koboToken = responseHeaders.getFirst(KoboHeaders.X_KOBO_SYNCTOKEN);
        if (koboToken != null) {
          BookloreSyncToken updated =
              BookloreSyncToken.builder()
                  ...
                  .build();
          responseHeaders.set(
              KoboHeaders.X_KOBO_SYNCTOKEN, bookloreSyncTokenGenerator.toBase64(updated));
        }
      }
      ...
      return new ResponseEntity<>(
          responseBody, responseHeaders, HttpStatus.valueOf(response.statusCode()));

    } catch (Exception e) {
      log.error("Failed to proxy request to Kobo", e);
      throw new ResponseStatusException(
          HttpStatus.INTERNAL_SERVER_ERROR, "Failed to proxy request to Kobo", e);
    }
  }
}
