...
public class KoboLibrarySyncService {
  ...
  public ResponseEntity<?> syncLibrary(BookLoreUser user, String token) {
    ...
    BookloreSyncToken syncToken =
        Optional.ofNullable(tokenGenerator.fromRequestHeaders(request))
            .orElse(new BookloreSyncToken());

    KoboLibrarySnapshotEntity currSnapshot =
    ...
    Optional<KoboLibrarySnapshotEntity> prevSnapshot =
        koboLibrarySnapshotService.findByIdAndUserId(
            syncToken.getLastSuccessfulSyncPointId(), user.getId());
    ...
    if (prevSnapshot.isPresent()) {
      ...
      koboLibrarySnapshotService.updateSyncedStatusForExistingBooks(
          prevSnapshot.get().getId(), currSnapshot.getId());

      Page<KoboSnapshotBookEntity> addedPage =
          koboLibrarySnapshotService.getNewlyAddedBooks(
              prevSnapshot.get().getId(),
              currSnapshot.getId(),
              PageRequest.of(0, maxRemaining),
              user.getId());
      ...
      if (addedPage.isLast() && maxRemaining > 0) {
        changedPage =
            koboLibrarySnapshotService.getChangedBooks(
                prevSnapshot.get().getId(), currSnapshot.getId(), PageRequest.of(0, maxRemaining));
      ...
      if (changedPage.isLast() && maxRemaining > 0) {
        removedPage =
            koboLibrarySnapshotService.getRemovedBooks(
                prevSnapshot.get().getId(),
                currSnapshot.getId(),
                user.getId(),
                PageRequest.of(0, maxRemaining));
      ...
      Set<Long> addedIds =
          addedAll.stream().map(KoboSnapshotBookEntity::getBookId).collect(Collectors.toSet());
      Set<Long> changedIds =
          changedAll.stream().map(KoboSnapshotBookEntity::getBookId).collect(Collectors.toSet());
      Set<Long> removedIds =
          removedAll.stream().map(KoboSnapshotBookEntity::getBookId).collect(Collectors.toSet());
      ...
      while (maxRemaining > 0) {
        Page<KoboSnapshotBookEntity> page =
            koboLibrarySnapshotService.getUnsyncedBooks(
                currSnapshot.getId(), PageRequest.of(0, maxRemaining));
    ...
    if (!shouldContinueSync) {
      ...
      Collection<Entitlement> syncResultsKobo =
          Optional.ofNullable(koboStoreResponse.getBody())
              .map(
                  body -> {
                    try {
      ...
      shouldContinueSync =
          "continue"
              .equalsIgnoreCase(
                  Optional.ofNullable(
                          koboStoreResponse.getHeaders().getFirst(KoboHeaders.X_KOBO_SYNC))
                      .orElse(""));

      String koboSyncTokenHeader =
          koboStoreResponse.getHeaders().getFirst(KoboHeaders.X_KOBO_SYNCTOKEN);
      syncToken =
          koboSyncTokenHeader != null ? tokenGenerator.fromBase64(koboSyncTokenHeader) : syncToken;
    }
    ...
    if (shouldContinueSync) {
      ...
      koboDeletedBookProgressRepository.deleteBySnapshotIdAndUserId(
          syncToken.getOngoingSyncPointId(), user.getId());
  ...
  private List<ChangedReadingState> syncReadingStatesToKobo(Long userId, String snapshotId) {
    ...
    List<ChangedReadingState> changedStates =
        entitlementService.generateChangedReadingStates(booksNeedingSync);
