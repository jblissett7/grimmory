...
public class KoboEntitlementService {
        ...
        .map(
            book ->
                NewEntitlement.builder()
                    .newEntitlement(
                        BookEntitlementContainer.builder()
  ...
  public List<? extends Entitlement> generateChangedEntitlements(
      Set<Long> bookIds, String token, boolean removed) {
    ...
    return books.stream()
        .filter(
            bookEntity ->
                bookEntity.getPrimaryBookFile() != null
                    && bookEntity.getPrimaryBookFile().getBookType() == BookFileType.EPUB)
        .map(
            book ->
  ...
  public List<ChangedReadingState> generateChangedReadingStates(
      List<UserBookProgressEntity> progressEntries) {
  ...
  public List<KoboTagWrapper> generateTags() {
    ...
    Set<Long> koboBookIDs =
        shelfRepository
            .findByUserIdAndName(userId, ShelfType.KOBO.getName())
            .orElseThrow(
                () -> new NoSuchElementException("Kobo shelf not found for user: " + userId))
            .getBookEntities()
            .stream()
            .filter(koboCompatibilityService::isBookSupportedForKobo)
        ...
        .map(
            shelf ->
                buildKoboTag(
                    ...
                    null,
                    null,
                    shelf.getBookEntities().stream().map(BookEntity::getId).toList(),
                    koboBookIDs))
        .forEach(tags::add);
    ...
    magicShelfRepository
        ...
        .forEach(
            magicShelf -> {
              try {
                List<Long> bookIds =
                    magicShelfBookService.getBookIdsByMagicShelfId(
                        userId, magicShelf.getId(), MAX_MAGIC_SHELF_BOOKS_FOR_KOBO);
                tags.add(
                    buildKoboTag(
                        "BL-MS-" + magicShelf.getId(),
                        magicShelf.getName(),
                        magicShelf.getCreatedAt().atOffset(ZoneOffset.UTC).toString(),
                        magicShelf.getUpdatedAt().atOffset(ZoneOffset.UTC).toString(),
                        bookIds,
                        koboBookIDs));
              } catch (Exception e) {
                log.warn(
                    "Skipping magic shelf '{}' during Kobo tag generation: {}",
                    magicShelf.getName(),
                    e.getMessage());
              }
            });
  ...
  private KoboTagWrapper buildKoboTag(
      String id,
      String name,
      String created,
      String modified,
      List<Long> bookIds,
      Set<Long> koboFilterIds) {
    List<KoboTag.KoboTagItem> items =
        bookIds.stream()
            .filter(koboFilterIds::contains)
            .map(
                bookId ->
                    KoboTag.KoboTagItem.builder()
    ...
    if (items.isEmpty()) {
      return KoboTagWrapper.builder()
          .deletedTag(
              KoboTagWrapper.WrappedTag.builder()
                  .tag(KoboTag.builder().id(id).lastModified(modified).build())
                  .build())
          .build();
    }
  ...
  private ChangedReadingState buildChangedReadingState(
      UserBookProgressEntity progress, String timestamp, OffsetDateTime now) {
    ...
    KoboReadingState.CurrentBookmark bookmark =
        (progress.getKoboProgressPercent() != null
                || (twoWaySync && progress.getEpubProgressPercent() != null))
    ...
    return ChangedReadingState.builder()
        .changedReadingState(
            ChangedReadingState.WrappedReadingState.builder().readingState(readingState).build())
        .build();
  }
  ...
  private KoboReadingState getReadingStateForBook(BookEntity book) {
    ...
    KoboReadingState existingState =
        readingStateRepository
            .findByEntitlementIdAndUserId(entitlementId, userId)
            .or(
                () ->
                    readingStateRepository
    ...
    Optional<UserBookProgressEntity> userProgress =
        progressRepository.findByUserIdAndBookId(
            authenticationService.getAuthenticatedUser().getId(), book.getId());

    boolean twoWaySync = koboSettingsService.getCurrentUserSettings().isTwoWayProgressSync();

    KoboReadingState.CurrentBookmark webReaderBookmark =
        twoWaySync
            ? userProgress
                .filter(
                    progress ->
    ...
    KoboReadingState.StatusInfo statusInfo =
        userProgress
            .map(
                progress ->
                    readingStateBuilder.buildStatusInfoFromProgress(progress, now.toString()))
            .orElseGet(
                () ->
  ...
  private BookEntitlement buildBookEntitlement(BookEntity book, boolean removed) {
    ...
    return BookEntitlement.builder()
        .activePeriod(BookEntitlement.ActivePeriod.builder().from(now.toString()).build())
        .removed(removed)
        .status("Active")
        .crossRevisionId(String.valueOf(book.getId()))
        .revisionId(String.valueOf(book.getId()))
  ...
  public KoboBookMetadata getMetadataForBook(long bookId, String token) {
    List<BookEntity> books =
        bookQueryService.findAllWithMetadataByIds(Set.of(bookId)).stream()
            .filter(koboCompatibilityService::isBookSupportedForKobo)
            .toList();
  ...
  private KoboBookMetadata mapToKoboMetadata(BookEntity book, String token) {
    ...
    if (metadata.getSeriesName() != null && !metadata.getSeriesName().isBlank()) {
      series =
          KoboBookMetadata.Series.builder()
              .id("series_" + metadata.getSeriesName().hashCode())
              .name(metadata.getSeriesName())
              .number(
                  metadata.getSeriesNumber() != null
                      ? BigDecimal.valueOf(metadata.getSeriesNumber())
                          .stripTrailingZeros()
                          .toPlainString()
                      : "1")
              .numberFloat(
                  metadata.getSeriesNumber() != null
                      ? metadata.getSeriesNumber().doubleValue()
                      : 1.0)
              .build();
    } else {
      series =
          KoboBookMetadata.Series.builder().id("").name("").number("").numberFloat(0.0).build();
    }
    ...
    return KoboBookMetadata.builder()
        ...
        .slug(
            metadata.getTitle() != null
                ? NON_ALPHANUMERIC_LOWERCASE_PATTERN
                    .matcher(metadata.getTitle().toLowerCase())
                    .replaceAll("-")
                : null)
        .coverImageId(book.getBookCoverHash())
        .workId(String.valueOf(book.getId()))
