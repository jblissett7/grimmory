...
public class KoboReadingStateService {
  ...
  public KoboReadingStateResponse saveReadingState(List<KoboReadingState> readingStates) {
    ...
    List<KoboReadingStateResponse.UpdateResult> updateResults =
        koboReadingStates.stream()
            .map(
                state ->
                    KoboReadingStateResponse.UpdateResult.builder()
    ...
    return dtos.stream()
        .map(
            dto -> {
              ...
              log.debug(
                  "Kobo reading state lookup: entitlementId={}, foundExisting={}",
                  entitlementId,
                  existingOpt.isPresent());
              KoboReadingStateEntity entity =
                  existingOpt
                      ...
                      .orElseGet(
                          () -> {
                            ...
                            newEntity.setPriorityTimestamp(
                                mapper.cleanString(computePriorityTimestamp(dto)));
  ...
  public void deleteReadingState(Long bookId) {
    ...
    repository
        .findByEntitlementIdAndUserId(String.valueOf(bookId), userId)
        .ifPresent(repository::delete);
  }

  public List<KoboReadingState> getReadingState(String entitlementId) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    Optional<KoboReadingState> readingState =
        repository
          ...
          .ifPresent(
              progress -> {
                ...
                KoboReadingState.CurrentBookmark webReaderBookmark =
                    readingStateBuilder.buildBookmarkFromProgress(progress);
          ...
          .filter(
              progress ->
                  progress.getKoboProgressPercent() != null
                      || progress.getKoboLocation() != null
                      || (twoWaySync && progress.getEpubProgressPercent() != null))
          .map(
              progress ->
                  readingStateBuilder.buildReadingStateFromProgress(entitlementId, progress));
    } catch (NumberFormatException e) {
  ...
  private void syncKoboProgressToUserBookProgress(KoboReadingState readingState, Long userId) {
    try {
      ...
      // But only if the progress percentage has changed from last time, or the read status has
      // changed
      if (progress.getKoboProgressPercent() != null
          && (!progress.getKoboProgressPercent().equals(prevousKoboProgressPercent)
              || progress.getReadStatus() != previousReadStatus)) {
        hardcoverSyncService.syncProgressToHardcover(
            book.getId(), progress.getKoboProgressPercent(), userId);
      }
    } catch (NumberFormatException e) {
  ...
  private boolean crossPopulateEpubFieldsFromKobo(
      UserBookProgressEntity progress,
    ...
    UserBookFileProgressEntity fileProgress =
        book.getPrimaryBookFile() != null
            ? fileProgressRepository
                .findByUserIdAndBookFileId(userId, book.getPrimaryBookFile().getId())
                .orElse(null)
            : null;

    Instant bookmarkTime = parseTimestamp(bookmark.getLastModified());
    boolean webReaderIsNewer =
        fileProgress != null
  ...
  private void normalizeResponseTimestamps(KoboReadingState state) {
    ...
    if (state.getStatusInfo() != null) {
      state
          .getStatusInfo()
          .setLastModified(normalizeTimestampValue(state.getStatusInfo().getLastModified()));
    }
    if (state.getStatistics() != null) {
      state
          .getStatistics()
          .setLastModified(normalizeTimestampValue(state.getStatistics().getLastModified()));
    }
    if (state.getCurrentBookmark() != null) {
      state
          .getCurrentBookmark()
          .setLastModified(normalizeTimestampValue(state.getCurrentBookmark().getLastModified()));
    }
  }
  ...
  private String normalizeTimestampValue(String value) {
    ...
    try {
      ...
      return KOBO_TIMESTAMP_FORMAT.format(
          offsetDateTime.toInstant().truncatedTo(ChronoUnit.SECONDS));
    } catch (Exception ignored) {
    }
    try {
  ...
  private KoboReadingStateEntity mergeReadingState(
      KoboReadingStateEntity existing, KoboReadingState incoming) {
  ...
  private KoboReadingState mergeReadingState(KoboReadingState existing, KoboReadingState incoming) {
    ...
    if (incoming.getStatusInfo() != null
        && isIncomingNewer(
            incoming.getStatusInfo().getLastModified(),
            statusInfo != null ? statusInfo.getLastModified() : null)) {
      statusInfo = incoming.getStatusInfo();
    }
    if (incoming.getStatistics() != null
        && isIncomingNewer(
            incoming.getStatistics().getLastModified(),
            statistics != null ? statistics.getLastModified() : null)) {
      statistics = incoming.getStatistics();
    }
    if (incoming.getCurrentBookmark() != null
        && isIncomingNewer(
            incoming.getCurrentBookmark().getLastModified(),
  ...
  private String computePriorityTimestamp(KoboReadingState state) {
    Instant maxInstant =
        maxInstant(
            ...
            state.getCurrentBookmark() != null
                ? state.getCurrentBookmark().getLastModified()
                : null);
    if (maxInstant == null) {
  ...
  private boolean shouldPreserveCurrentStatus(UserBookProgressEntity progress, Instant now) {
    ...
    boolean hasPendingStatusUpdate =
        statusSentTime == null || statusModifiedTime.isAfter(statusSentTime);
  ...
  private ReadStatus deriveStatusFromProgress(double progressPercent) {
    ...
    float finishedThreshold =
        settings.getProgressMarkAsFinishedThreshold() != null
            ? settings.getProgressMarkAsFinishedThreshold()
            : 99f;
    float readingThreshold =
        settings.getProgressMarkAsReadingThreshold() != null
