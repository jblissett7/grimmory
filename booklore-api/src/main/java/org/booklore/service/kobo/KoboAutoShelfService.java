...
public class KoboAutoShelfService {
  ...
  public void autoAddBookToKoboShelves(Long bookId) {
    ...
    List<KoboUserSettingsEntity> eligibleUsers =
        koboUserSettingsRepository.findByAutoAddToShelfTrueAndSyncEnabledTrue();
    ...
    List<Long> userIds = eligibleUsers.stream().map(KoboUserSettingsEntity::getUserId).toList();

    List<ShelfEntity> shelves =
        shelfRepository.findByUserIdInAndName(userIds, ShelfType.KOBO.getName());

    Map<Long, ShelfEntity> shelfByUser =
        shelves.stream().collect(Collectors.toMap(s -> s.getUser().getId(), s -> s));
