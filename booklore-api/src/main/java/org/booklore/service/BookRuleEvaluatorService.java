...
public class BookRuleEvaluatorService {
  ...
  private static final Set<RuleField> COMPOSITE_FIELDS =
      Set.of(RuleField.SERIES_STATUS, RuleField.SERIES_GAPS, RuleField.SERIES_POSITION);
  ...
  public Specification<BookEntity> toSpecification(GroupRule groupRule, Long userId) {
    return (root, query, cb) -> {
      ...
      Join<BookEntity, UserBookProgressEntity> progressJoin =
          root.join("userBookProgress", JoinType.LEFT);
  ...
  private Predicate buildRulePredicate(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildWithinLast(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
  ...
  private Predicate buildOlderThan(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
  ...
  private Predicate buildThisPeriod(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
  ...
  private Predicate buildCompositeFieldPredicate(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
    ...
    Predicate result =
        switch (rule.getField()) {
          case SERIES_STATUS ->
              buildSeriesStatusPredicate(value, query, cb, root, hasSeries, userId);
          case SERIES_GAPS -> buildSeriesGapsPredicate(value, query, cb, root, hasSeries);
          case SERIES_POSITION ->
              buildSeriesPositionPredicate(value, query, cb, root, progressJoin, hasSeries, userId);
  ...
  private Predicate buildMetadataPresencePredicate(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildFieldPresencePredicate(
    ...
    return switch (metadataField) {
      ...
      case "comicCoverArtists" ->
          comicCreatorPresence(query, cb, root, ComicCreatorRole.COVER_ARTIST);
  ...
  private Predicate collectionPresence(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root, String collectionName) {
    Subquery<Long> sub = query.subquery(Long.class);
  ...
  private Predicate comicCollectionPresence(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root, String collectionName) {
    Subquery<Long> sub = query.subquery(Long.class);
    Root<BookEntity> subRoot = sub.from(BookEntity.class);
    Join<Object, Object> metadataJoin = subRoot.join("metadata", JoinType.INNER);
  ...
  private Predicate comicCreatorPresence(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root, ComicCreatorRole role) {
    Subquery<Long> sub = query.subquery(Long.class);
  ...
  private Predicate buildSeriesStatusPredicate(
      String value,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
    ...
    Predicate condition =
        switch (value) {
          case "reading" ->
              seriesHasReadStatus(query, cb, root, userId, List.of("READING", "RE_READING"));
          case "not_started" ->
              cb.not(
                  seriesHasReadStatus(
                      query,
                      ...
                      root,
                      userId,
                      List.of("READ", "READING", "RE_READING", "PARTIALLY_READ")));
          case "fully_read" -> seriesAllRead(query, cb, root, userId);
          case "completed" -> seriesOwnsLastBook(query, cb, root);
          case "ongoing" ->
              cb.and(seriesHasTotal(query, cb, root), cb.not(seriesOwnsLastBook(query, cb, root)));
          default -> cb.conjunction();
        };
    return cb.and(hasSeries, condition);
  }

  private Predicate seriesHasReadStatus(
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Long userId,
    ...
    sub.select(cb.literal(1L))
        .where(
            cb.equal(
                subRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
            cb.equal(subProgress.get("user").get("id"), userId),
            subProgress.get("readStatus").as(String.class).in(statuses));
    return cb.exists(sub);
  }

  private Predicate seriesAllRead(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root, Long userId) {
    ...
    notReadSub
        ...
        .where(
            cb.equal(
                nrRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
  ...
  private Predicate seriesOwnsLastBook(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root) {
    Subquery<Integer> totalSub = query.subquery(Integer.class);
    Root<BookEntity> totalRoot = totalSub.from(BookEntity.class);
    totalSub
        .select(cb.max(totalRoot.get("metadata").get("seriesTotal")))
        .where(
            cb.equal(
                totalRoot.get("metadata").get("seriesName"),
                root.get("metadata").get("seriesName")),
  ...
  private Predicate seriesHasTotal(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root) {
    Subquery<Long> sub = query.subquery(Long.class);
    Root<BookEntity> subRoot = sub.from(BookEntity.class);
    sub.select(cb.literal(1L))
  ...
  private Predicate buildSeriesGapsPredicate(
      String value,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate seriesHasAnyGap(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root) {
    ...
    countSub
        .select(
            cb.countDistinct(
                cb.function("FLOOR", Integer.class, cRoot.get("metadata").get("seriesNumber"))))
        .where(
            cb.equal(
                cRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
    ...
    maxSub
        .select(
            cb.max(cb.function("FLOOR", Integer.class, mRoot.get("metadata").get("seriesNumber"))))
        .where(
            cb.equal(
                mRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
            cb.isNotNull(mRoot.get("metadata").get("seriesNumber")));

    return cb.lt(countSub, maxSub.as(Long.class));
  }

  private Predicate seriesMissingFirst(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root) {
    Subquery<Long> sub = query.subquery(Long.class);
    Root<BookEntity> subRoot = sub.from(BookEntity.class);
    sub.select(cb.literal(1L))
  ...
  private Predicate seriesHasDuplicateNumber(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root) {
    ...
    totalSub
        ...
        .where(
            cb.equal(
                tRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
    ...
    distinctSub
        .select(cb.countDistinct(dRoot.get("metadata").get("seriesNumber")))
        .where(
            cb.equal(
                dRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
            cb.isNotNull(dRoot.get("metadata").get("seriesNumber")));

    return cb.gt(totalSub, distinctSub);
  }

  private Predicate buildSeriesPositionPredicate(
      String value,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate isLastInSeries(
      CriteriaQuery<?> query, CriteriaBuilder cb, Root<BookEntity> root) {
    Subquery<Float> maxSub = query.subquery(Float.class);
    Root<BookEntity> mRoot = maxSub.from(BookEntity.class);
    maxSub
        .select(cb.max(mRoot.get("metadata").get("seriesNumber")))
        .where(
            cb.equal(
                mRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
            cb.isNotNull(mRoot.get("metadata").get("seriesNumber")));
  ...
  private Predicate isNextUnread(
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin,
    ...
    lowerUnreadSub
        ...
        .where(
            cb.equal(
                luRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
            cb.isNotNull(luRoot.get("metadata").get("seriesNumber")),
            cb.lt(
                luRoot.get("metadata").get("seriesNumber"),
                root.get("metadata").get("seriesNumber")),
            cb.or(
    ...
    priorReadSub
        ...
        .where(
            cb.equal(
                prRoot.get("metadata").get("seriesName"), root.get("metadata").get("seriesName")),
            cb.isNotNull(prRoot.get("metadata").get("seriesNumber")),
            cb.lt(
                prRoot.get("metadata").get("seriesNumber"),
                root.get("metadata").get("seriesNumber")),
  ...
  private Predicate buildNotEquals(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
    ...
    if (rule.getField() == RuleField.READ_STATUS
        && !"UNSET".equals(String.valueOf(rule.getValue()))) {
      return cb.or(cb.isNull(progressJoin.get("readStatus")), negated);
    }
    return negated;
  }

  private Predicate buildContains(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildStartsWith(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildEndsWith(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildStringPredicate(
    ...
    if (isArrayField(field)) {
      ...
      subquery
          .select(cb.literal(1L))
          .where(cb.equal(subRoot.get("id"), root.get("id")), predicateBuilder.apply(nameField));
  ...
  private Predicate buildGreaterThan(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
    return buildComparisonPredicate(
        rule,
        cb,
        root,
        progressJoin,
  ...
  private Predicate buildGreaterThanEqual(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
    return buildComparisonPredicate(
        ...
        (field, localDateValue) ->
            cb.greaterThanOrEqualTo(field.as(LocalDate.class), localDateValue),
        (field, numValue) -> cb.ge(toNumericExpression(field), numValue));
  }

  private Predicate buildLessThan(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
  ...
  private Predicate buildLessThanEqual(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
  ...
  private Predicate buildInBetween(
      Rule rule,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
  ...
  private Predicate buildIsEmpty(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildIncludesAny(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildExcludesAll(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
    ...
    Predicate negated =
        cb.not(
            buildFieldInPredicate(
                rule.getField(), field -> field, ruleList, cb, root, progressJoin));
  ...
  private Predicate buildIncludesAll(
      Rule rule,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
      Root<BookEntity> root,
  ...
  private Predicate buildFieldInPredicate(
      RuleField ruleField,
      ...
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
    Expression<?> field =
        fieldTransformer.apply(getFieldExpression(ruleField, cb, root, progressJoin));
    if (field == null) return cb.conjunction();

    if (ruleField == RuleField.READ_STATUS) {
      boolean hasUnset = ruleList.stream().anyMatch("UNSET"::equals);
      List<String> nonUnsetValues =
          ruleList.stream().filter(v -> !"UNSET".equals(v)).collect(Collectors.toList());

      if (hasUnset && !nonUnsetValues.isEmpty()) {
        return cb.or(cb.isNull(field), field.as(String.class).in(nonUnsetValues));
      } else if (hasUnset) {
        return cb.isNull(field);
      } else {
    ...
    List<String> lowerList =
        ruleList.stream().map(String::toLowerCase).collect(Collectors.toList());
    return cb.lower(field.as(String.class)).in(lowerList);
  }

  private Expression<?> getFieldExpression(
      RuleField field,
      CriteriaBuilder cb,
      Root<BookEntity> root,
      Join<BookEntity, UserBookProgressEntity> progressJoin) {
  ...
  private boolean isArrayField(RuleField field) {
    return field == RuleField.AUTHORS
        || field == RuleField.CATEGORIES
        || field == RuleField.MOODS
        || field == RuleField.TAGS
        || field == RuleField.GENRE
        || field == RuleField.SHELF;
  }
  ...
  private Predicate buildArrayFieldPredicate(
      RuleField field,
      List<String> values,
      CriteriaQuery<?> query,
      CriteriaBuilder cb,
    ...
    if (includesAll) {
      ...
      subquery
          .select(cb.literal(1L))
          .where(cb.equal(subRoot.get("id"), root.get("id")), cb.lower(nameField).in(lowerValues));
      return cb.exists(subquery);
    }
  }
  ...
  private Object normalizeValue(Object value, RuleField field) {
    ...
    if (field == RuleField.DATE_FINISHED
        || field == RuleField.LAST_READ_TIME
  ...
  private static final Set<RuleField> NUMERIC_FIELDS =
      Set.of(
          RuleField.METADATA_SCORE,
          RuleField.FILE_SIZE,
          RuleField.PAGE_COUNT,
          RuleField.SERIES_NUMBER,
          RuleField.SERIES_TOTAL,
          RuleField.AGE_RATING,
          RuleField.PERSONAL_RATING,
          RuleField.READING_PROGRESS,
          RuleField.AUDIOBOOK_DURATION,
          RuleField.AMAZON_RATING,
          RuleField.AMAZON_REVIEW_COUNT,
          RuleField.GOODREADS_RATING,
          RuleField.GOODREADS_REVIEW_COUNT,
          RuleField.HARDCOVER_RATING,
          RuleField.HARDCOVER_REVIEW_COUNT,
          RuleField.LUBIMYCZYTAC_RATING,
          RuleField.RANOBEDB_RATING,
          RuleField.AUDIBLE_RATING,
          RuleField.AUDIBLE_REVIEW_COUNT,
          RuleField.AUDIOBOOK_CHAPTER_COUNT,
          RuleField.AUDIOBOOK_BITRATE);
