...
public class ReadingSessionService {
  ...
  public List<ReadingSessionHeatmapResponse> getSessionHeatmapForYear(int year) {
    ...
    return readingSessionRepository
        .findSessionCountsByUserAndYear(userId, year, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                ReadingSessionHeatmapResponse.builder()
                    .date(dto.getDate())
                    .count(dto.getCount())
                    .build())
  ...
  public List<ReadingSessionHeatmapResponse> getSessionHeatmapForMonth(int year, int month) {
    ...
    return readingSessionRepository
        .findSessionCountsByUserAndYearAndMonth(userId, year, month, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                ReadingSessionHeatmapResponse.builder()
  ...
  public List<ReadingSessionTimelineResponse> getSessionTimelineForWeek(int year, int week) {
    ...
    LocalDate date = LocalDate.of(year, 1, 1).with(WeekFields.ISO.weekOfYear(), week);
    LocalDateTime startOfWeek =
        date.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY)).atStartOfDay();
    LocalDateTime endOfWeek =
        date.with(TemporalAdjusters.nextOrSame(DayOfWeek.SUNDAY)).plusDays(1).atStartOfDay();

    return readingSessionRepository
        .findSessionTimelineByUserAndWeek(
            userId,
            startOfWeek.atZone(ZoneId.systemDefault()).toInstant(),
            endOfWeek.atZone(ZoneId.systemDefault()).toInstant())
        .stream()
        .map(
            dto ->
                ReadingSessionTimelineResponse.builder()
  ...
  public List<ReadingSpeedResponse> getReadingSpeedForYear(int year) {
    ...
    return readingSessionRepository.findReadingSpeedByUserAndYear(userId, year).stream()
        .map(
            dto ->
                ReadingSpeedResponse.builder()
                    .date(dto.getDate())
  ...
  public List<PeakHoursResponse> getPeakReadingHours(Integer year, Integer month) {
    ...
    return readingSessionRepository
        .findPeakReadingHoursByUser(userId, year, month, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                PeakHoursResponse.builder()
  ...
  public List<FavoriteReadingDaysResponse> getFavoriteReadingDays(Integer year, Integer month) {
    ...
    return readingSessionRepository
        .findFavoriteReadingDaysByUser(userId, year, month, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                FavoriteReadingDaysResponse.builder()
  ...
  public List<GenreStatisticsResponse> getGenreStatistics() {
    ...
    return readingSessionRepository.findGenreStatisticsByUser(userId).stream()
        .map(
            dto -> {
              double avgSessionsPerBook =
                  dto.getBookCount() > 0
  ...
  public List<CompletionTimelineResponse> getCompletionTimeline(int year) {
    ...
    userBookProgressRepository
        .findCompletionTimelineByUser(userId, year)
        .forEach(
            dto -> {
              String key = dto.getYear() + "-" + dto.getMonth();
              timelineMap
                  .computeIfAbsent(key, k -> new EnumMap<>(ReadStatus.class))
  ...
  public Page<ReadingSessionResponse> getReadingSessionsForBook(Long bookId, int page, int size) {
    ...
    Page<ReadingSessionEntity> sessions =
        readingSessionRepository.findByUserIdAndBookId(userId, bookId, pageable);

    return sessions.map(
        session ->
            ReadingSessionResponse.builder()
                .id(session.getId())
  ...
  public List<BookCompletionHeatmapResponse> getBookCompletionHeatmap() {
    ...
    return userBookProgressRepository
        .findBookCompletionHeatmap(userId, startYear, currentYear)
        .stream()
        .map(
            dto ->
                BookCompletionHeatmapResponse.builder()
                    .year(dto.getYear())
                    .month(dto.getMonth())
                    .count(dto.getCount())
                    .build())
        ...
        sessions.stream()
            .collect(
                Collectors.groupingBy(
                    PageTurnerSessionDto::getBookId, LinkedHashMap::new, Collectors.toList()));

    Set<Long> bookIds = sessionsByBook.keySet();
    Map<Long, List<String>> bookCategories = new HashMap<>();
    if (!bookIds.isEmpty()) {
      bookRepository
          .findAllWithMetadataByIds(bookIds)
          .forEach(
              book -> {
                List<String> categories =
                    book.getMetadata() != null && book.getMetadata().getCategories() != null
    ...
    return sessionsByBook.entrySet().stream()
        ...
        .map(
            entry -> {
                      ...
                      .map(
                          s ->
                              s.getDurationSeconds() != null
                                  ? s.getDurationSeconds().doubleValue()
                                  : 0.0)
                      .collect(Collectors.toList());
              ...
              double firstThreeQuartersAvg =
                  durations.subList(0, lastQuarterStart).stream()
                      .mapToDouble(Double::doubleValue)
                      .average()
                      .orElse(0);
              double lastQuarterAvg =
                  durations.subList(lastQuarterStart, totalSessions).stream()
                      .mapToDouble(Double::doubleValue)
                      .average()
                      .orElse(0);
              ...
              int gripScore =
                  (int)
                      Math.round(
                          Math.min(
                              100, Math.max(0, accelScore * 35 + gapScore * 35 + burstScore * 30)));
  ...
  public List<CompletionRaceResponse> getCompletionRace(int year) {
    ...
    var allSessions =
        readingSessionRepository.findCompletionRaceSessionsByUserAndYear(userId, year);

    // Collect unique book IDs in order of appearance, take last N (most recently finished)
    LinkedHashSet<Long> allBookIds =
        allSessions.stream()
    ...
    return allSessions.stream()
        ...
        .map(
            dto ->
                CompletionRaceResponse.builder()
                    .bookId(dto.getBookId())
                    .bookTitle(dto.getBookTitle())
  ...
  public List<ReadingSessionHeatmapResponse> getReadingDates() {
    ...
    return readingSessionRepository.findAllSessionCountsByUser(userId, getTimezoneOffset()).stream()
        .map(
            dto ->
                ReadingSessionHeatmapResponse.builder()
                    .date(dto.getDate())
  ...
  public BookDistributionsResponse getBookDistributions() {
    ...
    // Rating distribution
    List<BookDistributionsResponse.RatingBucket> ratingBuckets =
        userBookProgressRepository.findRatingDistributionByUser(userId).stream()
            .map(
                dto ->
                    BookDistributionsResponse.RatingBucket.builder()
    ...
    // Status distribution
    List<BookDistributionsResponse.StatusBucket> statusBuckets =
        userBookProgressRepository.findStatusDistributionByUser(userId).stream()
            .map(
                dto ->
                    BookDistributionsResponse.StatusBucket.builder()
    ...
    List<ProgressPercentDto> progressRows =
        userBookProgressRepository.findAllProgressPercentsByUser(userId);
    long[] bucketCounts =
        new long[6]; // Not Started, Just Started, Getting Into It, Halfway Through, Almost Done,
    // Completed
    ...
    for (int i = 0; i < 6; i++) {
      progressBuckets.add(
          BookDistributionsResponse.ProgressBucket.builder()
  ...
  private float maxProgress(ProgressPercentDto row) {
    float max = 0f;
    if (row.getKoreaderProgressPercent() != null)
      max = Math.max(max, row.getKoreaderProgressPercent());
  ...
  public List<SessionScatterResponse> getSessionScatter(int year) {
    ...
    return readingSessionRepository
        .findSessionScatterByUserAndYear(userId, year, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                SessionScatterResponse.builder()
                    .hourOfDay(dto.getHourOfDay())
  ...
  public ReadingStreakResponse getReadingStreak() {
    ...
    List<ReadingSessionCountDto> allDates =
        readingSessionRepository.findAllSessionCountsByUser(userId, getTimezoneOffset());
    ...
    for (LocalDate date = startDate; !date.isAfter(today); date = date.plusDays(1)) {
      last52Weeks.add(
          ReadingStreakResponse.ReadingStreakDay.builder()
              .date(date)
  ...
  public List<BookTimelineResponse> getBookTimeline(int year) {
    ...
    return readingSessionRepository.findBookTimelineByUserAndYear(userId, year, tzOffset).stream()
        .map(
            dto ->
                BookTimelineResponse.builder()
                    .bookId(dto.getBookId())
                    .title(dto.getTitle())
                    .pageCount(dto.getPageCount())
                    .firstSessionDate(
                        dto.getFirstSessionDate() != null
                            ? dto.getFirstSessionDate().toLocalDate()
                            : null)
                    .lastSessionDate(
                        dto.getLastSessionDate() != null
  ...
  public List<ListeningHeatmapResponse> getListeningHeatmapForMonth(int year, int month) {
    ...
    return readingSessionRepository
        .findListeningSessionsByUserAndMonth(userId, year, month, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                ListeningHeatmapResponse.builder()
                    .date(dto.getDate())
  ...
  public List<WeeklyListeningTrendResponse> getWeeklyListeningTrend(int weeks) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    return readingSessionRepository
        .findWeeklyListeningTrend(userId, weeks, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                WeeklyListeningTrendResponse.builder()
                    .year(dto.getYear())
                    .week(dto.getWeek())
                    .totalDurationSeconds(dto.getTotalDurationSeconds())
                    .sessions(dto.getSessions())
  ...
  public ListeningCompletionResponse getListeningCompletion() {
    ...
    for (var dto : progressList) {
      ...
      if (maxProg >= 0.98f) {
        ...
        inProgress.add(
            ListeningCompletionResponse.AudiobookCompletionEntry.builder()
                .bookId(dto.getBookId())
                .title(dto.getTitle())
                .progressPercent(Math.round(maxProg * 10.0) / 10.0)
                .totalDurationSeconds(
                    dto.getTotalDurationSeconds() != null ? dto.getTotalDurationSeconds() : 0L)
                .listenedDurationSeconds(
                    dto.getListenedDurationSeconds() != null
                        ? dto.getListenedDurationSeconds()
                        : 0L)
                .build());
      }
    }
    ...
    inProgress.sort(
        (a, b) -> Long.compare(b.getListenedDurationSeconds(), a.getListenedDurationSeconds()));
  ...
  public List<MonthlyPaceResponse> getMonthlyListeningPace(int months) {
    ...
    var durationsByMonth =
        readingSessionRepository.findMonthlyListeningDurations(userId, getTimezoneOffset());
  ...
  public List<PeakHoursResponse> getListeningPeakHours(Integer year, Integer month) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    return readingSessionRepository
        .findListeningPeakHoursByUser(userId, year, month, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                PeakHoursResponse.builder()
  ...
  public List<GenreStatisticsResponse> getListeningGenreStatistics() {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    return readingSessionRepository.findListeningGenreStatisticsByUser(userId).stream()
        .map(
            dto -> {
              double avgSessionsPerBook =
                  dto.getBookCount() > 0
  ...
  public List<ListeningAuthorResponse> getListeningAuthorStats() {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    return readingSessionRepository.findListeningAuthorStatsByUser(userId).stream()
        .map(
            dto ->
                ListeningAuthorResponse.builder()
  ...
  public List<SessionScatterResponse> getListeningSessionScatter() {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    return readingSessionRepository
        .findListeningSessionScatterByUser(userId, getTimezoneOffset())
        .stream()
        .map(
            dto ->
                SessionScatterResponse.builder()
  ...
  public List<LongestAudiobookResponse> getListeningLongestBooks() {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    return readingSessionRepository.findAudiobookProgressByUser(userId).stream()
        .sorted(
            (a, b) ->
                Long.compare(
