    ...
    try {
      ...
      try (ByteArrayInputStream bais = new ByteArrayInputStream(coverData)) {
        ...
        if (originalImage == null) {
          log.warn(
              "Cover image found but could not be decoded (possibly SVG or unsupported format) in EPUB '{}'",
      ...
      log.error(
          "Error generating cover for EPUB '{}': {}", bookFile.getFileName(), e.getMessage(), e);
      return false;
    }
  }
  ...
  private void setBookMetadata(BookEntity bookEntity) {
    ...
    if (epubMetadata.getCategories() != null) {
      Set<String> validSubjects =
          epubMetadata.getCategories().stream()
              .filter(
                  s ->
                      s != null
                          && !s.isBlank()
