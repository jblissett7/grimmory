...
public class PdfProcessor extends AbstractFileProcessor implements BookFileProcessor {
  ...
  public boolean generateCover(BookEntity bookEntity, BookFileEntity bookFile) {
    ...
    try (RandomAccessReadBufferedFile randomAccessRead = new RandomAccessReadBufferedFile(pdfFile);
      ...
      log.error(
          "Out of memory (heap space exhausted) while generating cover for '{}'. Skipping cover generation.",
      ...
      log.warn(
          "Corrupted PDF structure for '{}'. Skipping cover generation.", bookFile.getFileName());
      return false;
    } catch (Exception e) {
  ...
  private void extractAndSetMetadata(BookEntity bookEntity) {
    try {
      BookMetadata extracted =
          pdfMetadataExtractor.extractMetadata(new File(FileUtils.getBookFullPath(bookEntity)));
  ...
  private boolean generateCoverImageAndSave(Long bookId, PDDocument document) throws IOException {
    ...
    try {
      ...
      log.error(
          "Out of memory (heap space exhausted) while generating cover for bookId {}. Skipping cover generation.",
          bookId);
      System.gc(); // Hint to JVM to reclaim memory
      return false;
    } finally {
