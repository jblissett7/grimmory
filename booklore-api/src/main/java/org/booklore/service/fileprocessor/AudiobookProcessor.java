...
public class AudiobookProcessor extends AbstractFileProcessor implements BookFileProcessor {
  ...
  public boolean generateCover(BookEntity bookEntity, boolean isFolderBased) {
    try {
      ...
      if (audioFile == null || !audioFile.exists()) {
        log.debug(
            "Audio file not found for audiobook '{}'",
      ...
      log.error(
          "Error generating cover for audiobook '{}': {}",
          bookEntity.getPrimaryBookFile().getFileName(),
          e.getMessage(),
          e);
      return false;
  ...
  private boolean generateCoverFromFile(BookEntity bookEntity, BookFileEntity audiobookFile) {
    try {
      ...
      if (audioFile == null) {
        log.warn(
            "Could not resolve audio file path for audiobook '{}' (bookId: {})",
            audiobookFile.getFileName(),
            bookEntity.getId());
        return false;
      }
      if (!audioFile.exists()) {
        log.warn(
            "Audio file does not exist: {} for audiobook '{}' (bookId: {})",
      ...
      log.error(
          "Error generating cover for audiobook '{}' (bookId: {}): {}",
          audiobookFile.getFileName(),
          bookEntity.getId(),
          e.getMessage(),
          e);
  ...
  private boolean extractAndSaveCover(BookEntity bookEntity, File audioFile, boolean throwOnNoCover)
      throws Exception {
    byte[] coverData = audiobookMetadataExtractor.extractCover(audioFile);

    if (coverData == null) {
      log.warn(
          "No embedded cover image found in audiobook file '{}' (bookId: {})",
          audioFile.getAbsolutePath(),
          bookEntity.getId());
      if (throwOnNoCover) {
    ...
    log.debug(
        "Found cover data ({} bytes) in audiobook file '{}'",
        coverData.length,
        audioFile.getName());
  ...
  private File getAudioFileForBookFile(BookEntity bookEntity, BookFileEntity bookFile) {
    ...
    log.debug(
        "Constructed audiobook path: {}, isFolderBased: {}", filePath, bookFile.isFolderBased());
  ...
  private void setBookMetadata(BookEntity bookEntity, boolean isFolderBased) {
    ...
    if (bookFile == null || !bookFile.exists()) {
      log.warn(
          "Audio file not found for metadata extraction: {}",
          bookEntity.getPrimaryBookFile().getFileName());
      return;
    }
    ...
    metadata.setLanguage(
        truncate((lang == null || "UND".equalsIgnoreCase(lang)) ? "en" : lang, 1000));
    ...
    if (audioMetadata.getCategories() != null) {
      Set<String> validCategories =
          audioMetadata.getCategories().stream()
              .filter(
                  s ->
                      s != null
                          && !s.isBlank()
  ...
  private List<BookFileEntity.AudioFileChapter> mapChapters(
      List<AudiobookMetadata.ChapterInfo> chapters) {
