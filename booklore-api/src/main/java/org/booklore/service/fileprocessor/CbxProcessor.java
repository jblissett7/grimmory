...
public class CbxProcessor extends AbstractFileProcessor implements BookFileProcessor {
  ...
  private static final Pattern IMAGE_EXTENSION_PATTERN =
      Pattern.compile(".*\\.(jpg|jpeg|png|webp)");
  private static final Pattern IMAGE_EXTENSION_CASE_INSENSITIVE_PATTERN =
      Pattern.compile("(?i).*\\.(jpg|jpeg|png|webp)");
  ...
  public BookEntity processNewFile(LibraryFile libraryFile) {
    ...
    bookEntity
        .getPrimaryBookFile()
        .setArchiveType(
            ArchiveUtils.detectArchiveType(new File(FileUtils.getBookFullPath(bookEntity))));
      ...
      if (imageOptional.isPresent()) {
        ...
        try {
          ...
          if (saved) {
            ...
            log.warn(
                "Could not save image extracted from CBZ as cover for '{}'",
  ...
  private Optional<BufferedImage> findAndReadFirstImage(ZipFile zipFile) {
    return Collections.list(zipFile.getEntries()).stream()
        .filter(
            e ->
                !e.isDirectory()
                    && IMAGE_EXTENSION_CASE_INSENSITIVE_PATTERN.matcher(e.getName()).matches())
        ...
        .map(
            entry -> {
              try (InputStream is = zipFile.getInputStream(entry)) {
                ...
                log.warn(
                    "Failed to read image from ZIP entry {}: {}", entry.getName(), e.getMessage());
                return null;
              }
            });
  }
    ...
    try (SevenZFile sevenZFile = SevenZFile.builder().setFile(file).get()) {
      ...
      while ((entry = sevenZFile.getNextEntry()) != null) {
        if (!entry.isDirectory()
            && IMAGE_EXTENSION_CASE_INSENSITIVE_PATTERN.matcher(entry.getName()).matches()) {
  ...
  private Optional<BufferedImage> extractFirstImageFromRar(File file) {
    try (Archive archive = new Archive(file)) {
      List<FileHeader> imageHeaders =
          archive.getFileHeaders().stream()
              .filter(
                  h ->
                      !h.isDirectory()
                          && IMAGE_EXTENSION_PATTERN
                              .matcher(h.getFileName().toLowerCase())
                              .matches())
      ...
      for (FileHeader header : imageHeaders) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
          ...
          return Optional.ofNullable(
              FileService.readImage(new ByteArrayInputStream(baos.toByteArray())));
        } catch (Exception e) {
  ...
  private void extractAndSetMetadata(BookEntity bookEntity) {
    try {
      BookMetadata extracted =
          cbxMetadataExtractor.extractMetadata(new File(FileUtils.getBookFullPath(bookEntity)));
      if (extracted == null) {
      ...
      if (extracted.getCategories() != null) {
        Set<String> validCategories =
            extracted.getCategories().stream()
                .filter(
                    s ->
                        s != null
                            && !s.isBlank()
      ...
      if (extracted.getMoods() != null && !extracted.getMoods().isEmpty()) {
        Set<String> validMoods =
            extracted.getMoods().stream()
      ...
      log.warn(
          "Failed to extract ComicInfo metadata for '{}': {}",
  ...
  private void saveComicMetadata(BookEntity bookEntity, ComicMetadata comicDto) {
    ...
    if (bookId == null) {
      log.warn(
          "Cannot save comic metadata - book ID is null for '{}'",
          bookEntity.getPrimaryBookFile().getFileName());
      return;
    }
    ...
    comic.setBlackAndWhite(
        comicDto.getBlackAndWhite() != null ? comicDto.getBlackAndWhite() : Boolean.FALSE);
    comic.setManga(comicDto.getManga() != null ? comicDto.getManga() : Boolean.FALSE);
    comic.setReadingDirection(
        comicDto.getReadingDirection() != null ? comicDto.getReadingDirection() : "ltr");
