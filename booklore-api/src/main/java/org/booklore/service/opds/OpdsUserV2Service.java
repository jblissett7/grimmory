...
public class OpdsUserV2Service {
  ...
  public OpdsUserV2 createOpdsUser(OpdsUserV2CreateRequest request) {
    try {
              ...
              .orElseThrow(
                  () ->
                      new UsernameNotFoundException(
                          "User not found with ID: " + bookLoreUser.getId()));

      OpdsUserV2Entity opdsUserV2 =
          OpdsUserV2Entity.builder()
              .user(userEntity)
      ...
      auditService.log(
          AuditAction.OPDS_USER_CREATED,
          "OpdsUser",
          result.getId(),
          "Created OPDS user: " + request.getUsername());
      return result;
  ...
  public void deleteOpdsUser(Long userId) {
    BookLoreUser bookLoreUser = authenticationService.getAuthenticatedUser();
    OpdsUserV2Entity user =
        opdsUserV2Repository
            .findById(userId)
    ...
    auditService.log(
        AuditAction.OPDS_USER_DELETED, "OpdsUser", userId, "Deleted OPDS user: " + username);
  }
  ...
  public OpdsUserV2 updateOpdsUser(Long userId, OpdsUserV2UpdateRequest request) {
    ...
    auditService.log(
        AuditAction.OPDS_USER_UPDATED,
        "OpdsUser",
        userId,
        "Updated OPDS user: " + user.getUsername());
    return result;
  }
