...
public class MagicShelfBookService {
  ...
  public Page<Book> getBooksByMagicShelfId(Long userId, Long magicShelfId, int page, int size) {
    ...
    try {
      ...
      Specification<BookEntity> specification =
          ruleEvaluatorService.toSpecification(groupRule, userId);
      ...
      List<BookEntity> filteredEntities =
          contentRestrictionService.applyRestrictions(booksPage.getContent(), userId);
      List<Book> books =
      ...
      throw new RuntimeException(
          "Failed to parse or execute magic shelf rules: " + e.getMessage(), e);
    }
  }
  ...
  public String getMagicShelfName(Long magicShelfId) {
    return magicShelfRepository
        .findById(magicShelfId)
