...
public class OpdsBookService {
  ...
  public Page<Book> getBooksPage(
      Long userId, String query, Long libraryId, Set<Long> shelfIds, int page, int size) {
    if (userId == null) {
    ...
    if (entity.getPermissions() == null
        || (!entity.getPermissions().isPermissionAccessOpds()
            && !entity.getPermissions().isPermissionAdmin())) {
      throw ApiError.FORBIDDEN.createException("You are not allowed to access this resource");
    }

    BookLoreUser user = bookLoreUserTransformer.toDTO(entity);
    boolean isAdmin = user.getPermissions().isAdmin();
    Set<Long> userLibraryIds =
    ...
    if (shelfIds != null && !shelfIds.isEmpty()) {
      ...
      Page<Book> books =
          query != null && !query.isBlank()
              ? searchByMetadataInShelvesPageInternal(
                  BookUtils.normalizeForSearch(query), shelfIds, page, size, userId)
    ...
    if (libraryId != null) {
      ...
      Page<Book> books =
          query != null && !query.isBlank()
              ? searchByMetadataInLibrariesPageInternal(
                  BookUtils.normalizeForSearch(query), Set.of(libraryId), page, size, userId)
    ...
    Page<Book> books =
        query != null && !query.isBlank()
            ? searchByMetadataInLibrariesPageInternal(
                BookUtils.normalizeForSearch(query), userLibraryIds, page, size, userId)
  ...
  public List<Book> getRandomBooks(Long userId, int count) {
    ...
    List<BookEntity> books =
        bookOpdsRepository.findAllWithMetadataByIds(ids.stream().limit(count).toList());
    if (userId != null) {
  ...
  public Page<Book> getBooksByAuthorName(Long userId, String authorName, int page, int size) {
    ...
    Page<Long> idPage =
        bookOpdsRepository.findBookIdsByAuthorNameAndLibraryIds(authorName, libraryIds, pageable);
    if (idPage.isEmpty()) {
      return new PageImpl<>(List.of(), pageable, 0);
    }

    List<BookEntity> books =
        bookOpdsRepository.findAllWithFullMetadataByIdsAndLibraryIds(
            idPage.getContent(), libraryIds);
  ...
  public Page<Book> getBooksBySeriesName(Long userId, String seriesName, int page, int size) {
    ...
    Page<Long> idPage =
        bookOpdsRepository.findBookIdsBySeriesNameAndLibraryIds(seriesName, libraryIds, pageable);
    if (idPage.isEmpty()) {
      return new PageImpl<>(List.of(), pageable, 0);
    }
  ...
  private Page<Book> getBooksByLibraryIdsPageInternal(
      Set<Long> libraryIds, int page, int size, Long userId) {
    Pageable pageable = PageRequest.of(Math.max(page, 0), size);
    ...
    List<BookEntity> books =
        bookOpdsRepository.findAllWithMetadataByIdsAndLibraryIds(idPage.getContent(), libraryIds);
    return createPageFromEntities(books, idPage, pageable, userId);
  }

  private Page<Book> getRecentBooksByLibraryIdsPageInternal(
      Set<Long> libraryIds, int page, int size, Long userId) {
    Pageable pageable = PageRequest.of(Math.max(page, 0), size);
  ...
  private Page<Book> getBooksByShelfIdPageInternal(Long shelfId, int page, int size, Long userId) {
    ...
    List<BookEntity> books =
        bookOpdsRepository.findAllWithMetadataByIdsAndShelfId(idPage.getContent(), shelfId);
    return createPageFromEntities(books, idPage, pageable, userId);
  }

  private Page<Book> getBooksByShelfIdsPageInternal(
      Set<Long> shelfIds, int page, int size, Long userId) {
    Pageable pageable = PageRequest.of(Math.max(page, 0), size);
    ...
    List<BookEntity> books =
        bookOpdsRepository.findAllWithMetadataByIdsAndShelfIds(idPage.getContent(), shelfIds);
    return createPageFromEntities(books, idPage, pageable, userId);
  }
  ...
  private Page<Book> searchByMetadataInLibrariesPageInternal(
      String text, Set<Long> libraryIds, int page, int size, Long userId) {
    Pageable pageable = PageRequest.of(Math.max(page, 0), size);

    Page<Long> idPage =
        bookOpdsRepository.findBookIdsByMetadataSearchAndLibraryIds(text, libraryIds, pageable);
    if (idPage.isEmpty()) {
      return new PageImpl<>(List.of(), pageable, 0);
    }
  ...
  private Page<Book> searchByMetadataInShelvesPageInternal(
      String text, Set<Long> shelfIds, int page, int size, Long userId) {
    Pageable pageable = PageRequest.of(Math.max(page, 0), size);

    Page<Long> idPage =
        bookOpdsRepository.findBookIdsByMetadataSearchAndShelfIds(text, shelfIds, pageable);
    if (idPage.isEmpty()) {
      return new PageImpl<>(List.of(), pageable, 0);
    }

    List<BookEntity> books =
        bookOpdsRepository.findAllWithFullMetadataByIdsAndShelfIds(idPage.getContent(), shelfIds);
    return createPageFromEntities(books, idPage, pageable, userId);
  }
  ...
  private Page<Book> createPageFromEntities(
      List<BookEntity> books, Page<Long> idPage, Pageable pageable, Long userId) {
    Map<Long, BookEntity> bookMap =
        books.stream().collect(Collectors.toMap(BookEntity::getId, Function.identity()));

    List<BookEntity> orderedEntities =
    ...
    List<Book> sortedBooks = orderedEntities.stream().map(bookMapper::toBook).toList();
    ...
    switch (sortOrder) {
      case TITLE_ASC ->
          sortedBooks.sort(
              (b1, b2) -> {
                String title1 =
                    b1.getMetadata() != null && b1.getMetadata().getTitle() != null
                        ? b1.getMetadata().getTitle()
                        : "";
                String title2 =
                    b2.getMetadata() != null && b2.getMetadata().getTitle() != null
      ...
      case RATING_DESC ->
          sortedBooks.sort(
              (b1, b2) -> {
                ...
                if (rating2 == null) return -1;
                int ratingComp =
                    Float.compare(rating2, rating1); // Descending order (highest first)
                if (ratingComp != 0) return ratingComp;
                // Same rating, fall back to addedOn descending
                return compareByAddedOn(b2, b1);
              });
