...
public class OpdsFeedService {
    ...
    String feed =
        """
            ...
            + """
                ...
                """
                .formatted(
                    now(), escapeXml("/api/v1/opds/catalog?page=1&size=" + DEFAULT_PAGE_SIZE))
            + """
                          <entry>
      ...
      feed.append(
          """
              ...
              .formatted(
                  ...
                  now(),
                  escapeXml(
                      "/api/v1/opds/catalog?author="
                          + java.net.URLEncoder.encode(
                              author, java.nio.charset.StandardCharsets.UTF_8)),
                  escapeXml(author)));
    }
      ...
      feed.append(
          """
              ...
              .formatted(
                  ...
                  now(),
                  escapeXml(
                      "/api/v1/opds/catalog?series="
                          + java.net.URLEncoder.encode(
                              series, java.nio.charset.StandardCharsets.UTF_8)),
                  escapeXml(series)));
    }
  ...
  public String generateCatalogFeed(HttpServletRequest request) {
    ...
    int size =
        Math.min(
            parseLongParam(request, "size", (long) DEFAULT_PAGE_SIZE).intValue(), MAX_PAGE_SIZE);
    ...
    if (magicShelfId != null) {
      booksPage =
          magicShelfBookService.getBooksByMagicShelfId(userId, magicShelfId, page - 1, size);
  ...
  private void appendPaginationLinks(
      StringBuilder feed, HttpServletRequest request, int currentPage, int totalPages, int size) {
    if (totalPages > 0) {
  ...
  private void appendLinks(StringBuilder feed, Book book) {
    ...
    if (book.getMetadata() != null && book.getMetadata().getCoverUpdatedOn() != null) {
      String coverUrl =
          "/api/v1/opds/" + book.getId() + "/cover?" + book.getMetadata().getCoverUpdatedOn();
      feed.append("    <link rel=\"http://opds-spec.org/image\" href=\"")
          .append(escapeXml(coverUrl))
  ...
  private String determineFeedTitle(
      Long libraryId, Set<Long> shelfIds, Long magicShelfId, String author, String series) {
    if (magicShelfId != null) {
  ...
  private String determineFeedId(
      Long libraryId, Set<Long> shelfIds, Long magicShelfId, String author, String series) {
    if (magicShelfId != null) {
    ...
    if (shelfIds != null && !shelfIds.isEmpty()) {
      ...
      return "urn:booklore:shelves:"
          + String.join(",", shelfIds.stream().map(String::valueOf).sorted().toList());
    }
    if (libraryId != null) {
  ...
  private boolean hasValidFilePath(BookFile bookFile) {
    return bookFile != null && bookFile.getFileName() != null && bookFile.getFilePath() != null;
  }
    ...
    return switch (bookFile.getBookType()) {
      ...
      case FB2 -> {
        if (hasValidFilePath(bookFile)) {
          ArchiveUtils.ArchiveType type =
              ArchiveUtils.detectArchiveType(new File(bookFile.getFilePath()));
  ...
  private OpdsSortOrder getSortOrder() {
    OpdsUserDetails details = authenticationService.getOpdsUser();
    return details != null
            && details.getOpdsUserV2() != null
            && details.getOpdsUserV2().getSortOrder() != null
