...
public class LibraryFileHelper {
  ...
  private List<LibraryFile> findLibraryFilesFlat(
      LibraryPathEntity pathEntity, LibraryEntity libraryEntity) throws IOException {
    ...
    // Collapse audio files: 2+ audio files in same directory become a single folder-based
    // audiobook.
    // A single audio file (e.g. standalone m4b) stays as an individual file entry.
    // Audio files at the library root are always added individually (no folder to name the
    // audiobook after).
  ...
  List<LibraryFile> filterByAllowedFormats(
      List<LibraryFile> files, List<BookFileType> allowedFormats) {
  ...
  private List<LibraryFile> findLibraryFiles(
      LibraryPathEntity pathEntity, LibraryEntity libraryEntity) throws IOException {
    Path libraryPath = Path.of(pathEntity.getPath());
    List<LibraryFile> libraryFiles = new ArrayList<>();
    ...
    Files.walkFileTree(
        ...
        new SimpleFileVisitor<>() {
            ...
            if (audioFiles != null
              ...
              if (!dir.equals(libraryPath)) {
                if (FileUtils.isSeriesFolder(audioFiles)) {
                  log.info(
                      "Detected series folder: {} ({} audio files with distinct titles)",
                      dir.getFileName(),
                      audioFiles.size());
                  addIndividualAudioFiles(audioFiles, libraryEntity, pathEntity, libraryFiles);
                } else {
                  log.info(
                      "Detected folder-based audiobook: {} ({} audio files)",
                      dir.getFileName(),
                      audioFiles.size());
          ...
          private void addIndividualAudioFiles(
            ...
            for (Path audioFile : audioFiles) {
              ...
              libraryFiles.add(
                  LibraryFile.builder()
                      ...
                      .fileName(fileName)
                      .bookFileType(
                          ext.map(BookFileExtension::getType).orElse(BookFileType.AUDIOBOOK))
                      .build());
            }
          }
