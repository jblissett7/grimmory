...
public class BookGroupingService {
  ...
  public Map<String, List<LibraryFile>> groupForInitialScan(
      List<LibraryFile> newFiles, LibraryEntity libraryEntity) {
  ...
  private Map<String, List<LibraryFile>> groupByFolder(List<LibraryFile> files) {
    ...
    for (LibraryFile file : files) {
      ...
      if (subPath.isEmpty()) {
        ...
        // fileName="audiobook-folder"). Their subPath points to the parent because
        // getRelativeSubPath
        // takes .getParent() (designed for files, not directories). Reconstruct the actual folder
        // path
        // so sibling audiobook folders stay separate and absorption searches from the correct
        // level.
        String effectiveSubPath =
            file.isFolderBased() ? subPath + "/" + file.getFileName() : subPath;
  ...
  private BookEntity findMatchingBook(LibraryFile file, LibraryOrganizationMode mode) {
    BookEntity filelessMatch =
        switch (mode) {
          case BOOK_PER_FILE, BOOK_PER_FOLDER ->
              findExactFilelessMatch(file, file.getLibraryEntity());
    ...
    List<BookEntity> activeBooksInDirectory =
  ...
  private BookEntity findExactFilelessMatch(LibraryFile file, LibraryEntity library) {
    ...
    for (BookEntity book : filelessBooks) {
      if (book.getLibraryPath() != null
          && !book.getLibraryPath().getId().equals(fileLibraryPathId)) {
        continue;
      }
      ...
      if (book.getMetadata() != null && book.getMetadata().getTitle() != null) {
        ...
        if (fileBaseName.equals(bookTitle)) {
          log.debug(
              "Exact matched file '{}' to fileless book '{}' (title: {})",
              file.getFileName(),
              book.getId(),
    ...
    for (BookEntity book : filelessBooks) {
      ...
      if (book.getMetadata() != null && book.getMetadata().getTitle() != null) {
        ...
        if (similarity >= FILELESS_MATCH_THRESHOLD) {
          log.debug(
              "Matched file '{}' to fileless book '{}' (title: {})",
              file.getFileName(),
              book.getId(),
              book.getMetadata().getTitle());
          return book;
  ...
  private BookEntity findMatchBookPerFolderWithAbsorption(
      LibraryFile file, List<BookEntity> booksInDirectory) {
  ...
  private BookEntity findMatchAutoDetect(LibraryFile file, List<BookEntity> booksInDirectory) {
    ...
    if (booksWithFiles.size() == 1) {
      ...
      if (isFileNameCompatible(file, book)) {
        log.debug(
            "AUTO_DETECT: Single book in folder '{}', attaching '{}' to '{}'",
            file.getFileSubPath(),
            file.getFileName(),
  ...
  private BookEntity findBestMatch(LibraryFile file, List<BookEntity> booksWithFiles) {
    ...
    if (bestMatch != null) {
      log.debug(
          "Fuzzy matched '{}' to '{}' (similarity: {})",
          file.getFileName(),
          bestMatch.getPrimaryBookFile().getFileName(),
          bestSimilarity);
    }
    return bestMatch;
  }
