...
public class FileAsBookProcessor {
  ...
  @Transactional
  public void processLibraryFilesGrouped(
      Map<String, List<LibraryFile>> groups, LibraryEntity libraryEntity) {
  ...
  private void processGroup(List<LibraryFile> group, LibraryEntity libraryEntity) {
    ...
    List<LibraryFile> additionalFiles = group.stream().filter(f -> !f.equals(primary)).toList();
  ...
  private Optional<LibraryFile> findBestPrimaryFile(
      List<LibraryFile> group, LibraryEntity libraryEntity) {
    List<BookFileType> formatPriority = libraryEntity.getFormatPriority();
    return group.stream()
        .filter(f -> f.getBookFileType() != null)
        .min(
            Comparator.<LibraryFile, Integer>comparing(
                    f -> {
  ...
  private void createAdditionalBookFile(BookEntity bookEntity, LibraryFile file) {
    Optional<BookFileEntity> existing =
    ...
    try {
      ...
      log.info(
          "Attached additional format {} to book: {}",
          file.getFileName(),
  ...
  void generateCoverFromAdditionalFile(BookEntity bookEntity, LibraryFile additionalFile) {
    ...
    if (!bookEntity.hasFiles()) {
      try {
        ...
        log.warn(
            "Failed to generate cover from additional file {}: {}",
  ...
  private void generateAudiobookCoverFromFile(BookEntity bookEntity, LibraryFile audioFile) {
    try {
      ...
      log.warn(
          "Error generating audiobook cover from {}: {}", audioFile.getFileName(), e.getMessage());
    }
  }
  ...
  private File getFileForCoverExtraction(LibraryFile libraryFile) {
    if (libraryFile.isFolderBased()) {
      ...
      return FileUtils.getFirstAudioFileInFolder(folderPath).map(Path::toFile).orElse(null);
    } else {
