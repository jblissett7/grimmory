...
public class LibraryRescanHelper {
  ...
  public void handleRescanOptions(RescanLibraryContext context, String taskId) {
    ...
    sendTaskProgressNotification(
        taskId,
        0,
        String.format("Starting rescan for library: %s", library.getName()),
    ...
    for (BookEntity bookEntity : bookEntities) {
      ...
      if (taskId != null && cancellationManager.isTaskCancelled(taskId)) {
        ...
        sendTaskProgressNotification(
            taskId,
            (processedBooks * 100) / totalBooks,
            String.format(
                "Rescan cancelled for library: %s (%d/%d books processed)",
                library.getName(), processedBooks, totalBooks),
            TaskStatus.CANCELLED);
        break;
      }

      log.info(
          "Processing book: library={}, bookId={}, fileName={}",
          library.getName(),
          bookEntity.getId(),
      ...
      sendTaskProgressNotification(
          ...
          progressPercentage,
          String.format(
              "Processing: %s (Library: %s)",
              bookEntity.getPrimaryBookFile().getFileName(), library.getName()),
          TaskStatus.IN_PROGRESS);

      try {
        BookMetadata bookMetadata =
            metadataExtractorFactory.extractMetadata(
                bookEntity.getPrimaryBookFile().getBookType(),
                bookEntity.getFullFilePath().toFile());
        if (bookMetadata == null) {
          log.warn(
              "No metadata extracted for book id={} path={}",
              bookEntity.getId(),
              bookEntity.getFullFilePath());
          continue;
        }
        MetadataUpdateContext metadataUpdateContext =
            MetadataUpdateContext.builder()
                .bookEntity(bookEntity)
                .metadataUpdateWrapper(
                    MetadataUpdateWrapper.builder().metadata(bookMetadata).build())
        ...
        if (bookEntity.getPrimaryBookFile().getBookType() == BookFileType.AUDIOBOOK
            && bookMetadata.getAudiobookMetadata() != null) {
          audiobookProcessor.setAudiobookTechnicalMetadata(bookEntity, bookMetadata);
        }
      } catch (Exception e) {
        log.error(
            "Failed to update metadata for book id={} path={}: {}",
            bookEntity.getId(),
            bookEntity.getFullFilePath(),
            e.getMessage(),
            e);
    ...
    if (taskId == null || !cancellationManager.isTaskCancelled(taskId)) {
      sendTaskProgressNotification(
          ...
          100,
          String.format(
              "Rescan completed for library: %s (%d books processed)",
              library.getName(), processedBooks),
          TaskStatus.COMPLETED);
    }
  }

  private void sendTaskProgressNotification(
      String taskId, int progress, String message, TaskStatus taskStatus) {
    try {
      TaskProgressPayload payload =
      ...
      log.error(
          "Failed to send task progress notification for taskId={}: {}", taskId, e.getMessage(), e);
    }
  }
}
