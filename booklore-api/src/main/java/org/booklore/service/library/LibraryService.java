...
public class LibraryService {
  ...
  /**
   * Checks whether a library is currently being scanned. Can be used by other components (e.g.,
   * file watcher) to avoid processing files while a full scan is in progress.
   */
  ...
  public Library updateLibrary(CreateLibraryRequest request, Long libraryId) {
    ...
    Set<String> updatedPaths =
        request.getPaths().stream().map(LibraryPath::getPath).collect(Collectors.toSet());
    ...
    if (!deletedPaths.isEmpty()) {
      Set<LibraryPathEntity> pathsToRemove =
          library.getLibraryPaths().stream()
      ...
      List<Long> books =
          bookRepository.findAllBookIdsByLibraryPathIdIn(
    ...
    if (!newPaths.isEmpty()) {
      Set<LibraryPathEntity> newPathEntities =
          newPaths.stream()
      ...
      SecurityContextVirtualThread.runWithSecurityContext(
          () -> {
            if (!scanningLibraries.add(libraryId)) {
              log.warn(
                  "Library {} is already being scanned, skipping duplicate process request",
                  libraryId);
              return;
            }
            try {
    ...
    auditService.log(
        AuditAction.LIBRARY_UPDATED, "Library", libraryId, "Updated library: " + library.getName());
  ...
  public Library createLibrary(CreateLibraryRequest request) {
    ...
    auditService.log(
        AuditAction.LIBRARY_CREATED,
        "Library",
        libraryEntity.getId(),
        "Created library: " + libraryEntity.getName());
  ...
  public void rescanLibrary(long libraryId) {
    LibraryEntity lib =
        libraryRepository
            ...
            .orElseThrow(() -> ApiError.LIBRARY_NOT_FOUND.createException(libraryId));
    auditService.log(
        AuditAction.LIBRARY_SCANNED, "Library", libraryId, "Scanned library: " + lib.getName());

    SecurityContextVirtualThread.runWithSecurityContext(
        () -> {
          if (!scanningLibraries.add(libraryId)) {
            log.warn(
                "Library {} is already being scanned, skipping duplicate rescan request",
                libraryId);
            return;
          }
          try {
            RescanLibraryContext context =
  ...
  public void deleteLibrary(long id) {
    ...
    Set<Long> bookIds =
        library.getBookEntities().stream().map(BookEntity::getId).collect(Collectors.toSet());
  ...
  public Book getBook(long libraryId, long bookId) {
    ...
    BookEntity bookEntity =
        bookRepository
            .findBookByIdAndLibraryId(bookId, libraryId)
  ...
  public Library setFileNamingPattern(long libraryId, String pattern) {
    ...
    auditService.log(
        AuditAction.NAMING_PATTERN_CHANGED,
        "Library",
        libraryId,
        "Changed naming pattern for library: " + library.getName() + " to: " + pattern);
    return result;
  }
  ...
  public int scanLibraryPaths(CreateLibraryRequest request) {
    ...
    Set<BookFileType> allowedFormats =
        request.getAllowedFormats() != null && !request.getAllowedFormats().isEmpty()
