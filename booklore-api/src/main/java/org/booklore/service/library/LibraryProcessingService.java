...
public class LibraryProcessingService {
  ...
  public void processLibrary(long libraryId) {
    ...
    notificationService.sendMessage(
        Topic.LOG, LogNotification.info("Started processing library: " + libraryEntity.getName()));
    try {
      ...
      Map<String, List<LibraryFile>> groups =
          bookGroupingService.groupForInitialScan(newFiles, libraryEntity);
      fileAsBookProcessor.processLibraryFilesGrouped(groups, libraryEntity);

      notificationService.sendMessage(
          Topic.LOG,
          LogNotification.info("Finished processing library: " + libraryEntity.getName()));
    } catch (IOException e) {
      log.error("Failed to process library {}: {}", libraryEntity.getName(), e.getMessage(), e);
      notificationService.sendMessage(
          Topic.LOG,
          LogNotification.error(
              "Failed to process library: " + libraryEntity.getName() + " - " + e.getMessage()));
  ...
  public void rescanLibrary(RescanLibraryContext context) throws IOException {
    LibraryEntity libraryEntity =
        libraryRepository
            .findById(context.getLibraryId())
            .orElseThrow(() -> ApiError.LIBRARY_NOT_FOUND.createException(context.getLibraryId()));
    notificationService.sendMessage(
        Topic.LOG, LogNotification.info("Started refreshing library: " + libraryEntity.getName()));
    ...
    List<LibraryFile> filteredFiles =
        libraryFileHelper.filterByAllowedFormats(
    ...
    if (existingBookCount > 0 && allLibraryFiles.isEmpty()) {
      ...
      log.error(
          "Library '{}' has {} existing books but scan found 0 files. Paths may be offline: {}",
    ...
    if (!additionalFileIds.isEmpty()) {
      log.info(
          "Detected {} removed additional files in library: {}",
          additionalFileIds.size(),
          libraryEntity.getName());
    ...
    BookGroupingService.GroupingResult groupingResult =
        bookGroupingService.groupForRescan(newFiles, libraryEntity);

    // Auto-attach files to existing books
    for (Map.Entry<BookEntity, List<LibraryFile>> entry :
        groupingResult.filesToAttach().entrySet()) {
    ...
    notificationService.sendMessage(
        Topic.LOG, LogNotification.info("Finished refreshing library: " + libraryEntity.getName()));
  }
  ...
  protected static List<Long> detectDeletedBookIds(
      List<LibraryFile> libraryFiles, LibraryEntity libraryEntity) {
    Set<Path> currentFullPaths =
  ...
  protected List<LibraryFile> detectNewBookPaths(
      List<LibraryFile> libraryFiles, LibraryEntity libraryEntity) {
  ...
  private void autoAttachFile(BookEntity book, LibraryFile file) {
    ...
    var existing =
        bookAdditionalFileRepository.findByLibraryPath_IdAndFileSubPathAndFileName(
    ...
    if (book.getLibraryPath() == null) {
      ...
      log.warn(
          "Cannot attach file '{}' to book id={}: file is in libraryPath {} but book is in libraryPath {}",
          file.getFileName(),
          book.getId(),
          file.getLibraryPathEntity().getId(),
          book.getLibraryPath().getId());
      return;
    }
    ...
    try {
      ...
      log.info(
          "Auto-attached new format {} to existing book: {}", file.getFileName(), primaryFileName);
  ...
  private String generateUniqueKey(BookEntity book) {
    ...
    return generateKey(
        book.getLibraryPath().getId(), primaryFile.getFileSubPath(), primaryFile.getFileName());
  }

  private String generateUniqueKey(BookFileEntity file) {
    return generateKey(
        file.getBook().getLibraryPath().getId(), file.getFileSubPath(), file.getFileName());
  }
  ...
  protected List<Long> detectDeletedAdditionalFiles(
      List<LibraryFile> libraryFiles, LibraryEntity libraryEntity) {
    Set<String> currentFileKeys =
        libraryFiles.stream().map(this::generateUniqueKey).collect(Collectors.toSet());

    List<BookFileEntity> allAdditionalFiles =
        bookAdditionalFileRepository.findByLibraryId(libraryEntity.getId());
