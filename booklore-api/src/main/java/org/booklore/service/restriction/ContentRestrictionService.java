...
public class ContentRestrictionService {
  ...
  public ContentRestriction getRestriction(Long restrictionId) {
    return restrictionRepository
        ...
        .map(this::toDto)
        .orElseThrow(
            () -> ApiError.GENERIC_NOT_FOUND.createException("Content restriction not found"));
  }
  ...
  public ContentRestriction addRestriction(Long userId, ContentRestriction restriction) {
    ...
    UserContentRestrictionEntity entity =
        UserContentRestrictionEntity.builder()
            .user(user)
  ...
  @Transactional
  public List<ContentRestriction> updateRestrictions(
      Long userId, List<ContentRestriction> restrictions) {
    BookLoreUserEntity user =
        userRepository
            .findById(userId)
            .orElseThrow(() -> ApiError.USER_NOT_FOUND.createException(userId));
    ...
    List<UserContentRestrictionEntity> entities =
        restrictions.stream()
            .map(
                r ->
                    UserContentRestrictionEntity.builder()
  ...
  public List<BookEntity> applyRestrictions(List<BookEntity> books, Long userId) {
    ...
    Set<String> excludedCategories =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.CATEGORY, ContentRestrictionMode.EXCLUDE);
    Set<String> excludedTags =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.TAG, ContentRestrictionMode.EXCLUDE);
    Set<String> excludedMoods =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.MOOD, ContentRestrictionMode.EXCLUDE);
    Set<String> excludedContentRatings =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.CONTENT_RATING, ContentRestrictionMode.EXCLUDE);

    Set<String> allowedCategories =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.CATEGORY, ContentRestrictionMode.ALLOW_ONLY);
    Set<String> allowedTags =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.TAG, ContentRestrictionMode.ALLOW_ONLY);
    Set<String> allowedMoods =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.MOOD, ContentRestrictionMode.ALLOW_ONLY);
    Set<String> allowedContentRatings =
        getValuesForTypeAndMode(
            restrictions, ContentRestrictionType.CONTENT_RATING, ContentRestrictionMode.ALLOW_ONLY);
    ...
    return books.stream()
        .filter(
            book ->
                !hasExcludedContent(
                    book, excludedCategories, excludedTags, excludedMoods, excludedContentRatings))
        .filter(
            book ->
                matchesAllowList(
                    book, allowedCategories, allowedTags, allowedMoods, allowedContentRatings))
        .filter(book -> isWithinAgeRating(book, maxAgeRating))
        .collect(Collectors.toList());
  }

  private Set<String> getValuesForTypeAndMode(
  ...
  private boolean matchesAllowList(
      BookEntity book,
      Set<String> allowedCategories,
      Set<String> allowedTags,
      Set<String> allowedMoods,
    ...
    if (allowedCategories.isEmpty()
        && allowedTags.isEmpty()
        && allowedMoods.isEmpty()
        && allowedContentRatings.isEmpty()) {
      return true;
    }
