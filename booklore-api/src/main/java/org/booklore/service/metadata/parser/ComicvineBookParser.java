...
public class ComicvineBookParser implements BookParser, DetailedMetadataProvider {
  ...
  private static final Pattern SERIES_ISSUE_PATTERN =
      Pattern.compile("^(.+?)\\s+#?(\\d+(?:\\.\\d+)?)(?:\\s|$)", Pattern.CASE_INSENSITIVE);
  private static final Pattern DIGITAL_PATTERN =
      Pattern.compile("\\(digital\\)", Pattern.CASE_INSENSITIVE);
  ...
  private static final Pattern SPECIAL_ISSUE_PATTERN =
      Pattern.compile("(annual|special|one-?shot)\\s+(\\d+)", Pattern.CASE_INSENSITIVE);
  private static final Pattern YEAR_PATTERN = Pattern.compile("\\(?(\\d{4})\\)?");
  private static final long MIN_REQUEST_INTERVAL_MS = 2000;

  private static final String VOLUME_FIELDS =
      "id,name,publisher,start_year,count_of_issues,description,deck,image,site_detail_url,aliases,first_issue,last_issue";
  private static final String ISSUE_LIST_FIELDS =
      "api_detail_url,cover_date,store_date,description,deck,id,image,issue_number,name,volume,site_detail_url,aliases,person_credits,character_credits,team_credits,story_arc_credits,location_credits";
  private static final String ISSUE_DETAIL_FIELDS =
      "api_detail_url,cover_date,store_date,description,deck,id,image,issue_number,name,person_credits,volume,site_detail_url,aliases,character_credits,team_credits,story_arc_credits,location_credits";
  private static final String SEARCH_FIELDS =
      "api_detail_url,cover_date,store_date,description,deck,id,image,issue_number,name,publisher,volume,site_detail_url,resource_type,start_year,count_of_issues,aliases,person_credits";
  ...
  private final Map<String, CachedVolumes> volumeCache =
  ...
  public BookMetadata fetchTopMetadata(Book book, FetchMetadataRequest fetchMetadataRequest) {
    ...
    if (top.getComicvineId() != null
        && (top.getComicMetadata() == null || !hasComicDetails(top.getComicMetadata()))) {
  ...
  public List<BookMetadata> getMetadataListByTerm(String term) {
    ...
    if (seriesAndIssue.issue() != null) {
      log.info(
          "Attempting structured search for Series: '{}', Issue: '{}', Year: '{}', Type: '{}'",
          seriesAndIssue.series(),
          seriesAndIssue.issue(),
          seriesAndIssue.year(),
          seriesAndIssue.issueType());
      List<BookMetadata> preciseResults =
          searchVolumesAndIssues(
    ...
    if (seriesAndIssue.issue() != null
        && seriesAndIssue.remainder() != null
        && !seriesAndIssue.remainder().isBlank()) {
  ...
  private List<BookMetadata> tryAlternativeSeriesNames(SeriesAndIssue original) {
    ...
    for (String altSeries : alternatives) {
      ...
      List<BookMetadata> results =
          searchVolumesAndIssues(altSeries, original.issue(), original.year());
      if (!results.isEmpty()) {
        return results;
      }
    }
  ...
  private List<BookMetadata> searchVolumesAndIssues(
      String seriesName, String issueNumber, Integer extractedYear) {
    ...
    if (seriesName.endsWith(" " + issueNumber) || seriesName.endsWith(" " + normalizedIssue)) {
      seriesName =
          seriesName
              .replaceAll("\\s+" + Pattern.quote(issueNumber) + "$", "")
    ...
    log.debug(
        "searchVolumesAndIssues: seriesName='{}', issueNumber='{}', year='{}'",
        finalSeriesName,
        issueNumber,
        extractedYear);
    ...
    for (int i = 0; i < limit; i++) {
      ...
      log.debug(
          "Checking volume: id='{}', name='{}', start_year='{}'",
          volume.getId(),
          volume.getName(),
          volume.getStartYear());
      ...
      if (!issues.isEmpty()) {
        ...
        if (extractedYear != null && matchesYear(volume, extractedYear)) {
          log.info(
              "Found match in year-aligned volume '{}' ({}). Stopping further volume searches.",
              volume.getName(),
              volume.getStartYear());
          break;
        }
  ...
  private int calculateVolumeScore(
      Comic volume, String seriesName, String normalizedIssue, Integer extractedYear) {
    int score = 0;
    ...
    if (volume.getName() != null && volume.getName().equalsIgnoreCase(seriesName)) {
      score += 50;
    } else if (volume.getName() != null
        && volume.getName().toLowerCase().contains(seriesName.toLowerCase())) {
      score += 25;
    }
  ...
  private List<Comic> searchVolumes(String seriesName) {
    ...
    URI uri =
        UriComponentsBuilder.fromUriString(COMICVINE_URL)
            .path("/volumes/")
  ...
  private List<BookMetadata> searchIssuesInVolume(Comic volume, String issueNumber) {
    ...
    String normalizedIssue = normalizeIssueNumber(issueNumber);
    log.debug(
        "searchIssuesInVolume: volumeId='{}', original='{}', normalized='{}'",
        volume.getId(),
        issueNumber,
        normalizedIssue);
    ...
    if (response != null && response.getResults() != null && !response.getResults().isEmpty()) {
      ...
      if (!issueNumbersMatch(normalizedIssue, returnedIssue)) {
        log.warn(
            "Issue number mismatch! Requested '{}', got '{}' from volume {}",
            normalizedIssue,
            returnedIssue,
            volume.getId());
        return Collections.emptyList();
      ...
      boolean hasCredits =
          hasAnyCredits(
              firstIssue.getPersonCredits(),
              firstIssue.getCharacterCredits(),
              firstIssue.getTeamCredits(),
              firstIssue.getStoryArcCredits(),
              firstIssue.getLocationCredits());
      ...
      return Collections.singletonList(
          Objects.requireNonNullElseGet(detailed, () -> convertToBookMetadata(firstIssue, volume)));
    }
    return Collections.emptyList();
  }
  ...
  private <T> T sendRequestWithRetry(URI uri, Class<T> responseType, int retriesLeft) {
    if (rateLimited.get()) {
      ...
      if (currentTime < rateLimitResetTime.get()) {
        log.warn(
            "ComicVine API is currently rate limited. Skipping request. Rate limit resets at: {}",
    ...
    try {
      ...
      HttpRequest request =
          HttpRequest.newBuilder()
              ...
              .header(
                  "User-Agent",
                  "BookLore/1.0 (Book and Comic Metadata Fetcher; +https://github.com/booklore-app/booklore)")
              .GET()
              .build();
      ...
      log.debug(
          "ComicVine API call #{} completed: status={}, size={}bytes",
          callNumber,
          response.statusCode(),
      ...
      if (response.statusCode() == 200) {
        ...
        log.warn(
            "ComicVine API returned status {}. Retrying... ({} retries left)",
            response.statusCode(),
            retriesLeft);
        try {
          Thread.sleep(2000);
        ...
        log.error(
            "Comicvine API returned status code {}. Body: {}",
            response.statusCode(),
            response.body());
      }
    } catch (IOException e) {
      if (retriesLeft > 0) {
        log.warn(
            "IOException during ComicVine request. Retrying... ({} retries left)", retriesLeft, e);
        try {
          Thread.sleep(1000);
        } catch (InterruptedException ignored) {
        }
  ...
  private void handleRateLimit(HttpResponse<String> response) {
    log.error(
        "ComicVine API rate limit exceeded (Error {}). Setting rate limit flag.",
        response.statusCode());
    ...
    if (!retryAfterHeaders.isEmpty()) {
      try {
        ...
        log.warn(
            "Could not parse Retry-After header '{}', using default 1 hour delay",
            retryAfterHeaders.getFirst());
      }
    }
  ...
  private BookMetadata convertToBookMetadata(Comic comic, Comic volumeContext) {
    ...
    if (metadata.getSeriesName() == null || metadata.getSeriesNumber() == null) {
      log.warn(
          "Incomplete metadata for issue {}: missing series name or number",
          metadata.getComicvineId());
    }
    if (metadata.getAuthors().isEmpty()) {
      log.debug(
          "No authors found for issue {} ({})", metadata.getComicvineId(), metadata.getTitle());
    }
  ...
  private BookMetadata convertToBookMetadata(
      ComicvineIssueResponse.IssueResults issue, int issueId, Comic volumeContext) {
  ...
  private ComicMetadata buildComicMetadata(
    ...
    if (personCredits != null && !personCredits.isEmpty()) {
      ...
      personCredits.stream()
          ...
          .filter(
              pc ->
                  Arrays.stream(pc.getRole().toLowerCase().split(","))
                      .map(String::trim)
  ...
  private String formatTitle(String seriesName, String issueNumber, String issueName) {
    ...
    if (issueName != null && !issueName.isBlank()) {
      ...
      boolean isGeneric =
          ISSUE_NUMBER_PATTERN.matcher(lowerName).matches()
              || lowerName.equals(seriesName.toLowerCase());
      if (!isGeneric) {
        title += " - " + issueName;
      }
    }
  ...
  private List<String> extractAuthors(List<Comic.PersonCredit> personCredits) {
    ...
    if (authors.isEmpty()) {
      List<String> allRoles =
          personCredits.stream().map(pc -> pc.getName() + " (" + pc.getRole() + ")").toList();
  ...
  private String getSearchTerm(Book book, FetchMetadataRequest request) {
    ...
    } else if (book.getPrimaryFile() != null
        && book.getPrimaryFile().getFileName() != null
        && !book.getPrimaryFile().getFileName().isEmpty()) {
  ...
  private SeriesAndIssue extractSeriesAndIssue(String term) {
    ...
    if (matcher.find()) {
      ...
      log.debug(
          "Extracted - Series: '{}', Issue: '{}', Remainder: '{}'", series, issueNum, remainder);
  ...
  private String getApiToken() {
    String apiToken =
        appSettingService.getAppSettings().getMetadataProviderSettings().getComicvine().getApiKey();
  ...
  private record SeriesAndIssue(
      String series, String issue, Integer year, String issueType, String remainder) {}
}
