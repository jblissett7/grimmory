...
public class SidecarMetadataMapper {
  ...
  public SidecarMetadata toSidecarMetadata(BookMetadataEntity entity, String coverFileName) {
    ...
    SidecarBookMetadata bookMetadata =
        SidecarBookMetadata.builder()
    ...
    if (StringUtils.hasText(coverFileName)) {
      coverInfo = SidecarCoverInfo.builder().source("external").path(coverFileName).build();
    }
  ...
  public BookMetadata toBookMetadata(SidecarMetadata sidecar) {
    ...
    if (m.getSeries() != null) {
      builder
          .seriesName(m.getSeries().getName())
    ...
    if (m.getRatings() != null) {
      ...
      if (r.getAudible() != null) {
        builder
            .audibleRating(r.getAudible().getAverage())
  ...
  private List<String> extractNames(List<AuthorEntity> entities) {
    ...
    return entities.stream().map(AuthorEntity::getName).toList();
  }

  private Set<String> extractCategoryNames(Set<CategoryEntity> entities) {
    if (entities == null) return null;
    return entities.stream().map(CategoryEntity::getName).collect(Collectors.toSet());
  }

  private Set<String> extractMoodNames(Set<MoodEntity> entities) {
    if (entities == null) return null;
    return entities.stream().map(MoodEntity::getName).collect(Collectors.toSet());
  }
  ...
  private SidecarSeries buildSeries(BookMetadataEntity entity) {
    if (entity.getSeriesName() == null
        && entity.getSeriesNumber() == null
        && entity.getSeriesTotal() == null) {
      return null;
    }
  ...
  private SidecarRating buildRating(Double average, Integer count) {
    ...
    return SidecarRating.builder().average(average).count(count).build();
  }
