...
public class RanobeDbParser implements BookParser {
  ...
  public List<BookMetadata> getMetadataListByTerm(String term, Boolean fetchTop) {
    ...
    try {
      ...
      URI uri =
          UriComponentsBuilder.fromUriString(RANOBEDB_URL)
              .path("/books")
              .queryParam("q", term)
  ...
  private List<BookMetadata> parseRanobeDbApiResponse(String responseBody, Boolean fetchTop)
      throws IOException {
    RanobedbSearchResponse searchResponse =
        OBJECT_MAPPER.readValue(responseBody, RanobedbSearchResponse.class);
    if (searchResponse.getBooks() == null) {
      return Collections.emptyList();
    }
    if (fetchTop && !searchResponse.getBooks().isEmpty()) {
      BookMetadata topMetadata =
          searchResultToBookMetadata(searchResponse.getBooks().getFirst().getId());
  ...
  private BookMetadata searchResultToBookMetadata(int bookId) {
    ...
    try {
      ...
      if (response.statusCode() == 200) {
        RanobedbBookResponse responstObj =
            OBJECT_MAPPER.readValue(response.body(), RanobedbBookResponse.class);
        RanobedbBookResponse.Book book = responstObj.getBook();
        if (book == null) {
          return null;
        }

        RanobedbBookResponse.TitleEntry englishTitleEntry =
            book.getTitles().stream()
                .filter(titleEntry -> "en".equalsIgnoreCase(titleEntry.getLang()))
                .filter(titleEntry -> titleEntry.getOfficial())
                .findFirst()
                .orElse(null);

        RanobedbBookResponse.Release englishRelease =
            book.getReleases().stream()
                .filter(release -> "en".equalsIgnoreCase(release.getLang()))
                .findFirst()
                .orElse(null);

        RanobedbBookResponse.Publisher englishPublisher =
            book.getPublishers().stream()
                .filter(publisher -> "en".equalsIgnoreCase(publisher.getLang()))
                .filter(
                    publisher ->
                        RanobedbBookResponse.PublisherType.PUBLISHER.equals(
                            publisher.getPublisherType()))
                .findFirst()
                .orElse(null);

        List<RanobedbBookResponse.SeriesBook> seriesBooks =
            book.getSeries() != null ? book.getSeries().getBooks() : List.of();
        int seriesIndex =
            IntStream.range(0, seriesBooks.size())
        ...
        if (book.getSeries() != null
            && book.getSeries().getTitle() != null
            && title.startsWith(book.getSeries().getTitle())) {
        ...
        return BookMetadata.builder()
            ...
            .thumbnailUrl(
                book.getImage() != null ? RANOBEDB_IMAGE_URL + book.getImage().getFilename() : null)
            ...
            .publishedDate(
                englishRelease != null
                    ? parseDate(englishRelease.getReleaseDate())
                    : parseDate(book.getCReleaseDate()))
            .build();
      } else {
  ...
  private LocalDate parseDate(Long dateInt) {
    ...
    try {
      ...
      return LocalDate.of(
          (int) (dateInt / 10000), (int) ((dateInt / 100) % 100), (int) (dateInt % 100));
