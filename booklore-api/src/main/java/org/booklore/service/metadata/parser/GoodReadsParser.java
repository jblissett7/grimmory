...
public class GoodReadsParser implements BookParser, DetailedMetadataProvider {
  ...
  public BookMetadata fetchTopMetadata(Book book, FetchMetadataRequest fetchMetadataRequest) {
    ...
    if (existingGoodreadsId != null) {
      ...
      try {
        ...
        log.warn(
            "GoodReads: Failed to parse details for existing ID: {}, falling back to search",
            existingGoodreadsId);
      } catch (Exception e) {
        log.warn(
            "GoodReads: Error fetching existing ID {}: {}, falling back to search",
            existingGoodreadsId,
            e.getMessage());
      }
    }

    Optional<BookMetadata> preview =
        fetchMetadataPreviews(book, fetchMetadataRequest).stream().findFirst();
    if (preview.isEmpty()) {
      return null;
    }
  ...
  public List<BookMetadata> fetchMetadata(Book book, FetchMetadataRequest fetchMetadataRequest) {
    ...
    if (isbn != null && !isbn.isBlank()) {
      ...
      String ogUrl =
          Optional.ofNullable(doc.selectFirst("meta[property=og:url]"))
    ...
    List<BookMetadata> previews =
        fetchMetadataPreviews(book, fetchMetadataRequest).stream()
            .limit(COUNT_DETAILED_METADATA_TO_GET)
            .toList();
    List<BookMetadata> results = fetchMetadataUsingPreviews(previews);

    if (results.isEmpty()
        && fetchMetadataRequest.getTitle() != null
        && !fetchMetadataRequest.getTitle().isBlank()
        && fetchMetadataRequest.getAuthor() != null
        && !fetchMetadataRequest.getAuthor().isBlank()) {
      log.info("GoodReads: No results with title+author, retrying with title only.");
      FetchMetadataRequest titleOnlyRequest =
          FetchMetadataRequest.builder()
              ...
              .build();
      previews =
          fetchMetadataPreviews(book, titleOnlyRequest).stream()
  ...
  private BookMetadata parseBookDetails(Document document, String goodreadsId) {
    BookMetadata.BookMetadataBuilder builder =
        BookMetadata.builder().goodreadsId(goodreadsId).provider(MetadataProvider.GoodReads);

    try {
      JSONObject apolloStateJson =
          getJson(document)
              .getJSONObject("props")
      ...
      appSettingService.getAppSettings().getMetadataPublicReviewsSettings().getProviders().stream()
    ...
    while (count < maxReviews && index < allReviewKeys.size()) {
      ...
      try {
        ...
        if (userJson != null) {
          ...
          followersCount =
              userJson.has("followersCount") ? userJson.optInt("followersCount") : null;
          textReviewsCount =
              userJson.has("textReviewsCount") ? userJson.optInt("textReviewsCount") : null;
        }
  ...
  private void extractSeriesDetails(
      JSONObject apolloStateJson,
      LinkedHashSet<String> keySet,
      BookMetadata.BookMetadataBuilder builder) {
  ...
  private void extractBookDetails(
      JSONObject apolloStateJson,
      LinkedHashSet<String> keySet,
      BookMetadata.BookMetadataBuilder builder) {
    ...
    if (detailsJson != null) {
      builder
          .pageCount(parseNumber(detailsJson.optString("numPages"), Integer::parseInt))
    ...
    if (bookSeriesJson != null && bookSeriesJson.length() > 0) {
      ...
      if (firstElement != null) {
        builder.seriesNumber(
            parseNumber(firstElement.optString("userPosition"), Float::parseFloat));
      }
    }
  }

  private void extractWorkDetails(
      JSONObject apolloStateJson,
      LinkedHashSet<String> keySet,
      BookMetadata.BookMetadataBuilder builder) {
    ...
    if (statsJson != null) {
      builder
          .goodreadsRating(parseNumber(statsJson.optString("averageRating"), Double::parseDouble))
          .goodreadsReviewCount(
              parseNumber(statsJson.optString("ratingsCount"), Integer::parseInt));
    }
  }
  ...
  private String findKeyByPrefix(LinkedHashSet<String> keySet, String prefix) {
    return keySet.stream().filter(key -> key.contains(prefix)).findFirst().orElse(null);
  }
    ...
    try {
      ...
      for (Element previewBook : previewBooks) {
        ...
        if (queryAuthor != null && !queryAuthor.isBlank()) {
          List<String> queryAuthorTokens =
              List.of(WHITESPACE_PATTERN.split(queryAuthor.toLowerCase()));
          boolean matches =
              authors.stream()
  ...
  private String getSearchTerm(Book book, FetchMetadataRequest request) {
    ...
    return (book.getPrimaryFile() != null
