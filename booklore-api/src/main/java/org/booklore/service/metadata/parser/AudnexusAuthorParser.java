...
public class AudnexusAuthorParser implements AuthorParser {
  ...
  public List<AuthorSearchResult> searchAuthors(String name, String region) {
    try {
      ...
      URI uri =
          UriComponentsBuilder.fromUriString(BASE_URL + "/authors")
      ...
      if (response.statusCode() == 200) {
        AudnexusAuthorResponse[] authors =
            objectMapper.readValue(response.body(), AudnexusAuthorResponse[].class);
  ...
  public AuthorSearchResult getAuthorByAsin(String asin, String region) {
    try {
      ...
      URI uri =
          UriComponentsBuilder.fromUriString(BASE_URL + "/authors/" + asin)
              .queryParam("region", region)
              .build()
              .toUri();
