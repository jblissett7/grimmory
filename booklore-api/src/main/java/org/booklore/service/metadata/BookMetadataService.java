...
public class BookMetadataService {
  ...
  public Flux<BookMetadata> getProspectiveMetadataListForBookId(
      long bookId, FetchMetadataRequest request) {
  ...
  public List<BookMetadata> fetchMetadataListFromAProvider(
      MetadataProvider provider, Book book, FetchMetadataRequest request) {
  ...
  public BookMetadata lookupByIsbn(IsbnLookupRequest request) {
    ...
    FetchMetadataRequest fetchRequest =
        FetchMetadataRequest.builder().isbn(request.getIsbn()).providers(providers).build();
    ...
    for (MetadataProvider provider : providers) {
      try {
        List<BookMetadata> results =
            fetchMetadataListFromAProvider(provider, emptyBook, fetchRequest);
  ...
  private List<MetadataProvider> deriveProviderChainFromSettings() {
    try {
      MetadataRefreshOptions options =
          appSettingService.getAppSettings().getDefaultMetadataRefreshOptions();
      ...
      log.warn(
          "Failed to derive provider chain from settings, falling back to default: {}",
          e.getMessage());
    }
    return List.of(MetadataProvider.Google);
  }

  public BookMetadata getDetailedProviderMetadata(
      MetadataProvider provider, String providerItemId) {
  ...
  public void toggleFieldLocks(List<Long> bookIds, Map<String, String> fieldActions) {
    Map<String, String> fieldMapping = Map.of("thumbnailLocked", "coverLocked");
    List<BookMetadataEntity> metadataEntities =
        bookMetadataRepository.getMetadataForBookIds(bookIds).stream().distinct().toList();

    for (BookMetadataEntity metadataEntity : metadataEntities) {
      fieldActions.forEach(
          (field, action) -> {
            String entityField = fieldMapping.getOrDefault(field, field);
            try {
              String setterName =
                  "set" + Character.toUpperCase(entityField.charAt(0)) + entityField.substring(1);
              ...
              throw new RuntimeException(
                  "Failed to invoke setter for field: "
                      + entityField
                      + " on bookId: "
                      + metadataEntity.getBookId(),
                  e);
            }
          });
    }
  ...
  public List<BookMetadata> toggleAllLock(ToggleAllLockRequest request) {
    ...
    List<BookEntity> books =
        bookQueryService.findAllWithMetadataByIds(request.getBookIds()).stream()
            .peek(book -> book.getMetadata().applyLockToAllFields(lock))
            .toList();
    bookRepository.saveAll(books);
    return books.stream()
        .map(b -> bookMetadataMapper.toBookMetadata(b.getMetadata(), false))
  ...
  public BookMetadata getFileMetadata(long bookId) {
    ...
    if (primaryFile == null) {
      throw ApiError.GENERIC_BAD_REQUEST.createException(
          "Book has no file to extract metadata from");
    }
    return metadataExtractorFactory.extractMetadata(
        primaryFile.getBookType(), new File(FileUtils.getBookFullPath(bookEntity)));
  }

  @Transactional
  public void bulkUpdateMetadata(
    ...
    for (Long bookId : request.getBookIds()) {
      try {
        processSingleBookUpdate(
            bookId, bookMetadata, clearFlags, mergeCategories, mergeMoods, mergeTags);
      } catch (Exception e) {
  ...
  private void processSingleBookUpdate(
      Long bookId,
      BookMetadata bookMetadata,
      MetadataClearFlags clearFlags,
      boolean mergeCategories,
      boolean mergeMoods,
      boolean mergeTags) {
    ...
    transactionTemplate.execute(
        status -> {
          ...
          notificationService.sendMessage(
              Topic.BOOK_UPDATE, bookMapper.toBookWithDescription(book, true));
          return null;
        });
  }
}
