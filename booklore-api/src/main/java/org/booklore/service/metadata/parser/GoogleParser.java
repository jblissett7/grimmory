...
public class GoogleParser implements BookParser {
  ...
  public GoogleParser(
      ObjectMapper objectMapper, AppSettingService appSettingService, HttpClient httpClient) {
  ...
  public List<BookMetadata> fetchMetadata(Book book, FetchMetadataRequest fetchMetadataRequest) {
    ...
    if (fetchMetadataRequest.getIsbn() != null && !fetchMetadataRequest.getIsbn().isBlank()) {
      List<BookMetadata> isbnResults =
          getMetadataListByIsbn(ParserUtils.cleanIsbn(fetchMetadataRequest.getIsbn()));
      if (!isbnResults.isEmpty()) {
        return isbnResults;
      }
      log.info(
          "Google Books: ISBN search returned no results, falling back to Title+Author search.");
    }
  ...
  private List<BookMetadata> fetchFromApi(String query, boolean isIsbnSearch) {
    try {
      ...
      UriComponentsBuilder uriBuilder =
          UriComponentsBuilder.fromUriString(getApiUrl())
              .queryParam("q", query)
  ...
  private List<BookMetadata> handleApiResponse(HttpResponse<String> response) throws IOException {
    ...
    log.error(
        "Google Books API request failed. Status: {}, Response: {}", statusCode, response.body());
    return List.of();
  }
  ...
  private List<BookMetadata> filterIrrelevantResults(List<BookMetadata> results) {
    ...
    return results.stream().filter(this::isRelevantResult).collect(Collectors.toList());
  }
  ...
  private boolean isRelevantResult(BookMetadata metadata) {
    ...
    boolean hasDescription =
        metadata.getDescription() != null && metadata.getDescription().length() > 10;
  ...
  private List<BookMetadata> parseGoogleBooksApiResponse(String responseBody) throws IOException {
    GoogleBooksApiResponse googleBooksApiResponse =
        objectMapper.readValue(responseBody, GoogleBooksApiResponse.class);
  ...
  private BookMetadata convertToFetchedBookMetadata(GoogleBooksApiResponse.Item item) {
    ...
    Integer pageCount =
        volumeInfo.getPrintedPageCount() != null && volumeInfo.getPrintedPageCount() > 0
  ...
  /**
   * Extract series number from common title patterns. Handles formats like: - "One Piece, Vol. 100"
   * - "Naruto Volume 5" - "Batman #123" - "Spider-Man 2099 2" - "Title (Band 5)"
   */
  private SeriesData extractSeriesFromTitle(String title) {
    ...
    Pattern volPattern =
        Pattern.compile(
            "(.+?)(?:,\\s*)?(?:Vol\\.?|Volume)\\s*(\\d+(?:\\.\\d+)?)", Pattern.CASE_INSENSITIVE);
    ...
    Pattern bandPattern =
        Pattern.compile("(.+?)(?:,\\s*)?Band\\s*(\\d+(?:\\.\\d+)?)", Pattern.CASE_INSENSITIVE);
  ...
  private String getExternalUrl(GoogleBooksApiResponse.Item.VolumeInfo volumeInfo) {
    ...
    if (volumeInfo.getCanonicalVolumeLink() != null
        && !volumeInfo.getCanonicalVolumeLink().isBlank()) {
  ...
  private Map<String, String> extractISBNs(
      List<GoogleBooksApiResponse.Item.IndustryIdentifier> identifiers) {
    if (identifiers == null) return Map.of();

    return identifiers.stream()
        .filter(
            identifier ->
                "ISBN_13".equals(identifier.getType()) || "ISBN_10".equals(identifier.getType()))
        .collect(
            Collectors.toMap(
  ...
  private String getSearchTerm(Book book, FetchMetadataRequest request) {
    String searchTerm =
        Optional.ofNullable(request.getTitle())
            ...
            .orElseGet(
                () ->
                    Optional.ofNullable(book.getPrimaryFile())
  ...
  /**
   * Parse date string from Google Books API. Handles various formats: - "2021-05-15" (full date) -
   * "2021-05" (year-month) - "2021" (year only)
   */
  ...
  private String getApiUrl() {
    MetadataProviderSettings.Google googleSettings =
        appSettingService.getAppSettings().getMetadataProviderSettings().getGoogle();
  ...
  /** Sort results by metadata completeness. Items with more populated fields come first. */
  private List<BookMetadata> sortByCompleteness(List<BookMetadata> results) {
    ...
    return results.stream()
        .sorted((a, b) -> Integer.compare(countPopulatedFields(b), countPopulatedFields(a)))
        .collect(Collectors.toList());
  }
