...
public class EpubMetadataExtractor implements FileMetadataExtractor {
  ...
  private static final Map<String, BiConsumer<BookMetadata.BookMetadataBuilder, String>>
      CALIBRE_IDENTIFIER_PREFIXES =
          Map.of(
  ...
  private static final Map<String, BiConsumer<BookMetadata.BookMetadataBuilder, String>>
      CALIBRE_FIELD_MAPPINGS =
          Map.ofEntries(
              ...
              Map.entry(
                  "#series_total", (builder, value) -> safeParseInt(value, builder::seriesTotal)),
              Map.entry(
                  "#amazon_rating",
                  (builder, value) -> safeParseDouble(value, builder::amazonRating)),
              Map.entry(
                  "#amazon_review_count",
                  (builder, value) -> safeParseInt(value, builder::amazonReviewCount)),
              Map.entry(
                  "#goodreads_rating",
                  (builder, value) -> safeParseDouble(value, builder::goodreadsRating)),
              Map.entry(
                  "#goodreads_review_count",
                  (builder, value) -> safeParseInt(value, builder::goodreadsReviewCount)),
              Map.entry(
                  "#hardcover_rating",
                  (builder, value) -> safeParseDouble(value, builder::hardcoverRating)),
              Map.entry(
                  "#hardcover_review_count",
                  (builder, value) -> safeParseInt(value, builder::hardcoverReviewCount)),
              Map.entry(
                  "#lubimyczytac_rating",
                  (builder, value) -> safeParseDouble(value, builder::lubimyczytacRating)),
              Map.entry(
                  "#ranobedb_rating",
                  (builder, value) -> safeParseDouble(value, builder::ranobedbRating)),
              Map.entry(
                  "#age_rating",
                  (builder, value) ->
                      safeParseInt(
              ...
              Map.entry(
                  ...
                  (builder, value) -> {
                    ...
                    if (Set.of("EVERYONE", "TEEN", "MATURE", "ADULT", "EXPLICIT")
                        .contains(normalized)) {
  ...
  public byte[] extractCover(File epubFile) {
    ...
    try (FileInputStream fis = new FileInputStream(epubFile)) {
      try {
        ...
        log.debug(
            "epub4j threw NullPointerException (likely malformed NCX) in {}: {}",
            epubFile.getName(),
            e.getMessage());
      } catch (Exception e) {
        log.debug(
            "epub4j failed to parse EPUB for cover extraction (will try fallbacks) in {}: {}",
            epubFile.getName(),
            e.getMessage());
      }
  ...
  public BookMetadata extractMetadata(File epubFile) {
    try (ZipFile zip = new ZipFile(epubFile)) {
          ...
          for (int i = 0; i < children.getLength(); i++) {
            ...
            switch (tag) {
              ...
              case "meta" -> {
                ...
                String content =
                    el.hasAttribute("content") ? el.getAttribute("content").trim() : text;
                ...
                if (!seriesFound
                    && ((BookLoreMetadata.NS_PREFIX + ":series").equals(prop)
                        || "calibre:series".equals(name)
                        || "belongs-to-collection".equals(prop))) {
                  builderMeta.seriesName(content);
                  seriesFound = true;
                }
                if (!seriesIndexFound
                    && ((BookLoreMetadata.NS_PREFIX + ":series_index").equals(prop)
                        || "calibre:series_index".equals(name)
                        || "group-position".equals(prop))) {
                  try {
                ...
                if ("calibre:pages".equals(name)
                    || "pagecount".equals(name)
                    || "schema:pagecount".equals(prop)
                    || "media:pagecount".equals(prop)
                    || (BookLoreMetadata.NS_PREFIX + ":page_count").equals(prop)) {
                ...
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":goodreads_id"))
                  builderMeta.goodreadsId(content);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":comicvine_id"))
                  builderMeta.comicvineId(content);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":ranobedb_id"))
                  builderMeta.ranobedbId(content);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":hardcover_id"))
                  builderMeta.hardcoverId(content);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":google_books_id"))
                  builderMeta.googleId(content);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":lubimyczytac_id"))
                  builderMeta.lubimyczytacId(content);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":page_count"))
                  safeParseInt(content, builderMeta::pageCount);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":subtitle"))
                  builderMeta.subtitle(content);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":series_total"))
                  safeParseInt(content, builderMeta::seriesTotal);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":rating")) {
                  /* Generic rating not supported */
                } else if (key.equals(BookLoreMetadata.NS_PREFIX + ":amazon_rating"))
                  safeParseDouble(content, builderMeta::amazonRating);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":amazon_review_count"))
                  safeParseInt(content, builderMeta::amazonReviewCount);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":goodreads_rating"))
                  safeParseDouble(content, builderMeta::goodreadsRating);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":goodreads_review_count"))
                  safeParseInt(content, builderMeta::goodreadsReviewCount);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":hardcover_rating"))
                  safeParseDouble(content, builderMeta::hardcoverRating);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":hardcover_review_count"))
                  safeParseInt(content, builderMeta::hardcoverReviewCount);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":lubimyczytac_rating"))
                  safeParseDouble(content, builderMeta::lubimyczytacRating);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":ranobedb_rating"))
                  safeParseDouble(content, builderMeta::ranobedbRating);
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":age_rating"))
                  safeParseInt(
                      content,
                      v -> {
                        if (VALID_AGE_RATINGS.contains(v)) builderMeta.ageRating(v);
                      });
                else if (key.equals(BookLoreMetadata.NS_PREFIX + ":content_rating"))
                  builderMeta.contentRating(content);
                ...
                if (!scheme.isEmpty()) {
                  ...
                  if (colonIdx > 0) {
                    ...
                    if (!"calibre".equals(prefix) && !"uuid".equals(prefix)) {
                      BiConsumer<BookMetadata.BookMetadataBuilder, String> setter =
                          CALIBRE_IDENTIFIER_PREFIXES.get(prefix);
                      if (setter != null) {
      ...
      log.error(
          "Failed to read metadata from EPUB file {}: {}", epubFile.getName(), e.getMessage(), e);
      return null;
    }
  }
  ...
  private void extractCalibreUserMetadata(
    ...
    while (keys.hasNext()) {
      ...
      try {
        ...
        if ("#moods".equals(fieldName)) {
          ...
          BiConsumer<BookMetadata.BookMetadataBuilder, String> mapper =
              CALIBRE_FIELD_MAPPINGS.get(fieldName);
          if (mapper != null) {
  ...
  /**
   * Parses a string that may be either a JSON array (e.g., ["item1", "item2"]) or a CSV (item1,
   * item2). Returns a Set of parsed values.
   */
