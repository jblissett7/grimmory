...
public class MetadataManagementService {
  ...
  public void consolidateMetadata(
      MergeMetadataType metadataType, List<String> targetValues, List<String> valuesToMerge) {
  ...
  private void consolidateAuthors(
      List<String> targetValues, List<String> valuesToMerge, boolean moveFile) {
    List<AuthorEntity> targetAuthors =
        targetValues.stream()
            .map(
                name ->
                    authorRepository
    ...
    List<AuthorEntity> authorsToMerge =
        valuesToMerge.stream()
    ...
    for (AuthorEntity oldAuthor : authorsToMerge) {
      List<BookMetadataEntity> booksWithOldAuthor =
          bookMetadataRepository.findAllByAuthorsContaining(oldAuthor);
    ...
    log.info(
        "Consolidated {} authors into {}: {}", authorsToMerge.size(), targetValues, valuesToMerge);
  }

  private void consolidateCategories(
      List<String> targetValues, List<String> valuesToMerge, boolean moveFile) {
    List<CategoryEntity> targetCategories =
        targetValues.stream()
            .map(
                name ->
                    categoryRepository
    ...
    List<CategoryEntity> categoriesToMerge =
        valuesToMerge.stream()
    ...
    for (CategoryEntity oldCategory : categoriesToMerge) {
      List<BookMetadataEntity> booksWithOldCategory =
          bookMetadataRepository.findAllByCategoriesContaining(oldCategory);
    ...
    log.info(
        "Consolidated {} categories into {}: {}",
        categoriesToMerge.size(),
        targetValues,
        valuesToMerge);
  }

  private void consolidateMoods(
      List<String> targetValues, List<String> valuesToMerge, boolean moveFile) {
    List<MoodEntity> targetMoods =
        targetValues.stream()
            .map(
                name ->
                    moodRepository
    ...
    for (MoodEntity oldMood : moodsToMerge) {
      List<BookMetadataEntity> booksWithOldMood =
          bookMetadataRepository.findAllByMoodsContaining(oldMood);
  ...
  private void consolidateTags(
    ...
    for (TagEntity oldTag : tagsToMerge) {
      List<BookMetadataEntity> booksWithOldTag =
          bookMetadataRepository.findAllByTagsContaining(oldTag);
  ...
  private void consolidateSeries(
      List<String> targetValues, List<String> valuesToMerge, boolean moveFile) {
    ...
    for (String oldSeriesName : valuesToMerge) {
      List<BookMetadataEntity> booksWithOldSeries =
          bookMetadataRepository.findAllBySeriesNameIgnoreCase(oldSeriesName);
    ...
    log.info(
        "Consolidated {} series into '{}': {}",
        valuesToMerge.size(),
        targetSeriesName,
        valuesToMerge);
  }

  private void consolidatePublishers(
      List<String> targetValues, List<String> valuesToMerge, boolean moveFile) {
    if (targetValues.size() != 1) {
    ...
    for (String oldPublisher : valuesToMerge) {
      List<BookMetadataEntity> booksWithOldPublisher =
          bookMetadataRepository.findAllByPublisherIgnoreCase(oldPublisher);
    ...
    log.info(
        "Consolidated {} publishers into '{}': {}",
        valuesToMerge.size(),
        targetPublisher,
        valuesToMerge);
  }

  private void consolidateLanguages(
      List<String> targetValues, List<String> valuesToMerge, boolean moveFile) {
    if (targetValues.size() != 1) {
    ...
    for (String oldLanguage : valuesToMerge) {
      List<BookMetadataEntity> booksWithOldLanguage =
          bookMetadataRepository.findAllByLanguageIgnoreCase(oldLanguage);
  ...
  private void deleteAuthors(List<String> valuesToDelete, boolean moveFile) {
    List<AuthorEntity> authorsToDelete =
        valuesToDelete.stream()
    ...
    for (AuthorEntity author : authorsToDelete) {
      List<BookMetadataEntity> booksWithAuthor =
          bookMetadataRepository.findAllByAuthorsContaining(author);
  ...
  private void deleteCategories(List<String> valuesToDelete, boolean moveFile) {
    List<CategoryEntity> categoriesToDelete =
        valuesToDelete.stream()
            .map(categoryRepository::findByNameIgnoreCase)
            .filter(java.util.Optional::isPresent)
            .map(java.util.Optional::get)
    ...
    for (CategoryEntity category : categoriesToDelete) {
      List<BookMetadataEntity> booksWithCategory =
          bookMetadataRepository.findAllByCategoriesContaining(category);
  ...
  private void deleteMoods(List<String> valuesToDelete, boolean moveFile) {
    ...
    for (MoodEntity mood : moodsToDelete) {
      List<BookMetadataEntity> booksWithMood =
          bookMetadataRepository.findAllByMoodsContaining(mood);
  ...
  private void deleteSeries(List<String> valuesToDelete, boolean moveFile) {
    for (String seriesName : valuesToDelete) {
      List<BookMetadataEntity> booksWithSeries =
          bookMetadataRepository.findAllBySeriesNameIgnoreCase(seriesName);
  ...
  private void deletePublishers(List<String> valuesToDelete, boolean moveFile) {
    for (String publisher : valuesToDelete) {
      List<BookMetadataEntity> booksWithPublisher =
          bookMetadataRepository.findAllByPublisherIgnoreCase(publisher);
  ...
  private void deleteLanguages(List<String> valuesToDelete, boolean moveFile) {
    for (String language : valuesToDelete) {
      List<BookMetadataEntity> booksWithLanguage =
          bookMetadataRepository.findAllByLanguageIgnoreCase(language);
