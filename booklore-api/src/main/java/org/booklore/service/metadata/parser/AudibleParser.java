...
public class AudibleParser implements BookParser, DetailedMetadataProvider {
  ...
  private static final Pattern SERIES_BOOK_PATTERN =
      Pattern.compile(",?\\s*Book\\s*(\\d+(?:\\.\\d+)?)$");
  ...
  private BookMetadata buildBookMetadataFromJsonLd(Document doc, String audibleId) {
    ...
    AudiobookMetadata audiobookMetadata =
        AudiobookMetadata.builder().durationSeconds(durationSeconds).build();
  ...
  private String getDomain() {
    ...
    if (settings != null
        && settings.getAudible() != null
        && settings.getAudible().getDomain() != null) {
