...
public class LubimyCzytacParser implements BookParser {
  ...
  public List<BookMetadata> fetchMetadata(Book book, FetchMetadataRequest fetchMetadataRequest) {
    ...
    if (settings == null
        || settings.getLubimyczytac() == null
        || !settings.getLubimyczytac().isEnabled()) {
        ...
        if (metadata != null) {
          ...
          if (searchingByIsbn) {
            if (isbnMatches(metadata, searchIsbn)) {
              ...
              log.debug(
                  "Skipping book {} - ISBN doesn't match search ISBN {}",
                  metadata.getTitle(),
                  searchIsbn);
            }
          } else {
  ...
  private Document fetchWithRetry(String url) {
    for (int attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        return Jsoup.connect(url).userAgent(USER_AGENT).timeout(CONNECTION_TIMEOUT_MS).get();
