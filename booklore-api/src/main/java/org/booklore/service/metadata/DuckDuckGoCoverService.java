...
public class DuckDuckGoCoverService implements BookCoverProvider {
  ...
  private static final String SEARCH_PARAMS_SQUARE =
      "&iar=images&iaf=size%3ALarge%2Clayout%3ASquare";
  ...
  private static final Map<String, String> HTML_HEADERS =
      Map.ofEntries(
          Map.entry(
              "accept",
  ...
  public List<CoverImage> getCovers(CoverFetchRequest request) {
    ...
    List<CoverImage> siteFilteredImages =
        fetchImagesFromApi(
            searchTerm + " (site:amazon.com OR site:goodreads.com)",
            siteSearchToken,
            cookies,
            siteUrl,
            jsonParams);
    ...
    if (generalMatcher.find()) {
      ...
      generalBookImages =
          fetchImagesFromApi(
              searchTerm, generalSearchToken, generalCookies, generalUrl, jsonParams);
      ...
      Set<String> siteUrls =
          siteFilteredImages.stream().map(CoverImage::getUrl).collect(Collectors.toSet());
  ...
  public List<CoverImage> searchImages(String searchTerm) {
    ...
    List<CoverImage> images =
        fetchImagesFromApi(searchTerm, searchToken, cookies, searchUrl, jsonParams);
  ...
  private List<CoverImage> fetchImagesFromApi(
      String query,
      String searchToken,
