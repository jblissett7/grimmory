...
public abstract class MobiBaseMetadataExtractor implements FileMetadataExtractor {
  ...
  protected MobiHeader readMobiHeader(RandomAccessFile raf, PalmDB palmDB) throws IOException {
    ...
    log.debug(
        "PalmDOC: compression={}, textLength={}, recordCount={}, recordSize={}",
        compression,
        textLength,
        recordCount,
        recordSize);
  ...
  protected void processFormatSpecificHeader(
      RandomAccessFile raf, PalmDBRecord record0, MobiHeader header, int headerLength)
      throws IOException {
    // Override in subclasses for format-specific processing
  }

  protected void readExthRecords(RandomAccessFile raf, MobiHeader header, Charset charset)
      throws IOException {
  ...
  protected byte[] extractImageFromRecord(RandomAccessFile raf, PalmDBRecord record)
      throws IOException {
    ...
    if (data.length > 4) {
      if ((data[0] == (byte) 0xFF && data[1] == (byte) 0xD8)
          ...
          (data[0] == (byte) 0x89
              ...
              && data[3] == (byte) 0x47)
          || // PNG
          (data[0] == (byte) 0x47 && data[1] == (byte) 0x49 && data[2] == (byte) 0x46)) { // GIF
        return data;
      }
    }
