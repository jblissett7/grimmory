...
public class PdfMetadataExtractor implements FileMetadataExtractor {
        ...
        try (InputStream is = metadata.createInputStream()) {
          if (is == null) {
            ...
            if (StringUtils.isNotBlank(rawXmp)) {
              ...
              Document doc =
                  dBuilder.parse(new ByteArrayInputStream(rawXmp.getBytes(StandardCharsets.UTF_8)));
              ...
              if (log.isDebugEnabled()) {
                ...
                log.debug(
                    "PDF XMP extraction results - subtitle: '{}', moods: {}, tags: {}",
                    debugMeta.getSubtitle(),
                    debugMeta.getMoods(),
                    debugMeta.getTags());
              }
  ...
  private void extractDublinCoreMetadata(
      XPath xpath, Document doc, BookMetadata.BookMetadataBuilder builder)
    ...
    Set<String> subjects =
        new HashSet<>(xpathEvaluateMultiple(xpath, doc, "//dc:subject/rdf:Bag/rdf:li/text()"));
    if (!subjects.isEmpty()) {
  ...
  private void extractCalibreMetadata(
      XPath xpath, Document doc, BookMetadata.BookMetadataBuilder builder) {
    try {
  ...
  private void extractBookloreMetadata(
      XPath xpath, Document doc, BookMetadata.BookMetadataBuilder builder) {
    try {
      ...
      if (moods.isEmpty()) {
        ...
        if (StringUtils.isNotBlank(moodsLegacy)) {
          moods =
              Arrays.stream(moodsLegacy.split(";"))
                  .map(String::trim)
                  .filter(StringUtils::isNotBlank)
                  .collect(Collectors.toSet());
        }
      ...
      if (tags.isEmpty()) {
        ...
        if (StringUtils.isNotBlank(tagsLegacy)) {
          tags =
              Arrays.stream(tagsLegacy.split(";"))
                  .map(String::trim)
                  .filter(StringUtils::isNotBlank)
                  .collect(Collectors.toSet());
        }
      ...
      extractBookloreRating(
          xpath, doc, "goodreadsRating", "GoodreadsRating", builder::goodreadsRating);
      extractBookloreRating(
          xpath, doc, "hardcoverRating", "HardcoverRating", builder::hardcoverRating);
      extractBookloreRating(
          xpath, doc, "lubimyczytacRating", "LubimyczytacRating", builder::lubimyczytacRating);
      extractBookloreRating(
          xpath, doc, "ranobedbRating", "RanobedbRating", builder::ranobedbRating);
  ...
  private void extractBookloreRating(
      XPath xpath,
      Document doc,
      String newName,
      String legacyName,
  ...
  private Map<String, String> extractIdentifiers(XPath xpath, Document doc)
      throws XPathExpressionException {
    Map<String, String> ids = new HashMap<>();
    NodeList idNodes =
        (NodeList) xpath.evaluate("//xmp:Identifier/rdf:Bag/rdf:li", doc, XPathConstants.NODESET);
  ...
  private String xpathEvaluate(XPath xpath, Document doc, String expression)
      throws XPathExpressionException {
    String result = xpath.evaluate(expression, doc);
    return result == null ? "" : result.trim();
  }

  private String xpathEvaluate(XPath xpath, org.w3c.dom.Node node, String expression)
      throws XPathExpressionException {
    String result = xpath.evaluate(expression, node);
    return result == null ? "" : result.trim();
  }

  private List<String> xpathEvaluateMultiple(XPath xpath, Document doc, String expression)
      throws XPathExpressionException {
