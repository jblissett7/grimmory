...
public class AudiobookMetadataExtractor implements FileMetadataExtractor {
  ...
  public BookMetadata extractMetadata(File audioFile) {
    try {
      ...
      if (StringUtils.isNotBlank(year)) {
        try {
          int yearInt =
              Integer.parseInt(year.trim().substring(0, Math.min(4, year.trim().length())));
      ...
      log.error(
          "Failed to read metadata from audio file {}: {}", audioFile.getName(), e.getMessage(), e);
      return BookMetadata.builder().title(FilenameUtils.getBaseName(audioFile.getName())).build();
    }
  }

  private void extractAudioHeaderInfo(
      AudiobookMetadata.AudiobookMetadataBuilder builder, AudioHeader header) {
    if (header == null) {
      return;
    }
  ...
  private List<AudiobookMetadata.ChapterInfo> extractChapters(
      File audioFile, AudioFile taggedFile, AudioHeader header) {
    List<AudiobookMetadata.ChapterInfo> chapters = extractId3v2Chapters(taggedFile);
    if (chapters != null && !chapters.isEmpty()) {
      log.debug(
          "Extracted {} chapters from ID3v2 CHAP frames for {}",
          chapters.size(),
          audioFile.getName());
      return chapters;
    }
  ...
  private List<AudiobookMetadata.ChapterInfo> extractId3v2Chapters(AudioFile taggedFile) {
    try {
      ...
      frameList.sort(
          (f1, f2) -> {
            if (f1.getBody() instanceof FrameBodyCHAP chap1
                && f2.getBody() instanceof FrameBodyCHAP chap2) {
  ...
  private List<AudiobookMetadata.ChapterInfo> extractChaptersWithFfprobe(File audioFile) {
    ...
    try {
      ...
      if (ffprobeBinary == null) {
        log.debug(
            "ffprobe binary not available, skipping chapter extraction for {}",
            audioFile.getName());
        return null;
      }
      ...
      for (int i = 0; i < chaptersNode.size(); i++) {
        ...
        if (chapterNode.has("start_time")) {
          ...
          String timeBase =
              chapterNode.has("time_base") ? chapterNode.get("time_base").asText() : "1/1000";
      ...
      log.debug(
          "Failed to extract chapters with ffprobe from {}: {}",
          audioFile.getName(),
          e.getMessage());
      return null;
    }
  ...
  public byte[] extractCover(File audioFile) {
    try {
      ...
      log.warn(
          "Failed to extract cover from audio file {}: {}", audioFile.getName(), e.getMessage());
      return null;
    }
  }
}
