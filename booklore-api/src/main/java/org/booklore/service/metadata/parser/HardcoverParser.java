...
public class HardcoverParser implements BookParser {
  ...
  public List<BookMetadata> fetchMetadata(Book book, FetchMetadataRequest fetchMetadataRequest) {
    ...
    if (searchByIsbn) {
      ...
      List<GraphQLResponse.BookWithEditions> hits =
          hardcoverBookSearchService.searchBookByIsbn(isbnCleaned);
  ...
  private List<BookMetadata> processBooksWithEditions(
      List<GraphQLResponse.BookWithEditions> books) {
    ...
    for (GraphQLResponse.BookWithEditions book : books) {
      ...
      for (GraphQLResponse.Edition edition : book.getEditions()) {
        log.debug(
            "Processing edition '{}' with id '{}' of book '{}'",
            edition.getTitle(),
            edition.getId(),
            book.getTitle());
  ...
  private List<BookMetadata> processHits(
      List<GraphQLResponse.Hit> hits, FetchMetadataRequest request, boolean searchByIsbn) {
    ...
    // Filter by author
    List<GraphQLResponse.Document> matchedDocs =
        hits.stream()
  ...
  private boolean filterByAuthor(
      GraphQLResponse.Document doc,
      String searchAuthor,
      boolean searchByIsbn,
    ...
    List<String> actualAuthorTokens =
    ...
    for (String actual : actualAuthorTokens) {
      for (String query : searchAuthorTokens) {
        ...
        int maxScore =
            Math.max(fuzzyScore.fuzzyScore(query, query), fuzzyScore.fuzzyScore(actual, actual));
  ...
  private BookMetadata mapDocumentToMetadata(
      GraphQLResponse.Document doc, FetchMetadataRequest request, boolean fetchDetailedMoods) {
  ...
  private BookMetadata mapEditionToMetadata(
      GraphQLResponse.Edition edition, GraphQLResponse.BookWithEditions book) {
    BookMetadata metadata = new BookMetadata();
    ...
    if (edition.getCachedContributors() != null) {
      metadata.setAuthors(
          edition.getCachedContributors().stream()
      ...
      if (book.getFeaturedBookSeries().getPosition() != null) {
        try {
          metadata.setSeriesNumber(
              Float.parseFloat(String.valueOf(book.getFeaturedBookSeries().getPosition())));
    ...
    if (cachedTags != null && cachedTags.getMood() != null && !cachedTags.getMood().isEmpty()) {
      Set<String> basicFilteredMoods =
          HardcoverMoodFilter.filterMoodsWithCounts(cachedTags.getMood());
      metadata.setMoods(
          basicFilteredMoods.stream()
              .map(WordUtils::capitalizeFully)
              .collect(Collectors.toCollection(LinkedHashSet::new)));
    }

    if (cachedTags != null && cachedTags.getGenre() != null && !cachedTags.getGenre().isEmpty()) {
      Set<String> filteredGenres =
          HardcoverMoodFilter.filterGenresWithCounts(cachedTags.getGenre());
      metadata.setCategories(
          filteredGenres.stream().map(WordUtils::capitalizeFully).collect(Collectors.toSet()));
    }
    ...
    if (cachedTags != null && cachedTags.getTag() != null && !cachedTags.getTag().isEmpty()) {
      ...
      metadata.setTags(
          filteredTags.stream().map(WordUtils::capitalizeFully).collect(Collectors.toSet()));
    }
  ...
  private void mapSeriesInfo(GraphQLResponse.Document doc, BookMetadata metadata) {
    ...
    if (doc.getFeaturedSeries().getPosition() != null) {
      try {
        metadata.setSeriesNumber(
            Float.parseFloat(String.valueOf(doc.getFeaturedSeries().getPosition())));
      } catch (NumberFormatException ignored) {
      }
    }
  }

  private void mapTagsAndMoods(
      GraphQLResponse.Document doc,
      BookMetadata metadata,
      String bookId,
      boolean fetchDetailedMoods) {
    ...
    if ((metadata.getCategories() == null || metadata.getCategories().isEmpty())
        && doc.getGenres() != null
        && !doc.getGenres().isEmpty()) {
      metadata.setCategories(
          doc.getGenres().stream().map(WordUtils::capitalizeFully).collect(Collectors.toSet()));
    }
    ...
    if ((metadata.getTags() == null || metadata.getTags().isEmpty())
        ...
        && !doc.getTags().isEmpty()) {
      metadata.setTags(
          doc.getTags().stream().map(WordUtils::capitalizeFully).collect(Collectors.toSet()));
    }
  }
  ...
  private boolean tryFetchDetailedMoods(String bookId, BookMetadata metadata) {
    try {
      ...
      Set<String> filteredMoods =
          HardcoverMoodFilter.filterMoodsWithCounts(details.getCachedTags());
      if (!filteredMoods.isEmpty()) {
        metadata.setMoods(
            filteredMoods.stream()
                .map(WordUtils::capitalizeFully)
                .collect(Collectors.toCollection(LinkedHashSet::new)));
      }

      Set<String> filteredGenres =
          HardcoverMoodFilter.filterGenresWithCounts(details.getCachedTags());
      if (!filteredGenres.isEmpty()) {
        metadata.setCategories(
            filteredGenres.stream()
                .map(WordUtils::capitalizeFully)
                .collect(Collectors.toCollection(LinkedHashSet::new)));
      }
  ...
  private void mapIsbns(
      GraphQLResponse.Document doc, FetchMetadataRequest request, BookMetadata metadata) {
    ...
    if (StringUtils.isBlank(inputIsbn)) {
      ...
      // If we searched by ISBN but got no exact matches, get response ISBN that most closely
      // matches it
