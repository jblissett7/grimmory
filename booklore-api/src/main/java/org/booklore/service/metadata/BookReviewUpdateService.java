...
public class BookReviewUpdateService {
  ...
  public void addReviewsToBook(List<BookReview> bookReviews, BookMetadataEntity e) {
    BookMetadata tempMetadata = BookMetadata.builder().bookReviews(bookReviews).build();
  ...
  public void updateBookReviews(
    ...
    if (!isFieldUpdateAllowed(false, metadata.getBookReviews())
        || metadata.getBookReviews() == null) {
      return;
    }
  ...
  private void replaceReviewsInEntity(List<BookReview> reviews, BookMetadataEntity entity) {
    entity.getReviews().clear();
    Set<BookReviewEntity> newReviews =
        reviews.stream()
