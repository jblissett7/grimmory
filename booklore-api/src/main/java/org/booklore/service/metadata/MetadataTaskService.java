...
public class MetadataTaskService {
  ...
  public Optional<MetadataTaskDetailsResponse> getTaskWithProposals(String taskId) {
    return metadataFetchTaskRepository.findById(taskId).map(this::buildTaskDetailsResponse);
  }

  private MetadataTaskDetailsResponse buildTaskDetailsResponse(MetadataFetchJobEntity task) {
    List<FetchedProposal> proposals =
        task.getProposals().stream()
            .filter(p -> p.getStatus() == FetchedMetadataProposalStatus.FETCHED)
            .map(fetchedProposalMapper::toDto)
            .toList();

    MetadataFetchTask taskDto =
        MetadataFetchTask.builder()
            .id(task.getTaskId())
  ...
  public boolean deleteTaskAndProposals(String taskId) {
    return metadataFetchTaskRepository
        .findById(taskId)
        .map(
            task -> {
    ...
    return tasks.stream()
        ...
        .map(
            task -> {
              ...
              List<MetadataFetchProposalEntity> remaining =
                  proposals.stream()
              ...
              if (task.getStatus() == MetadataFetchTaskStatus.ERROR) {
                total =
                    task.getTotalBooksCount() != null
                        ? task.getTotalBooksCount()
                        : remaining.size();
                message =
                    String.format(
                        "Metadata fetch failed, processed %d of %d books.", completedCount, total);
                status = "ERROR";
              } else {
                total = remaining.size();
                message =
                    String.format("Metadata fetch completed! %d books need review.", fetchedCount);
                status = "COMPLETED";
                completedCount = (int) acceptedCount;
              }

              return new MetadataBatchProgressNotification(
                  task.getTaskId(), completedCount, total, message, status, true);
            })
