...
public class OidcGroupMappingService {
  ...
  public OidcGroupMapping create(OidcGroupMapping dto) {
    ...
    auditService.log(
        AuditAction.OIDC_GROUP_MAPPING_CREATED,
        "Created OIDC group mapping: " + saved.getOidcGroupClaim());
    return mapper.toDto(saved);
  }

  public OidcGroupMapping update(Long id, OidcGroupMapping dto) {
    OidcGroupMappingEntity existing =
        repository
            .findById(id)
            .orElseThrow(
                () -> ApiError.GENERIC_NOT_FOUND.createException("OIDC group mapping not found"));
    ...
    auditService.log(
        AuditAction.OIDC_GROUP_MAPPING_UPDATED,
  ...
  public void delete(Long id) {
    ...
    auditService.log(
        AuditAction.OIDC_GROUP_MAPPING_DELETED,
  ...
  public void syncUserGroups(BookLoreUserEntity user, List<String> groups) {
    ...
    log.info(
        "Synced OIDC group permissions for user '{}' (mode: {})", user.getUsername(), syncMode);
  }

  private void applyPermissions(
      UserPermissionsEntity perms, Set<String> permissions, boolean isAdmin, boolean additive) {
    if (!additive) {
      ...
      perms.setPermissionManageMetadataConfig(
          permissions.contains("permissionManageMetadataConfig"));
      ...
      perms.setPermissionManageGlobalPreferences(
          permissions.contains("permissionManageGlobalPreferences"));
      ...
      perms.setPermissionBulkAutoFetchMetadata(
          permissions.contains("permissionBulkAutoFetchMetadata"));
      perms.setPermissionBulkCustomFetchMetadata(
          permissions.contains("permissionBulkCustomFetchMetadata"));
      ...
      perms.setPermissionBulkLockUnlockMetadata(
          permissions.contains("permissionBulkLockUnlockMetadata"));
      perms.setPermissionBulkResetBookloreReadProgress(
          permissions.contains("permissionBulkResetBookloreReadProgress"));
      perms.setPermissionBulkResetKoReaderReadProgress(
          permissions.contains("permissionBulkResetKoReaderReadProgress"));
      perms.setPermissionBulkResetBookReadStatus(
          permissions.contains("permissionBulkResetBookReadStatus"));
    } else {
      ...
      if (permissions.contains("permissionManageMetadataConfig"))
      ...
      if (permissions.contains("permissionAccessLibraryStats"))
        perms.setPermissionAccessLibraryStats(true);
      if (permissions.contains("permissionAccessUserStats"))
        perms.setPermissionAccessUserStats(true);
      if (permissions.contains("permissionAccessTaskManager"))
        perms.setPermissionAccessTaskManager(true);
      if (permissions.contains("permissionManageGlobalPreferences"))
      ...
      if (permissions.contains("permissionBulkAutoFetchMetadata"))
        perms.setPermissionBulkAutoFetchMetadata(true);
      if (permissions.contains("permissionBulkCustomFetchMetadata"))
        perms.setPermissionBulkCustomFetchMetadata(true);
      if (permissions.contains("permissionBulkEditMetadata"))
        perms.setPermissionBulkEditMetadata(true);
      if (permissions.contains("permissionBulkRegenerateCover"))
        perms.setPermissionBulkRegenerateCover(true);
      if (permissions.contains("permissionMoveOrganizeFiles"))
        perms.setPermissionMoveOrganizeFiles(true);
      if (permissions.contains("permissionBulkLockUnlockMetadata"))
        perms.setPermissionBulkLockUnlockMetadata(true);
      if (permissions.contains("permissionBulkResetBookloreReadProgress"))
        perms.setPermissionBulkResetBookloreReadProgress(true);
      if (permissions.contains("permissionBulkResetKoReaderReadProgress"))
        perms.setPermissionBulkResetKoReaderReadProgress(true);
      if (permissions.contains("permissionBulkResetBookReadStatus"))
