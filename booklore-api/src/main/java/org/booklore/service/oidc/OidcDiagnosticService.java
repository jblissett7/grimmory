  ...
  public OidcTestResult testConnection(OidcProviderDetails providerDetails) {
    ...
    try {
      ...
      if (doc == null) {
        checks.add(
            new OidcTestCheck(
                "Discovery Document", CheckStatus.FAIL, "Empty response from discovery endpoint"));
        return new OidcTestResult(false, checks);
      }
      checks.add(
          new OidcTestCheck(
              "Discovery Document", CheckStatus.PASS, "Successfully fetched from " + discoveryUrl));
    } catch (Exception e) {
      checks.add(
          new OidcTestCheck(
              "Discovery Document", CheckStatus.FAIL, "Failed to fetch: " + e.getMessage()));
      return new OidcTestResult(false, checks);
    }
    ...
    if (authEndpoint != null && !authEndpoint.isBlank()) {
      ...
      checks.add(
          new OidcTestCheck(
              "Authorization Endpoint", CheckStatus.FAIL, "Not found in discovery document"));
      hasFailure = true;
    }
    ...
    if (tokenEndpoint != null && !tokenEndpoint.isBlank()) {
      ...
      checks.add(
          new OidcTestCheck("Token Endpoint", CheckStatus.FAIL, "Not found in discovery document"));
      hasFailure = true;
    }
    ...
    if (jwksUri != null && !jwksUri.isBlank()) {
      ...
      checks.add(
          new OidcTestCheck("JWKS URI", CheckStatus.FAIL, "Not found in discovery document"));
      hasFailure = true;
    }
    ...
    if (jwksUri != null && !jwksUri.isBlank()) {
      try {
        ...
        checks.add(
            new OidcTestCheck(
                "JWKS Keys", CheckStatus.FAIL, "Failed to fetch JWKS: " + e.getMessage()));
        hasFailure = true;
      }
    } else {
    ...
    if (scopes != null) {
      ...
      if (missing.isEmpty()) {
        checks.add(
            new OidcTestCheck(
                "Required Scopes", CheckStatus.PASS, "openid, profile, email all supported"));
      } else {
        checks.add(
            new OidcTestCheck(
                "Required Scopes",
      ...
      checks.add(
          new OidcTestCheck(
              "Required Scopes",
              CheckStatus.WARN,
              "scopes_supported not listed in discovery document"));
    }
    ...
    if (responseTypes != null && responseTypes.contains("code")) {
      checks.add(
          new OidcTestCheck(
              "Response Type 'code'", CheckStatus.PASS, "Authorization code flow supported"));
    } else if (responseTypes != null) {
      checks.add(
          new OidcTestCheck(
              "Response Type 'code'", CheckStatus.FAIL, "Authorization code flow not supported"));
      hasFailure = true;
    } else {
      checks.add(
          new OidcTestCheck(
              "Response Type 'code'", CheckStatus.WARN, "response_types_supported not listed"));
    }
    ...
    if (codeChallengeMethodsSupported != null && codeChallengeMethodsSupported.contains("S256")) {
      checks.add(
          new OidcTestCheck(
              "PKCE (S256)", CheckStatus.PASS, "S256 code challenge method supported"));
    } else if (codeChallengeMethodsSupported != null) {
      checks.add(
          new OidcTestCheck(
              "PKCE (S256)",
              CheckStatus.WARN,
              "S256 not listed, available: " + String.join(", ", codeChallengeMethodsSupported)));
    } else {
      checks.add(
          new OidcTestCheck(
              "PKCE (S256)",
              CheckStatus.WARN,
              "code_challenge_methods_supported not listed (PKCE may still work)"));
    }
    ...
    if (endSessionEndpoint != null && !endSessionEndpoint.isBlank()) {
      ...
      checks.add(
          new OidcTestCheck(
              ...
              CheckStatus.WARN,
              "Not available (RP-initiated logout won't work)"));
    }

    Object backchannelLogout = doc.get("backchannel_logout_supported");
    if (Boolean.TRUE.equals(backchannelLogout)) {
      checks.add(
          new OidcTestCheck("Back-Channel Logout", CheckStatus.PASS, "Supported by provider"));
    } else {
      checks.add(
          new OidcTestCheck(
              "Back-Channel Logout", CheckStatus.WARN, "Not supported or not advertised"));
    }
