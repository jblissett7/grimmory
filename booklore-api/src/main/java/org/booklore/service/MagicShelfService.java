...
public class MagicShelfService {
  ...
  private List<MagicShelf> getShelvesForUser(Long userId) {
    ...
    List<MagicShelf> publicShelves =
        magicShelfRepository.findAllByIsPublicIsTrue().stream()
            .map(this::toDto)
  ...
  public MagicShelf createOrUpdateShelf(MagicShelf dto) {
    ...
    if (dto.getId() != null) {
      ...
      if (existing.isPublic()
          && !authenticationService.getAuthenticatedUser().getPermissions().isAdmin()) {
      ...
      auditService.log(
          AuditAction.MAGIC_SHELF_UPDATED,
          "MagicShelf",
          dto.getId(),
          "Updated magic shelf: " + dto.getName());
      return result;
    }
    if (magicShelfRepository.existsByUserIdAndName(userId, dto.getName())) {
      throw new IllegalArgumentException(
          "A shelf with the same name already exists for this user.");
    }
    MagicShelf result = toDto(magicShelfRepository.save(toEntity(dto, userId)));
    auditService.log(
        AuditAction.MAGIC_SHELF_CREATED,
        "MagicShelf",
        result.getId(),
        "Created magic shelf: " + dto.getName());
    return result;
  }
  ...
  public void deleteShelf(Long id) {
    ...
    auditService.log(
        AuditAction.MAGIC_SHELF_DELETED, "MagicShelf", id, "Deleted magic shelf: " + shelfName);
  }
