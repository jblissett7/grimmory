...
public class LibraryWatchService {
  ...
  public LibraryWatchService(LibraryFileEventProcessor eventProcessor) throws IOException {
    ...
    this.registrationExecutor =
        Executors.newSingleThreadExecutor(Thread.ofVirtual().name("watch-registrar").factory());
  }
          ...
          if (isDir && kind == StandardWatchEventKinds.ENTRY_CREATE) {
            ...
            if (parentEntry != null) {
              registrationExecutor.submit(
                  () -> registerRecursive(fullPath, parentEntry.libraryId()));
            }
          }
  ...
  private void handleOverflow(Path directory) {
    ...
    if (entry != null) {
      log.warn(
          "OVERFLOW for directory: {}. Queuing rescan for library {}.",
          directory,
          entry.libraryId());
      eventProcessor.processEvent(
          StandardWatchEventKinds.ENTRY_CREATE, entry.libraryId(), directory, true);
    } else {
            ...
            libraryPath -> {
              ...
              if (Files.isDirectory(rootPath)) {
                try (Stream<Path> pathStream = Files.walk(rootPath)) {
                  ...
                  log.error(
                      "Failed to register paths for library '{}': {}",
                      library.getName(),
                      e.getMessage(),
                      e);
                }
  ...
  public void unregisterLibrary(Long libraryId) {
    Set<Path> pathsToRemove =
        watches.entrySet().stream()
  ...
  private void registerRecursive(Path root, long libraryId) {
    try (Stream<Path> stream = Files.walk(root)) {
      stream.filter(Files::isDirectory).forEach(p -> registerPath(p, libraryId));
    } catch (IOException e) {
  ...
  private void unregisterRecursive(Path root) {
    Set<Path> toRemove =
        watches.keySet().stream().filter(p -> p.startsWith(root)).collect(Collectors.toSet());
  ...
  public void registerLibraryPaths(long libraryId, Path libraryRoot) {
    ...
    try {
      ...
      log.error(
          "Failed to register library paths for libraryId {} at {}", libraryId, libraryRoot, e);
    }
  }
