...
public class EmailProviderV2Service {
  ...
  public List<EmailProviderV2> getEmailProviders() {
    ...
    return userProviders.stream().map(entity -> mapper.toDTO(entity, defaultProviderId)).toList();
  }

  public EmailProviderV2 getEmailProvider(Long id) {
    BookLoreUser user = authService.getAuthenticatedUser();
    EmailProviderV2Entity entity =
        repository
            .findAccessibleProvider(id, user.getId())
  ...
  public EmailProviderV2 createEmailProvider(CreateEmailProviderRequest request) {
    ...
    Long defaultProviderId = getDefaultProviderIdForUser(user.getId());
    auditService.log(
        AuditAction.EMAIL_PROVIDER_CREATED,
        "EmailProvider",
        savedEntity.getId(),
        "Created email provider: " + savedEntity.getHost() + ":" + savedEntity.getPort());
  ...
  public EmailProviderV2 updateEmailProvider(Long id, CreateEmailProviderRequest request) {
    BookLoreUser user = authService.getAuthenticatedUser();
    EmailProviderV2Entity existingProvider =
        repository
    ...
    auditService.log(
        AuditAction.EMAIL_PROVIDER_UPDATED,
        "EmailProvider",
        id,
        "Updated email provider: " + updatedEntity.getHost() + ":" + updatedEntity.getPort());
  ...
  public void deleteEmailProvider(Long id) {
    ...
    for (UserEmailProviderPreferenceEntity preference : preferencesUsingProvider) {
      List<EmailProviderV2Entity> availableProviders =
          getAccessibleProvidersForUser(preference.getUserId());
      availableProviders.removeIf(p -> p.getId().equals(id));

      if (!availableProviders.isEmpty()) {
        EmailProviderV2Entity newDefault =
            availableProviders.get(ThreadLocalRandom.current().nextInt(availableProviders.size()));
    ...
    auditService.log(
        AuditAction.EMAIL_PROVIDER_DELETED, "EmailProvider", id, "Deleted email provider");
  }
  ...
  private void setDefaultProviderForUser(Long userId, Long providerId) {
    UserEmailProviderPreferenceEntity preference =
        preferenceRepository
            .findByUserId(userId)
            .orElse(UserEmailProviderPreferenceEntity.builder().userId(userId).build());
