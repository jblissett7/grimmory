...
public class BookRecommendationService {
  ...
  public List<BookRecommendation> getRecommendations(Long bookId, int limit) {
    ...
    if (recommendations == null || recommendations.isEmpty()) {
      log.info(
          "Recommendations for book ID {} are missing or empty. Computing similarity...", bookId);
      recommendations = findSimilarBookIds(bookId, limit);
      book.setSimilarBooksJson(recommendations);
      bookRepository.save(book);
    }

    Set<Long> recommendedBookIds =
        recommendations.stream().map(BookRecommendationLite::getB).collect(Collectors.toSet());
    ...
    Map<Long, BookEntity> recommendedBooksMap =
        bookQueryService.findAllWithMetadataByIds(recommendedBookIds).stream()
            .filter(
                b -> {
                  if (accessibleLibraryIds == null) {
                    return true;
                  }
                  return b.getLibrary() != null
                      && accessibleLibraryIds.contains(b.getLibrary().getId());
                })
        ...
        .map(
            rec -> {
              ...
              return new BookRecommendation(
                  bookMapper.toBookWithDescription(bookEntity, false), rec.getS());
            })
  ...
  protected List<BookRecommendation> findSimilarBooks(Long bookId, int limit) {
    ...
    List<SimpleEntry<BookEntity, Double>> scored =
        candidates.stream()
            ...
            .map(
                candidate ->
                    new SimpleEntry<>(
                        candidate, similarityService.calculateSimilarity(target, candidate)))
