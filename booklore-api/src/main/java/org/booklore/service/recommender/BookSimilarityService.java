...
public class BookSimilarityService {
  ...
  private static final Pattern NON_ALPHANUMERIC_EXCEPT_SPACE_PATTERN =
  ...
  public double calculateSimilarity(BookEntity a, BookEntity b) {
    ...
    score +=
        SimilarityWeight.AUTHORS.getWeight()
            * jaccardSimilarity(extractNames(metaA.getAuthors()), extractNames(metaB.getAuthors()));

    score +=
        SimilarityWeight.CATEGORIES.getWeight()
            * jaccardSimilarity(
                extractNames(metaA.getCategories()), extractNames(metaB.getCategories()));

    score +=
        SimilarityWeight.TITLE.getWeight()
    ...
    score +=
        SimilarityWeight.RATING.getWeight()
  ...
  private Map<String, Integer> tokenize(String text) {
    ...
    String[] tokens =
        WHITESPACE_PATTERN.split(
            NON_ALPHANUMERIC_EXCEPT_SPACE_PATTERN.matcher(text.toLowerCase()).replaceAll(""));
