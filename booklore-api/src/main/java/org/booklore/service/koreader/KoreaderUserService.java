...
public class KoreaderUserService {
  ...
  public KoreaderUser upsertUser(String username, String rawPassword) {
    ...
    log.info(
        "upsertUser: {} KoreaderUser [id={}, username='{}'] for BookLoreUser='{}'",
  ...
  public KoreaderUser getUser() {
    ...
    KoreaderUserEntity user =
        koreaderUserRepository
            .findByBookLoreUserId(id)
            .orElseThrow(
                () ->
                    ApiError.GENERIC_NOT_FOUND.createException(
                        "Koreader user not found for BookLore user ID: " + id));
