...
public class KoreaderService {
  ...
  public KoreaderProgress getProgress(String bookHash) {
    ...
    UserBookProgressEntity progress =
        findUserProgress(authDetails.getBookLoreUserId(), book.getId());

    log.info(
        "getProgress: fetched progress='{}' percentage={} for userId={} bookHash={}",
        progress.getKoreaderProgress(),
        progress.getKoreaderProgressPercent(),
        authDetails.getBookLoreUserId(),
        bookHash);
  ...
  public void saveProgress(String bookHash, KoreaderProgress koProgress) {
    ...
    log.info(
        "saveProgress: saved progress='{}' percentage={} for userId={} bookHash={}",
        koProgress.getProgress(),
        koProgress.getPercentage(),
        authDetails.getBookLoreUserId(),
        bookHash);
    ...
    // changed
    if (koProgress.getPercentage() != null
        && (!koProgress.getPercentage().equals(previousProgressPercent)
      ...
      hardcoverSyncService.syncProgressToHardcover(
          book.getId(), progressPercent, authDetails.getBookLoreUserId());
    }
  }

  private void saveToFileProgress(
      BookLoreUserEntity user, BookEntity book, UserBookProgressEntity progress) {
    try {
  ...
  private void updateProgressData(
    ...
    if (syncWithBookloreReader && koProgress.getProgress() != null) {
      try {
        String cfi =
            epubCfiService.convertXPointerToCfi(book.getFullFilePath(), koProgress.getProgress());

        float percent = koProgress.getPercentage() * 100f;
        float rounded = BigDecimal.valueOf(percent).setScale(1, RoundingMode.HALF_UP).floatValue();
  ...
  private BookEntity findBookByHash(String bookHash) {
    return bookRepository
        .findByCurrentHash(bookHash)
        .orElseThrow(
            () ->
                ApiError.GENERIC_NOT_FOUND.createException("Book not found for hash " + bookHash));
  }
  ...
  private BookLoreUserEntity findBookLoreUser(long userId) {
    return userRepository
        .findById(userId)
        .orElseThrow(
            () -> ApiError.GENERIC_NOT_FOUND.createException("User not found with id " + userId));
  }
  ...
  private UserBookProgressEntity findUserProgress(long userId, Long bookId) {
    return progressRepository
        ...
        .orElseThrow(
            () ->
                ApiError.GENERIC_NOT_FOUND.createException("No progress found for user and book"));
  }
