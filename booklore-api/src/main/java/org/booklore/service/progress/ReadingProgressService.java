...
public class ReadingProgressService {
  ...
  public Map<Long, UserBookFileProgressEntity> fetchUserFileProgress(
      Long userId, Set<Long> bookIds) {
    if (bookIds.isEmpty()) {
  ...
  public Optional<UserBookFileProgressEntity> fetchUserFileProgressForFile(
      Long userId, Long bookFileId) {
  ...
  public void enrichBookWithProgress(
      Book book, UserBookProgressEntity progress, UserBookFileProgressEntity fileProgress) {
    if (progress != null) {
      book.setReadStatus(
          progress.getReadStatus() == null
              ? String.valueOf(ReadStatus.UNSET)
              : String.valueOf(progress.getReadStatus()));
  ...
  private void setBookProgress(Book book, UserBookProgressEntity progress) {
    ...
    if (progress.getEpubProgress() != null || progress.getEpubProgressPercent() != null) {
      book.setEpubProgress(
          EpubProgress.builder()
  ...
  private void setBookProgressFromFileProgress(Book book, UserBookFileProgressEntity fileProgress) {
    BookFileType type =
        fileProgress.getBookFile() != null ? fileProgress.getBookFile().getBookType() : null;
    if (type == null) return;
      ...
      case CBX ->
          book.setCbxProgress(
              CbxProgress.builder()
                  ...
                  .build());
      case AUDIOBOOK ->
          book.setAudiobookProgress(
              AudiobookProgress.builder()
  ...
  public void updateReadProgress(ReadProgressRequest request) {
    BookEntity book =
        bookRepository
            .findByIdWithBookFiles(request.getBookId())
    ...
    if (hasProgressData) {
      if (request.getFileProgress() != null) {
        ...
        BookFileEntity bookFile =
            bookFileRepository
                .findById(fileProgress.bookFileId())
                .orElseThrow(
                    () -> ApiError.GENERIC_NOT_FOUND.createException("Book file not found"));
  ...
  private void saveToUserBookFileProgress(
      BookLoreUserEntity user, BookFileProgress fileProgress, Instant now) {
  ...
  private void saveToUserBookFileProgressFromLegacy(
    ...
    switch (bookFile.getBookType()) {
      case PDF -> {
        entity.setPositionData(
            progress.getPdfProgress() != null ? String.valueOf(progress.getPdfProgress()) : null);
      ...
      case CBX -> {
        entity.setPositionData(
            progress.getCbxProgress() != null ? String.valueOf(progress.getCbxProgress()) : null);
  ...
  private void updateProgressFromFileProgress(
      UserBookProgressEntity progress, BookFileType bookType, BookFileProgress fileProgress) {
    switch (bookType) {
      case PDF -> {
        progress.setPdfProgress(
            fileProgress.positionData() != null
                ? Integer.parseInt(fileProgress.positionData())
                : null);
  ...
  private Float updateProgressByBookType(
      UserBookProgressEntity progress, BookFileType bookType, ReadProgressRequest request) {
  ...
  private void setProgressPercent(
      UserBookProgressEntity progress, BookFileType type, Float percentage) {
    switch (type) {
  ...
  private void validateResetPermission(
      List<Long> bookIds, BookLoreUser user, ResetProgressType type) {
  ...
  private List<BookStatusUpdateResponse> buildResetResponses(
      List<Long> bookIds, Set<Long> existingBookIds, Instant now) {
