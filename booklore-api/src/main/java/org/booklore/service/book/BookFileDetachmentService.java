...
public class BookFileDetachmentService {
  ...
  public DetachBookFileResponse detachBookFile(Long bookId, Long fileId, boolean copyMetadata) {
    ...
    BookFileEntity targetFile =
        sourceBook.getBookFiles().stream()
            .filter(f -> f.getId().equals(fileId))
            .findFirst()
            .orElseThrow(
                () -> ApiError.GENERIC_BAD_REQUEST.createException("File not found on this book"));
    ...
    if (targetFile.isBookFormat()) {
      long bookFormatCount =
          sourceBook.getBookFiles().stream().filter(BookFileEntity::isBookFormat).count();
      if (bookFormatCount == 1) {
        boolean hasSupplementaryOnly =
            sourceBook.getBookFiles().stream().anyMatch(f -> !f.isBookFormat());
    ...
    try {
      ...
      log.warn(
          "Failed to generate cover for detached book {}: {}", newBook.getId(), e.getMessage());
    }

    auditService.log(
        AuditAction.BOOK_FILE_DETACHED,
        "Book",
        bookId,
        "Detached file '"
