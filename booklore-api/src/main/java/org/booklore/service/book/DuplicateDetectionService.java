...
public class DuplicateDetectionService {
  ...
  private static final List<BookFileType> DEFAULT_FORMAT_PRIORITY =
      List.of(
          BookFileType.EPUB,
          BookFileType.PDF,
          BookFileType.AZW3,
  ...
  public List<DuplicateGroup> findDuplicates(DuplicateDetectionRequest request) {
    ...
    List<BookFileType> formatPriority =
        books.getFirst().getLibrary() != null
  ...
  private List<DuplicateGroup> findByIsbn(
      List<BookEntity> books, Set<Long> alreadyGrouped, List<BookFileType> formatPriority) {
  ...
  private List<DuplicateGroup> findByExternalId(
      List<BookEntity> books, Set<Long> alreadyGrouped, List<BookFileType> formatPriority) {
  ...
  private List<DuplicateGroup> findByTitleAuthor(
      List<BookEntity> books, Set<Long> alreadyGrouped, List<BookFileType> formatPriority) {
    ...
    for (List<BookEntity> group : titleGroups.values()) {
      ...
      List<BookEntity> withAuthors =
          group.stream()
              .filter(
                  b ->
                      b.getMetadata().getAuthors() != null
                          && !b.getMetadata().getAuthors().isEmpty())
              .toList();
  ...
  private List<DuplicateGroup> findByDirectory(
      List<BookEntity> books, Set<Long> alreadyGrouped, List<BookFileType> formatPriority) {
    ...
    for (BookEntity book : books) {
      ...
      BookFileEntity primary =
          bookFiles.stream().filter(BookFileEntity::isBookFormat).findFirst().orElse(null);
  ...
  private List<DuplicateGroup> findByFilename(
      List<BookEntity> books, Set<Long> alreadyGrouped, List<BookFileType> formatPriority) {
  ...
  private List<DuplicateGroup> buildGroups(
      Map<String, List<BookEntity>> groupMap,
      String reason,
      Set<Long> alreadyGrouped,
      List<BookFileType> formatPriority) {
    List<DuplicateGroup> result = new ArrayList<>();
  ...
  private DuplicateGroup toDuplicateGroup(
      List<BookEntity> entities, String reason, List<BookFileType> formatPriority) {
    BookEntity suggested =
        entities.stream()
            .max(
                Comparator.comparingInt((BookEntity b) -> formatPriorityScore(b, formatPriority))
                    .thenComparingLong(
                        b ->
                            b.getBookFiles() == null
                                ? 0
                                : b.getBookFiles().stream()
                                    .filter(BookFileEntity::isBookFormat)
                                    .count())
                    .thenComparing(
                        b -> b.getMetadataMatchScore() != null ? b.getMetadataMatchScore() : 0f)
                    .thenComparing(BookEntity::getId, Comparator.reverseOrder()))
            .orElse(entities.getFirst());

    List<Book> books = entities.stream().map(bookMapper::toBook).toList();
