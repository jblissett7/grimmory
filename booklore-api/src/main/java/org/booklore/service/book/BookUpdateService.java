...
public class BookUpdateService {
  ...
  public void updateBookViewerSetting(long bookId, BookViewerSettings bookViewerSettings) {
    ...
    switch (primaryFile.getBookType()) {
      ...
      case EPUB, FB2, MOBI, AZW3 ->
          updateEbookViewerSettings(bookId, user.getId(), bookViewerSettings);
  ...
  public List<BookStatusUpdateResponse> updateReadStatus(List<Long> bookIds, String status) {
    ...
    createNewProgress(
        user.getId(), bookIds, existingProgressBookIds, readStatus, now, dateFinished);

    return buildStatusUpdateResponses(bookIds, readStatus, now, dateFinished);
  }

  @Transactional
  public List<PersonalRatingUpdateResponse> updatePersonalRating(
      List<Long> bookIds, Integer rating) {
    BookLoreUser user = authenticationService.getAuthenticatedUser();
    Set<Long> existingProgressBookIds = validateBooksAndGetExistingProgress(user.getId(), bookIds);

    if (!existingProgressBookIds.isEmpty()) {
      userBookProgressRepository.bulkUpdatePersonalRating(
          user.getId(), new ArrayList<>(existingProgressBookIds), rating);
    }
  ...
  public List<PersonalRatingUpdateResponse> resetPersonalRating(List<Long> bookIds) {
    ...
    if (!existingProgressBookIds.isEmpty()) {
      userBookProgressRepository.bulkUpdatePersonalRating(
          user.getId(), new ArrayList<>(existingProgressBookIds), null);
    }
  ...
  @Transactional
  public List<Book> assignShelvesToBooks(
      Set<Long> bookIds, Set<Long> shelfIdsToAssign, Set<Long> shelfIdsToUnassign) {
    BookLoreUser user = authenticationService.getAuthenticatedUser();
  ...
  private PdfViewerPreferencesEntity findOrCreatePdfPreferences(long bookId, Long userId) {
    return pdfViewerPreferencesRepository
        ...
        .orElseGet(
            () ->
                pdfViewerPreferencesRepository.save(
                    PdfViewerPreferencesEntity.builder().bookId(bookId).userId(userId).build()));
  }
  ...
  private NewPdfViewerPreferencesEntity findOrCreateNewPdfPreferences(long bookId, Long userId) {
    return newPdfViewerPreferencesRepository
        ...
        .orElseGet(
            () ->
                newPdfViewerPreferencesRepository.save(
                    NewPdfViewerPreferencesEntity.builder().bookId(bookId).userId(userId).build()));
  }
  ...
  private EbookViewerPreferenceEntity findOrCreateEbookPreferences(long bookId, Long userId) {
    return ebookViewerPreferenceRepository
        ...
        .orElseGet(
            () ->
                ebookViewerPreferenceRepository.save(
                    EbookViewerPreferenceEntity.builder().bookId(bookId).userId(userId).build()));
  }
  ...
  private CbxViewerPreferencesEntity findOrCreateCbxPreferences(long bookId, Long userId) {
    return cbxViewerPreferencesRepository
        ...
        .orElseGet(
            () ->
                cbxViewerPreferencesRepository.save(
                    CbxViewerPreferencesEntity.builder().bookId(bookId).userId(userId).build()));
  }

  private void updateExistingProgress(
      Long userId, Set<Long> bookIds, ReadStatus status, Instant now, Instant dateFinished) {
    if (!bookIds.isEmpty()) {
      userBookProgressRepository.bulkUpdateReadStatus(
          userId, new ArrayList<>(bookIds), status, now, dateFinished);
    }
  }

  private void createNewProgress(
      Long userId,
      List<Long> allBookIds,
      Set<Long> existingBookIds,
      ReadStatus status,
      Instant now,
      Instant dateFinished) {
    Set<Long> newProgressBookIds =
        allBookIds.stream().filter(id -> !existingBookIds.contains(id)).collect(Collectors.toSet());

    if (newProgressBookIds.isEmpty()) return;

    BookLoreUserEntity userEntity = findUserOrThrow(userId);
    List<UserBookProgressEntity> newProgressEntities =
        newProgressBookIds.stream()
  ...
  private UserBookProgressEntity createProgressEntity(
      BookLoreUserEntity user, Long bookId, ReadStatus status, Instant now, Instant dateFinished) {
  ...
  private void createProgressForRating(
      Long userId, List<Long> allBookIds, Set<Long> existingBookIds, Integer rating) {
    Set<Long> newProgressBookIds =
        allBookIds.stream().filter(id -> !existingBookIds.contains(id)).collect(Collectors.toSet());
  ...
  private UserBookProgressEntity createProgressEntityWithRating(
      BookLoreUserEntity user, Long bookId, Integer rating) {
    UserBookProgressEntity progress = new UserBookProgressEntity();
    progress.setUser(user);
  ...
  private void validateShelfOwnership(
      BookLoreUserEntity user, Set<Long> shelfIdsToAssign, Set<Long> shelfIdsToUnassign) {
    Set<Long> userShelfIds =
        user.getShelves().stream().map(ShelfEntity::getId).collect(Collectors.toSet());

    if (!userShelfIds.containsAll(shelfIdsToAssign)) {
      throw ApiError.GENERIC_UNAUTHORIZED.createException(
          "Cannot assign shelves that do not belong to the user.");
    }
    if (!userShelfIds.containsAll(shelfIdsToUnassign)) {
      throw ApiError.GENERIC_UNAUTHORIZED.createException(
          "Cannot unassign shelves that do not belong to the user.");
    }
  }

  private void updateBookShelves(
      List<BookEntity> books, List<ShelfEntity> shelvesToAssign, Set<Long> shelfIdsToUnassign) {
  ...
  private Book buildBook(
    ...
    readingProgressService.enrichBookWithProgress(
        book, progressMap.get(bookEntity.getId()), fileProgressMap.get(bookEntity.getId()));
    return book;
  }
  ...
  private void validateBulkOperationPermission(
      List<Long> bookIds, BookLoreUser user, UserPermission permission) {
  ...
  private List<BookStatusUpdateResponse> buildStatusUpdateResponses(
      List<Long> bookIds, ReadStatus status, Instant now, Instant dateFinished) {
    return bookIds.stream()
        .map(
            bookId ->
                BookStatusUpdateResponse.builder()
                    .bookId(bookId)
                    .readStatus(status)
  ...
  private List<PersonalRatingUpdateResponse> buildRatingUpdateResponses(
      List<Long> bookIds, Integer rating) {
    return bookIds.stream()
        .map(
            bookId ->
                PersonalRatingUpdateResponse.builder()
                    .bookId(bookId)
