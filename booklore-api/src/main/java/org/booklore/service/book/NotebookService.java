...
public class NotebookService {
  ...
  public Page<NotebookEntry> getNotebookEntries(
      int page, int size, Set<String> types, Long bookId, String search, String sort) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    Pageable pageable = PageRequest.of(page, size, toSort(sort));
    return repository
        .findEntries(userId, types, bookId, wrapSearch(search), pageable)
        .map(NotebookService::toDto);
  }

  @Transactional(readOnly = true)
  public List<NotebookEntry> getAllNotebookEntries(
      Set<String> types, Long bookId, String search, String sort) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
  ...
  public List<NotebookBookOption> getBooksWithAnnotations(String search) {
    Long userId = authenticationService.getAuthenticatedUser().getId();
    return repository
        .findBooksWithAnnotations(userId, wrapSearch(search), Pageable.ofSize(BOOK_OPTIONS_LIMIT))
        .stream()
