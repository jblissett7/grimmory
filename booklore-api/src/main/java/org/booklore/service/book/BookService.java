...
public class BookService {
  ...
  public List<Book> getBookDTOs(boolean includeDescription) {
    ...
    List<Book> books =
        isAdmin
            ...
            : bookQueryService.getAllBooksByLibraryIds(
                getUserLibraryIds(user), includeDescription, user.getId());
    ...
    books.forEach(
        book -> {
          ...
          book.setShelves(
              !includeDescription && filtered != null && filtered.isEmpty() ? null : filtered);
        });
  ...
  public BookViewerSettings getBookViewerSetting(long bookId, long bookFileId) {
    ...
    BookFileEntity bookFile =
        bookEntity.getBookFiles().stream()
            ...
            .orElseThrow(
                () ->
                    ApiError.FILE_NOT_FOUND.createException("Book file not found: " + bookFileId));
    ...
    if (bookType == BookFileType.EPUB
      ...
      ebookViewerPreferencesRepository
          .findByBookIdAndUserId(bookId, user.getId())
          .ifPresent(
              epubPref ->
                  settingsBuilder.ebookSettings(
      ...
      newPdfViewerPreferencesRepository
          ...
          .ifPresent(
              pdfPref ->
                  settingsBuilder.newPdfSettings(
  ...
  public Resource getBookCover(String coverHash) {
    BookEntity bookEntity =
        bookRepository
            .findByBookCoverHash(coverHash)
            .orElseThrow(() -> ApiError.BOOK_NOT_FOUND.createException(coverHash));
  ...
  public void streamBookContent(
      long bookId, String bookType, HttpServletRequest request, HttpServletResponse response)
      throws IOException {
    BookEntity bookEntity =
        bookRepository
            .findById(bookId)
        ...
        try {
          if (Files.exists(fullFilePath)) {
            try {
              ...
              log.warn(
                  "Failed to unregister monitoring for path: {}", fullFilePath.getParent(), ex);
            }
          ...
          monitoringRegistrationService.registerSpecificPath(
              fullFilePath.getParent(), book.getLibrary().getId());
        }
      }
    }
