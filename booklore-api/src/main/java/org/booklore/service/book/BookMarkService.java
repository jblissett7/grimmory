...
public class BookMarkService {
  ...
  public List<BookMark> getBookmarksForBook(Long bookId) {
    ...
    return bookMarkRepository
        .findByBookIdAndUserIdOrderByPriorityAscCreatedAtDesc(bookId, userId)
        .stream()
        .map(mapper::toDto)
        .toList();
  }
  ...
  public BookMark createBookmark(CreateBookMarkRequest request) {
    ...
    if (request.isAudiobookBookmark()) {
      validateNoDuplicateAudiobookBookmark(
          request.getPositionMs(), request.getTrackIndex(), request.getBookId(), userId);
  ...
  public BookMark updateBookmark(Long bookmarkId, UpdateBookMarkRequest request) {
    ...
    if (request.getCfi() != null) {
      validateNoDuplicateBookmark(
          request.getCfi(), bookmark.getBookId(), bookmark.getUserId(), bookmarkId);
    }
  ...
  /**
   * Priority: 1 (highest/most important) to 5 (lowest/least important). Bookmarks are sorted by
   * priority ascending (1 first), then by creation date descending.
   */
  private void validateNoDuplicateBookmark(
      String cfi, Long bookId, Long userId, Long excludeBookmarkId) {
    boolean exists =
        (excludeBookmarkId == null)
            ? bookMarkRepository.existsByCfiAndBookIdAndUserId(cfi, bookId, userId)
            : bookMarkRepository.existsByCfiAndBookIdAndUserIdExcludeId(
                cfi, bookId, userId, excludeBookmarkId);
  ...
  private void validateNoDuplicateAudiobookBookmark(
      Long positionMs, Integer trackIndex, Long bookId, Long userId) {
    boolean exists =
        bookMarkRepository.existsByPositionMsNearAndBookIdAndUserId(
            positionMs, trackIndex, bookId, userId);
    if (exists) {
