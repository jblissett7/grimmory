...
public class BookCreatorService {
  ...
  public BookEntity createShellBook(LibraryFile libraryFile, BookFileType bookFileType) {
    Optional<BookEntity> existingBookOpt =
        bookRepository.findByLibraryIdAndLibraryPathIdAndFileSubPathAndFileName(
    ...
    BookMetadataEntity metadata = BookMetadataEntity.builder().book(bookEntity).build();
  ...
  public void addCategoriesToBook(Set<String> categories, BookEntity bookEntity) {
    ...
    categories.stream()
        ...
        .map(
            truncated ->
                categoryRepository
                    .findByName(truncated)
                    .orElseGet(
                        () ->
                            categoryRepository.save(
                                CategoryEntity.builder().name(truncated).build())))
  ...
  public void addAuthorsToBook(Collection<String> authors, BookEntity bookEntity) {
    ...
    bookEntity
        .getMetadata()
        .updateSearchText(); // Manually trigger search text update since collection modification
    // doesn't trigger @PreUpdate
  }
        ...
        .map(
            truncated ->
                moodRepository
                    .findByName(truncated)
                    .orElseGet(
                        () -> moodRepository.save(MoodEntity.builder().name(truncated).build())))
        ...
        .map(
            truncated ->
                tagRepository
                    .findByName(truncated)
                    .orElseGet(
                        () -> tagRepository.save(TagEntity.builder().name(truncated).build())))
          ...
          .map(
              name ->
                  comicTeamRepository
                      ...
                      .orElseGet(
                          () ->
                              comicTeamRepository.save(
                                  ComicTeamEntity.builder().name(name).build())))
    ...
    for (String name : names) {
      ...
      ComicCreatorEntity creator =
          comicCreatorRepository
              .findByName(truncatedName)
              .orElseGet(
                  () ->
                      comicCreatorRepository.save(
                          ComicCreatorEntity.builder().name(truncatedName).build()));
