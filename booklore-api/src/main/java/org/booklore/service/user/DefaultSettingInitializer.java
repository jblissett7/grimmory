...
public class DefaultSettingInitializer {
  ...
  private static final Set<Long> initializedUsers =
  ...
  private void addSettingIfMissing(BookLoreUserEntity user, UserSettingKey key, Object value) {
    boolean exists =
        user.getSettings().stream().anyMatch(s -> s.getSettingKey().equals(key.getDbKey()));
    if (!exists) {
      try {
        String serializedValue =
            key.isJson() ? objectMapper.writeValueAsString(value) : value.toString();
              ...
              try {
                var current =
                    objectMapper.readValue(
                        setting.getSettingValue(), BookLoreUser.UserSettings.PerBookSetting.class);
                if (current.getCbx() == null) {
                  current.setCbx(
                      BookLoreUser.UserSettings.PerBookSetting.GlobalOrIndividual.Individual);
