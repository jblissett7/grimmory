...
public class UserService {
  ...
  public BookLoreUser updateUser(Long id, UserUpdateRequest updateRequest) {
    BookLoreUserEntity user =
        userRepository.findById(id).orElseThrow(() -> ApiError.USER_NOT_FOUND.createException(id));
    ...
    if (updateRequest.getPermissions() != null && getMyself().getPermissions().isAdmin()) {
      ...
      auditService.log(
          ...
          "User",
          id,
          "Changed permissions for user: " + user.getUsername());
    }
  ...
  public void deleteUser(Long id) {
    BookLoreUserEntity userToDelete =
        userRepository.findById(id).orElseThrow(() -> ApiError.USER_NOT_FOUND.createException(id));
    ...
    auditService.log(
        AuditAction.USER_DELETED, "User", id, "Deleted user: " + userToDelete.getUsername());
  }
  ...
  public void changePassword(ChangePasswordRequest changePasswordRequest) {
    ...
    BookLoreUserEntity bookLoreUserEntity =
        userRepository
            .findById(bookLoreUser.getId())
    ...
    if (!passwordEncoder.matches(
        changePasswordRequest.getCurrentPassword(), bookLoreUserEntity.getPasswordHash())) {
      throw ApiError.PASSWORD_INCORRECT.createException();
    }

    if (passwordEncoder.matches(
        changePasswordRequest.getNewPassword(), bookLoreUserEntity.getPasswordHash())) {
    ...
    bookLoreUserEntity.setPasswordHash(
    ...
    auditService.log(
        ...
        "User",
        bookLoreUser.getId(),
        "Password changed by user: " + bookLoreUser.getUsername());
  }
  ...
  public void changeUserPassword(ChangeUserPasswordRequest request) {
    ...
    auditService.log(
        ...
        "User",
        request.getUserId(),
        "Password changed for user: " + userEntity.getUsername());
  }
