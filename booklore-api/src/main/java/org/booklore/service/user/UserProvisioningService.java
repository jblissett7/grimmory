...
public class UserProvisioningService {
  ...
  public void provisionInternalUser(UserCreateRequest request) {
    Optional<BookLoreUserEntity> existingUser =
    ...
    permissions.setPermissionBulkResetBookloreReadProgress(
        request.isPermissionBulkResetBookloreReadProgress());
    permissions.setPermissionBulkResetKoReaderReadProgress(
        request.isPermissionBulkResetKoReaderReadProgress());
  ...
  @Transactional
  public BookLoreUserEntity provisionOidcUser(
      String username,
      String email,
      String name,
      String oidcSubject,
    ...
    if (defaultPermissions != null) {
      ...
      perms.setPermissionManageMetadataConfig(
          defaultPermissions.contains("permissionManageMetadataConfig"));
      perms.setPermissionAccessBookdrop(defaultPermissions.contains("permissionAccessBookdrop"));
      perms.setPermissionAccessLibraryStats(
          defaultPermissions.contains("permissionAccessLibraryStats"));
      perms.setPermissionAccessUserStats(defaultPermissions.contains("permissionAccessUserStats"));
      perms.setPermissionAccessTaskManager(
          defaultPermissions.contains("permissionAccessTaskManager"));
  ...
  /**
   * Create and persist a remote-provisioned user based on incoming headers. This is the preferred
   * (non-deprecated) entry point for remote provisioning.
   */
  @Transactional
  public BookLoreUserEntity provisionRemoteUserFromHeaders(
      String name, String username, String email, String groups) {
    boolean isAdmin = false;
    if (groups != null && appProperties.getRemoteAuth().getAdminGroup() != null) {
      String groupsContent = groups.trim();
      if (groupsContent.length() >= 2
          && groupsContent.charAt(0) == '['
          && groupsContent.charAt(groupsContent.length() - 1) == ']') {
    ...
    if (oidcAutoProvisionDetails != null
        && oidcAutoProvisionDetails.getDefaultPermissions() != null) {
      List<String> defaultPermissions = oidcAutoProvisionDetails.getDefaultPermissions();
      ...
      permissions.setPermissionManageLibrary(
          defaultPermissions.contains("permissionManageLibrary"));
      ...
      permissions.setPermissionManageMetadataConfig(
          defaultPermissions.contains("permissionManageMetadataConfig"));
      permissions.setPermissionAccessBookdrop(
          defaultPermissions.contains("permissionAccessBookdrop"));
      permissions.setPermissionAccessLibraryStats(
          defaultPermissions.contains("permissionAccessLibraryStats"));
      permissions.setPermissionAccessUserStats(
          defaultPermissions.contains("permissionAccessUserStats"));
      permissions.setPermissionAccessTaskManager(
          defaultPermissions.contains("permissionAccessTaskManager"));
      permissions.setPermissionManageGlobalPreferences(
          defaultPermissions.contains("permissionManageGlobalPreferences"));
    ...
    if (isAdmin) {
      ...
      user.setLibraries(new ArrayList<>(libraries));
    } else if (oidcAutoProvisionDetails != null
        && oidcAutoProvisionDetails.getDefaultLibraryIds() != null
        && !oidcAutoProvisionDetails.getDefaultLibraryIds().isEmpty()) {
      List<LibraryEntity> libraries =
          libraryRepository.findAllById(oidcAutoProvisionDetails.getDefaultLibraryIds());
      user.setLibraries(new ArrayList<>(libraries));
    }
  ...
  protected BookLoreUserEntity createUser(BookLoreUserEntity user) {
    ...
    auditService.log(
        AuditAction.USER_CREATED, "User", save.getId(), "Created user: " + save.getUsername());
    return save;
  }
}
