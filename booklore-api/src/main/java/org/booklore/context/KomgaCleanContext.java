...
/**
 * ThreadLocal context to track whether the Komga API "clean" mode is enabled. When clean mode is
 * enabled: - Fields ending with "Lock" are excluded from JSON serialization - Null values are
 * excluded from JSON serialization - Metadata fields (language, summary, etc.) are allowed to be
 * null
 */
