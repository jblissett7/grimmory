...
@Mapper(
    ...
    uses = {
      BookMetadataMapper.class,
      ShelfMapper.class,
      AdditionalFileMapper.class,
      ComicMetadataMapper.class
    },
...
public interface BookMapper {
  ...
  @Mapping(
      source = "bookFiles",
      target = "alternativeFormats",
      qualifiedByName = "mapAlternativeFormats")
  @Mapping(
      source = "bookFiles",
      target = "supplementaryFiles",
      qualifiedByName = "mapSupplementaryFiles")
  ...
  default List<String> mapAuthors(List<AuthorEntity> authors) {
    ...
    return authors.stream().map(AuthorEntity::getName).toList();
  }

  default Set<String> mapCategories(Set<CategoryEntity> categories) {
    if (categories == null) return null;
    return categories.stream().map(CategoryEntity::getName).collect(Collectors.toSet());
  }

  default Set<String> mapMoods(Set<MoodEntity> moods) {
    if (moods == null) return null;
    return moods.stream().map(MoodEntity::getName).collect(Collectors.toSet());
  }
  ...
  default LibraryPath mapLibraryPathIdOnly(LibraryPathEntity entity) {
    ...
    return LibraryPath.builder().id(entity.getId()).build();
  }
  ...
  default List<BookFile> mapSupplementaryFiles(List<BookFileEntity> bookFiles) {
    if (bookFiles == null) return null;
    return bookFiles.stream().filter(bf -> !bf.isBook()).map(this::toBookFile).toList();
  }

  default BookFileEntity getPrimaryBookFile(List<BookFileEntity> bookFiles) {
    if (bookFiles == null || bookFiles.isEmpty()) return null;

    List<BookFileEntity> bookFormats = bookFiles.stream().filter(BookFileEntity::isBook).toList();
    ...
    if (library != null
      ...
      for (BookFileType format : library.getFormatPriority()) {
        var match = bookFormats.stream().filter(bf -> bf.getBookType() == format).findFirst();
        if (match.isPresent()) {
          return match.get();
        }
      }
  ...
  default void mapAudiobookMetadata(BookEntity bookEntity, @MappingTarget Book book) {
    ...
    if (audiobookFile != null && audiobookFile.getDurationSeconds() != null) {
      ...
      if (audiobookFile.getChapters() != null) {
        chapters =
            audiobookFile.getChapters().stream()
                .map(
                    ch ->
