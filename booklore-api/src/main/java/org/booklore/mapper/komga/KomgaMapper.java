...
public class KomgaMapper {
  ...
  public KomgaLibraryDto toKomgaLibraryDto(LibraryEntity library) {
    return KomgaLibraryDto.builder()
        ...
        .root(
            library.getLibraryPaths() != null && !library.getLibraryPaths().isEmpty()
  ...
  public KomgaBookDto toKomgaBookDto(BookEntity book) {
    ...
    return KomgaBookDto.builder()
        ...
        .number(
            metadata != null && metadata.getSeriesNumber() != null
  ...
  public KomgaSeriesDto toKomgaSeriesDto(
      String seriesName, Long libraryId, List<BookEntity> books) {
  ...
  private KomgaMediaDto toKomgaMediaDto(BookEntity book, BookMetadataEntity metadata) {
    ...
    Integer pageCount =
        metadata != null && metadata.getPageCount() != null ? metadata.getPageCount() : 0;
    ...
    if (metadata.getAuthors() != null) {
      authors =
          metadata.getAuthors().stream()
              .map(author -> KomgaAuthorDto.builder().name(author.getName()).role("writer").build())
              .collect(Collectors.toList());
    }

    List<String> tags = new ArrayList<>();
    if (metadata.getTags() != null) {
      tags = metadata.getTags().stream().map(TagEntity::getName).collect(Collectors.toList());
    }
  ...
  private KomgaSeriesMetadataDto aggregateSeriesMetadata(
      String seriesName, List<BookEntity> books) {
    BookEntity firstBook = books.get(0);
    ...
    if (firstMetadata != null) {
      if (firstMetadata.getCategories() != null) {
        genres =
            firstMetadata.getCategories().stream()
                .map(CategoryEntity::getName)
                .collect(Collectors.toList());
      }
      if (firstMetadata.getTags() != null) {
        tags =
            firstMetadata.getTags().stream().map(TagEntity::getName).collect(Collectors.toList());
      }
    }
  ...
  public String getBookSeriesName(BookEntity book) {
    ...
    String bookSeriesName =
        metadata != null && metadata.getSeriesName() != null
            ...
            : (groupUnknown
                ? UNKNOWN_SERIES
                : (metadata.getTitle() != null ? metadata.getTitle() : bookFile.getFileName()));
  ...
  private String generateSeriesId(BookEntity book) {
    ...
    return libraryId
        + "-"
        + NON_ALPHANUMERIC_PATTERN.matcher(seriesName.toLowerCase()).replaceAll("-");
  }
  ...
  /**
   * Helper method to return null for empty strings in clean mode. In clean mode, we want to allow
   * null values so they can be filtered out.
   */
  ...
  /**
   * Helper method to return null for empty integer in clean mode. In clean mode, we want to allow
   * null values so they can be filtered out.
   */
  ...
  /**
   * Helper method to return null for empty float in clean mode. In clean mode, we want to allow
   * null values so they can be filtered out.
   */
