...
public class UnrarHelper {
  ...
  public static boolean isAvailable() {
    ...
    synchronized (UnrarHelper.class) {
      ...
      try {
        Process process = new ProcessBuilder(getUnrarBin()).redirectErrorStream(true).start();
  ...
  public static List<String> listEntries(Path rarPath) throws IOException {
    ProcessBuilder pb =
        new ProcessBuilder(getUnrarBin(), "lb", rarPath.toAbsolutePath().toString());
  ...
  public static void extractEntry(Path rarPath, String entryName, OutputStream out)
      throws IOException {
    ProcessBuilder pb =
        new ProcessBuilder(
    ...
    if (process.exitValue() != 0) {
      throw new IOException(
          "unrar extract failed (exit " + process.exitValue() + ") for entry: " + entryName);
    }
  }
  ...
  public static void extractAll(Path rarPath, Path targetDir) throws IOException {
    ProcessBuilder pb =
        new ProcessBuilder(
            ...
            "x",
            "-o+",
            rarPath.toAbsolutePath().toString(),
    ...
    if (process.exitValue() != 0) {
      throw new IOException(
          "unrar extractAll failed (exit " + process.exitValue() + ") for: " + rarPath);
    }
  }
