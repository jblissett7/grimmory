...
public class FileService {
  ...
  public String getThumbnailFile(long bookId) {
    return Paths.get(
            appProperties.getPathConfig(), IMAGES_DIR, String.valueOf(bookId), THUMBNAIL_FILENAME)
        .toString();
  }

  public String getCoverFile(long bookId) {
    return Paths.get(
            appProperties.getPathConfig(), IMAGES_DIR, String.valueOf(bookId), COVER_FILENAME)
        .toString();
  }

  public String getAudiobookThumbnailFile(long bookId) {
    return Paths.get(
            appProperties.getPathConfig(),
            IMAGES_DIR,
            String.valueOf(bookId),
            AUDIOBOOK_THUMBNAIL_FILENAME)
        .toString();
  }
  ...
  public String getAudiobookCoverFile(long bookId) {
    return Paths.get(
            ...
            IMAGES_DIR,
            String.valueOf(bookId),
            AUDIOBOOK_COVER_FILENAME)
        .toString();
  }

  public String getAuthorImagesFolder(long authorId) {
    return Paths.get(appProperties.getPathConfig(), AUTHOR_IMAGES_DIR, String.valueOf(authorId))
        .toString();
  }
  ...
  public static String getBackgroundUrl(String filename, Long userId) {
    if (userId != null) {
      return Paths.get("/", BACKGROUNDS_DIR, "user-" + userId, filename)
          .toString()
          .replace("\\", "/");
    }
  ...
  public String getBookMetadataBackupPath(long bookId) {
    return Paths.get(appProperties.getPathConfig(), "metadata_backup", String.valueOf(bookId))
        .toString();
  }
  ...
  public String getTempBookdropCoverImagePath(long bookdropFileId) {
    return Paths.get(appProperties.getPathConfig(), "bookdrop_temp", bookdropFileId + ".jpg")
        .toString();
  }
  ...
  public static BufferedImage readImage(byte[] imageData) throws IOException {
    ...
    try (ImageInputStream iis =
      ...
      throw new IOException(
          "ImageIO decode failed (possibly unsupported format): " + e.getMessage(), e);
    }
  ...
  private BufferedImage downloadImageFromUrlInternal(String imageUrl) throws IOException {
    ...
    while (redirectCount <= MAX_REDIRECTS) {
      ...
      for (InetAddress inetAddress : inetAddresses) {
        if (isInternalAddress(inetAddress)) {
          throw new SecurityException(
              "URL points to a local or private internal network address: "
                  + host
                  + " ("
      ...
      ResponseEntity<byte[]> response =
          noRedirectRestTemplate.exchange(currentUrl, HttpMethod.GET, entity, byte[].class);
  ...
  public void createThumbnailFromFile(long bookId, MultipartFile file) {
    try {
      ...
      if (originalImage == null) {
        log.warn(
            "Could not decode image from file, skipping thumbnail creation for book: {}", bookId);
        return;
      }
  ...
  public void createAuthorThumbnailFromUrl(long authorId, String imageUrl) {
    try {
      ...
      if (originalImage == null) {
        log.warn(
            "Skipping author thumbnail creation for author {}: download/decode failed", authorId);
        return;
      }
      ...
      log.warn(
          "Failed to create author thumbnail from URL for author {}: {}", authorId, e.getMessage());
    }
  }
    ...
    try {
      ...
      if (scale < 1.0) {
        resized =
            resizeImage(
                rgbImage,
                (int) (rgbImage.getWidth() * scale),
                (int) (rgbImage.getHeight() * scale));
        rgbImage.flush();
        rgbImage = resized;
      }
  ...
  public void createAudiobookThumbnailFromFile(long bookId, MultipartFile file) {
    try {
      ...
      if (originalImage == null) {
        log.warn(
            "Could not decode image from file, skipping audiobook thumbnail creation for book: {}",
            bookId);
        return;
      }
  ...
  public void createAudiobookThumbnailFromBytes(long bookId, byte[] imageBytes) {
    try {
      ...
      log.error(
          "An error occurred while creating audiobook thumbnail from bytes: {}", e.getMessage(), e);
      throw ApiError.FILE_READ_ERROR.createException(e.getMessage());
    }
  }
  ...
  public void createAudiobookThumbnailFromUrl(long bookId, String imageUrl) {
    try {
      ...
      if (originalImage == null) {
        log.warn(
            "Skipping audiobook thumbnail creation for book {}: download/decode failed", bookId);
        return;
      }
      boolean success = saveAudiobookCoverImages(originalImage, bookId);
      if (!success) {
      ...
      log.error(
          "An error occurred while creating audiobook thumbnail from URL: {}", e.getMessage(), e);
      throw ApiError.FILE_READ_ERROR.createException(e.getMessage());
    }
  }

  public boolean saveAudiobookCoverImages(BufferedImage coverImage, long bookId)
      throws IOException {
    BufferedImage rgbImage = null;
    BufferedImage resized = null;
    BufferedImage thumb = null;
    try {
      ...
      rgbImage =
          new BufferedImage(
              coverImage.getWidth(), coverImage.getHeight(), BufferedImage.TYPE_INT_RGB);
      Graphics2D g = rgbImage.createGraphics();
  ...
  private BufferedImage applyCoverCropping(BufferedImage image) {
    ...
    boolean isExtremelyTall =
        settings.isVerticalCroppingEnabled() && heightToWidthRatio > threshold;
    if (isExtremelyTall) {
      int croppedHeight = (int) (width * TARGET_COVER_ASPECT_RATIO);
      log.debug(
          "Cropping tall image: {}x{} (ratio {}) -> {}x{}, smartCrop={}",
          width,
          height,
          String.format("%.2f", heightToWidthRatio),
          width,
          croppedHeight,
          smartCrop);
    ...
    boolean isExtremelyWide =
        settings.isHorizontalCroppingEnabled() && widthToHeightRatio > threshold;
    if (isExtremelyWide) {
      int croppedWidth = (int) (height / TARGET_COVER_ASPECT_RATIO);
      log.debug(
          "Cropping wide image: {}x{} (ratio {}) -> {}x{}, smartCrop={}",
          width,
          height,
          String.format("%.2f", widthToHeightRatio),
          croppedWidth,
          height,
          smartCrop);
  ...
  private BufferedImage cropFromTop(
      BufferedImage image, int targetWidth, int targetHeight, boolean smartCrop) {
    int startY = 0;
    if (smartCrop) {
  ...
  private BufferedImage cropFromLeft(
      BufferedImage image, int targetWidth, int targetHeight, boolean smartCrop) {
    int startX = 0;
    if (smartCrop) {
