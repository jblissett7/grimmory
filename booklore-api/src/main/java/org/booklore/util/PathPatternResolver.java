...
public class PathPatternResolver {
  ...
  public String resolvePattern(BookEntity book, String pattern) {
    ...
    return resolvePattern(
        book, primaryFile, pattern, primaryFile != null && primaryFile.isFolderBased());
  }
  ...
  public String resolvePattern(
      BookEntity book, BookFileEntity bookFile, String pattern, boolean folderBased) {
    String currentFilename =
        bookFile != null && bookFile.getFileName() != null ? bookFile.getFileName().trim() : "";
  ...
  public String resolvePattern(
      BookMetadataEntity metadata, String pattern, String filename, boolean folderBased) {
    MetadataProvider metadataProvider = MetadataProvider.from(metadata);
    return resolvePattern(metadataProvider, pattern, filename, folderBased);
  }

  private String resolvePattern(
      MetadataProvider metadata, String pattern, String filename, boolean folderBased) {
    ...
    String title =
        sanitize(
            metadata != null && metadata.getTitle() != null ? metadata.getTitle() : filenameBase);
        ...
        sanitize(
            metadata != null
                ? (metadata.getIsbn13() != null
                    ? metadata.getIsbn13()
                    : metadata.getIsbn10() != null ? metadata.getIsbn10() : "")
                : "");
  ...
  private String resolvePatternWithValues(
      String pattern, Map<String, String> values, String currentFilename, boolean folderBased) {
    String extension = "";
    if (!folderBased) {
    ...
    while (placeholderMatcher.find()) {
      ...
      if (values.containsKey(fieldName)) {
        ...
        placeholderMatcher.appendReplacement(
            finalResult, Matcher.quoteReplacement(placeholderMatcher.group()));
      }
    }
    ...
    if (!folderBased
        && !usedFallbackFilename
        && !patternIncludesExtension
        && !patternIncludesFullFilename
        && !extension.isBlank()) {
      result += "." + extension;
    }
    ...
    return WHITESPACE_PATTERN
        .matcher(
            CONTROL_CHARACTER_PATTERN
                .matcher(INVALID_CHARS_PATTERN.matcher(input).replaceAll(""))
                .replaceAll(""))
        .replaceAll(" ")
        .trim();
  }
  ...
  public String truncatePathComponent(String component, int maxBytes) {
    ...
    if (!truncated.equals(component)) {
      log.debug(
          "Truncated path component from {} to {} bytes for filesystem safety",
          bytes.length,
  ...
  private record BookMetadataProvider(BookMetadata metadata) implements MetadataProvider {
    ...
    public List<String> getAuthors() {
      return metadata.getAuthors() != null
          ? metadata.getAuthors().stream().toList()
  ...
  private record BookMetadataEntityProvider(BookMetadataEntity metadata)
