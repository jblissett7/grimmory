...
public class BookFileGroupingUtils {
  ...
  private static final Pattern FORMAT_INDICATOR_PATTERN =
      Pattern.compile(
  ...
  private static final Pattern TRAILING_AUTHOR_PATTERN =
      Pattern.compile(
          "\\s*[-–—]\\s+(?:[A-Z](?:\\.[A-Z])*\\.\\s+)?[A-Z][a-z]+(?:\\s+[A-Z](?:\\.[A-Z])*\\.?)?(?:\\s+[A-Z][a-z]+)*\\s*$");

  // Matches ", The" or ", A" or ", An" at end
  private static final Pattern ARTICLE_SUFFIX_PATTERN =
      Pattern.compile(",\\s*(The|A|An)\\s*$", Pattern.CASE_INSENSITIVE);

  // Series patterns: "Book 1", "Vol. 2", "Part 3", "#4", etc.
  private static final Pattern SERIES_NUMBER_PATTERN =
      Pattern.compile(
          "(?:book|vol(?:ume)?|part|chapter|episode|#|no\\.?)\\s*(\\d+)", Pattern.CASE_INSENSITIVE);

  // Trailing number pattern: "book1", "book 2", "title_03"
  private static final Pattern TRAILING_NUMBER_PATTERN = Pattern.compile("[\\s_-]?(\\d{1,3})\\s*$");

  // Part/disc indicators: "(part 1)", "[pt 2]", "- disc 1", "cd 3", etc.
  // "part" requires brackets or dash prefix to avoid conflict with series labels ("Part 1" = series
  // entry).
  // "pt", "disc", "disk", "cd" are unambiguous and can be matched bare.
  private static final Pattern PART_INDICATOR_PATTERN =
      Pattern.compile(
          "\\s*(?:"
  ...
  private static final Pattern BARE_NUMBER_PREFIX_PATTERN =
      Pattern.compile("^(\\d{1,3})(?:\\.|\\s*-)\\s+(.+)$");
  ...
  private static final Set<String> KNOWN_EXTENSIONS =
      Set.of("pdf", "epub", "cbz", "cbr", "cb7", "mobi", "azw3", "azw", "fb2", "m4b", "m4a", "mp3");
  ...
  public double calculateSimilarity(String s1, String s2) {
    ...
    int maxScore = Math.max(fuzzyScore.fuzzyScore(s1, s1), fuzzyScore.fuzzyScore(s2, s2));
   ...
   *
   * <p>Strategy: 1. Group files by folder (libraryPathId + fileSubPath) 2. For files in subfolders:
   * use folder name as reference title - Files matching folder name (substring/fuzzy) → group
   * together - Series entries (numbered) → keep separate - Unrelated files → separate books 3. For
   * root-level files: use exact grouping key matching
   */
  public Map<String, List<LibraryFile>> groupByBaseName(List<LibraryFile> libraryFiles) {
    ...
    for (Map.Entry<String, List<LibraryFile>> folderEntry : byFolder.entrySet()) {
      ...
      Map<String, List<LibraryFile>> folderGroups =
          groupFilesInFolder(filesInFolder, fileSubPath, libraryPathId);
  ...
  private Map<String, List<LibraryFile>> groupFilesInFolder(
      List<LibraryFile> files, String fileSubPath, Long libraryPathId) {
    Map<String, List<LibraryFile>> result = new LinkedHashMap<>();
    ...
    for (Map.Entry<String, List<LibraryFile>> entry : matchesFolderByNumber.entrySet()) {
      ...
      log.debug(
          "Grouped {} files matching folder '{}'{}: {}",
          filesWithNumber.size(),
          folderName,
    ...
    if (!unmatched.isEmpty()) {
      Map<String, List<LibraryFile>> clusters =
          clusterBySimilarity(unmatched, libraryPathId, fileSubPath);
  ...
  /**
   * Checks if a file's grouping key matches the folder name. Uses substring matching and fuzzy
   * similarity.
   */
  ...
  /**
   * Clusters unmatched files by fuzzy similarity. Uses union-find to group similar files together.
   * Files with different trailing numbers are kept separate (e.g., "book1" vs "book2").
   */
  private Map<String, List<LibraryFile>> clusterBySimilarity(
      List<LibraryFile> files, Long libraryPathId, String fileSubPath) {
    Map<String, List<LibraryFile>> result = new LinkedHashMap<>();

    if (files.size() == 1) {
      LibraryFile file = files.get(0);
      String key =
          libraryPathId + ":" + fileSubPath + ":single:" + extractGroupingKey(file.getFileName());
      result.put(key, new ArrayList<>(List.of(file)));
      return result;
    }

    // Extract keys for all files
    List<String> keys = files.stream().map(f -> extractGroupingKey(f.getFileName())).toList();
  ...
  /**
   * Checks if two keys have different trailing numbers. Returns true if both have trailing numbers
   * AND they're different. This prevents grouping "book1" with "book2".
   */
